using System;
using System.Diagnostics;
using System.Runtime.CompilerServices;
using UnityEngine;
using UnityEngine.Rendering;

// Image 4: HG.RenderPipelines.Runtime.dll - Assembly: HG.RenderPipelines.Runtime, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null - Types 37354-38879

namespace HG.Rendering.Runtime
{
	[DisallowMultipleComponent]
	public class ClothGroupInfo : MonoBehaviour // TypeDefIndex: 37551
	{
		// Fields
		[NonSerialized]
		public int groupIdx; // 0x18
		[NonSerialized]
		public ClothInfo[] clothInfos; // 0x20
		[NonSerialized]
		public bool isBuilt; // 0x28
		[NonSerialized]
		public Bounds groupBounds; // 0x2C
		[NonSerialized]
		public ClothGroupMeta groupMeta; // 0x44
		[NonSerialized]
		public DynamicArray<ClothMetaData> clothMetaDataArray; // 0x98
		[NonSerialized]
		public DynamicArray<ClothNodeData> clothNodeDataArray; // 0xA0
		[NonSerialized]
		public CompressedInt2Array clothBatchMetaArray; // 0xA8
		[NonSerialized]
		public CompressedInt2Array clothBatchCacheMapArray; // 0xB0
	
		// Constructors
		public ClothGroupInfo() {} // 0x0000000189C67980-0x0000000189C67A80
		// ClothGroupInfo()
		void HG::Rendering::Runtime::ClothGroupInfo::ClothGroupInfo(ClothGroupInfo *this, MethodInfo *method)
		{
		  DynamicArray_1_HG_Rendering_RenderGraphModule_HGRenderGraph_CompiledResourceInfo_ *v3; // rax
		  __int64 v4; // rdx
		  __int64 v5; // rcx
		  DynamicArray_1_HG_Rendering_RenderGraphModule_HGRenderGraph_CompiledResourceInfo_ *v6; // rdi
		  Type *v7; // rdx
		  PropertyInfo_1 *v8; // r8
		  Int32__Array **v9; // r9
		  DynamicArray_1_UnityEngine_Experimental_Rendering_RenderGraphModule_RenderGraph_CompiledResourceInfo_ *v10; // rax
		  DynamicArray_1_UnityEngine_Experimental_Rendering_RenderGraphModule_RenderGraph_CompiledResourceInfo_ *v11; // rdi
		  Type *v12; // rdx
		  PropertyInfo_1 *v13; // r8
		  Int32__Array **v14; // r9
		  CompressedInt2Array *v15; // rax
		  ClothInfo__Array *v16; // rdi
		  Type *v17; // rdx
		  PropertyInfo_1 *v18; // r8
		  Int32__Array **v19; // r9
		  CompressedInt2Array *v20; // rax
		  CompressedInt2Array *v21; // rdi
		  Type *v22; // rdx
		  PropertyInfo_1 *v23; // r8
		  Int32__Array **v24; // r9
		  MethodInfo *v25; // [rsp+20h] [rbp-8h]
		  MethodInfo *v26; // [rsp+20h] [rbp-8h]
		  MethodInfo *v27; // [rsp+20h] [rbp-8h]
		  MethodInfo *v28; // [rsp+20h] [rbp-8h]
		
		  v3 = (DynamicArray_1_HG_Rendering_RenderGraphModule_HGRenderGraph_CompiledResourceInfo_ *)sub_18002C620(TypeInfo::UnityEngine::Rendering::DynamicArray<HG::Rendering::Runtime::ClothMetaData>);
		  v6 = v3;
		  if ( !v3 )
		    goto LABEL_6;
		  UnityEngine::Rendering::DynamicArray<HG::Rendering::RenderGraphModule::HGRenderGraph::CompiledResourceInfo>::DynamicArray(
		    v3,
		    MethodInfo::UnityEngine::Rendering::DynamicArray<HG::Rendering::Runtime::ClothMetaData>::DynamicArray);
		  this[1].fields._._._._.m_CachedPtr = v6;
		  sub_18002D1B0((SingleFieldAccessor *)&this[1].fields, v7, v8, v9, v25);
		  v10 = (DynamicArray_1_UnityEngine_Experimental_Rendering_RenderGraphModule_RenderGraph_CompiledResourceInfo_ *)sub_18002C620(TypeInfo::UnityEngine::Rendering::DynamicArray<HG::Rendering::Runtime::ClothNodeData>);
		  v11 = v10;
		  if ( !v10 )
		    goto LABEL_6;
		  UnityEngine::Rendering::DynamicArray<UnityEngine::Experimental::Rendering::RenderGraphModule::RenderGraph::CompiledResourceInfo>::DynamicArray(
		    v10,
		    MethodInfo::UnityEngine::Rendering::DynamicArray<HG::Rendering::Runtime::ClothNodeData>::DynamicArray);
		  *(_QWORD *)&this[1].fields.groupIdx = v11;
		  sub_18002D1B0((SingleFieldAccessor *)&this[1].fields.groupIdx, v12, v13, v14, v26);
		  v15 = (CompressedInt2Array *)sub_18002C620(TypeInfo::HG::Rendering::Runtime::CompressedInt2Array);
		  v16 = (ClothInfo__Array *)v15;
		  if ( !v15
		    || (HG::Rendering::Runtime::CompressedInt2Array::CompressedInt2Array(v15, 0LL),
		        this[1].fields.clothInfos = v16,
		        sub_18002D1B0((SingleFieldAccessor *)&this[1].fields.clothInfos, v17, v18, v19, v27),
		        v20 = (CompressedInt2Array *)sub_18002C620(TypeInfo::HG::Rendering::Runtime::CompressedInt2Array),
		        (v21 = v20) == 0LL) )
		  {
		LABEL_6:
		    sub_1800D8260(v5, v4);
		  }
		  HG::Rendering::Runtime::CompressedInt2Array::CompressedInt2Array(v20, 0LL);
		  *(_QWORD *)&this[1].fields.isBuilt = v21;
		  sub_18002D1B0((SingleFieldAccessor *)&this[1].fields.isBuilt, v22, v23, v24, v28);
		  RootMotion::Singleton<System::Object>::Singleton((Singleton_1_System_Object__1 *)this, 0LL);
		}
		
	
		// Methods
		public void RecomputeBounds() {} // 0x0000000189C677CC-0x0000000189C67980
		// Void RecomputeBounds()
		void HG::Rendering::Runtime::ClothGroupInfo::RecomputeBounds(ClothGroupInfo *this, MethodInfo *method)
		{
		  ClothInfo *v3; // rdx
		  ClothInfo__Array *v4; // rcx
		  int32_t v5; // r8d
		  MethodInfo *v6; // r9
		  unsigned int i; // ebx
		  ClothInfo__Array *clothInfos; // rax
		  Vector3 *Vector; // rax
		  __int64 v10; // xmm1_8
		  ClothInfo *v11; // rax
		  __int64 v12; // xmm0_8
		  ILFixDynamicMethodWrapper_2 *Patch; // rax
		  float v14[4]; // [rsp+20h] [rbp-68h]
		  Vector3 v15; // [rsp+40h] [rbp-48h] BYREF
		  Bounds v16; // [rsp+50h] [rbp-38h] BYREF
		
		  if ( !IFix::WrappersManagerImpl::IsPatched(1241, 0LL) )
		  {
		    for ( i = 0; ; ++i )
		    {
		      clothInfos = this->fields.clothInfos;
		      if ( !clothInfos )
		        break;
		      if ( (signed int)i >= clothInfos->max_length.size )
		        return;
		      v4 = this->fields.clothInfos;
		      if ( i >= clothInfos->max_length.size )
		        goto LABEL_22;
		      v3 = v4->vector[i];
		      if ( !v3 )
		        break;
		      Vector = UnityEngine::Animator::GetVector(&v15, (Animator *)v3, v5, v6);
		      *(_QWORD *)v14 = *(_QWORD *)&v3->fields.clothBounds.m_Extents.x;
		      if ( (float)((float)((float)((float)(v14[1] - COERCE_FLOAT(HIDWORD(*(_QWORD *)&Vector->x)))
		                                 * (float)(v14[1] - COERCE_FLOAT(HIDWORD(*(_QWORD *)&Vector->x))))
		                         + (float)((float)(v14[0] - COERCE_FLOAT(*(_QWORD *)&Vector->x))
		                                 * (float)(v14[0] - COERCE_FLOAT(*(_QWORD *)&Vector->x))))
		                 + (float)((float)(v3->fields.clothBounds.m_Extents.z - Vector->z)
		                         * (float)(v3->fields.clothBounds.m_Extents.z - Vector->z))) < 9.9999994e-11 )
		      {
		        v4 = this->fields.clothInfos;
		        if ( !v4 )
		          break;
		        if ( i >= v4->max_length.size )
		          goto LABEL_22;
		        v4 = (ClothInfo__Array *)v4->vector[i];
		        if ( !v4 )
		          break;
		        HG::Rendering::Runtime::ClothInfo::RecomputeBounds((ClothInfo *)v4, 0LL);
		      }
		      v4 = this->fields.clothInfos;
		      if ( i )
		      {
		        if ( !v4 )
		          break;
		        if ( i >= v4->max_length.size )
		          goto LABEL_22;
		        v3 = v4->vector[i];
		        if ( !v3 )
		          break;
		        v10 = *(_QWORD *)&v3->fields.clothBounds.m_Extents.y;
		        *(_OWORD *)&v16.m_Center.x = *(_OWORD *)&v3->fields.clothBounds.m_Center.x;
		        *(_QWORD *)&v16.m_Extents.y = v10;
		        UnityEngine::Bounds::Encapsulate(&this->fields.groupBounds, &v16, 0LL);
		      }
		      else
		      {
		        if ( !v4 )
		          break;
		        if ( !v4->max_length.size )
		LABEL_22:
		          sub_1800D2AB0(v4, v3);
		        v11 = v4->vector[0];
		        if ( !v11 )
		          break;
		        v12 = *(_QWORD *)&v11->fields.clothBounds.m_Extents.y;
		        *(_OWORD *)&this->fields.groupBounds.m_Center.x = *(_OWORD *)&v11->fields.clothBounds.m_Center.x;
		        *(_QWORD *)&this->fields.groupBounds.m_Extents.y = v12;
		      }
		    }
		LABEL_24:
		    sub_1800D8260(v4, v3);
		  }
		  Patch = IFix::WrappersManagerImpl::GetPatch(1241, 0LL);
		  if ( !Patch )
		    goto LABEL_24;
		  IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_0((ILFixDynamicMethodWrapper_39 *)Patch, (Object *)this, 0LL);
		}
		
		public void FillPadding() {} // 0x0000000189C6774C-0x0000000189C677CC
		// Void FillPadding()
		void HG::Rendering::Runtime::ClothGroupInfo::FillPadding(ClothGroupInfo *this, MethodInfo *method)
		{
		  __int64 v3; // rdx
		  DynamicArray_1_Unity_Mathematics_int4_ *v4; // rcx
		  __int64 v5; // rax
		  ILFixDynamicMethodWrapper_2 *Patch; // rax
		  int4 value; // [rsp+20h] [rbp-18h] BYREF
		
		  if ( !IFix::WrappersManagerImpl::IsPatched(1243, 0LL) )
		  {
		    v5 = *(_QWORD *)&this[1].fields.isBuilt;
		    if ( v5 )
		    {
		      if ( *(_DWORD *)(v5 + 24) )
		        return;
		      v4 = *(DynamicArray_1_Unity_Mathematics_int4_ **)(v5 + 16);
		      value = 0LL;
		      if ( v4 )
		      {
		        UnityEngine::Rendering::DynamicArray<Unity::Mathematics::int4>::Add(
		          v4,
		          &value,
		          MethodInfo::UnityEngine::Rendering::DynamicArray<Unity::Mathematics::int4>::Add);
		        return;
		      }
		    }
		LABEL_7:
		    sub_1800D8260(v4, v3);
		  }
		  Patch = IFix::WrappersManagerImpl::GetPatch(1243, 0LL);
		  if ( !Patch )
		    goto LABEL_7;
		  IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_0((ILFixDynamicMethodWrapper_39 *)Patch, (Object *)this, 0LL);
		}
		
	}
}
