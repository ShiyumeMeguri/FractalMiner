using System;
using System.Runtime.CompilerServices;
using System.Runtime.InteropServices;
using HG.Rendering.RenderGraphModule;
using Unity.Collections;
using UnityEngine;
using UnityEngine.Experimental.Rendering;
using UnityEngine.HyperGryphEngineCode;
using UnityEngine.Rendering;

namespace HG.Rendering.Runtime
{
	internal class UberPostPassUtils
	{
		public UberPostPassUtils()
		{
			// // Void Lerp[HGWindConfig](HGWindConfig ByRef, HGWindConfig ByRef, Single)
			// void HG::Rendering::Runtime::HGCelestialConfig::HGCelestialAdvancedObjectConfig::Lerp<HG::Rendering::Runtime::HGWindConfig>(
			//         HGCelestialConfig_HGCelestialAdvancedObjectConfig *this,
			//         HGWindConfig *cSrc,
			//         HGWindConfig *cDst,
			//         float t,
			//         MethodInfo *method)
			// {
			//   ;
			// }
			// 
		}

		internal static float GetPostExposureLinear(HGCamera camera)
		{
			// // Single GetPostExposureLinear(HGCamera)
			// float HG::Rendering::Runtime::UberPostPassUtils::GetPostExposureLinear(HGCamera *camera, MethodInfo *method)
			// {
			//   __int64 *static_fields; // rcx
			//   int *wrapperArray; // rdx
			//   HGEnvironmentVolumeCameraComponent *m_envVolumeCameraComponent; // rax
			//   HGEnvironmentPhase *m_interpolatedPhase; // rax
			//   HGColorGradingConfig *p_colorGradingConfig; // rbx
			//   double v8; // xmm0_8
			//   __int64 v9; // rax
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   __int64 v11; // rax
			//   ILFixDynamicMethodWrapper_2 *v12; // rax
			//   __int64 v13; // rax
			//   ILFixDynamicMethodWrapper_2 *v14; // rax
			// 
			//   if ( !byte_18D8EDA2E )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGColorGradingConfig);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGEnvironmentManager);
			//     byte_18D8EDA2E = 1;
			//   }
			//   if ( !byte_18D8EDC37 )
			//   {
			//     sub_18003C530(&TypeInfo::IFix::ILFixDynamicMethodWrapper);
			//     byte_18D8EDC37 = 1;
			//   }
			//   if ( !TypeInfo::IFix::ILFixDynamicMethodWrapper._1.cctor_finished_or_no_cctor )
			//     il2cpp_runtime_class_init_0(TypeInfo::IFix::ILFixDynamicMethodWrapper, method);
			//   static_fields = (__int64 *)TypeInfo::IFix::ILFixDynamicMethodWrapper;
			//   wrapperArray = (int *)TypeInfo::IFix::ILFixDynamicMethodWrapper.static_fields.wrapperArray;
			//   if ( !wrapperArray )
			//     goto LABEL_33;
			//   if ( wrapperArray[6] > 1024 )
			//   {
			//     if ( !TypeInfo::IFix::ILFixDynamicMethodWrapper._1.cctor_finished_or_no_cctor )
			//       il2cpp_runtime_class_init_0(TypeInfo::IFix::ILFixDynamicMethodWrapper, wrapperArray);
			//     wrapperArray = (int *)TypeInfo::IFix::ILFixDynamicMethodWrapper.static_fields;
			//     v9 = *(_QWORD *)wrapperArray;
			//     if ( !*(_QWORD *)wrapperArray )
			//       goto LABEL_33;
			//     if ( *(_DWORD *)(v9 + 24) <= 0x400u )
			//       goto LABEL_58;
			//     if ( *(_QWORD *)(v9 + 8224) )
			//     {
			//       Patch = IFix::WrappersManagerImpl::GetPatch(1024, 0LL);
			//       if ( Patch )
			//       {
			//         *(float *)&v8 = IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_43(
			//                           (ILFixDynamicMethodWrapper_16 *)Patch,
			//                           (Object *)camera,
			//                           0LL);
			//         return *(float *)&v8;
			//       }
			//       goto LABEL_33;
			//     }
			//   }
			//   if ( !TypeInfo::HG::Rendering::Runtime::HGEnvironmentManager._1.cctor_finished_or_no_cctor )
			//     il2cpp_runtime_class_init_0(TypeInfo::HG::Rendering::Runtime::HGEnvironmentManager, wrapperArray);
			//   if ( !byte_18D8EDC5D )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGEnvironmentManager);
			//     byte_18D8EDC5D = 1;
			//   }
			//   if ( !byte_18D8EDC37 )
			//   {
			//     sub_18003C530(&TypeInfo::IFix::ILFixDynamicMethodWrapper);
			//     byte_18D8EDC37 = 1;
			//   }
			//   if ( !TypeInfo::IFix::ILFixDynamicMethodWrapper._1.cctor_finished_or_no_cctor )
			//     il2cpp_runtime_class_init_0(TypeInfo::IFix::ILFixDynamicMethodWrapper, wrapperArray);
			//   static_fields = (__int64 *)TypeInfo::IFix::ILFixDynamicMethodWrapper;
			//   wrapperArray = (int *)TypeInfo::IFix::ILFixDynamicMethodWrapper.static_fields.wrapperArray;
			//   if ( !wrapperArray )
			//     goto LABEL_33;
			//   if ( wrapperArray[6] <= 439 )
			//     goto LABEL_62;
			//   if ( !TypeInfo::IFix::ILFixDynamicMethodWrapper._1.cctor_finished_or_no_cctor )
			//     il2cpp_runtime_class_init_0(TypeInfo::IFix::ILFixDynamicMethodWrapper, wrapperArray);
			//   wrapperArray = (int *)TypeInfo::IFix::ILFixDynamicMethodWrapper.static_fields;
			//   v11 = *(_QWORD *)wrapperArray;
			//   if ( !*(_QWORD *)wrapperArray )
			//     goto LABEL_33;
			//   if ( *(_DWORD *)(v11 + 24) <= 0x1B7u )
			//     goto LABEL_58;
			//   if ( *(_QWORD *)(v11 + 3544) )
			//   {
			//     v12 = IFix::WrappersManagerImpl::GetPatch(439, 0LL);
			//     if ( !v12 )
			//       goto LABEL_33;
			//     m_interpolatedPhase = IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_199(v12, (Object *)camera, 0LL);
			//   }
			//   else
			//   {
			// LABEL_62:
			//     if ( !camera )
			//       goto LABEL_33;
			//     m_envVolumeCameraComponent = camera.fields.m_envVolumeCameraComponent;
			//     if ( !m_envVolumeCameraComponent )
			//       goto LABEL_33;
			//     if ( m_envVolumeCameraComponent.fields.m_useEnvVolumeInterpolatedPhase )
			//     {
			//       m_interpolatedPhase = m_envVolumeCameraComponent.fields.m_interpolatedPhase;
			//     }
			//     else
			//     {
			//       if ( !TypeInfo::HG::Rendering::Runtime::HGEnvironmentManager._1.cctor_finished_or_no_cctor )
			//         il2cpp_runtime_class_init_0(TypeInfo::HG::Rendering::Runtime::HGEnvironmentManager, wrapperArray);
			//       m_interpolatedPhase = HG::Rendering::Runtime::HGEnvironmentManager::get_s_interpolatedPhase(0LL);
			//     }
			//   }
			//   if ( !m_interpolatedPhase )
			//     goto LABEL_33;
			//   p_colorGradingConfig = &m_interpolatedPhase.fields.colorGradingConfig;
			//   if ( !TypeInfo::HG::Rendering::Runtime::HGColorGradingConfig._1.cctor_finished_or_no_cctor )
			//     il2cpp_runtime_class_init_0(TypeInfo::HG::Rendering::Runtime::HGColorGradingConfig, wrapperArray);
			//   if ( !byte_18D8EDC37 )
			//   {
			//     sub_18003C530(&TypeInfo::IFix::ILFixDynamicMethodWrapper);
			//     byte_18D8EDC37 = 1;
			//   }
			//   if ( !TypeInfo::IFix::ILFixDynamicMethodWrapper._1.cctor_finished_or_no_cctor )
			//     il2cpp_runtime_class_init_0(TypeInfo::IFix::ILFixDynamicMethodWrapper, wrapperArray);
			//   static_fields = (__int64 *)TypeInfo::IFix::ILFixDynamicMethodWrapper;
			//   wrapperArray = (int *)TypeInfo::IFix::ILFixDynamicMethodWrapper.static_fields.wrapperArray;
			//   if ( !wrapperArray )
			//     goto LABEL_33;
			//   if ( wrapperArray[6] > 1025 )
			//   {
			//     if ( !TypeInfo::IFix::ILFixDynamicMethodWrapper._1.cctor_finished_or_no_cctor )
			//       il2cpp_runtime_class_init_0(TypeInfo::IFix::ILFixDynamicMethodWrapper, wrapperArray);
			//     static_fields = (__int64 *)TypeInfo::IFix::ILFixDynamicMethodWrapper.static_fields;
			//     v13 = *static_fields;
			//     if ( !*static_fields )
			//       goto LABEL_33;
			//     if ( *(_DWORD *)(v13 + 24) > 0x401u )
			//     {
			//       if ( !*(_QWORD *)(v13 + 8232) )
			//         goto LABEL_32;
			//       v14 = IFix::WrappersManagerImpl::GetPatch(1025, 0LL);
			//       if ( v14 )
			//       {
			//         IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_343(v14, p_colorGradingConfig, 0LL);
			//         goto LABEL_32;
			//       }
			// LABEL_33:
			//       sub_180B536AC(static_fields, wrapperArray);
			//     }
			// LABEL_58:
			//     sub_180070270(static_fields, wrapperArray);
			//   }
			// LABEL_32:
			//   v8 = sub_1802EB1B0((_DWORD)static_fields, wrapperArray);
			//   return *(float *)&v8;
			// }
			// 
			return 0f;
		}

		internal static GraphicsFormat GetPostprocessTextureFormat(bool enableAlpha, GraphicsFormat ppFormat)
		{
			// // GraphicsFormat GetPostprocessTextureFormat(Boolean, GraphicsFormat)
			// GraphicsFormat__Enum HG::Rendering::Runtime::UberPostPassUtils::GetPostprocessTextureFormat(
			//         bool enableAlpha,
			//         GraphicsFormat__Enum ppFormat,
			//         MethodInfo *method)
			// {
			//   GraphicsFormat__Enum result; // eax
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   __int64 v7; // rdx
			//   __int64 v8; // rcx
			// 
			//   if ( IFix::WrappersManagerImpl::IsPatched(2387, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2387, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v8, v7);
			//     return IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_862(Patch, enableAlpha, ppFormat, 0LL);
			//   }
			//   else
			//   {
			//     result = GraphicsFormat__Enum_R16G16B16A16_SFloat;
			//     if ( !enableAlpha )
			//       return ppFormat;
			//   }
			//   return result;
			// }
			// 
			return (GraphicsFormat)GraphicsFormat.None;
		}

		internal static GraphicsFormat GetPostprocessComputeTextureFormat(bool enableAlpha)
		{
			// // GraphicsFormat GetPostprocessComputeTextureFormat(Boolean)
			// GraphicsFormat__Enum HG::Rendering::Runtime::UberPostPassUtils::GetPostprocessComputeTextureFormat(
			//         bool enableAlpha,
			//         MethodInfo *method)
			// {
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   __int64 v5; // rdx
			//   __int64 v6; // rcx
			// 
			//   if ( !byte_18D9194BC )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGUtils);
			//     byte_18D9194BC = 1;
			//   }
			//   if ( IFix::WrappersManagerImpl::IsPatched(2388, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2388, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v6, v5);
			//     return IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_863(Patch, enableAlpha, 0LL);
			//   }
			//   else
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGUtils);
			//     return HG::Rendering::Runtime::HGUtils::GetDeviceSupportedRWTextureFormat(enableAlpha, 0LL);
			//   }
			// }
			// 
			return (GraphicsFormat)GraphicsFormat.None;
		}

		internal static bool ShouldRenderPostProcess(HGCamera camera)
		{
			// // Boolean ShouldRenderPostProcess(HGCamera)
			// bool HG::Rendering::Runtime::UberPostPassUtils::ShouldRenderPostProcess(HGCamera *camera, MethodInfo *method)
			// {
			//   struct ILFixDynamicMethodWrapper_2__Class *v3; // rcx
			//   ILFixDynamicMethodWrapper_2__Array *wrapperArray; // rdx
			//   Camera *v5; // rdi
			//   unsigned int (__fastcall *v6)(Camera *); // rax
			//   __int64 v7; // rdx
			//   bool v8; // al
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   __int64 v11; // rax
			//   ILFixDynamicMethodWrapper_2 *v12; // rax
			//   BitArray128 bitDatas; // [rsp+20h] [rbp-38h]
			//   FrameSettings P0; // [rsp+30h] [rbp-28h] BYREF
			// 
			//   if ( !byte_18D8EDA2F )
			//   {
			//     sub_18003C530(&TypeInfo::UnityEngine::Rendering::CoreUtils);
			//     byte_18D8EDA2F = 1;
			//   }
			//   if ( !byte_18D8EDC37 )
			//   {
			//     sub_18003C530(&TypeInfo::IFix::ILFixDynamicMethodWrapper);
			//     byte_18D8EDC37 = 1;
			//   }
			//   v3 = TypeInfo::IFix::ILFixDynamicMethodWrapper;
			//   if ( !TypeInfo::IFix::ILFixDynamicMethodWrapper._1.cctor_finished_or_no_cctor )
			//   {
			//     il2cpp_runtime_class_init_0(TypeInfo::IFix::ILFixDynamicMethodWrapper, method);
			//     v3 = TypeInfo::IFix::ILFixDynamicMethodWrapper;
			//   }
			//   wrapperArray = v3.static_fields.wrapperArray;
			//   if ( !wrapperArray )
			//     goto LABEL_24;
			//   if ( wrapperArray.max_length.size > 883 )
			//   {
			//     if ( !v3._1.cctor_finished_or_no_cctor )
			//     {
			//       il2cpp_runtime_class_init_0(v3, wrapperArray);
			//       v3 = TypeInfo::IFix::ILFixDynamicMethodWrapper;
			//     }
			//     wrapperArray = v3.static_fields.wrapperArray;
			//     if ( !wrapperArray )
			//       goto LABEL_24;
			//     if ( wrapperArray.max_length.size <= 0x373u )
			//       goto LABEL_42;
			//     if ( wrapperArray[24].vector[19] )
			//     {
			//       Patch = IFix::WrappersManagerImpl::GetPatch(883, 0LL);
			//       if ( Patch )
			//         return IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_8(
			//                  (ILFixDynamicMethodWrapper_27 *)Patch,
			//                  (Object *)camera,
			//                  0LL);
			//       goto LABEL_24;
			//     }
			//   }
			//   if ( !camera )
			//     goto LABEL_24;
			//   v5 = camera.fields.camera;
			//   if ( !v5 )
			//     goto LABEL_24;
			//   v6 = (unsigned int (__fastcall *)(Camera *))qword_18D8F4200;
			//   if ( !qword_18D8F4200 )
			//   {
			//     v6 = (unsigned int (__fastcall *)(Camera *))il2cpp_resolve_icall_0("UnityEngine.Camera::get_cameraType()");
			//     if ( !v6 )
			//     {
			//       v11 = sub_1802DBBE8("UnityEngine.Camera::get_cameraType()");
			//       sub_18000F750(v11, 0LL);
			//     }
			//     qword_18D8F4200 = (__int64)v6;
			//   }
			//   if ( v6(v5) == 4 )
			//     return 1;
			//   bitDatas = camera.fields._frameSettings_k__BackingField.bitDatas;
			//   P0.bitDatas = bitDatas;
			//   *(_QWORD *)&P0.materialQuality = *(_QWORD *)&camera.fields._frameSettings_k__BackingField.materialQuality;
			//   if ( !byte_18D8EDC37 )
			//   {
			//     sub_18003C530(&TypeInfo::IFix::ILFixDynamicMethodWrapper);
			//     byte_18D8EDC37 = 1;
			//   }
			//   v3 = TypeInfo::IFix::ILFixDynamicMethodWrapper;
			//   if ( !TypeInfo::IFix::ILFixDynamicMethodWrapper._1.cctor_finished_or_no_cctor )
			//   {
			//     il2cpp_runtime_class_init_0(TypeInfo::IFix::ILFixDynamicMethodWrapper, v7);
			//     v3 = TypeInfo::IFix::ILFixDynamicMethodWrapper;
			//   }
			//   wrapperArray = v3.static_fields.wrapperArray;
			//   if ( !wrapperArray )
			//     goto LABEL_24;
			//   if ( wrapperArray.max_length.size > 679 )
			//   {
			//     if ( !v3._1.cctor_finished_or_no_cctor )
			//     {
			//       il2cpp_runtime_class_init_0(v3, wrapperArray);
			//       v3 = TypeInfo::IFix::ILFixDynamicMethodWrapper;
			//     }
			//     v3 = (struct ILFixDynamicMethodWrapper_2__Class *)v3.static_fields.wrapperArray;
			//     if ( !v3 )
			//       goto LABEL_24;
			//     if ( LODWORD(v3._0.namespaze) > 0x2A7 )
			//     {
			//       if ( !v3[14]._1.typeHierarchy )
			//         goto LABEL_19;
			//       v12 = IFix::WrappersManagerImpl::GetPatch(679, 0LL);
			//       if ( v12 )
			//       {
			//         v8 = IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_256(v12, &P0, FrameSettingsField__Enum_Postprocess, 0LL);
			//         goto LABEL_20;
			//       }
			// LABEL_24:
			//       sub_180B536AC(v3, wrapperArray);
			//     }
			// LABEL_42:
			//     sub_180070270(v3, wrapperArray);
			//   }
			// LABEL_19:
			//   v8 = (bitDatas.data1 & 0x8000) != 0LL;
			// LABEL_20:
			//   if ( v8 )
			//   {
			//     if ( !TypeInfo::UnityEngine::Rendering::CoreUtils._1.cctor_finished_or_no_cctor )
			//       il2cpp_runtime_class_init_0(TypeInfo::UnityEngine::Rendering::CoreUtils, wrapperArray);
			//     return 1;
			//   }
			//   return 0;
			// }
			// 
			return default(bool);
		}

		private static float GetEV100AtSlot(Vector2 exposureEv100Range, int slot)
		{
			// // Single GetEV100AtSlot(Vector2, Int32)
			// float HG::Rendering::Runtime::UberPostPassUtils::GetEV100AtSlot(
			//         Vector2 exposureEv100Range,
			//         int32_t slot,
			//         MethodInfo *method)
			// {
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   __int64 v7; // rdx
			//   __int64 v8; // rcx
			// 
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2390, 0LL) )
			//     return (float)((float)((float)(exposureEv100Range.y - exposureEv100Range.x) * 0.0625) * (float)slot)
			//          + exposureEv100Range.x;
			//   Patch = IFix::WrappersManagerImpl::GetPatch(2390, 0LL);
			//   if ( !Patch )
			//     sub_180B536AC(v8, v7);
			//   return IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_864(Patch, exposureEv100Range, slot, 0LL);
			// }
			// 
			return 0f;
		}

		private static float CalculateAverageLogLuminance(Vector2 exposureEv100Range, Vector2 exposureEv100HistogramRange, Vector2 pixelLuminanceRange, int threadGroupsX, int threadGroupsY)
		{
			// // Single CalculateAverageLogLuminance(Vector2, Vector2, Vector2, Int32, Int32)
			// float HG::Rendering::Runtime::UberPostPassUtils::CalculateAverageLogLuminance(
			//         Vector2 exposureEv100Range,
			//         Vector2 exposureEv100HistogramRange,
			//         Vector2 pixelLuminanceRange,
			//         int32_t threadGroupsX,
			//         int32_t threadGroupsY,
			//         MethodInfo *method)
			// {
			//   int32_t v10; // ebx
			//   int v11; // r14d
			//   float v12; // xmm8_4
			//   float v13; // xmm7_4
			//   float EV100AtSlot; // xmm0_4
			//   int v15; // esi
			//   float i; // xmm6_4
			//   ILFixDynamicMethodWrapper_2 *Patch; // rcx
			//   MonitorData *monitor; // rdx
			//   int v19; // eax
			//   int32_t v20; // edi
			//   int v21; // esi
			//   int v22; // r12d
			//   float v23; // xmm6_4
			//   float v24; // xmm11_4
			//   float v25; // xmm12_4
			//   float v26; // xmm0_4
			//   float v27; // xmm10_4
			//   int v28; // ebx
			//   float v29; // xmm7_4
			//   int v30; // eax
			//   float v31; // xmm0_4
			//   float v32; // xmm1_4
			//   float v33; // xmm0_4
			//   float v34; // xmm0_4
			// 
			//   if ( !byte_18D9194BD )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     byte_18D9194BD = 1;
			//   }
			//   if ( IFix::WrappersManagerImpl::IsPatched(2391, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2391, 0LL);
			//     if ( Patch )
			//       return IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_865(
			//                Patch,
			//                exposureEv100Range,
			//                exposureEv100HistogramRange,
			//                pixelLuminanceRange,
			//                threadGroupsX,
			//                threadGroupsY,
			//                0LL);
			// LABEL_35:
			//     sub_180B536AC(Patch, monitor);
			//   }
			//   v10 = 0;
			//   v11 = 0;
			//   v12 = 0.0;
			//   v13 = 0.0;
			//   do
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     EV100AtSlot = HG::Rendering::Runtime::UberPostPassUtils::GetEV100AtSlot(exposureEv100Range, v10, 0LL);
			//     if ( exposureEv100HistogramRange.x <= EV100AtSlot && EV100AtSlot <= exposureEv100HistogramRange.y )
			//     {
			//       v15 = 0;
			//       for ( i = 0.0; v15 < threadGroupsY * threadGroupsX; i = i + *((float *)monitor + v19 + 8) )
			//       {
			//         sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//         Patch = (ILFixDynamicMethodWrapper_2 *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//         monitor = Patch.monitor;
			//         if ( !monitor )
			//           goto LABEL_35;
			//         v19 = v11 + v15;
			//         if ( (unsigned int)(v11 + v15) >= *((_DWORD *)monitor + 6) )
			// LABEL_33:
			//           sub_180070270(Patch, monitor);
			//         ++v15;
			//       }
			//       v13 = v13 + i;
			//     }
			//     ++v10;
			//     v11 += threadGroupsY * threadGroupsX;
			//   }
			//   while ( v10 < 16 );
			//   v20 = 0;
			//   v21 = 0;
			//   v22 = threadGroupsY * threadGroupsX;
			//   v23 = 0.0;
			//   v24 = pixelLuminanceRange.x * v13;
			//   v25 = pixelLuminanceRange.y * v13;
			//   do
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     v26 = HG::Rendering::Runtime::UberPostPassUtils::GetEV100AtSlot(exposureEv100Range, v20, 0LL);
			//     v27 = v26;
			//     if ( exposureEv100HistogramRange.x <= v26 && v26 <= exposureEv100HistogramRange.y )
			//     {
			//       v28 = 0;
			//       v29 = 0.0;
			//       if ( v22 > 0 )
			//       {
			//         do
			//         {
			//           sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//           Patch = (ILFixDynamicMethodWrapper_2 *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//           monitor = Patch.monitor;
			//           if ( !monitor )
			//             goto LABEL_35;
			//           v30 = v21 + v28;
			//           if ( (unsigned int)(v21 + v28) >= *((_DWORD *)monitor + 6) )
			//             goto LABEL_33;
			//           ++v28;
			//           v29 = v29 + *((float *)monitor + v30 + 8);
			//         }
			//         while ( v28 < v22 );
			//         if ( v29 != 0.0 )
			//         {
			//           v31 = v29 + v23;
			//           if ( v23 <= v24 )
			//             v32 = v24;
			//           else
			//             v32 = v23;
			//           if ( v25 <= v31 )
			//             v31 = v25;
			//           v33 = v31 - v32;
			//           if ( v33 < 0.0 )
			//             v33 = 0.0;
			//           v23 = v23 + v29;
			//           v12 = v12 + (float)((float)(v33 / v29) * (float)(v29 * v27));
			//         }
			//       }
			//     }
			//     ++v20;
			//     v21 += v22;
			//   }
			//   while ( v20 < 16 );
			//   v34 = 1.0;
			//   if ( v23 >= 1.0 )
			//     v34 = v23;
			//   return v12 / v34;
			// }
			// 
			return 0f;
		}

		internal static void PrepareAutoExposureDataFromEnvVolume(HGCamera hgCamera, HGAutoExposureConfig aeConfig, ref UberPostPassUtils.HGAutoExposureData data)
		{
			// // Void PrepareAutoExposureDataFromEnvVolume(HGCamera, HGAutoExposureConfig, UberPostPassUtils+HGAutoExposureData ByRef)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareAutoExposureDataFromEnvVolume(
			//         HGCamera *hgCamera,
			//         HGAutoExposureConfig *aeConfig,
			//         UberPostPassUtils_HGAutoExposureData **data,
			//         MethodInfo *method)
			// {
			//   float deltaTime; // xmm6_4
			//   void *v8; // rdx
			//   PassConstructorID__Enum__Array *v9; // r8
			//   HGCamera *v10; // r9
			//   char *static_fields; // rcx
			//   HGCamera *v12; // r9
			//   HGRenderPathBase_HGRenderPathResources *v13; // rdx
			//   PassConstructorID__Enum__Array *v14; // r8
			//   bool v15; // zf
			//   UberPostPassUtils_HGAutoExposureData *v16; // rsi
			//   HGRenderPipeline *currentPipeline; // rax
			//   HGRenderPipelineRuntimeResources *defaultResources; // rax
			//   PassConstructorID__Enum__Array *v19; // r8
			//   HGCamera *v20; // r9
			//   HGRenderPipelineRuntimeResources_ShaderResources *shaders; // rax
			//   UberPostPassUtils_HGAutoExposureData *v22; // rsi
			//   int32_t Kernel; // eax
			//   __int64 v24; // xmm0_8
			//   ComputeShader *v25; // rsi
			//   ComputeShader *v26; // rsi
			//   UberPostPassUtils_HGAutoExposureData *v27; // rsi
			//   UberPostPassUtils_HGAutoExposureData *v28; // rsi
			//   struct UberPostPassUtils__Class *v29; // rcx
			//   int32_t count; // eax
			//   UberPostPassUtils_HGAutoExposureData *v31; // rax
			//   int32_t threadGroupsX; // r12d
			//   int32_t threadGroupsY; // esi
			//   ComputeBuffer *v34; // rax
			//   ComputeBuffer *v35; // r15
			//   HGRenderPathBase_HGRenderPathResources *v36; // rdx
			//   PassConstructorID__Enum__Array *v37; // r8
			//   HGCamera *v38; // r9
			//   __int64 v39; // r8
			//   __int64 v40; // rax
			//   HGRenderPathBase_HGRenderPathResources *v41; // rdx
			//   PassConstructorID__Enum__Array *v42; // r8
			//   HGCamera *v43; // r9
			//   UberPostPassUtils_HGAutoExposureData *v44; // rsi
			//   PassConstructorID__Enum__Array *v45; // r8
			//   HGCamera *v46; // r9
			//   UberPostPassUtils_HGAutoExposureData *v47; // rsi
			//   bool IsActive; // al
			//   PassConstructorID__Enum__Array *v49; // r8
			//   HGCamera *v50; // r9
			//   __int64 v51; // xmm0_8
			//   __m128i v52; // xmm2
			//   __m128 v53; // xmm2
			//   __int64 v54; // xmm1_8
			//   __m128 v55; // xmm2
			//   unsigned __int32 v56; // xmm0_4
			//   __m128 v57; // xmm2
			//   float y; // xmm0_4
			//   __m128i v59; // xmm2
			//   UberPostPassUtils_HGAutoExposureData *v60; // rbx
			//   int v61; // eax
			//   __int128 v62; // xmm1
			//   __int128 v63; // xmm0
			//   MethodInfo *v64; // [rsp+28h] [rbp-29h]
			//   MethodInfo *v65; // [rsp+28h] [rbp-29h]
			//   MethodInfo *v66; // [rsp+28h] [rbp-29h]
			//   MethodInfo *v67; // [rsp+28h] [rbp-29h]
			//   MethodInfo *v68; // [rsp+28h] [rbp-29h]
			//   MethodInfo *v69; // [rsp+30h] [rbp-21h]
			//   MethodInfo *v70; // [rsp+30h] [rbp-21h]
			//   MethodInfo *v71; // [rsp+30h] [rbp-21h]
			//   MethodInfo *v72; // [rsp+30h] [rbp-21h]
			//   MethodInfo *v73; // [rsp+30h] [rbp-21h]
			//   HGAutoExposureConfig v74; // [rsp+48h] [rbp-9h] BYREF
			// 
			//   if ( !byte_18D9194BE )
			//   {
			//     sub_18003C530(&TypeInfo::UnityEngine::ComputeBuffer);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGAutoExposureConfig);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGRenderPipeline);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//     sub_18003C530(&TypeInfo::System::Single);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     sub_18003C530(&off_18D4F3920);
			//     byte_18D9194BE = 1;
			//   }
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2392, 0LL) )
			//   {
			//     deltaTime = UnityEngine::Time::get_deltaTime(0LL);
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     static_fields = (char *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//     *((float *)static_fields + 8) = deltaTime;
			//     if ( !hgCamera )
			//       goto LABEL_55;
			//     TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.m_currentExposure = hgCamera.fields.exposureAdaptation;
			//     TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.m_targetExposure = hgCamera.fields.exposureTargetAdaptation;
			//     TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.autoExposure_histogramBuffer = hgCamera.fields.autoExposureHistogramBuffer;
			//     sub_1800054D0(
			//       (HGRenderPathScene *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields,
			//       (HGRenderPathBase_HGRenderPathResources *)v8,
			//       v9,
			//       v10,
			//       v64,
			//       v69);
			//     v12 = (HGCamera *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils;
			//     TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.m_autoExposure_histogramRes = hgCamera.fields.autoExposureHistogramRes;
			//     sub_1800054D0(
			//       (HGRenderPathScene *)&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.m_autoExposure_histogramRes,
			//       v13,
			//       v14,
			//       v12,
			//       v65,
			//       v70);
			//     v15 = aeConfig.autoExposureMode == 0;
			//     *(_QWORD *)&v74.autoExposureEvClampRange.y = *(_QWORD *)&aeConfig.autoExposureEvClampRange.y;
			//     if ( !v15 )
			//     {
			// LABEL_38:
			//       v47 = *data;
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGAutoExposureConfig);
			//       IsActive = HG::Rendering::Runtime::HGAutoExposureConfig::IsActive(aeConfig, 0LL);
			//       if ( v47 )
			//       {
			//         v51 = *(_QWORD *)&aeConfig.autoExposureEvClampRange.y;
			//         v52 = *(__m128i *)&aeConfig.autoExposureMode;
			//         v47.fields.autoExposureActive = IsActive;
			//         static_fields = (char *)*data;
			//         *(_QWORD *)&v74.autoExposureEvClampRange.y = v51;
			//         if ( static_fields )
			//         {
			//           *((_DWORD *)static_fields + 15) = _mm_cvtsi128_si32(v52);
			//           static_fields = (char *)*data;
			//           *(_QWORD *)&v74.autoExposureEvClampRange.y = v51;
			//           v53 = *(__m128 *)&aeConfig.autoExposureEv100Range.x;
			//           if ( static_fields )
			//           {
			//             *((_DWORD *)static_fields + 16) = v53.m128_i32[0];
			//             v54 = v51;
			//             *((_DWORD *)static_fields + 17) = _mm_shuffle_ps(v53, v53, 85).m128_u32[0];
			//             static_fields = (char *)*data;
			//             *(_QWORD *)&v74.autoExposureEvClampRange.y = v51;
			//             v55 = *(__m128 *)&aeConfig.autoExposureEv100Range.x;
			//             if ( static_fields )
			//             {
			//               *(_QWORD *)&v74.autoExposureEvClampRange.y = v51;
			//               v56 = _mm_shuffle_ps(v55, v55, 170).m128_u32[0];
			//               *((_DWORD *)static_fields + 19) = _mm_shuffle_ps(v55, v55, 255).m128_u32[0];
			//               v57 = *(__m128 *)&aeConfig.autoExposureMeteringMode;
			//               *((_DWORD *)static_fields + 18) = v56;
			//               static_fields = (char *)*data;
			//               if ( *data )
			//               {
			//                 *((_DWORD *)static_fields + 21) = _mm_shuffle_ps(v57, v57, 170).m128_u32[0];
			//                 *((_DWORD *)static_fields + 20) = _mm_shuffle_ps(v57, v57, 85).m128_u32[0];
			//                 if ( *data )
			//                 {
			//                   (*data).fields.centerPixelWeight = 0.75;
			//                   static_fields = (char *)*data;
			//                   *(_QWORD *)&v74.autoExposureEvClampRange.y = v54;
			//                   if ( static_fields )
			//                   {
			//                     *((_DWORD *)static_fields + 23) = LODWORD(v74.autoExposureManualEvCompensationManual);
			//                     static_fields = (char *)*data;
			//                     *(_QWORD *)&v74.autoExposureEvClampRange.y = v54;
			//                     if ( static_fields )
			//                     {
			//                       *((_DWORD *)static_fields + 24) = LODWORD(aeConfig.autoExposureManualEvCompensationAuto);
			//                       static_fields = (char *)*data;
			//                       *(_QWORD *)&v74.autoExposureEvClampRange.y = v54;
			//                       if ( static_fields )
			//                       {
			//                         y = v74.autoExposureEvClampRange.y;
			//                         *((_DWORD *)static_fields + 25) = LODWORD(aeConfig.autoExposureEvClampRange.x);
			//                         *((float *)static_fields + 26) = y;
			//                         static_fields = (char *)*data;
			//                         if ( *data )
			//                         {
			//                           *((_DWORD *)static_fields + 27) = LODWORD(aeConfig.autoExposureLerpDownSpeed);
			//                           static_fields = (char *)*data;
			//                           if ( *data )
			//                           {
			//                             *((_DWORD *)static_fields + 28) = LODWORD(aeConfig.autoExposureLerpUpSpeed);
			//                             if ( *data )
			//                             {
			//                               v59 = *(__m128i *)&aeConfig.autoExposureMeteringMode;
			//                               (*data).fields.curveEditModeEnabled = 0;
			//                               static_fields = (char *)*data;
			//                               if ( *data )
			//                               {
			//                                 *((_DWORD *)static_fields + 30) = _mm_cvtsi128_si32(v59);
			//                                 static_fields = (char *)*data;
			//                                 if ( *data )
			//                                 {
			//                                   *((_QWORD *)static_fields + 18) = hgCamera;
			//                                   sub_1800054D0(
			//                                     (HGRenderPathScene *)(static_fields + 144),
			//                                     (HGRenderPathBase_HGRenderPathResources *)v8,
			//                                     v49,
			//                                     v50,
			//                                     v66,
			//                                     v71);
			//                                   v60 = *data;
			//                                   sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//                                   static_fields = (char *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//                                   if ( v60 )
			//                                   {
			//                                     v60.fields.deltaTime = *((float *)static_fields + 8);
			//                                     return;
			//                                   }
			//                                 }
			//                               }
			//                             }
			//                           }
			//                         }
			//                       }
			//                     }
			//                   }
			//                 }
			//               }
			//             }
			//           }
			//         }
			//       }
			// LABEL_55:
			//       sub_180B536AC(static_fields, v8);
			//     }
			//     v16 = *data;
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGRenderPipeline);
			//     currentPipeline = HG::Rendering::Runtime::HGRenderPipeline::get_currentPipeline(0LL);
			//     if ( !currentPipeline )
			//       goto LABEL_55;
			//     defaultResources = HG::Rendering::Runtime::HGRenderPipeline::get_defaultResources(currentPipeline, 0LL);
			//     if ( !defaultResources )
			//       goto LABEL_55;
			//     shaders = defaultResources.fields.shaders;
			//     if ( !shaders )
			//       goto LABEL_55;
			//     static_fields = (char *)shaders.fields.hgAutoExposureHistogramCS;
			//     if ( !v16 )
			//       goto LABEL_55;
			//     v16.fields.autoExposureHistogramCS = (ComputeShader *)static_fields;
			//     sub_1800054D0(
			//       (HGRenderPathScene *)&v16.fields.autoExposureHistogramCS,
			//       (HGRenderPathBase_HGRenderPathResources *)v8,
			//       v19,
			//       v20,
			//       v66,
			//       v71);
			//     v22 = *data;
			//     if ( !*data )
			//       goto LABEL_55;
			//     static_fields = (char *)v22.fields.autoExposureHistogramCS;
			//     if ( !static_fields )
			//       goto LABEL_55;
			//     Kernel = UnityEngine::ComputeShader::FindKernel(
			//                (ComputeShader *)static_fields,
			//                (String *)"HGLuminanceHistogramCS",
			//                0LL);
			//     v15 = aeConfig.autoExposureMeteringMode == 1;
			//     v24 = *(_QWORD *)&aeConfig.autoExposureEvClampRange.y;
			//     v22.fields.histogramKernel = Kernel;
			//     static_fields = (char *)*data;
			//     *(_QWORD *)&v74.autoExposureEvClampRange.y = v24;
			//     if ( v15 )
			//     {
			//       if ( !static_fields )
			//         goto LABEL_55;
			//       v26 = (ComputeShader *)*((_QWORD *)static_fields + 4);
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//       v8 = TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//       if ( !v26 )
			//         goto LABEL_55;
			//       UnityEngine::ComputeShader::EnableKeyword(v26, *((String **)v8 + 45), 0LL);
			//     }
			//     else
			//     {
			//       if ( !static_fields )
			//         goto LABEL_55;
			//       v25 = (ComputeShader *)*((_QWORD *)static_fields + 4);
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//       v8 = TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//       if ( !v25 )
			//         goto LABEL_55;
			//       UnityEngine::ComputeShader::DisableKeyword(v25, *((String **)v8 + 45), 0LL);
			//     }
			//     static_fields = (char *)*data;
			//     if ( !*data )
			//       goto LABEL_55;
			//     *((_DWORD *)static_fields + 31) = hgCamera.fields._taauRTSize_k__BackingField;
			//     static_fields = (char *)*data;
			//     if ( !*data )
			//       goto LABEL_55;
			//     *((_DWORD *)static_fields + 32) = HIDWORD(*(_QWORD *)&hgCamera.fields._taauRTSize_k__BackingField);
			//     if ( !*data )
			//       goto LABEL_55;
			//     v27 = *data;
			//     v27.fields.threadGroupsX = sub_1826E82D0();
			//     if ( !*data )
			//       goto LABEL_55;
			//     v28 = *data;
			//     v28.fields.threadGroupsY = sub_1826E82D0();
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     v29 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils;
			//     if ( TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.autoExposure_histogramBuffer )
			//     {
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       static_fields = (char *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.autoExposure_histogramBuffer;
			//       if ( !static_fields )
			//         goto LABEL_55;
			//       if ( UnityEngine::ComputeBuffer::IsValid((ComputeBuffer *)static_fields, 0LL) )
			//       {
			//         sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//         static_fields = (char *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.autoExposure_histogramBuffer;
			//         if ( !static_fields )
			//           goto LABEL_55;
			//         count = UnityEngine::ComputeBuffer::get_count((ComputeBuffer *)static_fields, 0LL);
			//         v8 = *data;
			//         if ( !*data )
			//           goto LABEL_55;
			//         if ( count == 16 * (*data).fields.threadGroupsY * *((_DWORD *)v8 + 33) )
			//         {
			// LABEL_36:
			//           v44 = *data;
			//           sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//           static_fields = (char *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//           if ( !v44 )
			//             goto LABEL_55;
			//           v44.fields.histogramBuffer = *(ComputeBuffer **)static_fields;
			//           sub_1800054D0(
			//             (HGRenderPathScene *)&v44.fields.histogramBuffer,
			//             (HGRenderPathBase_HGRenderPathResources *)v8,
			//             v45,
			//             v46,
			//             v67,
			//             v72);
			//           goto LABEL_38;
			//         }
			//       }
			//       v29 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils;
			//     }
			//     sub_180002C70(v29);
			//     static_fields = (char *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.autoExposure_histogramBuffer;
			//     if ( static_fields )
			//       UnityEngine::ComputeBuffer::Dispose((ComputeBuffer *)static_fields, 0LL);
			//     v31 = *data;
			//     if ( !*data )
			//       goto LABEL_55;
			//     threadGroupsX = v31.fields.threadGroupsX;
			//     threadGroupsY = v31.fields.threadGroupsY;
			//     v34 = (ComputeBuffer *)sub_180004920(TypeInfo::UnityEngine::ComputeBuffer);
			//     v35 = v34;
			//     if ( !v34 )
			//       goto LABEL_55;
			//     UnityEngine::ComputeBuffer::ComputeBuffer(v34, 16 * threadGroupsX * threadGroupsY, 4, 0LL);
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.autoExposure_histogramBuffer = v35;
			//     sub_1800054D0(
			//       (HGRenderPathScene *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields,
			//       v36,
			//       v37,
			//       v38,
			//       v67,
			//       v72);
			//     if ( !*data )
			//       goto LABEL_55;
			//     v40 = il2cpp_array_new_specific_0(
			//             TypeInfo::System::Single,
			//             (unsigned int)(16 * (*data).fields.threadGroupsX * (*data).fields.threadGroupsY),
			//             v39,
			//             *data);
			//     v41 = (HGRenderPathBase_HGRenderPathResources *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//     v41.settingParameters = (HGSettingParameters *)v40;
			//     sub_1800054D0(
			//       (HGRenderPathScene *)&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.m_autoExposure_histogramRes,
			//       v41,
			//       v42,
			//       v43,
			//       v68,
			//       v73);
			//     goto LABEL_36;
			//   }
			//   static_fields = (char *)IFix::WrappersManagerImpl::GetPatch(2392, 0LL);
			//   if ( !static_fields )
			//     goto LABEL_55;
			//   v61 = *(_DWORD *)&aeConfig.m_active;
			//   v62 = *(_OWORD *)&aeConfig.autoExposureEv100Range.x;
			//   *(_OWORD *)&v74.autoExposureMode = *(_OWORD *)&aeConfig.autoExposureMode;
			//   v63 = *(_OWORD *)&aeConfig.autoExposureMeteringMode;
			//   *(_DWORD *)&v74.m_active = v61;
			//   *(_OWORD *)&v74.autoExposureEv100Range.x = v62;
			//   *(_QWORD *)&v62 = *(_QWORD *)&aeConfig.autoExposureEvClampRange.y;
			//   *(_OWORD *)&v74.autoExposureMeteringMode = v63;
			//   *(_QWORD *)&v74.autoExposureEvClampRange.y = v62;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_866(
			//     (ILFixDynamicMethodWrapper_2 *)static_fields,
			//     (Object *)hgCamera,
			//     &v74,
			//     data,
			//     0LL);
			// }
			// 
		}

		internal static void AutoExposureUpdateData(ref UberPostPassUtils.HGAutoExposureData data)
		{
			// // Void AutoExposureUpdateData(UberPostPassUtils+HGAutoExposureData ByRef)
			// void HG::Rendering::Runtime::UberPostPassUtils::AutoExposureUpdateData(
			//         UberPostPassUtils_HGAutoExposureData **data,
			//         MethodInfo *method)
			// {
			//   char *v3; // rdx
			//   float *v4; // rcx
			//   UberPostPassUtils_HGAutoExposureData *v5; // rax
			//   UberPostPassUtils__StaticFields *static_fields; // rax
			//   float v7; // xmm6_4
			//   double v8; // xmm0_8
			//   float v9; // xmm6_4
			//   struct UberPostPassUtils__Class *v10; // rcx
			//   ComputeBuffer *histogramBuffer; // rsi
			//   Action_1_UnityEngine_Rendering_AsyncGPUReadbackRequest_ *_9__23_0; // rbx
			//   Object *v13; // rbp
			//   GenericDelegateCallerGen_FStructSize56_voidDelegate_1_Unity_Collections_NativeArray_1_System_Int32_ *v14; // rax
			//   HGRenderPathBase_HGRenderPathResources *v15; // rdx
			//   PassConstructorID__Enum__Array *v16; // r8
			//   HGCamera *v17; // r9
			//   unsigned int *v18; // rax
			//   __m128 v19; // xmm6
			//   __m128 v20; // xmm7
			//   __m128 v21; // xmm9
			//   __m128 v22; // xmm10
			//   __m128 v23; // xmm11
			//   __m128 v24; // xmm12
			//   float v25; // xmm13_4
			//   int32_t v26; // esi
			//   int32_t v27; // ebx
			//   struct UberPostPassUtils__Class *v28; // rbx
			//   float m_targetExposure; // xmm6_4
			//   double v30; // xmm0_8
			//   float v31; // xmm7_4
			//   __int64 v32; // rdx
			//   int v33; // ecx
			//   double v34; // xmm0_8
			//   UberPostPassUtils_HGAutoExposureData *v35; // rax
			//   Beyond::JobMathf *v36; // rcx
			//   double v37; // xmm0_8
			//   __int64 v38; // rax
			//   HGCamera *hgCamera; // rbx
			//   PassConstructorID__Enum__Array *v40; // r8
			//   HGCamera *v41; // r9
			//   PassConstructorID__Enum__Array *v42; // r8
			//   HGCamera *v43; // r9
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   MethodInfo *threadGroupsX; // [rsp+20h] [rbp-A8h]
			//   MethodInfo *threadGroupsXa; // [rsp+20h] [rbp-A8h]
			//   MethodInfo *threadGroupsY; // [rsp+28h] [rbp-A0h]
			//   MethodInfo *threadGroupsYa; // [rsp+28h] [rbp-A0h]
			//   AsyncGPUReadbackRequest v49; // [rsp+40h] [rbp-88h] BYREF
			// 
			//   if ( !byte_18D9194BF )
			//   {
			//     sub_18003C530(&TypeInfo::System::Action<UnityEngine::Rendering::AsyncGPUReadbackRequest>);
			//     sub_18003C530(&MethodInfo::HG::Rendering::Runtime::UberPostPassUtils::__c::_AutoExposureUpdateData_b__23_0);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils::__c);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     byte_18D9194BF = 1;
			//   }
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2393, 0LL) )
			//   {
			//     v5 = *data;
			//     if ( !*data )
			//       goto LABEL_38;
			//     if ( !v5.fields.autoExposureActive )
			//     {
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.m_targetExposure = 1.0;
			//       v4 = (float *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils;
			//       static_fields = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//       v7 = static_fields.m_autoExposure_deltaTime * 0.60000002;
			//       if ( static_fields.m_targetExposure > static_fields.m_currentExposure )
			//       {
			//         sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//         v4 = (float *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils;
			//         v7 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.m_autoExposure_deltaTime * 0.40000001;
			//       }
			// LABEL_28:
			//       sub_180002C70(v4);
			//       v37 = Beyond::JobMathf::ClampedLerp(
			//               v36,
			//               TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.m_targetExposure,
			//               v7);
			//       *(_DWORD *)(v38 + 16) = LODWORD(v37);
			//       if ( *data )
			//       {
			//         hgCamera = (*data).fields.hgCamera;
			//         sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//         v4 = (float *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//         if ( hgCamera )
			//         {
			//           hgCamera.fields.exposureTargetAdaptation = v4[5];
			//           if ( *data )
			//           {
			//             v3 = (char *)(*data).fields.hgCamera;
			//             v4 = (float *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//             if ( v3 )
			//             {
			//               *((float *)v3 + 434) = v4[4];
			//               v3 = (char *)*data;
			//               if ( *data )
			//               {
			//                 v3 = (char *)*((_QWORD *)v3 + 18);
			//                 v4 = (float *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//                 if ( v3 )
			//                 {
			//                   *((_QWORD *)v3 + 214) = *(_QWORD *)v4;
			//                   sub_1800054D0(
			//                     (HGRenderPathScene *)(v3 + 1712),
			//                     (HGRenderPathBase_HGRenderPathResources *)v3,
			//                     v40,
			//                     v41,
			//                     threadGroupsX,
			//                     threadGroupsY);
			//                   v3 = (char *)*data;
			//                   if ( *data )
			//                   {
			//                     v3 = (char *)*((_QWORD *)v3 + 18);
			//                     v4 = (float *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//                     if ( v3 )
			//                     {
			//                       *((_QWORD *)v3 + 215) = *((_QWORD *)v4 + 1);
			//                       sub_1800054D0(
			//                         (HGRenderPathScene *)(v3 + 1720),
			//                         (HGRenderPathBase_HGRenderPathResources *)v3,
			//                         v42,
			//                         v43,
			//                         threadGroupsXa,
			//                         threadGroupsYa);
			//                       return;
			//                     }
			//                   }
			//                 }
			//               }
			//             }
			//           }
			//         }
			//       }
			//       goto LABEL_38;
			//     }
			//     if ( !v5 )
			//       goto LABEL_38;
			//     if ( v5.fields.autoExposureMode )
			//     {
			//       v8 = sub_1802EB1B0((_DWORD)v4, v3);
			//       v9 = *(float *)&v8;
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.m_targetExposure = v9;
			//     }
			//     else
			//     {
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       v10 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils;
			//       if ( !TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.m_autoExposureReadyForNextFetch )
			//         goto LABEL_19;
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       v4 = (float *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//       *((_BYTE *)v4 + 36) = 0;
			//       if ( !*data )
			//         goto LABEL_38;
			//       histogramBuffer = (*data).fields.histogramBuffer;
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils::__c);
			//       _9__23_0 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils::__c.static_fields.__9__23_0;
			//       if ( !_9__23_0 )
			//       {
			//         sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils::__c);
			//         v13 = (Object *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils::__c.static_fields.__9;
			//         v14 = (GenericDelegateCallerGen_FStructSize56_voidDelegate_1_Unity_Collections_NativeArray_1_System_Int32_ *)sub_180004920(TypeInfo::System::Action<UnityEngine::Rendering::AsyncGPUReadbackRequest>);
			//         _9__23_0 = (Action_1_UnityEngine_Rendering_AsyncGPUReadbackRequest_ *)v14;
			//         if ( !v14 )
			//           goto LABEL_38;
			//         Beyond::Reflection::GenericDelegateCallerGen::FStructSize56_voidDelegate<Unity::Collections::NativeArray<int>>::FStructSize56_voidDelegate(
			//           v14,
			//           v13,
			//           MethodInfo::HG::Rendering::Runtime::UberPostPassUtils::__c::_AutoExposureUpdateData_b__23_0,
			//           0LL);
			//         TypeInfo::HG::Rendering::Runtime::UberPostPassUtils::__c.static_fields.__9__23_0 = _9__23_0;
			//         sub_1800054D0(
			//           (HGRenderPathScene *)&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils::__c.static_fields.__9__23_0,
			//           v15,
			//           v16,
			//           v17,
			//           threadGroupsX,
			//           threadGroupsY);
			//       }
			//       UnityEngine::Rendering::AsyncGPUReadback::Request(&v49, histogramBuffer, _9__23_0, 0LL);
			//       v18 = (unsigned int *)*data;
			//       if ( !*data )
			// LABEL_38:
			//         sub_180B536AC(v4, v3);
			//       v19 = (__m128)v18[16];
			//       v20 = (__m128)v18[17];
			//       v21 = (__m128)v18[18];
			//       v22 = (__m128)v18[19];
			//       v23 = (__m128)v18[20];
			//       v24 = (__m128)v18[21];
			//       v25 = *((float *)v18 + 24);
			//       v26 = v18[33];
			//       v27 = v18[34];
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       HG::Rendering::Runtime::UberPostPassUtils::CalculateTargetExposure(
			//         (Vector2)*(_OWORD *)&_mm_unpacklo_ps(v19, v20),
			//         (Vector2)*(_OWORD *)&_mm_unpacklo_ps(v21, v22),
			//         (Vector2)*(_OWORD *)&_mm_unpacklo_ps(v23, v24),
			//         v25,
			//         v26,
			//         v27,
			//         0LL);
			//     }
			//     v10 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils;
			// LABEL_19:
			//     sub_180002C70(v10);
			//     v28 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils;
			//     m_targetExposure = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.m_targetExposure;
			//     if ( !*data )
			//       goto LABEL_38;
			//     v30 = sub_1802EB1B0((_DWORD)v4, v3);
			//     v31 = *(float *)&v30;
			//     v34 = sub_1802EB1B0(v33, v32);
			//     if ( v31 > m_targetExposure )
			//     {
			//       m_targetExposure = v31;
			//     }
			//     else if ( m_targetExposure > *(float *)&v34 )
			//     {
			//       m_targetExposure = *(float *)&v34;
			//     }
			//     v28.static_fields.m_targetExposure = m_targetExposure;
			//     v35 = *data;
			//     if ( !*data )
			//       goto LABEL_38;
			//     v4 = (float *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils;
			//     v7 = v35.fields.lerpDownSpeed * v35.fields.deltaTime;
			//     v3 = (char *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//     if ( *((float *)v3 + 5) > *((float *)v3 + 4) )
			//     {
			//       if ( !v35 )
			//         goto LABEL_38;
			//       v7 = v35.fields.lerpUpSpeed * v35.fields.deltaTime;
			//     }
			//     goto LABEL_28;
			//   }
			//   Patch = IFix::WrappersManagerImpl::GetPatch(2393, 0LL);
			//   if ( !Patch )
			//     goto LABEL_38;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_869(Patch, data, 0LL);
			// }
			// 
		}

		internal static void ExecuteAutoExposure(HGRenderGraph renderGraph, TextureHandle inputTex, HGCamera hgCamera)
		{
			// // Void ExecuteAutoExposure(HGRenderGraph, TextureHandle, HGCamera)
			// // Hidden C++ exception states: #wind=1
			// void HG::Rendering::Runtime::UberPostPassUtils::ExecuteAutoExposure(
			//         HGRenderGraph *renderGraph,
			//         TextureHandle *inputTex,
			//         HGCamera *hgCamera,
			//         MethodInfo *method)
			// {
			//   HGEnvironmentPhase *InterpolatedPhase; // rax
			//   __int64 v8; // rdx
			//   __int64 v9; // rcx
			//   __int128 v10; // xmm7
			//   __int128 v11; // xmm8
			//   __int128 v12; // xmm9
			//   __int64 v13; // xmm6_8
			//   int v14; // ebx
			//   __int64 v15; // rdx
			//   UberPostPassUtils__StaticFields *static_fields; // rcx
			//   UberPostPassUtils_HGAutoExposureData *s_cachedAutoExposureData; // rbx
			//   __int64 v18; // rdx
			//   UberPostPassUtils__StaticFields *v19; // rcx
			//   UberPostPassUtils_HGAutoExposureData *v20; // rbx
			//   ProfilingSampler *v21; // rax
			//   __int64 v22; // rdx
			//   __int64 v23; // rcx
			//   Object *v24; // rbx
			//   __int64 v25; // rdx
			//   UberPostPassUtils__StaticFields *v26; // rcx
			//   UberPostPassUtils_HGAutoExposureData *v27; // rax
			//   Object__Class *autoExposureHistogramCS; // rcx
			//   int v29; // r8d
			//   unsigned __int64 v30; // r9
			//   signed __int64 v31; // rtt
			//   Object *v32; // r9
			//   UberPostPassUtils__StaticFields *v33; // rcx
			//   UberPostPassUtils_HGAutoExposureData *v34; // rax
			//   Object__Class *histogramBuffer; // rcx
			//   char v36; // r8
			//   unsigned __int64 v37; // r9
			//   char v38; // r8
			//   signed __int64 v39; // rtt
			//   UberPostPassUtils__StaticFields *v40; // rcx
			//   UberPostPassUtils_HGAutoExposureData *v41; // rax
			//   __int64 histogramKernel; // rcx
			//   UberPostPassUtils__StaticFields *v43; // rcx
			//   UberPostPassUtils_HGAutoExposureData *v44; // rax
			//   __int64 textureSizeX; // rcx
			//   UberPostPassUtils__StaticFields *v46; // rcx
			//   UberPostPassUtils_HGAutoExposureData *v47; // rax
			//   __int64 textureSizeY; // rcx
			//   UberPostPassUtils__StaticFields *v49; // rcx
			//   UberPostPassUtils_HGAutoExposureData *v50; // rax
			//   __int64 threadGroupsX; // rcx
			//   UberPostPassUtils__StaticFields *v52; // rcx
			//   UberPostPassUtils_HGAutoExposureData *v53; // rax
			//   __int64 threadGroupsY; // rcx
			//   UberPostPassUtils__StaticFields *v55; // rcx
			//   UberPostPassUtils_HGAutoExposureData *v56; // rax
			//   float x; // xmm0_4
			//   float y; // xmm1_4
			//   Object *v59; // rax
			//   UberPostPassUtils__StaticFields *v60; // rcx
			//   UberPostPassUtils_HGAutoExposureData *v61; // rax
			//   Object *v62; // rbx
			//   __int64 v63; // rdx
			//   __int64 v64; // rcx
			//   TextureHandle v65; // xmm0
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   __int64 v67; // rdx
			//   __int64 v68; // rcx
			//   Object *v69; // [rsp+40h] [rbp-D8h] BYREF
			//   HGAutoExposureConfig v70; // [rsp+50h] [rbp-C8h] BYREF
			//   HGRenderGraphBuilder v71; // [rsp+90h] [rbp-88h] BYREF
			//   TextureHandle v72; // [rsp+B0h] [rbp-68h] BYREF
			//   Il2CppExceptionWrapper *v73; // [rsp+C0h] [rbp-58h] BYREF
			// 
			//   if ( !byte_18D9194C0 )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGEnvironmentManager);
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::HGAutoExposurePassData>);
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::HGAutoExposurePassData>);
			//     sub_18003C530(&MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     sub_18003C530(&off_18D4F3978);
			//     byte_18D9194C0 = 1;
			//   }
			//   memset(&v71, 0, sizeof(v71));
			//   v69 = 0LL;
			//   if ( IFix::WrappersManagerImpl::IsPatched(2410, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2410, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v68, v67);
			//     *(TextureHandle *)&v70.autoExposureMode = *inputTex;
			//     IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_404(
			//       Patch,
			//       (Object *)renderGraph,
			//       (TextureHandle *)&v70,
			//       (Object *)hgCamera,
			//       0LL);
			//   }
			//   else
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGEnvironmentManager);
			//     InterpolatedPhase = HG::Rendering::Runtime::HGEnvironmentManager::GetInterpolatedPhase(hgCamera, 0LL);
			//     if ( !InterpolatedPhase )
			//       sub_180B536AC(v9, v8);
			//     v10 = *(_OWORD *)&InterpolatedPhase.fields.autoExposureConfig.autoExposureMode;
			//     v11 = *(_OWORD *)&InterpolatedPhase.fields.autoExposureConfig.autoExposureEv100Range.x;
			//     v12 = *(_OWORD *)&InterpolatedPhase.fields.autoExposureConfig.autoExposureMeteringMode;
			//     v13 = *(_QWORD *)&InterpolatedPhase.fields.autoExposureConfig.autoExposureEvClampRange.y;
			//     v14 = *(_DWORD *)&InterpolatedPhase.fields.autoExposureConfig.m_active;
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     *(_OWORD *)&v70.autoExposureMode = v10;
			//     *(_OWORD *)&v70.autoExposureEv100Range.x = v11;
			//     *(_OWORD *)&v70.autoExposureMeteringMode = v12;
			//     *(_QWORD *)&v70.autoExposureEvClampRange.y = v13;
			//     *(_DWORD *)&v70.m_active = v14;
			//     HG::Rendering::Runtime::UberPostPassUtils::PrepareAutoExposureDataFromEnvVolume(
			//       hgCamera,
			//       &v70,
			//       &TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_cachedAutoExposureData,
			//       0LL);
			//     static_fields = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//     s_cachedAutoExposureData = static_fields.s_cachedAutoExposureData;
			//     if ( !s_cachedAutoExposureData )
			//       sub_180B536AC(static_fields, v15);
			//     if ( s_cachedAutoExposureData.fields.autoExposureActive )
			//     {
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       v19 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//       v20 = v19.s_cachedAutoExposureData;
			//       if ( !v20 )
			//         sub_180B536AC(v19, v18);
			//       if ( !v20.fields.autoExposureMode )
			//       {
			//         v21 = UnityEngine::Rendering::ProfilingSampler::Get<System::Int32Enum>(
			//                 (Int32Enum__Enum)0x94u,
			//                 MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//         if ( !renderGraph )
			//           sub_180B536AC(v23, v22);
			//         HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<System::Object>(
			//           (HGRenderGraphBuilder *)&v70,
			//           renderGraph,
			//           (String *)"Auto Exposure",
			//           &v69,
			//           v21,
			//           1,
			//           ProfilingHGPass__Enum_None,
			//           MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::HGAutoExposurePassData>);
			//         *(_OWORD *)&v71.m_RenderPass = *(_OWORD *)&v70.autoExposureMode;
			//         *(_OWORD *)&v71.m_RenderGraph = *(_OWORD *)&v70.autoExposureEv100Range.x;
			//         *(_QWORD *)&v70.autoExposureMode = 0LL;
			//         *(_QWORD *)&v70.autoExposureLerpUpSpeed = &v71;
			//         try
			//         {
			//           v24 = v69;
			//           sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//           v26 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//           v27 = v26.s_cachedAutoExposureData;
			//           if ( !v27 )
			//             sub_1802DC2C8(v26, v25);
			//           autoExposureHistogramCS = (Object__Class *)v27.fields.autoExposureHistogramCS;
			//           if ( !v24 )
			//             sub_1802DC2C8(autoExposureHistogramCS, v25);
			//           v24[2].klass = autoExposureHistogramCS;
			//           v29 = dword_18D8E43F8;
			//           if ( dword_18D8E43F8 )
			//           {
			//             v30 = (((unsigned __int64)&v24[2] >> 12) & 0x1FFFFF) >> 6;
			//             _m_prefetchw(&qword_18D6405E0[v30 + 36190]);
			//             do
			//               v31 = qword_18D6405E0[v30 + 36190];
			//             while ( v31 != _InterlockedCompareExchange64(
			//                              &qword_18D6405E0[v30 + 36190],
			//                              v31 | (1LL << (((unsigned __int64)&v24[2] >> 12) & 0x3F)),
			//                              v31) );
			//             v29 = dword_18D8E43F8;
			//           }
			//           v32 = v69;
			//           v33 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//           v34 = v33.s_cachedAutoExposureData;
			//           if ( !v34 )
			//             sub_1802DC2C8(v33, qword_18D6405E0);
			//           histogramBuffer = (Object__Class *)v34.fields.histogramBuffer;
			//           if ( !v69 )
			//             sub_1802DC2C8(histogramBuffer, qword_18D6405E0);
			//           v69[3].klass = histogramBuffer;
			//           if ( v29 )
			//           {
			//             v36 = (unsigned __int64)&v32[3] >> 12;
			//             v37 = (((unsigned __int64)&v32[3] >> 12) & 0x1FFFFF) >> 6;
			//             v38 = v36 & 0x3F;
			//             _m_prefetchw(&qword_18D6405E0[v37 + 36190]);
			//             do
			//               v39 = qword_18D6405E0[v37 + 36190];
			//             while ( v39 != _InterlockedCompareExchange64(&qword_18D6405E0[v37 + 36190], v39 | (1LL << v38), v39) );
			//           }
			//           v40 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//           v41 = v40.s_cachedAutoExposureData;
			//           if ( !v41 )
			//             sub_1802DC2C8(v40, qword_18D6405E0);
			//           histogramKernel = (unsigned int)v41.fields.histogramKernel;
			//           if ( !v69 )
			//             sub_1802DC2C8(histogramKernel, qword_18D6405E0);
			//           LODWORD(v69[2].monitor) = histogramKernel;
			//           v43 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//           v44 = v43.s_cachedAutoExposureData;
			//           if ( !v44 )
			//             sub_1802DC2C8(v43, qword_18D6405E0);
			//           textureSizeX = (unsigned int)v44.fields.textureSizeX;
			//           if ( !v69 )
			//             sub_1802DC2C8(textureSizeX, qword_18D6405E0);
			//           LODWORD(v69[3].monitor) = textureSizeX;
			//           v46 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//           v47 = v46.s_cachedAutoExposureData;
			//           if ( !v47 )
			//             sub_1802DC2C8(v46, qword_18D6405E0);
			//           textureSizeY = (unsigned int)v47.fields.textureSizeY;
			//           if ( !v69 )
			//             sub_1802DC2C8(textureSizeY, qword_18D6405E0);
			//           HIDWORD(v69[3].monitor) = textureSizeY;
			//           v49 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//           v50 = v49.s_cachedAutoExposureData;
			//           if ( !v50 )
			//             sub_1802DC2C8(v49, qword_18D6405E0);
			//           threadGroupsX = (unsigned int)v50.fields.threadGroupsX;
			//           if ( !v69 )
			//             sub_1802DC2C8(threadGroupsX, qword_18D6405E0);
			//           LODWORD(v69[4].klass) = threadGroupsX;
			//           v52 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//           v53 = v52.s_cachedAutoExposureData;
			//           if ( !v53 )
			//             sub_1802DC2C8(v52, qword_18D6405E0);
			//           threadGroupsY = (unsigned int)v53.fields.threadGroupsY;
			//           if ( !v69 )
			//             sub_1802DC2C8(threadGroupsY, qword_18D6405E0);
			//           HIDWORD(v69[4].klass) = threadGroupsY;
			//           v55 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//           v56 = v55.s_cachedAutoExposureData;
			//           if ( !v56 )
			//             sub_1802DC2C8(v55, qword_18D6405E0);
			//           x = v56.fields.exposureEv100Range.x;
			//           y = v56.fields.exposureEv100Range.y;
			//           v59 = v69;
			//           if ( !v69 )
			//             sub_1802DC2C8(v55, qword_18D6405E0);
			//           *(float *)&v69[4].monitor = x;
			//           *((float *)&v59[4].monitor + 1) = y;
			//           v60 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//           v61 = v60.s_cachedAutoExposureData;
			//           if ( !v61 )
			//             sub_1802DC2C8(v60, qword_18D6405E0);
			//           if ( !v69 )
			//             sub_1802DC2C8(v60, qword_18D6405E0);
			//           *(float *)&v69[5].klass = v61.fields.centerPixelWeight;
			//           v62 = v69;
			//           v65 = *HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadTexture(&v72, &v71, inputTex, 0LL);
			//           if ( !v62 )
			//             sub_1802DC2C8(v64, v63);
			//           v62[1] = (Object)v65;
			//           HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<System::Object>(
			//             &v71,
			//             (RenderFunc_1_System_Object_ *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_autoExposureCSRenderFunc,
			//             0LL,
			//             0,
			//             MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::HGAutoExposurePassData>);
			//         }
			//         catch ( Il2CppExceptionWrapper *v73 )
			//         {
			//           *(Il2CppExceptionWrapper *)&v70.autoExposureMode = (Il2CppExceptionWrapper)v73.ex;
			//         }
			//         sub_180222690(&v70);
			//       }
			//     }
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     HG::Rendering::Runtime::UberPostPassUtils::AutoExposureUpdateData(
			//       &TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_cachedAutoExposureData,
			//       0LL);
			//   }
			// }
			// 
		}

		private static void CalculateTargetExposure(Vector2 exposureEv100Range, Vector2 exposureEv100HistogramRange, Vector2 pixelLuminanceRange, float evCompensationAuto, int threadGroupsX, int threadGroupsY)
		{
			// // Void CalculateTargetExposure(Vector2, Vector2, Vector2, Single, Int32, Int32)
			// void HG::Rendering::Runtime::UberPostPassUtils::CalculateTargetExposure(
			//         Vector2 exposureEv100Range,
			//         Vector2 exposureEv100HistogramRange,
			//         Vector2 pixelLuminanceRange,
			//         float evCompensationAuto,
			//         int32_t threadGroupsX,
			//         int32_t threadGroupsY,
			//         MethodInfo *method)
			// {
			//   __int64 v10; // rdx
			//   int v11; // ecx
			//   double v12; // xmm0_8
			//   float v13; // xmm6_4
			//   int v14; // xmm0_4
			//   __int64 v15; // rdx
			//   int v16; // ecx
			//   double v17; // xmm0_8
			//   __int64 v18; // rdx
			//   ILFixDynamicMethodWrapper_2 *Patch; // rcx
			// 
			//   if ( !byte_18D9194C1 )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     byte_18D9194C1 = 1;
			//   }
			//   if ( IFix::WrappersManagerImpl::IsPatched(2409, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2409, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(0LL, v18);
			//     IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_868(
			//       Patch,
			//       exposureEv100Range,
			//       exposureEv100HistogramRange,
			//       pixelLuminanceRange,
			//       evCompensationAuto,
			//       threadGroupsX,
			//       threadGroupsY,
			//       0LL);
			//   }
			//   else
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     HG::Rendering::Runtime::UberPostPassUtils::CalculateAverageLogLuminance(
			//       exposureEv100Range,
			//       exposureEv100HistogramRange,
			//       pixelLuminanceRange,
			//       threadGroupsX,
			//       threadGroupsY,
			//       0LL);
			//     v12 = sub_1802EB1B0(v11, v10);
			//     v13 = *(float *)&v12;
			//     v14 = 1008981770;
			//     if ( v13 < 0.0099999998 || (v14 = 1090519040, v13 > 8.0) )
			//       v13 = *(float *)&v14;
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     v17 = sub_1802EB1B0(v16, v15);
			//     TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.m_targetExposure = *(float *)&v17
			//                                                                                          * (float)(0.18000001 / v13);
			//   }
			// }
			// 
		}

		internal static Vector4 GetBloomThresholdParams(Bloom bloom)
		{
			// // Vector4 GetBloomThresholdParams(Bloom)
			// Vector4 *HG::Rendering::Runtime::UberPostPassUtils::GetBloomThresholdParams(
			//         Vector4 *__return_ptr retstr,
			//         Bloom *bloom,
			//         MethodInfo *method)
			// {
			//   __int64 v5; // rdx
			//   __int64 v6; // rcx
			//   float threshold; // xmm0_4
			//   float v8; // xmm0_4
			//   float v9; // xmm2_4
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   Vector4 v12; // [rsp+20h] [rbp-18h] BYREF
			// 
			//   if ( IFix::WrappersManagerImpl::IsPatched(2411, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2411, 0LL);
			//     if ( Patch )
			//     {
			//       *retstr = *IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_312(&v12, Patch, (Object *)bloom, 0LL);
			//       return retstr;
			//     }
			// LABEL_5:
			//     sub_180B536AC(v6, v5);
			//   }
			//   if ( !bloom )
			//     goto LABEL_5;
			//   threshold = HG::Rendering::Runtime::Bloom::get_threshold(bloom, 0LL);
			//   v8 = UnityEngine::Mathf::GammaToLinearSpace(threshold, 0LL);
			//   retstr.x = v8;
			//   v9 = (float)(v8 * 0.5) + 0.0000099999997;
			//   retstr.y = v8 - v9;
			//   retstr.w = 0.25 / v9;
			//   retstr.z = v9 + v9;
			//   return retstr;
			// }
			// 
			return null;
		}

		internal static Vector4 GetBloomCharacterThresholdParams(Bloom bloom)
		{
			// // Vector4 GetBloomCharacterThresholdParams(Bloom)
			// Vector4 *HG::Rendering::Runtime::UberPostPassUtils::GetBloomCharacterThresholdParams(
			//         Vector4 *__return_ptr retstr,
			//         Bloom *bloom,
			//         MethodInfo *method)
			// {
			//   void *characterThreshold; // rdx
			//   __int64 v6; // rcx
			//   float v7; // xmm0_4
			//   float v8; // xmm0_4
			//   float v9; // xmm6_4
			//   float v10; // xmm0_4
			//   float v11; // xmm0_4
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   Vector4 v14; // [rsp+20h] [rbp-28h] BYREF
			// 
			//   if ( IFix::WrappersManagerImpl::IsPatched(2412, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2412, 0LL);
			//     if ( Patch )
			//     {
			//       *retstr = *IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_312(&v14, Patch, (Object *)bloom, 0LL);
			//       return retstr;
			//     }
			// LABEL_7:
			//     sub_180B536AC(v6, characterThreshold);
			//   }
			//   if ( !bloom )
			//     goto LABEL_7;
			//   characterThreshold = bloom.fields.characterThreshold;
			//   if ( !characterThreshold )
			//     goto LABEL_7;
			//   v7 = sub_18003F9B0(10LL, characterThreshold);
			//   v8 = UnityEngine::Mathf::GammaToLinearSpace(v7, 0LL);
			//   characterThreshold = bloom.fields.characterSoftness;
			//   v9 = v8;
			//   if ( !characterThreshold )
			//     goto LABEL_7;
			//   v10 = sub_18003F9B0(10LL, characterThreshold);
			//   retstr.x = v9;
			//   v11 = (float)(v10 * v9) + 0.0000099999997;
			//   retstr.w = 0.25 / v11;
			//   retstr.y = v9 - v11;
			//   retstr.z = v11 + v11;
			//   return retstr;
			// }
			// 
			return null;
		}

		internal static void PrepareUberBloomParameters(HGSettingParameters settingParams, UberPostPassUtils.PPBloomData data, Bloom bloom, Vector4 bloomBicubicParams, HGCamera camera, Material ppMaterial)
		{
			// // Void PrepareUberBloomParameters(HGSettingParameters, UberPostPassUtils+PPBloomData, Bloom, Vector4, HGCamera, Material)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareUberBloomParameters(
			//         HGSettingParameters *settingParams,
			//         UberPostPassUtils_PPBloomData *data,
			//         Bloom *bloom,
			//         Vector4 *bloomBicubicParams,
			//         HGCamera *camera,
			//         Material *ppMaterial,
			//         MethodInfo *method)
			// {
			//   HGRenderPathBase_HGRenderPathResources *dirtTexture; // rdx
			//   ILFixDynamicMethodWrapper_2 *Patch; // rcx
			//   __int64 v13; // rdx
			//   int v14; // ecx
			//   double v15; // xmm0_8
			//   ColorParameter *tint; // r8
			//   float v17; // xmm8_4
			//   float v18; // xmm9_4
			//   __m128 v19; // xmm6
			//   MethodInfo *v20; // rdx
			//   MethodInfo *v21; // r9
			//   float v22; // xmm3_4
			//   Vector4 *one; // rax
			//   __m128i v24; // xmm11
			//   Object_1 *v25; // rbx
			//   Texture2D *blackTexture; // rax
			//   Texture *v27; // rbx
			//   bool dirtEnabled; // r14
			//   float v29; // xmm7_4
			//   float v30; // xmm7_4
			//   float m_X; // xmm6_4
			//   float v32; // xmm0_4
			//   float v33; // xmm6_4
			//   PassConstructorID__Enum__Array *v34; // r8
			//   HGCamera *v35; // r9
			//   float v36; // xmm10_4
			//   MethodInfo *v37; // r8
			//   float v38; // xmm1_4
			//   Vector4 v39; // xmm0
			//   Vector4 v40; // xmm1
			//   Vector4 v41; // xmm0
			//   HGShaderKeyWords__StaticFields *static_fields; // rcx
			//   MethodInfo *v43; // [rsp+28h] [rbp-81h]
			//   MethodInfo *v44; // [rsp+30h] [rbp-79h]
			//   Vector4 si128; // [rsp+48h] [rbp-61h] BYREF
			//   Vector4 v46; // [rsp+58h] [rbp-51h] BYREF
			//   Vector4 v47[7]; // [rsp+68h] [rbp-41h] BYREF
			// 
			//   if ( !byte_18D9194C2 )
			//   {
			//     sub_18003C530(&TypeInfo::UnityEngine::Rendering::ColorUtils);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//     sub_18003C530(&TypeInfo::UnityEngine::Object);
			//     sub_18003C530(&MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     byte_18D9194C2 = 1;
			//   }
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2413, 0LL) )
			//   {
			//     if ( bloom )
			//     {
			//       if ( !HG::Rendering::Runtime::Bloom::IsActive(bloom, 0LL) )
			//         return;
			//       if ( settingParams )
			//       {
			//         if ( !HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit(
			//                 settingParams.fields._bloomEnabled_k__BackingField,
			//                 MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit) )
			//           return;
			//         HG::Rendering::Runtime::Bloom::get_intensity(bloom, 0LL);
			//         v15 = sub_1802EB1B0(v14, v13);
			//         tint = bloom.fields.tint;
			//         v17 = 1.0;
			//         v18 = *(float *)&v15 - 1.0;
			//         if ( tint )
			//         {
			//           v46 = *(Vector4 *)sub_180040AD0(&v46, 10LL, tint);
			//           v19 = (__m128)_mm_loadu_si128((const __m128i *)sub_182F8C840(&si128, &v46));
			//           sub_180002C70(TypeInfo::UnityEngine::Rendering::ColorUtils);
			//           v22 = (float)((float)(v19.m128_f32[0] * 0.2126729)
			//                       + (float)(_mm_shuffle_ps(v19, v19, 85).m128_f32[0] * 0.7151522))
			//               + (float)(_mm_shuffle_ps(v19, v19, 170).m128_f32[0] * 0.072175004);
			//           if ( v22 <= 0.0 )
			//           {
			//             one = UnityEngine::Vector4::get_one(&v46, v20);
			//           }
			//           else
			//           {
			//             v46 = (Vector4)v19;
			//             one = UnityEngine::Vector4::op_Multiply(&si128, &v46, 1.0 / v22, v21);
			//           }
			//           v24 = _mm_loadu_si128((const __m128i *)one);
			//           dirtTexture = (HGRenderPathBase_HGRenderPathResources *)bloom.fields.dirtTexture;
			//           if ( dirtTexture )
			//           {
			//             v25 = (Object_1 *)sub_18004EF00(10LL, dirtTexture);
			//             sub_180002C70(TypeInfo::UnityEngine::Object);
			//             if ( UnityEngine::Object::op_Equality(v25, 0LL, 0LL) )
			//             {
			//               blackTexture = UnityEngine::Texture2D::get_blackTexture(0LL);
			//             }
			//             else
			//             {
			//               dirtTexture = (HGRenderPathBase_HGRenderPathResources *)bloom.fields.dirtTexture;
			//               if ( !dirtTexture )
			//                 goto LABEL_37;
			//               blackTexture = (Texture2D *)sub_18004EF00(10LL, dirtTexture);
			//             }
			//             v27 = (Texture *)blackTexture;
			//             dirtEnabled = HG::Rendering::Runtime::Bloom::get_dirtEnabled(bloom, 0LL);
			//             if ( v27 )
			//             {
			//               v29 = (float)(int)sub_18003ED00(5LL);
			//               v30 = v29 / (float)(int)sub_18003ED00(7LL);
			//               if ( camera )
			//               {
			//                 dirtTexture = (HGRenderPathBase_HGRenderPathResources *)bloom.fields.dirtIntensity;
			//                 m_X = (float)camera.fields._taauRTSize_k__BackingField.m_X;
			//                 v32 = (float)(int)HIDWORD(*(_QWORD *)&camera.fields._taauRTSize_k__BackingField);
			//                 si128 = (Vector4)_mm_load_si128((const __m128i *)&xmmword_18A357570);
			//                 v33 = m_X / v32;
			//                 if ( dirtTexture )
			//                 {
			//                   v36 = sub_18003F9B0(10LL, dirtTexture);
			//                   if ( v30 > v33 )
			//                   {
			//                     si128.x = v33 / v30;
			//                     si128.z = (float)(1.0 - (float)(v33 / v30)) * 0.5;
			//                   }
			//                   else if ( v33 > v30 )
			//                   {
			//                     si128.y = v30 / v33;
			//                     si128.w = (float)(1.0 - (float)(v30 / v33)) * 0.5;
			//                   }
			//                   if ( data )
			//                   {
			//                     data.fields.bloomDirtTexture = v27;
			//                     sub_1800054D0((HGRenderPathScene *)&data.fields.bloomDirtTexture, dirtTexture, v34, v35, v43, v44);
			//                     dirtTexture = (HGRenderPathBase_HGRenderPathResources *)bloom.fields.blendMode;
			//                     if ( dirtTexture )
			//                     {
			//                       if ( (unsigned int)sub_18003ED00(10LL) )
			//                         v38 = 1.0;
			//                       else
			//                         v38 = 0.0;
			//                       if ( !dirtEnabled )
			//                         v17 = 0.0;
			//                       v46.x = v18;
			//                       v46.z = v38;
			//                       v46.y = v18 * v36;
			//                       v46.w = v17;
			//                       v39 = v46;
			//                       v46 = (Vector4)v24;
			//                       data.fields.bloomParams = v39;
			//                       v40 = (Vector4)_mm_loadu_si128((const __m128i *)UnityEngine::Color::op_Implicit(
			//                                                                         (Color *)v47,
			//                                                                         &v46,
			//                                                                         v37));
			//                       data.fields.bloomDirtTileOffset = si128;
			//                       data.fields.bloomTint = v40;
			//                       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//                       v41 = (Vector4)_mm_loadu_si128((const __m128i *)HG::Rendering::Runtime::UberPostPassUtils::GetBloomThresholdParams(
			//                                                                         v47,
			//                                                                         bloom,
			//                                                                         0LL));
			//                       data.fields.bloomBicubicParams = *bloomBicubicParams;
			//                       data.fields.bloomThreshold = v41;
			//                       sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//                       static_fields = TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//                       dirtTexture = (HGRenderPathBase_HGRenderPathResources *)(dirtEnabled
			//                                                                              ? static_fields.s_BloomDirt
			//                                                                              : static_fields.s_Bloom);
			//                       Patch = (ILFixDynamicMethodWrapper_2 *)ppMaterial;
			//                       if ( ppMaterial )
			//                       {
			//                         UnityEngine::Material::EnableKeyword(ppMaterial, (String *)dirtTexture, 0LL);
			//                         return;
			//                       }
			//                     }
			//                   }
			//                 }
			//               }
			//             }
			//           }
			//         }
			//       }
			//     }
			// LABEL_37:
			//     sub_180B536AC(Patch, dirtTexture);
			//   }
			//   Patch = IFix::WrappersManagerImpl::GetPatch(2413, 0LL);
			//   if ( !Patch )
			//     goto LABEL_37;
			//   v46 = *bloomBicubicParams;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_870(
			//     Patch,
			//     (Object *)settingParams,
			//     (Object *)data,
			//     (Object *)bloom,
			//     &v46,
			//     (Object *)camera,
			//     (Object *)ppMaterial,
			//     0LL);
			// }
			// 
		}

		internal static void PrepareBloomPSMaterials(HGRenderPipelineMaterialCollector materialCollector, ref UberPostPassUtils.BloomPSMaterials bloomPSMaterials, HGRenderPipelineRuntimeResources defaultResources)
		{
			// // Void PrepareBloomPSMaterials(HGRenderPipelineMaterialCollector, UberPostPassUtils+BloomPSMaterials ByRef, HGRenderPipelineRuntimeResources)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareBloomPSMaterials(
			//         HGRenderPipelineMaterialCollector *materialCollector,
			//         UberPostPassUtils_BloomPSMaterials *bloomPSMaterials,
			//         HGRenderPipelineRuntimeResources *defaultResources,
			//         MethodInfo *method)
			// {
			//   __int64 v7; // r8
			//   __int64 v8; // r9
			//   int v9; // ebx
			//   HGRenderPathBase_HGRenderPathResources *v10; // rdx
			//   PassConstructorID__Enum__Array *v11; // r8
			//   HGCamera *v12; // r9
			//   __int64 v13; // r8
			//   __int64 v14; // r9
			//   HGRenderPathBase_HGRenderPathResources *v15; // rdx
			//   PassConstructorID__Enum__Array *v16; // r8
			//   HGCamera *v17; // r9
			//   __int64 v18; // r8
			//   __int64 v19; // r9
			//   HGRenderPathBase_HGRenderPathResources *v20; // rdx
			//   PassConstructorID__Enum__Array *v21; // r8
			//   HGCamera *v22; // r9
			//   __int128 v23; // xmm1
			//   HGRenderPathBase_HGRenderPathResources *v24; // rdx
			//   PassConstructorID__Enum__Array *v25; // r8
			//   HGCamera *v26; // r9
			//   HGRenderPipelineRuntimeResources_ShaderResources *v27; // rdx
			//   __int64 v28; // rcx
			//   HGRenderPipelineRuntimeResources_ShaderResources *shaders; // rax
			//   HGRenderPathBase_HGRenderPathResources *v30; // rdx
			//   PassConstructorID__Enum__Array *v31; // r8
			//   HGCamera *v32; // r9
			//   Material__Array *blur1stPassMaterials; // rdi
			//   Material *Material; // rax
			//   Material *v35; // rsi
			//   Material__Array *blur2ndPassMaterials; // rdi
			//   Material *v37; // rax
			//   Material *v38; // rsi
			//   Material__Array *bloomUpsampleMaterials; // rdi
			//   Material *v40; // rax
			//   Material *v41; // rsi
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   MethodInfo *methoda; // [rsp+20h] [rbp-48h]
			//   MethodInfo *methodc; // [rsp+20h] [rbp-48h]
			//   MethodInfo *methodd; // [rsp+20h] [rbp-48h]
			//   MethodInfo *methode; // [rsp+20h] [rbp-48h]
			//   MethodInfo *methodb; // [rsp+20h] [rbp-48h]
			//   MethodInfo *v48; // [rsp+28h] [rbp-40h]
			//   MethodInfo *v49; // [rsp+28h] [rbp-40h]
			//   MethodInfo *v50; // [rsp+28h] [rbp-40h]
			//   MethodInfo *v51; // [rsp+28h] [rbp-40h]
			//   MethodInfo *v52; // [rsp+28h] [rbp-40h]
			//   __m256i v53; // [rsp+30h] [rbp-38h] BYREF
			// 
			//   if ( !byte_18D8EDA30 )
			//   {
			//     sub_18003C530(&TypeInfo::UnityEngine::Material);
			//     byte_18D8EDA30 = 1;
			//   }
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2415, 0LL) )
			//   {
			//     v9 = 0;
			//     memset(&v53, 0, 24);
			//     v53.m256i_i64[3] = il2cpp_array_new_specific_0(TypeInfo::UnityEngine::Material, 17LL, v7, v8);
			//     sub_1800054D0((HGRenderPathScene *)&v53.m256i_u64[3], v10, v11, v12, methoda, v48);
			//     v53.m256i_i64[1] = il2cpp_array_new_specific_0(TypeInfo::UnityEngine::Material, 17LL, v13, v14);
			//     sub_1800054D0((HGRenderPathScene *)&v53.m256i_u64[1], v15, v16, v17, methodc, v49);
			//     v53.m256i_i64[2] = il2cpp_array_new_specific_0(TypeInfo::UnityEngine::Material, 17LL, v18, v19);
			//     sub_1800054D0((HGRenderPathScene *)&v53.m256i_u64[2], v20, v21, v22, methodd, v50);
			//     v23 = *(_OWORD *)&v53.m256i_u64[2];
			//     *(_OWORD *)&bloomPSMaterials.prefilterMaterial = *(_OWORD *)v53.m256i_i8;
			//     *(_OWORD *)&bloomPSMaterials.blur2ndPassMaterials = v23;
			//     sub_1800054D0((HGRenderPathScene *)bloomPSMaterials, v24, v25, v26, methode, v51);
			//     if ( defaultResources )
			//     {
			//       shaders = defaultResources.fields.shaders;
			//       if ( shaders )
			//       {
			//         if ( materialCollector )
			//         {
			//           bloomPSMaterials.prefilterMaterial = HG::Rendering::Runtime::HGRenderPipelineMaterialCollector::CreateMaterial(
			//                                                   materialCollector,
			//                                                   shaders.fields.bloomPS,
			//                                                   0,
			//                                                   0LL);
			//           sub_1800054D0((HGRenderPathScene *)bloomPSMaterials, v30, v31, v32, methodb, v52);
			//           while ( 1 )
			//           {
			//             v27 = defaultResources.fields.shaders;
			//             blur1stPassMaterials = bloomPSMaterials.blur1stPassMaterials;
			//             if ( !v27 )
			//               break;
			//             Material = HG::Rendering::Runtime::HGRenderPipelineMaterialCollector::CreateMaterial(
			//                          materialCollector,
			//                          v27.fields.bloomPS,
			//                          0,
			//                          0LL);
			//             v35 = Material;
			//             if ( !blur1stPassMaterials )
			//               break;
			//             sub_180036D40(blur1stPassMaterials, Material);
			//             sub_18000FDA0(blur1stPassMaterials, v9, v35);
			//             v27 = defaultResources.fields.shaders;
			//             blur2ndPassMaterials = bloomPSMaterials.blur2ndPassMaterials;
			//             if ( !v27 )
			//               break;
			//             v37 = HG::Rendering::Runtime::HGRenderPipelineMaterialCollector::CreateMaterial(
			//                     materialCollector,
			//                     v27.fields.bloomPS,
			//                     0,
			//                     0LL);
			//             v38 = v37;
			//             if ( !blur2ndPassMaterials )
			//               break;
			//             sub_180036D40(blur2ndPassMaterials, v37);
			//             sub_18000FDA0(blur2ndPassMaterials, v9, v38);
			//             v27 = defaultResources.fields.shaders;
			//             bloomUpsampleMaterials = bloomPSMaterials.bloomUpsampleMaterials;
			//             if ( !v27 )
			//               break;
			//             v40 = HG::Rendering::Runtime::HGRenderPipelineMaterialCollector::CreateMaterial(
			//                     materialCollector,
			//                     v27.fields.bloomPS,
			//                     0,
			//                     0LL);
			//             v41 = v40;
			//             if ( !bloomUpsampleMaterials )
			//               break;
			//             sub_180036D40(bloomUpsampleMaterials, v40);
			//             sub_18000FDA0(bloomUpsampleMaterials, v9++, v41);
			//             if ( v9 >= 17 )
			//               return;
			//           }
			//         }
			//       }
			//     }
			// LABEL_16:
			//     sub_180B536AC(v28, v27);
			//   }
			//   Patch = IFix::WrappersManagerImpl::GetPatch(2415, 0LL);
			//   if ( !Patch )
			//     goto LABEL_16;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_872(
			//     Patch,
			//     (Object *)materialCollector,
			//     bloomPSMaterials,
			//     (Object *)defaultResources,
			//     0LL);
			// }
			// 
		}

		internal static void DisposeBloomPSMaterials(ref UberPostPassUtils.BloomPSMaterials bloomPSMaterials)
		{
		}

		internal static GraphicsFormat GetBloomTextureFormat(bool useComputeShader, bool enableAlpha)
		{
			// // GraphicsFormat GetBloomTextureFormat(Boolean, Boolean)
			// GraphicsFormat__Enum HG::Rendering::Runtime::UberPostPassUtils::GetBloomTextureFormat(
			//         bool useComputeShader,
			//         bool enableAlpha,
			//         MethodInfo *method)
			// {
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   __int64 v7; // rdx
			//   __int64 v8; // rcx
			// 
			//   if ( !byte_18D9194C3 )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     byte_18D9194C3 = 1;
			//   }
			//   if ( IFix::WrappersManagerImpl::IsPatched(2418, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2418, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v8, v7);
			//     return IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_874(Patch, useComputeShader, enableAlpha, 0LL);
			//   }
			//   else
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     if ( useComputeShader )
			//       return HG::Rendering::Runtime::UberPostPassUtils::GetPostprocessComputeTextureFormat(enableAlpha, 0LL);
			//     else
			//       return HG::Rendering::Runtime::UberPostPassUtils::GetPostprocessTextureFormat(
			//                enableAlpha,
			//                GraphicsFormat__Enum_B10G11R11_UFloatPack32,
			//                0LL);
			//   }
			// }
			// 
			return (GraphicsFormat)GraphicsFormat.None;
		}

		internal static void PrepareBloomData(bool useComputeShader, HGRenderGraph renderGraph, HGRenderGraphBuilder builder, UberPostPassUtils.BloomPassData passData, Bloom bloom, HGCamera camera, bool enableAlpha, TextureHandle source, TextureHandle motionVector, HGRenderPathInternal renderPath, TAAUQuality taauQuality, BloomQuality bloomQuality, ref UberPostPassUtils.BloomPSMaterials bloomPSMaterials, out Vector4 bloomBicubicParam)
		{
			// // Void PrepareBloomData(Boolean, HGRenderGraph, HGRenderGraphBuilder, UberPostPassUtils+BloomPassData, Bloom, HGCamera, Boolean, TextureHandle, TextureHandle, HGRenderPathInternal, TAAUQuality, BloomQuality, UberPostPassUtils+BloomPSMaterials ByRef, Vector4 ByRef)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareBloomData(
			//         bool useComputeShader,
			//         HGRenderGraph *renderGraph,
			//         HGRenderGraphBuilder *builder,
			//         UberPostPassUtils_BloomPassData *passData,
			//         Bloom *bloom,
			//         HGCamera *camera,
			//         bool enableAlpha,
			//         TextureHandle *source,
			//         TextureHandle *motionVector,
			//         HGRenderPathInternal__Enum renderPath,
			//         TAAUQuality__Enum taauQuality,
			//         BloomQuality__Enum bloomQuality,
			//         UberPostPassUtils_BloomPSMaterials *bloomPSMaterials,
			//         Vector4 *bloomBicubicParam,
			//         MethodInfo *method)
			// {
			//   __int64 characterBloomControl; // rdx
			//   void *bloomPrefilterCS; // rcx
			//   TextureHandle *v21; // rax
			//   HGRenderPathBase_HGRenderPathResources *v22; // rdx
			//   PassConstructorID__Enum__Array *v23; // r8
			//   HGCamera *v24; // r9
			//   bool IsValid; // al
			//   __int64 v26; // r10
			//   Material__Array *v27; // r9
			//   HGRenderPathBase_HGRenderPathResources *v28; // rdx
			//   PassConstructorID__Enum__Array *v29; // r8
			//   __int64 v30; // r10
			//   Material__Array *v31; // r9
			//   HGRenderPathBase_HGRenderPathResources *v32; // rdx
			//   PassConstructorID__Enum__Array *v33; // r8
			//   __int64 v34; // r10
			//   Material__Array *v35; // r9
			//   HGRenderPathBase_HGRenderPathResources *v36; // rdx
			//   PassConstructorID__Enum__Array *v37; // r8
			//   Material *bloomPrefilterMat; // rdi
			//   String *v39; // rdx
			//   int i; // edi
			//   int v41; // eax
			//   Material *v42; // rsi
			//   String *s_HighQuality; // rdx
			//   Material *v44; // rdi
			//   Material *v45; // rdi
			//   HGRenderPipeline *currentPipeline; // rax
			//   HGRenderPipelineRuntimeResources *defaultResources; // rax
			//   PassConstructorID__Enum__Array *v48; // r8
			//   HGCamera *v49; // r9
			//   HGRenderPipelineRuntimeResources_ShaderResources *shaders; // rax
			//   int32_t Kernel; // eax
			//   HGRenderPipeline *v52; // rax
			//   HGRenderPipelineRuntimeResources *v53; // rax
			//   PassConstructorID__Enum__Array *v54; // r8
			//   HGCamera *v55; // r9
			//   HGRenderPipelineRuntimeResources_ShaderResources *v56; // rax
			//   HGRenderPipeline *v57; // rax
			//   HGRenderPipelineRuntimeResources *v58; // rax
			//   PassConstructorID__Enum__Array *v59; // r8
			//   HGCamera *v60; // r9
			//   HGRenderPipelineRuntimeResources_ShaderResources *v61; // rax
			//   int32_t v62; // eax
			//   HGRenderPipeline *v63; // rax
			//   HGRenderPipelineRuntimeResources *v64; // rax
			//   PassConstructorID__Enum__Array *v65; // r8
			//   HGCamera *v66; // r9
			//   HGRenderPipelineRuntimeResources_ShaderResources *v67; // rax
			//   float scatter; // xmm0_4
			//   Beyond::JobMathf *v69; // rcx
			//   double v70; // xmm0_8
			//   float v71; // xmm7_4
			//   float v72; // xmm10_4
			//   AttributeCollection *instance; // rax
			//   __int64 v74; // rdx
			//   __int64 v75; // r8
			//   __int128 v76; // xmm1
			//   float v77; // xmm0_4
			//   float v78; // xmm1_4
			//   float v79; // xmm1_4
			//   char v80; // dl
			//   __int64 v81; // rcx
			//   int v82; // r8d
			//   int32_t v83; // eax
			//   int v84; // ecx
			//   int32_t j; // edi
			//   __int64 v86; // rdx
			//   __int64 v87; // rcx
			//   double v88; // xmm0_8
			//   __int64 v89; // rdx
			//   int32_t v90; // r14d
			//   int32_t v91; // eax
			//   HGRenderPathBase_HGRenderPathResources *v92; // rdx
			//   PassConstructorID__Enum__Array *v93; // r8
			//   HGCamera *v94; // r9
			//   TextureHandle__Array *mipsDown; // r14
			//   TextureHandle *v96; // rax
			//   TextureHandle *v97; // rax
			//   TextureHandle *v98; // rax
			//   HGRenderPathBase_HGRenderPathResources *v99; // rdx
			//   PassConstructorID__Enum__Array *v100; // r8
			//   HGCamera *v101; // r9
			//   TextureHandle__Array *mipsUp; // r14
			//   TextureHandle *v103; // rax
			//   TextureHandle *v104; // rax
			//   TextureHandle *v105; // rax
			//   uint32_t *v106; // rax
			//   uint32_t v107; // xmm8_4
			//   __int64 v108; // rax
			//   int32_t v109; // xmm7_4
			//   float *v110; // rax
			//   uint32_t v111; // xmm0_4
			//   __int64 v112; // rdx
			//   __int64 v113; // rcx
			//   int32_t v114; // eax
			//   __int64 v115; // rcx
			//   __int64 v116; // rdx
			//   HGRenderPathBase_HGRenderPathResources *v117; // rdx
			//   PassConstructorID__Enum__Array *v118; // r8
			//   HGCamera *v119; // r9
			//   GraphicsFormat__Enum BloomTextureFormat; // eax
			//   TextureHandle__Array *v121; // rdi
			//   TextureHandle *v122; // rax
			//   __int128 v123; // xmm0
			//   __int128 v124; // xmm1
			//   MethodInfo *v125; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *v126; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *v127; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *v128; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *v129; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *v130; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *v131; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *v132; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *v133; // [rsp+30h] [rbp-D8h]
			//   MethodInfo *v134; // [rsp+30h] [rbp-D8h]
			//   MethodInfo *v135; // [rsp+30h] [rbp-D8h]
			//   MethodInfo *v136; // [rsp+30h] [rbp-D8h]
			//   MethodInfo *v137; // [rsp+30h] [rbp-D8h]
			//   MethodInfo *v138; // [rsp+30h] [rbp-D8h]
			//   MethodInfo *v139; // [rsp+30h] [rbp-D8h]
			//   MethodInfo *v140; // [rsp+30h] [rbp-D8h]
			//   TextureHandle v141; // [rsp+88h] [rbp-80h] BYREF
			//   Vector4 v142; // [rsp+98h] [rbp-70h] BYREF
			//   Vector2Int size; // [rsp+A8h] [rbp-60h]
			//   TextureDesc v144; // [rsp+B8h] [rbp-50h] BYREF
			//   HGRenderGraphBuilder v145; // [rsp+118h] [rbp+10h] BYREF
			//   TextureDesc v146; // [rsp+148h] [rbp+40h] BYREF
			//   TextureHandle v147; // [rsp+1A8h] [rbp+A0h] BYREF
			//   TextureHandle v148; // [rsp+1B8h] [rbp+B0h] BYREF
			//   TextureHandle v149; // [rsp+1C8h] [rbp+C0h] BYREF
			//   TextureHandle v150; // [rsp+1D8h] [rbp+D0h] BYREF
			//   TextureHandle v151; // [rsp+1E8h] [rbp+E0h] BYREF
			// 
			//   if ( !byte_18D9194C4 )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGRenderPipeline);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//     sub_18003C530(&TypeInfo::HG::Rendering::RenderGraphModule::TextureHandle);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     sub_18003C530(&off_18D4F3A70);
			//     sub_18003C530(&off_18D4F3A78);
			//     sub_18003C530(&off_18D4F3A88);
			//     sub_18003C530(&off_18D4F3AC0);
			//     sub_18003C530(&off_18D4F3AD8);
			//     byte_18D9194C4 = 1;
			//   }
			//   sub_1802F01E0(&v146, 0LL, 96LL);
			//   size = 0LL;
			//   sub_1802F01E0(&v144, 0LL, 96LL);
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2419, 0LL) )
			//   {
			//     if ( !passData )
			//       goto LABEL_129;
			//     passData.fields.enableAlpha = enableAlpha;
			//     passData.fields.viewCount = 1;
			//     passData.fields.bloomQuality = bloomQuality;
			//     sub_180002C70(TypeInfo::HG::Rendering::RenderGraphModule::TextureHandle);
			//     if ( HG::Rendering::RenderGraphModule::TextureHandle::IsValid(motionVector, 0LL) )
			//     {
			//       v21 = HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadTexture(&v141, builder, motionVector, 0LL);
			//     }
			//     else
			//     {
			//       sub_180002C70(TypeInfo::HG::Rendering::RenderGraphModule::TextureHandle);
			//       v21 = (TextureHandle *)sub_182E7CCD0(&v141);
			//     }
			//     passData.fields.motionVector = *v21;
			//     if ( !bloom )
			//       goto LABEL_129;
			//     characterBloomControl = (__int64)bloom.fields.characterBloomControl;
			//     if ( !characterBloomControl )
			//       goto LABEL_129;
			//     if ( !(unsigned __int8)sub_1800023D0(10LL, characterBloomControl) || taauQuality )
			//     {
			//       IsValid = 0;
			//     }
			//     else
			//     {
			//       sub_180002C70(TypeInfo::HG::Rendering::RenderGraphModule::TextureHandle);
			//       IsValid = HG::Rendering::RenderGraphModule::TextureHandle::IsValid(&passData.fields.motionVector, 0LL);
			//     }
			//     passData.fields.enableCharacterMask = IsValid;
			//     if ( useComputeShader )
			//     {
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGRenderPipeline);
			//       currentPipeline = HG::Rendering::Runtime::HGRenderPipeline::get_currentPipeline(0LL);
			//       if ( !currentPipeline )
			//         goto LABEL_129;
			//       defaultResources = HG::Rendering::Runtime::HGRenderPipeline::get_defaultResources(currentPipeline, 0LL);
			//       if ( !defaultResources )
			//         goto LABEL_129;
			//       shaders = defaultResources.fields.shaders;
			//       if ( !shaders )
			//         goto LABEL_129;
			//       passData.fields.bloomPrefilterCS = shaders.fields.bloomPrefilterCS;
			//       sub_1800054D0(
			//         (HGRenderPathScene *)&passData.fields.bloomPrefilterCS,
			//         (HGRenderPathBase_HGRenderPathResources *)characterBloomControl,
			//         v48,
			//         v49,
			//         v125,
			//         v133);
			//       bloomPrefilterCS = passData.fields.bloomPrefilterCS;
			//       if ( !bloomPrefilterCS )
			//         goto LABEL_129;
			//       Kernel = UnityEngine::ComputeShader::FindKernel((ComputeShader *)bloomPrefilterCS, (String *)"KMain", 0LL);
			//       bloomPrefilterCS = passData.fields.bloomPrefilterCS;
			//       passData.fields.bloomPrefilterKernel = Kernel;
			//       if ( !bloomPrefilterCS )
			//         goto LABEL_129;
			//       UnityEngine::ComputeShader::SetShaderKeywords((ComputeShader *)bloomPrefilterCS, 0LL, 0LL);
			//       v52 = HG::Rendering::Runtime::HGRenderPipeline::get_currentPipeline(0LL);
			//       if ( !v52 )
			//         goto LABEL_129;
			//       v53 = HG::Rendering::Runtime::HGRenderPipeline::get_defaultResources(v52, 0LL);
			//       if ( !v53 )
			//         goto LABEL_129;
			//       v56 = v53.fields.shaders;
			//       if ( !v56 )
			//         goto LABEL_129;
			//       passData.fields.bloomBlurCS = v56.fields.bloomBlurCS;
			//       sub_1800054D0(
			//         (HGRenderPathScene *)&passData.fields.bloomBlurCS,
			//         (HGRenderPathBase_HGRenderPathResources *)characterBloomControl,
			//         v54,
			//         v55,
			//         v130,
			//         v138);
			//       v57 = HG::Rendering::Runtime::HGRenderPipeline::get_currentPipeline(0LL);
			//       if ( !v57 )
			//         goto LABEL_129;
			//       v58 = HG::Rendering::Runtime::HGRenderPipeline::get_defaultResources(v57, 0LL);
			//       if ( !v58 )
			//         goto LABEL_129;
			//       v61 = v58.fields.shaders;
			//       if ( !v61 )
			//         goto LABEL_129;
			//       passData.fields.bloomBlurNonOptCS = v61.fields.bloomBlurNonOptCS;
			//       sub_1800054D0(
			//         (HGRenderPathScene *)&passData.fields.bloomBlurNonOptCS,
			//         (HGRenderPathBase_HGRenderPathResources *)characterBloomControl,
			//         v59,
			//         v60,
			//         v131,
			//         v139);
			//       bloomPrefilterCS = passData.fields.bloomBlurCS;
			//       if ( !bloomPrefilterCS )
			//         goto LABEL_129;
			//       v62 = UnityEngine::ComputeShader::FindKernel((ComputeShader *)bloomPrefilterCS, (String *)"KMain", 0LL);
			//       bloomPrefilterCS = passData.fields.bloomBlurCS;
			//       passData.fields.bloomBlurKernel = v62;
			//       if ( !bloomPrefilterCS )
			//         goto LABEL_129;
			//       passData.fields.bloomDownsampleKernel = UnityEngine::ComputeShader::FindKernel(
			//                                                  (ComputeShader *)bloomPrefilterCS,
			//                                                  (String *)"KDownsample",
			//                                                  0LL);
			//       v63 = HG::Rendering::Runtime::HGRenderPipeline::get_currentPipeline(0LL);
			//       if ( !v63 )
			//         goto LABEL_129;
			//       v64 = HG::Rendering::Runtime::HGRenderPipeline::get_defaultResources(v63, 0LL);
			//       if ( !v64 )
			//         goto LABEL_129;
			//       v67 = v64.fields.shaders;
			//       if ( !v67 )
			//         goto LABEL_129;
			//       passData.fields.bloomUpsampleCS = v67.fields.bloomUpsampleCS;
			//       sub_1800054D0(
			//         (HGRenderPathScene *)&passData.fields.bloomUpsampleCS,
			//         (HGRenderPathBase_HGRenderPathResources *)characterBloomControl,
			//         v65,
			//         v66,
			//         v132,
			//         v140);
			//       bloomPrefilterCS = passData.fields.bloomUpsampleCS;
			//       if ( !bloomPrefilterCS )
			//         goto LABEL_129;
			//       UnityEngine::ComputeShader::SetShaderKeywords((ComputeShader *)bloomPrefilterCS, 0LL, 0LL);
			//       bloomPrefilterCS = passData.fields.bloomUpsampleCS;
			//       if ( !bloomPrefilterCS )
			//         goto LABEL_129;
			//       passData.fields.bloomUpsampleKernel = UnityEngine::ComputeShader::FindKernel(
			//                                                (ComputeShader *)bloomPrefilterCS,
			//                                                (String *)"KMain",
			//                                                0LL);
			//       goto LABEL_68;
			//     }
			//     passData.fields.bloomPrefilterMat = bloomPSMaterials.prefilterMaterial;
			//     sub_1800054D0((HGRenderPathScene *)&passData.fields, v22, v23, v24, v125, v133);
			//     v27 = *(Material__Array **)(v26 + 8);
			//     passData.fields.bloomBlurMat1stPass = v27;
			//     sub_1800054D0((HGRenderPathScene *)&passData.fields.bloomBlurMat1stPass, v28, v29, (HGCamera *)v27, v126, v134);
			//     v31 = *(Material__Array **)(v30 + 16);
			//     passData.fields.bloomBlurMat2ndPass = v31;
			//     sub_1800054D0((HGRenderPathScene *)&passData.fields.bloomBlurMat2ndPass, v32, v33, (HGCamera *)v31, v127, v135);
			//     v35 = *(Material__Array **)(v34 + 24);
			//     passData.fields.bloomUpsampleMat = v35;
			//     sub_1800054D0((HGRenderPathScene *)&passData.fields.bloomUpsampleMat, v36, v37, (HGCamera *)v35, v128, v136);
			//     bloomPrefilterCS = passData.fields.bloomPrefilterMat;
			//     if ( !bloomPrefilterCS )
			//       goto LABEL_129;
			//     UnityEngine::Material::SetShaderKeywords((Material *)bloomPrefilterCS, 0LL, 0LL);
			//     if ( bloomQuality )
			//     {
			//       if ( bloomQuality == BloomQuality__Enum_Medium )
			//       {
			//         bloomPrefilterMat = passData.fields.bloomPrefilterMat;
			//         sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//         characterBloomControl = (__int64)TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//         if ( !bloomPrefilterMat )
			//           goto LABEL_129;
			//         v39 = *(String **)(characterBloomControl + 136);
			//       }
			//       else
			//       {
			//         if ( bloomQuality != BloomQuality__Enum_High )
			//         {
			// LABEL_26:
			//           for ( i = 0; i <= 16; ++i )
			//           {
			//             characterBloomControl = (__int64)bloom.fields.quality;
			//             if ( !characterBloomControl )
			//               goto LABEL_129;
			//             v41 = sub_18003ED00(10LL);
			//             bloomPrefilterCS = passData.fields.bloomUpsampleMat;
			//             if ( v41 )
			//             {
			//               if ( !bloomPrefilterCS )
			//                 goto LABEL_129;
			//               if ( (unsigned int)i >= *((_DWORD *)bloomPrefilterCS + 6) )
			//                 goto LABEL_48;
			//               v42 = (Material *)*((_QWORD *)bloomPrefilterCS + i + 4);
			//               sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//               if ( !v42 )
			//                 goto LABEL_129;
			//               s_HighQuality = TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_HighQuality;
			//             }
			//             else
			//             {
			//               if ( !bloomPrefilterCS )
			//                 goto LABEL_129;
			//               if ( (unsigned int)i >= *((_DWORD *)bloomPrefilterCS + 6) )
			// LABEL_48:
			//                 sub_180070270(bloomPrefilterCS, characterBloomControl);
			//               v42 = (Material *)*((_QWORD *)bloomPrefilterCS + i + 4);
			//               sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//               if ( !v42 )
			//                 goto LABEL_129;
			//               s_HighQuality = TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_LowQuality;
			//             }
			//             UnityEngine::Material::EnableKeyword(v42, s_HighQuality, 0LL);
			//           }
			//           v44 = passData.fields.bloomPrefilterMat;
			//           sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//           if ( enableAlpha )
			//           {
			//             characterBloomControl = (__int64)TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//             if ( !v44 )
			//               goto LABEL_129;
			//             UnityEngine::Material::EnableKeyword(v44, *(String **)(characterBloomControl + 152), 0LL);
			//           }
			//           else
			//           {
			//             bloomPrefilterCS = TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//             if ( !v44 )
			//               goto LABEL_129;
			//             UnityEngine::Material::DisableKeyword(v44, *((String **)bloomPrefilterCS + 19), 0LL);
			//           }
			//           v45 = passData.fields.bloomPrefilterMat;
			//           if ( passData.fields.enableCharacterMask )
			//           {
			//             sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//             characterBloomControl = (__int64)TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//             if ( !v45 )
			//               goto LABEL_129;
			//             UnityEngine::Material::EnableKeyword(v45, *(String **)(characterBloomControl + 160), 0LL);
			//           }
			//           else
			//           {
			//             sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//             bloomPrefilterCS = TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//             if ( !v45 )
			//               goto LABEL_129;
			//             UnityEngine::Material::DisableKeyword(v45, *((String **)bloomPrefilterCS + 20), 0LL);
			//           }
			// LABEL_68:
			//           scatter = HG::Rendering::Runtime::Bloom::get_scatter(bloom, 0LL);
			//           v70 = Beyond::JobMathf::ClampedLerp(v69, 0.94999999, scatter);
			//           passData.fields.bloomScatterParam = *(float *)&v70;
			//           sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//           passData.fields.thresholdParams = (Vector4)_mm_loadu_si128((const __m128i *)HG::Rendering::Runtime::UberPostPassUtils::GetBloomThresholdParams(
			//                                                                                          (Vector4 *)&v141,
			//                                                                                          bloom,
			//                                                                                          0LL));
			//           passData.fields.characterThresholdParams = (Vector4)_mm_loadu_si128((const __m128i *)HG::Rendering::Runtime::UberPostPassUtils::GetBloomCharacterThresholdParams(
			//                                                                                                   (Vector4 *)&v141,
			//                                                                                                   bloom,
			//                                                                                                   0LL));
			//           characterBloomControl = (__int64)bloom.fields.characterIntensity;
			//           if ( characterBloomControl )
			//           {
			//             v142 = (Vector4)COERCE_UNSIGNED_INT(sub_18003F9B0(10LL, characterBloomControl));
			//             passData.fields.characterParams = v142;
			//             HG::Rendering::Runtime::Bloom::get_resolution(bloom, 0LL);
			//             if ( camera )
			//             {
			//               if ( (int)HIDWORD(*(_QWORD *)&camera.fields._taauRTSize_k__BackingField) > 1080 )
			//               {
			//                 v71 = 1080.0 / (float)(int)HIDWORD(*(_QWORD *)&camera.fields._taauRTSize_k__BackingField);
			//                 v72 = v71;
			//               }
			//               else
			//               {
			//                 v71 = 1.0;
			//                 v72 = 1.0;
			//               }
			//               instance = (AttributeCollection *)HG::Rendering::Runtime::HGRenderPipelineSettingHub::get_instance(0LL);
			//               if ( instance )
			//               {
			//                 if ( System::ComponentModel::AttributeCollection::get_Count(instance, 0LL) == 4 )
			//                 {
			//                   v71 = 1.0;
			//                   v72 = 1.0;
			//                 }
			//                 characterBloomControl = (__int64)bloom.fields.anamorphic;
			//                 if ( characterBloomControl )
			//                 {
			//                   if ( (unsigned __int8)sub_1800023D0(10LL, characterBloomControl) )
			//                   {
			//                     v76 = *(_OWORD *)&camera.fields._physicalParameters_k__BackingField.m_BladeCount;
			//                     *(_OWORD *)&v145.m_RenderPass = *(_OWORD *)&camera.fields._physicalParameters_k__BackingField.m_Iso;
			//                     v77 = camera.fields._physicalParameters_k__BackingField.m_Anamorphism * 0.5;
			//                     *(_OWORD *)&v145.m_RenderGraph = v76;
			//                     if ( v77 >= 0.0 )
			//                       v78 = 1.0;
			//                     else
			//                       v78 = v77 + 1.0;
			//                     v72 = v72 * v78;
			//                     v79 = 1.0;
			//                     if ( v77 > 0.0 )
			//                       v79 = 1.0 - v77;
			//                     v71 = v71 * v79;
			//                   }
			//                   sub_182376F20(
			//                     (unsigned int)(int)(float)((float)(int)HIDWORD(*(_QWORD *)&camera.fields._taauRTSize_k__BackingField)
			//                                              * v71),
			//                     v74,
			//                     v75);
			//                   v83 = sub_182592070(v81, v80, v82);
			//                   if ( v83 < 1 )
			//                   {
			//                     v83 = 1;
			//                   }
			//                   else if ( v83 > 16 )
			//                   {
			//                     v83 = 16;
			//                   }
			//                   passData.fields.bloomMipCount = v83;
			//                   for ( j = 0; j < passData.fields.bloomMipCount; ++j )
			//                   {
			//                     v88 = sub_1802EB1B0(v84, characterBloomControl);
			//                     if ( camera.fields._DynResRequest_k__BackingField.hardwareEnabled )
			//                     {
			//                       v90 = sub_1826E82D0();
			//                       if ( v90 < 1 )
			//                         v90 = 1;
			//                       v91 = sub_1826E82D0();
			//                       if ( v91 < 1 )
			//                         v91 = 1;
			//                     }
			//                     else
			//                     {
			//                       v90 = sub_1825C6750(v87, v86);
			//                       if ( v90 < 1 )
			//                         v90 = 1;
			//                       v91 = sub_1825C6750(HIDWORD(*(_QWORD *)&camera.fields._taauRTSize_k__BackingField), v89);
			//                       characterBloomControl = 1LL;
			//                       if ( v91 < 1 )
			//                         v91 = 1;
			//                     }
			//                     bloomPrefilterCS = passData.fields.bloomMipInfo;
			//                     v142.z = (float)(1.0 / *(float *)&v88) * v72;
			//                     v142.w = (float)(1.0 / *(float *)&v88) * v71;
			//                     size.m_X = v90;
			//                     size.m_Y = v91;
			//                     v142.x = (float)v90;
			//                     v142.y = (float)v91;
			//                     if ( !bloomPrefilterCS )
			//                       goto LABEL_129;
			//                     v141 = (TextureHandle)v142;
			//                     sub_18004D910(bloomPrefilterCS, j, &v141);
			//                     HG::Rendering::RenderGraphModule::TextureDesc::TextureDesc(&v144, size, 0LL);
			//                     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//                     v144.colorFormat = HG::Rendering::Runtime::UberPostPassUtils::GetBloomTextureFormat(
			//                                          useComputeShader,
			//                                          enableAlpha,
			//                                          0LL);
			//                     v144.name = (String *)"BloomMipDown";
			//                     v144.enableRandomWrite = useComputeShader;
			//                     sub_1800054D0((HGRenderPathScene *)&v144.name, v92, v93, v94, v129, v137);
			//                     mipsDown = passData.fields.mipsDown;
			//                     v144.wrapMode = 1;
			//                     v144.filterMode = 1;
			//                     v146 = v144;
			//                     if ( useComputeShader )
			//                     {
			//                       v98 = HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::CreateTransientTexture(
			//                               &v149,
			//                               builder,
			//                               &v146,
			//                               0LL);
			//                       if ( !mipsDown )
			//                         goto LABEL_129;
			//                       v141 = *v98;
			//                       sub_1803EF6F4(mipsDown, j, &v141);
			//                     }
			//                     else
			//                     {
			//                       if ( !renderGraph )
			//                         goto LABEL_129;
			//                       v96 = HG::Rendering::RenderGraphModule::HGRenderGraph::CreateTexture(
			//                               &v147,
			//                               renderGraph,
			//                               &v146,
			//                               0LL);
			//                       if ( !mipsDown )
			//                         goto LABEL_129;
			//                       v141 = *v96;
			//                       sub_1803EF6F4(mipsDown, j, &v141);
			//                       bloomPrefilterCS = passData.fields.mipsDown;
			//                       if ( !bloomPrefilterCS )
			//                         goto LABEL_129;
			//                       v97 = (TextureHandle *)sub_18037A2E0(bloomPrefilterCS, j);
			//                       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadWriteTexture(&v148, builder, v97, 0LL);
			//                     }
			//                     if ( j && (!useComputeShader || j != passData.fields.bloomMipCount - 1) )
			//                     {
			//                       HG::Rendering::RenderGraphModule::TextureDesc::TextureDesc(&v144, size, 0LL);
			//                       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//                       v144.colorFormat = HG::Rendering::Runtime::UberPostPassUtils::GetBloomTextureFormat(
			//                                            useComputeShader,
			//                                            enableAlpha,
			//                                            0LL);
			//                       v144.name = (String *)"BloomMipUp";
			//                       v144.enableRandomWrite = useComputeShader;
			//                       sub_1800054D0((HGRenderPathScene *)&v144.name, v99, v100, v101, v129, v137);
			//                       v144.wrapMode = 1;
			//                       v144.filterMode = 1;
			//                       mipsUp = passData.fields.mipsUp;
			//                       v146 = v144;
			//                       if ( useComputeShader )
			//                       {
			//                         v105 = HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::CreateTransientTexture(
			//                                  (TextureHandle *)&v145,
			//                                  builder,
			//                                  &v146,
			//                                  0LL);
			//                         if ( !mipsUp )
			//                           goto LABEL_129;
			//                         v141 = *v105;
			//                         sub_1803EF6F4(mipsUp, j, &v141);
			//                       }
			//                       else
			//                       {
			//                         if ( !renderGraph )
			//                           goto LABEL_129;
			//                         v103 = HG::Rendering::RenderGraphModule::HGRenderGraph::CreateTexture(
			//                                  &v150,
			//                                  renderGraph,
			//                                  &v146,
			//                                  0LL);
			//                         if ( !mipsUp )
			//                           goto LABEL_129;
			//                         v141 = *v103;
			//                         sub_1803EF6F4(mipsUp, j, &v141);
			//                         bloomPrefilterCS = passData.fields.mipsUp;
			//                         if ( !bloomPrefilterCS )
			//                           goto LABEL_129;
			//                         v104 = (TextureHandle *)sub_18037A2E0(bloomPrefilterCS, j);
			//                         HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadWriteTexture(
			//                           &v151,
			//                           builder,
			//                           v104,
			//                           0LL);
			//                       }
			//                     }
			//                   }
			//                   bloomPrefilterCS = passData.fields.bloomMipInfo;
			//                   if ( bloomPrefilterCS )
			//                   {
			//                     v106 = (uint32_t *)sub_18003EB40(bloomPrefilterCS, 0LL);
			//                     bloomPrefilterCS = passData.fields.bloomMipInfo;
			//                     v107 = *v106;
			//                     if ( bloomPrefilterCS )
			//                     {
			//                       v108 = sub_18003EB40(bloomPrefilterCS, 0LL);
			//                       bloomPrefilterCS = passData.fields.bloomMipInfo;
			//                       v109 = *(_DWORD *)(v108 + 4);
			//                       if ( bloomPrefilterCS )
			//                       {
			//                         v110 = (float *)sub_18003EB40(bloomPrefilterCS, 0LL);
			//                         bloomPrefilterCS = passData.fields.bloomMipInfo;
			//                         if ( bloomPrefilterCS )
			//                         {
			//                           LODWORD(v142.x) = v107;
			//                           *(float *)&v111 = 1.0 / *v110;
			//                           LODWORD(v142.y) = v109;
			//                           LODWORD(v142.z) = v111;
			//                           v142.w = 1.0 / *(float *)(sub_18003EB40(bloomPrefilterCS, 0LL) + 4);
			//                           *bloomBicubicParam = v142;
			//                           bloomPrefilterCS = passData.fields.bloomMipInfo;
			//                           if ( bloomPrefilterCS )
			//                           {
			//                             sub_18003EB40(bloomPrefilterCS, 0LL);
			//                             bloomPrefilterCS = passData.fields.bloomMipInfo;
			//                             if ( bloomPrefilterCS )
			//                             {
			//                               sub_18003EB40(bloomPrefilterCS, 0LL);
			//                               v114 = sub_1825C6750(v113, v112);
			//                               v115 = HIDWORD(*(_QWORD *)&camera.fields._taauRTSize_k__BackingField);
			//                               size.m_X = v114;
			//                               size.m_Y = sub_1825C6750(v115, v116);
			//                               HG::Rendering::RenderGraphModule::TextureDesc::TextureDesc(&v144, size, 0LL);
			//                               v144.name = (String *)"Bloom final mip up";
			//                               sub_1800054D0((HGRenderPathScene *)&v144.name, v117, v118, v119, v129, v137);
			//                               sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//                               BloomTextureFormat = HG::Rendering::Runtime::UberPostPassUtils::GetBloomTextureFormat(
			//                                                      useComputeShader,
			//                                                      enableAlpha,
			//                                                      0LL);
			//                               v121 = passData.fields.mipsUp;
			//                               v144.colorFormat = BloomTextureFormat;
			//                               v144.wrapMode = 1;
			//                               v144.filterMode = 1;
			//                               v146 = v144;
			//                               v144.useMipMap = 0;
			//                               v144.enableRandomWrite = useComputeShader;
			//                               if ( renderGraph )
			//                               {
			//                                 v141 = *HG::Rendering::RenderGraphModule::HGRenderGraph::CreateTexture(
			//                                           (TextureHandle *)&v145,
			//                                           renderGraph,
			//                                           &v146,
			//                                           0LL);
			//                                 v122 = HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::WriteTexture(
			//                                          (TextureHandle *)&v145,
			//                                          builder,
			//                                          &v141,
			//                                          0LL);
			//                                 if ( v121 )
			//                                 {
			//                                   v141 = *v122;
			//                                   sub_1803EF6F4(v121, 0LL, &v141);
			//                                   passData.fields.source = *HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadTexture(
			//                                                                (TextureHandle *)&v145,
			//                                                                builder,
			//                                                                source,
			//                                                                0LL);
			//                                   return;
			//                                 }
			//                               }
			//                             }
			//                           }
			//                         }
			//                       }
			//                     }
			//                   }
			//                 }
			//               }
			//             }
			//           }
			// LABEL_129:
			//           sub_180B536AC(bloomPrefilterCS, characterBloomControl);
			//         }
			//         bloomPrefilterMat = passData.fields.bloomPrefilterMat;
			//         sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//         characterBloomControl = (__int64)TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//         if ( !bloomPrefilterMat )
			//           goto LABEL_129;
			//         v39 = *(String **)(characterBloomControl + 128);
			//       }
			//     }
			//     else
			//     {
			//       bloomPrefilterMat = passData.fields.bloomPrefilterMat;
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//       characterBloomControl = (__int64)TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//       if ( !bloomPrefilterMat )
			//         goto LABEL_129;
			//       v39 = *(String **)(characterBloomControl + 144);
			//     }
			//     UnityEngine::Material::EnableKeyword(bloomPrefilterMat, v39, 0LL);
			//     goto LABEL_26;
			//   }
			//   bloomPrefilterCS = IFix::WrappersManagerImpl::GetPatch(2419, 0LL);
			//   if ( !bloomPrefilterCS )
			//     goto LABEL_129;
			//   v141 = *motionVector;
			//   v123 = *(_OWORD *)&builder.m_RenderPass;
			//   v142 = (Vector4)*source;
			//   v124 = *(_OWORD *)&builder.m_RenderGraph;
			//   *(_OWORD *)&v145.m_RenderPass = v123;
			//   *(_OWORD *)&v145.m_RenderGraph = v124;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_875(
			//     (ILFixDynamicMethodWrapper_2 *)bloomPrefilterCS,
			//     useComputeShader,
			//     (Object *)renderGraph,
			//     &v145,
			//     (Object *)passData,
			//     (Object *)bloom,
			//     (Object *)camera,
			//     enableAlpha,
			//     (TextureHandle *)&v142,
			//     &v141,
			//     renderPath,
			//     taauQuality,
			//     bloomQuality,
			//     bloomPSMaterials,
			//     bloomBicubicParam,
			//     0LL);
			// }
			// 
		}

		private static Vector2Int GetMipsUp0SizeTempForCPP(Bloom bloom, HGCamera camera)
		{
			// // Vector2Int GetMipsUp0SizeTempForCPP(Bloom, HGCamera)
			// Vector2Int HG::Rendering::Runtime::UberPostPassUtils::GetMipsUp0SizeTempForCPP(
			//         Bloom *bloom,
			//         HGCamera *camera,
			//         MethodInfo *method)
			// {
			//   BoolParameter *anamorphic; // rdx
			//   __int64 v6; // rcx
			//   __int64 v7; // rdx
			//   int v8; // ecx
			//   __int64 v9; // rdx
			//   __int64 v10; // rcx
			//   __int64 v11; // rdx
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   Vector2Int v14; // [rsp+A8h] [rbp+20h]
			// 
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2421, 0LL) )
			//   {
			//     if ( bloom )
			//     {
			//       HG::Rendering::Runtime::Bloom::get_resolution(bloom, 0LL);
			//       if ( camera )
			//       {
			//         anamorphic = bloom.fields.anamorphic;
			//         if ( anamorphic )
			//         {
			//           sub_1800023D0(10LL, anamorphic);
			//           sub_1802EB1B0(v8, v7);
			//           v14.m_X = sub_1825C6750(v10, v9);
			//           v14.m_Y = sub_1825C6750(HIDWORD(*(_QWORD *)&camera.fields._taauRTSize_k__BackingField), v11);
			//           return v14;
			//         }
			//       }
			//     }
			// LABEL_7:
			//     sub_180B536AC(v6, anamorphic);
			//   }
			//   Patch = IFix::WrappersManagerImpl::GetPatch(2421, 0LL);
			//   if ( !Patch )
			//     goto LABEL_7;
			//   return IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_876(Patch, (Object *)bloom, (Object *)camera, 0LL);
			// }
			// 
			return null;
		}

		internal static TextureHandle BloomPass(HGRenderGraph renderGraph, HGCamera camera, HGSettingParameters settingParameters, Bloom bloom, bool enableAlpha, TextureHandle source, TextureHandle motionVector, TAAUQuality taauQuality, HGRenderPathInternal renderPath, ref UberPostPassUtils.BloomPSMaterials bloomPSMaterials, out Vector4 bloomBicubicParam)
		{
			// // TextureHandle BloomPass(HGRenderGraph, HGCamera, HGSettingParameters, Bloom, Boolean, TextureHandle, TextureHandle, TAAUQuality, HGRenderPathInternal, UberPostPassUtils+BloomPSMaterials ByRef, Vector4 ByRef)
			// // Hidden C++ exception states: #wind=1
			// TextureHandle *HG::Rendering::Runtime::UberPostPassUtils::BloomPass(
			//         TextureHandle *__return_ptr retstr,
			//         HGRenderGraph *renderGraph,
			//         HGCamera *camera,
			//         HGSettingParameters *settingParameters,
			//         Bloom *bloom,
			//         bool enableAlpha,
			//         TextureHandle *source,
			//         TextureHandle *motionVector,
			//         TAAUQuality__Enum taauQuality,
			//         HGRenderPathInternal__Enum renderPath,
			//         UberPostPassUtils_BloomPSMaterials *bloomPSMaterials,
			//         Vector4 *bloomBicubicParam,
			//         MethodInfo *method)
			// {
			//   TextureHandle *v16; // rdi
			//   __int64 v17; // rdx
			//   __int64 v18; // rcx
			//   __int64 v19; // rdx
			//   __int64 v20; // rcx
			//   BOOL v21; // r15d
			//   bool v22; // r12
			//   __int64 v23; // rdx
			//   __int64 v24; // rcx
			//   Int32Enum__Enum v25; // r13d
			//   HGRenderGraphDefaultResources *m_DefaultResources; // rbx
			//   TextureHandle blackTexture_k__BackingField; // xmm6
			//   ProfilingSampler *v28; // rax
			//   __int128 v29; // xmm6
			//   __int128 v30; // xmm7
			//   UberPostPassUtils_BloomPassData *v31; // r15
			//   __int64 v32; // rdx
			//   __int64 v33; // rcx
			//   __int64 v34; // r9
			//   MonitorData *mipsUp; // rcx
			//   __int64 v36; // rdx
			//   __int64 v37; // rcx
			//   ILFixDynamicMethodWrapper_2 *Patch; // rsi
			//   UberPostPassUtils_BloomPassData *v40; // [rsp+80h] [rbp-F8h] BYREF
			//   _QWORD v41[3]; // [rsp+88h] [rbp-F0h] BYREF
			//   HGRenderGraphBuilder v42; // [rsp+A0h] [rbp-D8h] BYREF
			//   TextureHandle v43; // [rsp+C0h] [rbp-B8h] BYREF
			//   TextureHandle v44; // [rsp+D0h] [rbp-A8h] BYREF
			//   HGRenderGraphBuilder v45; // [rsp+E0h] [rbp-98h] BYREF
			//   Il2CppExceptionWrapper *v46; // [rsp+100h] [rbp-78h] BYREF
			//   HGRenderGraphBuilder v47; // [rsp+110h] [rbp-68h] BYREF
			// 
			//   v16 = retstr;
			//   if ( !byte_18D9194C5 )
			//   {
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::BloomPassData>);
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::BloomPassData>);
			//     sub_18003C530(&MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//     sub_18003C530(&MethodInfo::HG::Rendering::Runtime::SettingParameter<UnityEngine::HyperGryphEngineCode::BloomQuality>::op_Implicit);
			//     sub_18003C530(&MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     sub_18003C530(&off_18D4F3A08);
			//     byte_18D9194C5 = 1;
			//   }
			//   v40 = 0LL;
			//   if ( IFix::WrappersManagerImpl::IsPatched(2422, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2422, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v37, v36);
			//     *(TextureHandle *)&v42.m_RenderPass = *motionVector;
			//     v44 = *source;
			//     *v16 = *IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_877(
			//               &v43,
			//               Patch,
			//               (Object *)renderGraph,
			//               (Object *)camera,
			//               (Object *)settingParameters,
			//               (Object *)bloom,
			//               enableAlpha,
			//               &v44,
			//               (TextureHandle *)&v42,
			//               taauQuality,
			//               renderPath,
			//               bloomPSMaterials,
			//               bloomBicubicParam,
			//               0LL);
			//   }
			//   else
			//   {
			//     if ( !bloom )
			//       sub_180B536AC(v18, v17);
			//     if ( HG::Rendering::Runtime::Bloom::IsActive(bloom, 0LL) )
			//     {
			//       if ( !settingParameters )
			//         sub_180B536AC(v20, v19);
			//       v21 = HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit(
			//               settingParameters.fields._bloomEnabled_k__BackingField,
			//               MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     }
			//     else
			//     {
			//       v21 = 0;
			//     }
			//     if ( !settingParameters )
			//       sub_180B536AC(v20, v19);
			//     v22 = HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit(
			//             settingParameters.fields._bloomUseComputeShader_k__BackingField,
			//             MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     v25 = HG::Rendering::Runtime::SettingParameter<System::Int32Enum>::op_Implicit(
			//             (SettingParameter_1_System_Int32Enum_ *)settingParameters.fields._bloomQuality_k__BackingField,
			//             MethodInfo::HG::Rendering::Runtime::SettingParameter<UnityEngine::HyperGryphEngineCode::BloomQuality>::op_Implicit);
			//     if ( !renderGraph )
			//       sub_180B536AC(v24, v23);
			//     m_DefaultResources = renderGraph.fields.m_DefaultResources;
			//     if ( !m_DefaultResources )
			//       sub_180B536AC(v24, v23);
			//     blackTexture_k__BackingField = m_DefaultResources.fields._blackTexture_k__BackingField;
			//     v43 = blackTexture_k__BackingField;
			//     *bloomBicubicParam = 0LL;
			//     if ( v21 )
			//     {
			//       v28 = UnityEngine::Rendering::ProfilingSampler::Get<System::Int32Enum>(
			//               (Int32Enum__Enum)0x95u,
			//               MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//       HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<System::Object>(
			//         &v42,
			//         renderGraph,
			//         (String *)"Bloom",
			//         (Object **)&v40,
			//         v28,
			//         1,
			//         ProfilingHGPass__Enum_None,
			//         MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::BloomPassData>);
			//       v45 = v42;
			//       v41[0] = 0LL;
			//       v41[1] = &v45;
			//       try
			//       {
			//         v29 = *(_OWORD *)&v45.m_RenderPass;
			//         v30 = *(_OWORD *)&v45.m_RenderGraph;
			//         v31 = v40;
			//         sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//         v44 = *motionVector;
			//         *(TextureHandle *)&v42.m_RenderPass = *source;
			//         *(_OWORD *)&v47.m_RenderPass = v29;
			//         *(_OWORD *)&v47.m_RenderGraph = v30;
			//         HG::Rendering::Runtime::UberPostPassUtils::PrepareBloomData(
			//           v22,
			//           renderGraph,
			//           &v47,
			//           v31,
			//           bloom,
			//           camera,
			//           enableAlpha,
			//           (TextureHandle *)&v42,
			//           &v44,
			//           renderPath,
			//           taauQuality,
			//           (BloomQuality__Enum)v25,
			//           bloomPSMaterials,
			//           bloomBicubicParam,
			//           0LL);
			//         sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//         if ( v22 )
			//           HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<System::Object>(
			//             &v45,
			//             (RenderFunc_1_System_Object_ *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_bloomCSRenderFunc,
			//             0LL,
			//             0,
			//             MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::BloomPassData>);
			//         else
			//           HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<System::Object>(
			//             &v45,
			//             (RenderFunc_1_System_Object_ *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_bloomPSRenderFunc,
			//             0LL,
			//             0,
			//             MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::BloomPassData>);
			//         if ( !v40 )
			//           sub_1802DC2C8(v33, v32);
			//         mipsUp = (MonitorData *)v40.fields.mipsUp;
			//         if ( !mipsUp )
			//           sub_1802DC2C8(0LL, v32);
			//         sub_18037A36C(mipsUp, &v42, 0LL, v34);
			//         blackTexture_k__BackingField = *(TextureHandle *)&v42.m_RenderPass;
			//         v43 = *(TextureHandle *)&v42.m_RenderPass;
			//       }
			//       catch ( Il2CppExceptionWrapper *v46 )
			//       {
			//         v41[0] = v46.ex;
			//         sub_180222690(v41);
			//         v16 = retstr;
			//         blackTexture_k__BackingField = v43;
			//         goto LABEL_21;
			//       }
			//       sub_180222690(v41);
			//     }
			// LABEL_21:
			//     *v16 = blackTexture_k__BackingField;
			//   }
			//   return v16;
			// }
			// 
			return null;
		}

		public static void PrepareColorGradingParametersFromEnvVolume(ref UberPostPassUtils.ColorGradingData data, HGCamera camera, int lutSize, Material lutBuilder2DMaterial)
		{
			// // Void PrepareColorGradingParametersFromEnvVolume(UberPostPassUtils+ColorGradingData ByRef, HGCamera, Int32, Material)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareColorGradingParametersFromEnvVolume(
			//         UberPostPassUtils_ColorGradingData **data,
			//         HGCamera *camera,
			//         int32_t lutSize,
			//         Material *lutBuilder2DMaterial,
			//         MethodInfo *method)
			// {
			//   HGEnvironmentPhase *InterpolatedPhase; // rax
			//   char *s_TonemappingACESmodified; // rdx
			//   int *static_fields; // rcx
			//   __int64 v12; // r8
			//   HGColorGradingConfig *p_colorGradingConfig; // rdx
			//   HGColorGradingConfig *v14; // rax
			//   __int64 v15; // r9
			//   HGColorGradingConfig *v16; // rcx
			//   Vector4 shadows; // xmm1
			//   __int128 v18; // xmm0
			//   __int128 v19; // xmm1
			//   __int128 v20; // xmm0
			//   __int128 v21; // xmm1
			//   __int128 v22; // xmm0
			//   __int128 v23; // xmm1
			//   __int128 v24; // xmm0
			//   Vector4 v25; // xmm1
			//   __int128 v26; // xmm1
			//   __int128 v27; // xmm0
			//   __int128 v28; // xmm1
			//   __int128 v29; // xmm0
			//   __int128 v30; // xmm1
			//   __int128 v31; // xmm0
			//   bool IsToneMappingActive; // al
			//   Material *v33; // rdi
			//   UberPostPassUtils_ColorGradingData *v34; // rdi
			//   float WhiteBalanceTemperature; // xmm7_4
			//   float WhiteBalanceTint; // xmm6_4
			//   Vector3 *ColorBalanceCoeffs; // rax
			//   UberPostPassUtils_ColorGradingData *v38; // rdi
			//   float ColorAdjustmentsContrast; // xmm0_4
			//   UberPostPassUtils_ColorGradingData *v40; // rdi
			//   float ChannelMixerRedOutBlueIn; // xmm0_4
			//   UberPostPassUtils_ColorGradingData *v42; // rdi
			//   float ChannelMixerGreenOutBlueIn; // xmm0_4
			//   UberPostPassUtils_ColorGradingData *v44; // rdi
			//   float ChannelMixerBlueOutBlueIn; // xmm0_4
			//   HGColorCurve *ColorCurves; // rax
			//   PassConstructorID__Enum__Array *v47; // r8
			//   HGCamera *v48; // r9
			//   __m128i v49; // xmm3
			//   __m128i v50; // xmm2
			//   PassConstructorID__Enum__Array *v51; // r8
			//   HGCamera *v52; // r9
			//   __m128i v53; // xmm4
			//   PassConstructorID__Enum__Array *v54; // r8
			//   HGCamera *v55; // r9
			//   PassConstructorID__Enum__Array *v56; // r8
			//   HGCamera *v57; // r9
			//   PassConstructorID__Enum__Array *v58; // r8
			//   HGCamera *v59; // r9
			//   PassConstructorID__Enum__Array *v60; // r8
			//   HGCamera *v61; // r9
			//   PassConstructorID__Enum__Array *v62; // r8
			//   HGCamera *v63; // r9
			//   __m128i v64; // xmm5
			//   PassConstructorID__Enum__Array *v65; // r8
			//   HGCamera *v66; // r9
			//   __int64 v67; // xmm5_8
			//   UberPostPassUtils_ColorGradingData *v68; // rdi
			//   MethodInfo *v69; // r8
			//   __m128i v70; // xmm1
			//   Vector4 si128; // xmm0
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   MethodInfo *limits; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *limitsa; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *limitsb; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *limitsc; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *limitsd; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *limitse; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *limitsf; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *limitsg; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *limitsh; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *v82; // [rsp+30h] [rbp-D8h]
			//   Color v83; // [rsp+38h] [rbp-D0h] BYREF
			//   Color v84[6]; // [rsp+48h] [rbp-C0h] BYREF
			//   HGColorGradingConfig colorGradingConfig; // [rsp+A8h] [rbp-60h] BYREF
			//   HGColorCurve v86; // [rsp+218h] [rbp+110h] BYREF
			//   int v87; // [rsp+268h] [rbp+160h] BYREF
			// 
			//   if ( !byte_18D9194C6 )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGColorGradingConfig);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGEnvironmentManager);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     byte_18D9194C6 = 1;
			//   }
			//   sub_1802F01E0(&colorGradingConfig, 0LL, 368LL);
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2425, 0LL) )
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGEnvironmentManager);
			//     InterpolatedPhase = HG::Rendering::Runtime::HGEnvironmentManager::GetInterpolatedPhase(camera, 0LL);
			//     if ( InterpolatedPhase )
			//     {
			//       v12 = 2LL;
			//       p_colorGradingConfig = &colorGradingConfig;
			//       v14 = &InterpolatedPhase.fields.colorGradingConfig;
			//       v15 = 2LL;
			//       v16 = v14;
			//       do
			//       {
			//         *(_OWORD *)&p_colorGradingConfig.tonemappingMode = *(_OWORD *)&v16.tonemappingMode;
			//         *(_OWORD *)&p_colorGradingConfig.colorLookupContribution = *(_OWORD *)&v16.colorLookupContribution;
			//         *(_OWORD *)&p_colorGradingConfig.colorAdjustmentsEnabled = *(_OWORD *)&v16.colorAdjustmentsEnabled;
			//         *(_OWORD *)&p_colorGradingConfig.colorAdjustmentsColorFilter.g = *(_OWORD *)&v16.colorAdjustmentsColorFilter.g;
			//         *(_OWORD *)&p_colorGradingConfig.colorAdjustmentsSaturation = *(_OWORD *)&v16.colorAdjustmentsSaturation;
			//         *(_OWORD *)&p_colorGradingConfig.channelMixerRedOutBlueIn = *(_OWORD *)&v16.channelMixerRedOutBlueIn;
			//         *(_OWORD *)&p_colorGradingConfig.channelMixerBlueOutRedIn = *(_OWORD *)&v16.channelMixerBlueOutRedIn;
			//         p_colorGradingConfig = (HGColorGradingConfig *)((char *)p_colorGradingConfig + 128);
			//         shadows = v16.shadowsMidtonesHighlights.shadows;
			//         v16 = (HGColorGradingConfig *)((char *)v16 + 128);
			//         *(Vector4 *)&p_colorGradingConfig[-1].colorCurves.masterOverriding = shadows;
			//         --v15;
			//       }
			//       while ( v15 );
			//       *(_OWORD *)&p_colorGradingConfig.tonemappingMode = *(_OWORD *)&v16.tonemappingMode;
			//       *(_OWORD *)&p_colorGradingConfig.colorLookupContribution = *(_OWORD *)&v16.colorLookupContribution;
			//       *(_OWORD *)&p_colorGradingConfig.colorAdjustmentsEnabled = *(_OWORD *)&v16.colorAdjustmentsEnabled;
			//       *(_OWORD *)&p_colorGradingConfig.colorAdjustmentsColorFilter.g = *(_OWORD *)&v16.colorAdjustmentsColorFilter.g;
			//       *(_OWORD *)&p_colorGradingConfig.colorAdjustmentsSaturation = *(_OWORD *)&v16.colorAdjustmentsSaturation;
			//       *(_OWORD *)&p_colorGradingConfig.channelMixerRedOutBlueIn = *(_OWORD *)&v16.channelMixerRedOutBlueIn;
			//       v18 = *(_OWORD *)&v16.channelMixerBlueOutRedIn;
			//       static_fields = &v87;
			//       *(_OWORD *)&p_colorGradingConfig.channelMixerBlueOutRedIn = v18;
			//       s_TonemappingACESmodified = (char *)*data;
			//       do
			//       {
			//         v19 = *(_OWORD *)&v14.colorLookupContribution;
			//         *(_OWORD *)static_fields = *(_OWORD *)&v14.tonemappingMode;
			//         v20 = *(_OWORD *)&v14.colorAdjustmentsEnabled;
			//         *((_OWORD *)static_fields + 1) = v19;
			//         v21 = *(_OWORD *)&v14.colorAdjustmentsColorFilter.g;
			//         *((_OWORD *)static_fields + 2) = v20;
			//         v22 = *(_OWORD *)&v14.colorAdjustmentsSaturation;
			//         *((_OWORD *)static_fields + 3) = v21;
			//         v23 = *(_OWORD *)&v14.channelMixerRedOutBlueIn;
			//         *((_OWORD *)static_fields + 4) = v22;
			//         v24 = *(_OWORD *)&v14.channelMixerBlueOutRedIn;
			//         *((_OWORD *)static_fields + 5) = v23;
			//         v25 = v14.shadowsMidtonesHighlights.shadows;
			//         v14 = (HGColorGradingConfig *)((char *)v14 + 128);
			//         *((_OWORD *)static_fields + 6) = v24;
			//         static_fields += 32;
			//         *((Vector4 *)static_fields - 1) = v25;
			//         --v12;
			//       }
			//       while ( v12 );
			//       v26 = *(_OWORD *)&v14.colorLookupContribution;
			//       *(_OWORD *)static_fields = *(_OWORD *)&v14.tonemappingMode;
			//       v27 = *(_OWORD *)&v14.colorAdjustmentsEnabled;
			//       *((_OWORD *)static_fields + 1) = v26;
			//       v28 = *(_OWORD *)&v14.colorAdjustmentsColorFilter.g;
			//       *((_OWORD *)static_fields + 2) = v27;
			//       v29 = *(_OWORD *)&v14.colorAdjustmentsSaturation;
			//       *((_OWORD *)static_fields + 3) = v28;
			//       v30 = *(_OWORD *)&v14.channelMixerRedOutBlueIn;
			//       *((_OWORD *)static_fields + 4) = v29;
			//       v31 = *(_OWORD *)&v14.channelMixerBlueOutRedIn;
			//       *((_OWORD *)static_fields + 5) = v30;
			//       *((_OWORD *)static_fields + 6) = v31;
			//       if ( s_TonemappingACESmodified )
			//       {
			//         *((_DWORD *)s_TonemappingACESmodified + 91) = v87;
			//         static_fields = (int *)*data;
			//         if ( *data )
			//         {
			//           *((_QWORD *)static_fields + 2) = lutBuilder2DMaterial;
			//           sub_1800054D0(
			//             (HGRenderPathScene *)(static_fields + 4),
			//             (HGRenderPathBase_HGRenderPathResources *)s_TonemappingACESmodified,
			//             0LL,
			//             0LL,
			//             limits,
			//             v82);
			//           if ( *data )
			//           {
			//             static_fields = (int *)(*data).fields.lutBuilder2DMaterial;
			//             if ( static_fields )
			//             {
			//               UnityEngine::Material::SetShaderKeywords((Material *)static_fields, 0LL, 0LL);
			//               sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGColorGradingConfig);
			//               IsToneMappingActive = HG::Rendering::Runtime::HGColorGradingConfig::IsToneMappingActive(
			//                                       &colorGradingConfig,
			//                                       0LL);
			//               static_fields = (int *)*data;
			//               if ( !IsToneMappingActive )
			//                 goto LABEL_14;
			//               if ( !static_fields )
			//                 goto LABEL_41;
			//               if ( static_fields[91] == 5 )
			//               {
			//                 v33 = (Material *)*((_QWORD *)static_fields + 2);
			//                 sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//                 s_TonemappingACESmodified = (char *)TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_TonemappingACESmodified;
			//               }
			//               else
			//               {
			// LABEL_14:
			//                 if ( !static_fields )
			//                   goto LABEL_41;
			//                 v33 = (Material *)*((_QWORD *)static_fields + 2);
			//                 sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//                 static_fields = (int *)TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//                 s_TonemappingACESmodified = (char *)*((_QWORD *)static_fields + 25);
			//               }
			//               if ( v33 )
			//               {
			//                 UnityEngine::Material::EnableKeyword(v33, (String *)s_TonemappingACESmodified, 0LL);
			//                 if ( *data )
			//                 {
			//                   (*data).fields.lutSize = lutSize;
			//                   v34 = *data;
			//                   sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGColorGradingConfig);
			//                   WhiteBalanceTemperature = HG::Rendering::Runtime::HGColorGradingConfig::GetWhiteBalanceTemperature(
			//                                               &colorGradingConfig,
			//                                               0LL);
			//                   WhiteBalanceTint = HG::Rendering::Runtime::HGColorGradingConfig::GetWhiteBalanceTint(
			//                                        &colorGradingConfig,
			//                                        0LL);
			//                   sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//                   ColorBalanceCoeffs = HG::Rendering::Runtime::UberPostPassUtils::GetColorBalanceCoeffs(
			//                                          (Vector3 *)&v83,
			//                                          WhiteBalanceTemperature,
			//                                          WhiteBalanceTint,
			//                                          0LL);
			//                   static_fields = (int *)LODWORD(ColorBalanceCoeffs.z);
			//                   if ( v34 )
			//                   {
			//                     *(_QWORD *)&v34.fields.lmsColorBalance.x = *(_QWORD *)&ColorBalanceCoeffs.x;
			//                     LODWORD(v34.fields.lmsColorBalance.z) = (_DWORD)static_fields;
			//                     v38 = *data;
			//                     v83.r = HG::Rendering::Runtime::HGColorGradingConfig::GetColorAdjustmentsHueShift(
			//                               &colorGradingConfig,
			//                               0LL)
			//                           / 360.0;
			//                     v83.g = (float)(HG::Rendering::Runtime::HGColorGradingConfig::GetColorAdjustmentsSaturation(
			//                                       &colorGradingConfig,
			//                                       0LL)
			//                                   / 100.0)
			//                           + 1.0;
			//                     ColorAdjustmentsContrast = HG::Rendering::Runtime::HGColorGradingConfig::GetColorAdjustmentsContrast(
			//                                                  &colorGradingConfig,
			//                                                  0LL);
			//                     v83.a = 0.0;
			//                     v83.b = (float)(ColorAdjustmentsContrast / 100.0) + 1.0;
			//                     if ( v38 )
			//                     {
			//                       v38.fields.hueSatCon = (Vector4)v83;
			//                       v40 = *data;
			//                       v83.r = HG::Rendering::Runtime::HGColorGradingConfig::GetChannelMixerRedOutRedIn(
			//                                 &colorGradingConfig,
			//                                 0LL)
			//                             / 100.0;
			//                       v83.g = HG::Rendering::Runtime::HGColorGradingConfig::GetChannelMixerRedOutGreenIn(
			//                                 &colorGradingConfig,
			//                                 0LL)
			//                             / 100.0;
			//                       ChannelMixerRedOutBlueIn = HG::Rendering::Runtime::HGColorGradingConfig::GetChannelMixerRedOutBlueIn(
			//                                                    &colorGradingConfig,
			//                                                    0LL);
			//                       v83.a = 0.0;
			//                       v83.b = ChannelMixerRedOutBlueIn / 100.0;
			//                       if ( v40 )
			//                       {
			//                         v40.fields.channelMixerR = (Vector4)v83;
			//                         v42 = *data;
			//                         v83.r = HG::Rendering::Runtime::HGColorGradingConfig::GetChannelMixerGreenOutRedIn(
			//                                   &colorGradingConfig,
			//                                   0LL)
			//                               / 100.0;
			//                         v83.g = HG::Rendering::Runtime::HGColorGradingConfig::GetChannelMixerGreenOutGreenIn(
			//                                   &colorGradingConfig,
			//                                   0LL)
			//                               / 100.0;
			//                         ChannelMixerGreenOutBlueIn = HG::Rendering::Runtime::HGColorGradingConfig::GetChannelMixerGreenOutBlueIn(
			//                                                        &colorGradingConfig,
			//                                                        0LL);
			//                         v83.a = 0.0;
			//                         v83.b = ChannelMixerGreenOutBlueIn / 100.0;
			//                         if ( v42 )
			//                         {
			//                           v42.fields.channelMixerG = (Vector4)v83;
			//                           v44 = *data;
			//                           v83.r = HG::Rendering::Runtime::HGColorGradingConfig::GetChannelMixerBlueOutRedIn(
			//                                     &colorGradingConfig,
			//                                     0LL)
			//                                 / 100.0;
			//                           v83.g = HG::Rendering::Runtime::HGColorGradingConfig::GetChannelMixerBlueOutGreenIn(
			//                                     &colorGradingConfig,
			//                                     0LL)
			//                                 / 100.0;
			//                           ChannelMixerBlueOutBlueIn = HG::Rendering::Runtime::HGColorGradingConfig::GetChannelMixerBlueOutBlueIn(
			//                                                         &colorGradingConfig,
			//                                                         0LL);
			//                           v83.a = 0.0;
			//                           v83.b = ChannelMixerBlueOutBlueIn / 100.0;
			//                           if ( v44 )
			//                           {
			//                             v44.fields.channelMixerB = (Vector4)v83;
			//                             s_TonemappingACESmodified = (char *)*data;
			//                             if ( *data )
			//                             {
			//                               HG::Rendering::Runtime::UberPostPassUtils::ComputeShadowsMidtonesHighlightsFromEnvVolume(
			//                                 &colorGradingConfig,
			//                                 (Vector4 *)(s_TonemappingACESmodified + 120),
			//                                 (Vector4 *)(s_TonemappingACESmodified + 136),
			//                                 (Vector4 *)(s_TonemappingACESmodified + 152),
			//                                 (Vector4 *)(s_TonemappingACESmodified + 168),
			//                                 0LL);
			//                               s_TonemappingACESmodified = (char *)*data;
			//                               if ( *data )
			//                               {
			//                                 HG::Rendering::Runtime::UberPostPassUtils::ComputeLiftGammaGainFromEnvVolume(
			//                                   &colorGradingConfig,
			//                                   (Vector4 *)(s_TonemappingACESmodified + 184),
			//                                   (Vector4 *)(s_TonemappingACESmodified + 200),
			//                                   (Vector4 *)(s_TonemappingACESmodified + 216),
			//                                   0LL);
			//                                 s_TonemappingACESmodified = (char *)*data;
			//                                 if ( *data )
			//                                 {
			//                                   HG::Rendering::Runtime::UberPostPassUtils::ComputeSplitToningFromEnvVolume(
			//                                     &colorGradingConfig,
			//                                     (Vector4 *)(s_TonemappingACESmodified + 232),
			//                                     (Vector4 *)(s_TonemappingACESmodified + 248),
			//                                     0LL);
			//                                   ColorCurves = HG::Rendering::Runtime::HGColorGradingConfig::GetColorCurves(
			//                                                   &v86,
			//                                                   &colorGradingConfig,
			//                                                   0LL);
			//                                   static_fields = (int *)*data;
			//                                   v49 = *(__m128i *)&ColorCurves.green;
			//                                   v50 = *(__m128i *)&ColorCurves.hueVsHue;
			//                                   if ( *data )
			//                                   {
			//                                     *((_QWORD *)static_fields + 34) = ColorCurves.master;
			//                                     sub_1800054D0(
			//                                       (HGRenderPathScene *)(static_fields + 68),
			//                                       (HGRenderPathBase_HGRenderPathResources *)s_TonemappingACESmodified,
			//                                       v47,
			//                                       v48,
			//                                       limitsa,
			//                                       v82);
			//                                     static_fields = (int *)*data;
			//                                     if ( *data )
			//                                     {
			//                                       *((_QWORD *)static_fields + 35) = _mm_srli_si128(v53, 8).m128i_u64[0];
			//                                       sub_1800054D0(
			//                                         (HGRenderPathScene *)(static_fields + 70),
			//                                         (HGRenderPathBase_HGRenderPathResources *)s_TonemappingACESmodified,
			//                                         v51,
			//                                         v52,
			//                                         limitsb,
			//                                         v82);
			//                                       static_fields = (int *)*data;
			//                                       if ( *data )
			//                                       {
			//                                         *((_QWORD *)static_fields + 36) = v49.m128i_i64[0];
			//                                         sub_1800054D0(
			//                                           (HGRenderPathScene *)(static_fields + 72),
			//                                           (HGRenderPathBase_HGRenderPathResources *)s_TonemappingACESmodified,
			//                                           v54,
			//                                           v55,
			//                                           limitsc,
			//                                           v82);
			//                                         static_fields = (int *)*data;
			//                                         if ( *data )
			//                                         {
			//                                           *((_QWORD *)static_fields + 37) = _mm_srli_si128(v49, 8).m128i_u64[0];
			//                                           sub_1800054D0(
			//                                             (HGRenderPathScene *)(static_fields + 74),
			//                                             (HGRenderPathBase_HGRenderPathResources *)s_TonemappingACESmodified,
			//                                             v56,
			//                                             v57,
			//                                             limitsd,
			//                                             v82);
			//                                           static_fields = (int *)*data;
			//                                           if ( *data )
			//                                           {
			//                                             *((_QWORD *)static_fields + 38) = v50.m128i_i64[0];
			//                                             sub_1800054D0(
			//                                               (HGRenderPathScene *)(static_fields + 76),
			//                                               (HGRenderPathBase_HGRenderPathResources *)s_TonemappingACESmodified,
			//                                               v58,
			//                                               v59,
			//                                               limitse,
			//                                               v82);
			//                                             static_fields = (int *)*data;
			//                                             if ( *data )
			//                                             {
			//                                               *((_QWORD *)static_fields + 39) = _mm_srli_si128(v50, 8).m128i_u64[0];
			//                                               sub_1800054D0(
			//                                                 (HGRenderPathScene *)(static_fields + 78),
			//                                                 (HGRenderPathBase_HGRenderPathResources *)s_TonemappingACESmodified,
			//                                                 v60,
			//                                                 v61,
			//                                                 limitsf,
			//                                                 v82);
			//                                               static_fields = (int *)*data;
			//                                               if ( *data )
			//                                               {
			//                                                 *((_QWORD *)static_fields + 40) = _mm_srli_si128(v64, 8).m128i_u64[0];
			//                                                 sub_1800054D0(
			//                                                   (HGRenderPathScene *)(static_fields + 80),
			//                                                   (HGRenderPathBase_HGRenderPathResources *)s_TonemappingACESmodified,
			//                                                   v62,
			//                                                   v63,
			//                                                   limitsg,
			//                                                   v82);
			//                                                 static_fields = (int *)*data;
			//                                                 if ( *data )
			//                                                 {
			//                                                   *((_QWORD *)static_fields + 41) = v67;
			//                                                   sub_1800054D0(
			//                                                     (HGRenderPathScene *)(static_fields + 82),
			//                                                     (HGRenderPathBase_HGRenderPathResources *)s_TonemappingACESmodified,
			//                                                     v65,
			//                                                     v66,
			//                                                     limitsh,
			//                                                     v82);
			//                                                   v68 = *data;
			//                                                   v83 = *HG::Rendering::Runtime::HGColorGradingConfig::GetColorAdjustmentsColorFilter(
			//                                                            &v83,
			//                                                            &colorGradingConfig,
			//                                                            0LL);
			//                                                   v83 = *(Color *)sub_182F8C840(v84, &v83);
			//                                                   v70 = _mm_loadu_si128((const __m128i *)UnityEngine::Color::op_Implicit(
			//                                                                                            v84,
			//                                                                                            (Vector4 *)&v83,
			//                                                                                            v69));
			//                                                   if ( v68 )
			//                                                   {
			//                                                     si128 = (Vector4)_mm_load_si128((const __m128i *)&xmmword_18A3576A0);
			//                                                     v68.fields.colorFilter = (Vector4)v70;
			//                                                     if ( *data )
			//                                                     {
			//                                                       (*data).fields.miscParams = si128;
			//                                                       return;
			//                                                     }
			//                                                   }
			//                                                 }
			//                                               }
			//                                             }
			//                                           }
			//                                         }
			//                                       }
			//                                     }
			//                                   }
			//                                 }
			//                               }
			//                             }
			//                           }
			//                         }
			//                       }
			//                     }
			//                   }
			//                 }
			//               }
			//             }
			//           }
			//         }
			//       }
			//     }
			// LABEL_41:
			//     sub_180B536AC(static_fields, s_TonemappingACESmodified);
			//   }
			//   Patch = IFix::WrappersManagerImpl::GetPatch(2425, 0LL);
			//   if ( !Patch )
			//     goto LABEL_41;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_881(
			//     Patch,
			//     data,
			//     (Object *)camera,
			//     lutSize,
			//     (Object *)lutBuilder2DMaterial,
			//     0LL);
			// }
			// 
		}

		private static bool IsAnyColorCurveDirty(ref UberPostPassUtils.ColorGradingData passData)
		{
			// // Boolean IsAnyColorCurveDirty(UberPostPassUtils+ColorGradingData ByRef)
			// bool HG::Rendering::Runtime::UberPostPassUtils::IsAnyColorCurveDirty(
			//         UberPostPassUtils_ColorGradingData **passData,
			//         MethodInfo *method)
			// {
			//   __int64 v3; // rdx
			//   TextureCurve *masterCurve; // rcx
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			// 
			//   if ( IFix::WrappersManagerImpl::IsPatched(2430, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2430, 0LL);
			//     if ( !Patch )
			//       goto LABEL_28;
			//     return IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_882(Patch, passData, 0LL);
			//   }
			//   else
			//   {
			//     if ( !*passData )
			//       goto LABEL_28;
			//     masterCurve = (*passData).fields.masterCurve;
			//     if ( !masterCurve )
			//       goto LABEL_28;
			//     if ( !UnityEngine::Rendering::TextureCurve::IsTextureCurveDirty(masterCurve, 0LL) )
			//     {
			//       if ( !*passData )
			//         goto LABEL_28;
			//       masterCurve = (*passData).fields.redCurve;
			//       if ( !masterCurve )
			//         goto LABEL_28;
			//       if ( !UnityEngine::Rendering::TextureCurve::IsTextureCurveDirty(masterCurve, 0LL) )
			//       {
			//         if ( !*passData )
			//           goto LABEL_28;
			//         masterCurve = (*passData).fields.greenCurve;
			//         if ( !masterCurve )
			//           goto LABEL_28;
			//         if ( !UnityEngine::Rendering::TextureCurve::IsTextureCurveDirty(masterCurve, 0LL) )
			//         {
			//           if ( !*passData )
			//             goto LABEL_28;
			//           masterCurve = (*passData).fields.blueCurve;
			//           if ( !masterCurve )
			//             goto LABEL_28;
			//           if ( !UnityEngine::Rendering::TextureCurve::IsTextureCurveDirty(masterCurve, 0LL) )
			//           {
			//             if ( !*passData )
			//               goto LABEL_28;
			//             masterCurve = (*passData).fields.hueVsHueCurve;
			//             if ( !masterCurve )
			//               goto LABEL_28;
			//             if ( !UnityEngine::Rendering::TextureCurve::IsTextureCurveDirty(masterCurve, 0LL) )
			//             {
			//               if ( !*passData )
			//                 goto LABEL_28;
			//               masterCurve = (*passData).fields.hueVsSatCurve;
			//               if ( !masterCurve )
			//                 goto LABEL_28;
			//               if ( !UnityEngine::Rendering::TextureCurve::IsTextureCurveDirty(masterCurve, 0LL) )
			//               {
			//                 if ( !*passData )
			//                   goto LABEL_28;
			//                 masterCurve = (*passData).fields.satVsSatCurve;
			//                 if ( !masterCurve )
			//                   goto LABEL_28;
			//                 if ( !UnityEngine::Rendering::TextureCurve::IsTextureCurveDirty(masterCurve, 0LL) )
			//                 {
			//                   if ( *passData )
			//                   {
			//                     masterCurve = (*passData).fields.lumVsSatCurve;
			//                     if ( masterCurve )
			//                       return UnityEngine::Rendering::TextureCurve::IsTextureCurveDirty(masterCurve, 0LL);
			//                   }
			// LABEL_28:
			//                   sub_180B536AC(masterCurve, v3);
			//                 }
			//               }
			//             }
			//           }
			//         }
			//       }
			//     }
			//     return 1;
			//   }
			// }
			// 
			return default(bool);
		}

		private static Vector3 GetColorBalanceCoeffs(float temperature, float tint)
		{
			// // Vector3 GetColorBalanceCoeffs(Single, Single)
			// Vector3 *HG::Rendering::Runtime::UberPostPassUtils::GetColorBalanceCoeffs(
			//         Vector3 *__return_ptr retstr,
			//         float temperature,
			//         float tint,
			//         MethodInfo *method)
			// {
			//   float v5; // xmm0_4
			//   float v6; // xmm6_4
			//   Vector3 *v7; // rax
			//   float v8; // xmm2_4
			//   float v9; // xmm1_4
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   __int64 v11; // rdx
			//   __int64 v12; // rcx
			//   Vector3 *v13; // rax
			//   __int64 v14; // xmm0_8
			//   Vector3 v16; // [rsp+40h] [rbp-38h] BYREF
			// 
			//   if ( !byte_18D9194C7 )
			//   {
			//     sub_18003C530(&TypeInfo::UnityEngine::Rendering::ColorUtils);
			//     byte_18D9194C7 = 1;
			//   }
			//   if ( IFix::WrappersManagerImpl::IsPatched(2426, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2426, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v12, v11);
			//     v13 = IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_547(&v16, Patch, temperature, tint, 0LL);
			//     v14 = *(_QWORD *)&v13.x;
			//     *(float *)&v13 = v13.z;
			//     *(_QWORD *)&retstr.x = v14;
			//     LODWORD(retstr.z) = (_DWORD)v13;
			//   }
			//   else
			//   {
			//     if ( (float)(temperature / 65.0) >= 0.0 )
			//       v5 = 0.050000001;
			//     else
			//       v5 = 0.1;
			//     v6 = 0.31270999 - (float)(v5 * (float)(temperature / 65.0));
			//     sub_180002C70(TypeInfo::UnityEngine::Rendering::ColorUtils);
			//     v7 = UnityEngine::Rendering::ColorUtils::CIExyToLMS(
			//            &v16,
			//            v6,
			//            (float)((float)((float)(v6 * 2.8699999) - (float)((float)(v6 * 3.0) * v6)) - 0.27509508)
			//          + (float)((float)(tint / 65.0) * 0.050000001),
			//            0LL);
			//     v8 = 1.08728 / v7.z;
			//     v9 = 1.0354199 / COERCE_FLOAT(HIDWORD(*(_QWORD *)&v7.x));
			//     retstr.x = 0.94923699 / COERCE_FLOAT(*(_QWORD *)&v7.x);
			//     retstr.y = v9;
			//     retstr.z = v8;
			//   }
			//   return retstr;
			// }
			// 
			return null;
		}

		private static void ComputeShadowsMidtonesHighlightsFromEnvVolume(ref HGColorGradingConfig colorGradingConfig, out Vector4 shadows, out Vector4 midtones, out Vector4 highlights, out Vector4 limits)
		{
			// // Void ComputeShadowsMidtonesHighlightsFromEnvVolume(HGColorGradingConfig ByRef, Vector4 ByRef, Vector4 ByRef, Vector4 ByRef, Vector4 ByRef)
			// void HG::Rendering::Runtime::UberPostPassUtils::ComputeShadowsMidtonesHighlightsFromEnvVolume(
			//         HGColorGradingConfig *colorGradingConfig,
			//         Vector4 *shadows,
			//         Vector4 *midtones,
			//         Vector4 *highlights,
			//         Vector4 *limits,
			//         MethodInfo *method)
			// {
			//   HGShadowsMidtonesHighlights *ShadowsMidtonesHighlights; // rax
			//   Vector4 v11; // xmm10
			//   Vector4 v12; // xmm11
			//   __m128 v13; // xmm7
			//   float v14; // xmm2_4
			//   MethodInfo *v15; // rdx
			//   float v16; // xmm6_4
			//   float v17; // xmm1_4
			//   float v18; // xmm1_4
			//   float v19; // xmm2_4
			//   MethodInfo *v20; // rdx
			//   float v21; // xmm1_4
			//   float v22; // xmm1_4
			//   float v23; // xmm2_4
			//   MethodInfo *v24; // rdx
			//   float v25; // xmm6_4
			//   __int64 v26; // rdx
			//   ILFixDynamicMethodWrapper_2 *Patch; // rcx
			//   Vector4 v28; // [rsp+40h] [rbp-108h]
			//   HGShadowsMidtonesHighlights v29[2]; // [rsp+A0h] [rbp-A8h] BYREF
			// 
			//   if ( !byte_18D9194C8 )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGColorGradingConfig);
			//     byte_18D9194C8 = 1;
			//   }
			//   if ( IFix::WrappersManagerImpl::IsPatched(2427, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2427, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(0LL, v26);
			//     IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_878(
			//       Patch,
			//       colorGradingConfig,
			//       shadows,
			//       midtones,
			//       highlights,
			//       limits,
			//       0LL);
			//   }
			//   else
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGColorGradingConfig);
			//     ShadowsMidtonesHighlights = HG::Rendering::Runtime::HGColorGradingConfig::GetShadowsMidtonesHighlights(
			//                                   v29,
			//                                   colorGradingConfig,
			//                                   0LL);
			//     v11 = ShadowsMidtonesHighlights.midtones;
			//     v12 = ShadowsMidtonesHighlights.highlights;
			//     v13 = *(__m128 *)&ShadowsMidtonesHighlights.shadowsStart;
			//     *shadows = ShadowsMidtonesHighlights.shadows;
			//     shadows.x = UnityEngine::Mathf::GammaToLinearSpace(shadows.x, 0LL);
			//     shadows.y = UnityEngine::Mathf::GammaToLinearSpace(shadows.y, 0LL);
			//     v14 = UnityEngine::Mathf::GammaToLinearSpace(shadows.z, 0LL);
			//     v16 = 1.0;
			//     if ( UnityEngine::Mathf::Sign(shadows.w, v15) >= 0.0 )
			//       v17 = 4.0;
			//     else
			//       v17 = 1.0;
			//     v18 = v17 * shadows.w;
			//     shadows.x = fmaxf(v18 + shadows.x, 0.0);
			//     shadows.y = fmaxf(v18 + shadows.y, 0.0);
			//     shadows.w = 0.0;
			//     shadows.z = fmaxf(v14 + v18, 0.0);
			//     *midtones = v11;
			//     midtones.x = UnityEngine::Mathf::GammaToLinearSpace(midtones.x, 0LL);
			//     midtones.y = UnityEngine::Mathf::GammaToLinearSpace(midtones.y, 0LL);
			//     v19 = UnityEngine::Mathf::GammaToLinearSpace(midtones.z, 0LL);
			//     if ( UnityEngine::Mathf::Sign(midtones.w, v20) >= 0.0 )
			//       v21 = 4.0;
			//     else
			//       v21 = 1.0;
			//     v22 = v21 * midtones.w;
			//     midtones.x = fmaxf(v22 + midtones.x, 0.0);
			//     midtones.y = fmaxf(v22 + midtones.y, 0.0);
			//     midtones.w = 0.0;
			//     midtones.z = fmaxf(v19 + v22, 0.0);
			//     *highlights = v12;
			//     highlights.x = UnityEngine::Mathf::GammaToLinearSpace(highlights.x, 0LL);
			//     highlights.y = UnityEngine::Mathf::GammaToLinearSpace(highlights.y, 0LL);
			//     v23 = UnityEngine::Mathf::GammaToLinearSpace(highlights.z, 0LL);
			//     if ( UnityEngine::Mathf::Sign(highlights.w, v24) >= 0.0 )
			//       v16 = 4.0;
			//     v25 = v16 * highlights.w;
			//     highlights.x = fmaxf(v25 + highlights.x, 0.0);
			//     highlights.y = fmaxf(v25 + highlights.y, 0.0);
			//     highlights.w = 0.0;
			//     LODWORD(v28.x) = v13.m128_i32[0];
			//     LODWORD(v28.y) = _mm_shuffle_ps(v13, v13, 85).m128_u32[0];
			//     LODWORD(v28.z) = _mm_shuffle_ps(v13, v13, 170).m128_u32[0];
			//     LODWORD(v28.w) = _mm_shuffle_ps(v13, v13, 255).m128_u32[0];
			//     highlights.z = fmaxf(v23 + v25, 0.0);
			//     *limits = v28;
			//   }
			// }
			// 
		}

		private static void ComputeLiftGammaGainFromEnvVolume(ref HGColorGradingConfig colorGradingConfig, out Vector4 lift, out Vector4 gamma, out Vector4 gain)
		{
			// // Void ComputeLiftGammaGainFromEnvVolume(HGColorGradingConfig ByRef, Vector4 ByRef, Vector4 ByRef, Vector4 ByRef)
			// void HG::Rendering::Runtime::UberPostPassUtils::ComputeLiftGammaGainFromEnvVolume(
			//         HGColorGradingConfig *colorGradingConfig,
			//         Vector4 *lift,
			//         Vector4 *gamma,
			//         Vector4 *gain,
			//         MethodInfo *method)
			// {
			//   HGLiftGammaGain *LiftGammaGain; // rax
			//   Vector4 v10; // xmm7
			//   Vector4 v11; // xmm11
			//   MethodInfo *v12; // r8
			//   __m128 v13; // xmm6
			//   float v14; // xmm2_4
			//   float v15; // xmm1_4
			//   float v16; // xmm0_4
			//   float v17; // xmm0_4
			//   MethodInfo *v18; // r8
			//   __m128 v19; // xmm3
			//   float v20; // xmm0_4
			//   float v21; // xmm1_4
			//   MethodInfo *v22; // r8
			//   __m128 v23; // xmm2
			//   float v24; // xmm0_4
			//   float x; // xmm1_4
			//   float v26; // xmm1_4
			//   float y; // xmm0_4
			//   float v28; // xmm0_4
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   __int64 v30; // rdx
			//   __int64 v31; // rcx
			//   Vector4 v32; // [rsp+38h] [rbp-71h] BYREF
			//   Color v33[4]; // [rsp+48h] [rbp-61h] BYREF
			//   HGLiftGammaGain v34[2]; // [rsp+90h] [rbp-19h] BYREF
			// 
			//   if ( !byte_18D9194C9 )
			//   {
			//     sub_18003C530(&TypeInfo::UnityEngine::Rendering::ColorUtils);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGColorGradingConfig);
			//     byte_18D9194C9 = 1;
			//   }
			//   if ( IFix::WrappersManagerImpl::IsPatched(2428, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2428, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v31, v30);
			//     IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_879(Patch, colorGradingConfig, lift, gamma, gain, 0LL);
			//   }
			//   else
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGColorGradingConfig);
			//     LiftGammaGain = HG::Rendering::Runtime::HGColorGradingConfig::GetLiftGammaGain(v34, colorGradingConfig, 0LL);
			//     v10 = LiftGammaGain.gamma;
			//     v11 = LiftGammaGain.gain;
			//     *lift = LiftGammaGain.lift;
			//     lift.x = UnityEngine::Mathf::GammaToLinearSpace(lift.x, 0LL) * 0.15000001;
			//     lift.y = UnityEngine::Mathf::GammaToLinearSpace(lift.y, 0LL) * 0.15000001;
			//     lift.z = UnityEngine::Mathf::GammaToLinearSpace(lift.z, 0LL) * 0.15000001;
			//     v32 = *lift;
			//     v13 = (__m128)_mm_loadu_si128((const __m128i *)UnityEngine::Color::op_Implicit(v33, &v32, v12));
			//     sub_180002C70(TypeInfo::UnityEngine::Rendering::ColorUtils);
			//     v14 = (float)((float)(v13.m128_f32[0] * 0.2126729) + (float)(_mm_shuffle_ps(v13, v13, 85).m128_f32[0] * 0.7151522))
			//         + (float)(_mm_shuffle_ps(v13, v13, 170).m128_f32[0] * 0.072175004);
			//     v15 = (float)(lift.y - v14) + lift.w;
			//     lift.x = (float)(lift.x - v14) + lift.w;
			//     v16 = lift.z - v14;
			//     lift.y = v15;
			//     v17 = v16 + lift.w;
			//     lift.w = 0.0;
			//     lift.z = v17;
			//     *gamma = v10;
			//     gamma.x = UnityEngine::Mathf::GammaToLinearSpace(gamma.x, 0LL) * 0.80000001;
			//     gamma.y = UnityEngine::Mathf::GammaToLinearSpace(gamma.y, 0LL) * 0.80000001;
			//     gamma.z = UnityEngine::Mathf::GammaToLinearSpace(gamma.z, 0LL) * 0.80000001;
			//     v32 = *gamma;
			//     v19 = (__m128)_mm_loadu_si128((const __m128i *)UnityEngine::Color::op_Implicit(v33, &v32, v18));
			//     v20 = gamma.w + 1.0;
			//     v19.m128_f32[0] = (float)((float)(v19.m128_f32[0] * 0.2126729)
			//                             + (float)(_mm_shuffle_ps(v19, v19, 85).m128_f32[0] * 0.7151522))
			//                     + (float)(_mm_shuffle_ps(v19, v19, 170).m128_f32[0] * 0.072175004);
			//     gamma.w = v20;
			//     gamma.x = 1.0 / fmaxf((float)(gamma.x - v19.m128_f32[0]) + v20, 0.001);
			//     gamma.y = 1.0 / fmaxf((float)(gamma.y - v19.m128_f32[0]) + gamma.w, 0.001);
			//     v21 = (float)(gamma.z - v19.m128_f32[0]) + gamma.w;
			//     gamma.w = 0.0;
			//     gamma.z = 1.0 / fmaxf(v21, 0.001);
			//     *gain = v11;
			//     gain.x = UnityEngine::Mathf::GammaToLinearSpace(gain.x, 0LL) * 0.80000001;
			//     gain.y = UnityEngine::Mathf::GammaToLinearSpace(gain.y, 0LL) * 0.80000001;
			//     v19.m128_f32[0] = UnityEngine::Mathf::GammaToLinearSpace(gain.z, 0LL) * 0.80000001;
			//     gain.z = v19.m128_f32[0];
			//     v32 = *gain;
			//     v23 = (__m128)_mm_loadu_si128((const __m128i *)UnityEngine::Color::op_Implicit(v33, &v32, v22));
			//     v24 = gain.w + 1.0;
			//     v23.m128_f32[0] = (float)((float)(v23.m128_f32[0] * 0.2126729)
			//                             + (float)(_mm_shuffle_ps(v23, v23, 85).m128_f32[0] * 0.7151522))
			//                     + (float)(_mm_shuffle_ps(v23, v23, 170).m128_f32[0] * 0.072175004);
			//     x = gain.x;
			//     gain.w = v24;
			//     v26 = (float)(x - v23.m128_f32[0]) + v24;
			//     y = gain.y;
			//     v19.m128_f32[0] = (float)(v19.m128_f32[0] - v23.m128_f32[0]) + gain.w;
			//     gain.x = v26;
			//     LODWORD(gain.z) = v19.m128_i32[0];
			//     v28 = (float)(y - v23.m128_f32[0]) + gain.w;
			//     gain.w = 0.0;
			//     gain.y = v28;
			//   }
			// }
			// 
		}

		private static void ComputeSplitToningFromEnvVolume(ref HGColorGradingConfig colorGradingCfg, out Vector4 shadows, out Vector4 highlights)
		{
			// // Void ComputeSplitToningFromEnvVolume(HGColorGradingConfig ByRef, Vector4 ByRef, Vector4 ByRef)
			// void HG::Rendering::Runtime::UberPostPassUtils::ComputeSplitToningFromEnvVolume(
			//         HGColorGradingConfig *colorGradingCfg,
			//         Vector4 *shadows,
			//         Vector4 *highlights,
			//         MethodInfo *method)
			// {
			//   MethodInfo *v7; // r8
			//   MethodInfo *v8; // r8
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   __int64 v10; // rdx
			//   __int64 v11; // rcx
			//   Color v12; // [rsp+30h] [rbp-28h] BYREF
			//   Color v13; // [rsp+40h] [rbp-18h] BYREF
			// 
			//   if ( !byte_18D9194CA )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGColorGradingConfig);
			//     byte_18D9194CA = 1;
			//   }
			//   if ( IFix::WrappersManagerImpl::IsPatched(2429, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2429, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v11, v10);
			//     IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_880(Patch, colorGradingCfg, shadows, highlights, 0LL);
			//   }
			//   else
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGColorGradingConfig);
			//     v12 = *HG::Rendering::Runtime::HGColorGradingConfig::GetSplitToningShadows(&v12, colorGradingCfg, 0LL);
			//     *(__m128i *)shadows = _mm_loadu_si128((const __m128i *)UnityEngine::Color::op_Implicit(&v13, (Vector4 *)&v12, v7));
			//     v12 = *HG::Rendering::Runtime::HGColorGradingConfig::GetSplitToningHighlights(&v13, colorGradingCfg, 0LL);
			//     *(__m128i *)highlights = _mm_loadu_si128((const __m128i *)UnityEngine::Color::op_Implicit(&v13, (Vector4 *)&v12, v8));
			//     shadows.w = HG::Rendering::Runtime::HGColorGradingConfig::GetSplitToningBalance(colorGradingCfg, 0LL) / 100.0;
			//     highlights.w = 0.0;
			//   }
			// }
			// 
		}

		public static void PrepareLutBuilderMaterial(UberPostPassUtils.ColorGradingData data, Material lutBuilderMaterial)
		{
			// // Void PrepareLutBuilderMaterial(UberPostPassUtils+ColorGradingData, Material)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareLutBuilderMaterial(
			//         UberPostPassUtils_ColorGradingData *data,
			//         Material *lutBuilderMaterial,
			//         MethodInfo *method)
			// {
			//   __int64 v5; // rdx
			//   TextureCurve *masterCurve; // rcx
			//   int32_t lutSize; // esi
			//   TextureHandle logLut; // xmm6
			//   int32_t OutputTexture; // r14d
			//   Texture *v10; // rax
			//   __m128i v11; // xmm1
			//   int32_t Size; // edx
			//   int32_t Lut_Params; // edx
			//   MethodInfo *v14; // r8
			//   int32_t v15; // r10d
			//   HGShaderIDs__StaticFields *static_fields; // rcx
			//   HGShaderIDs__StaticFields *v17; // rcx
			//   HGShaderIDs__StaticFields *v18; // rcx
			//   HGShaderIDs__StaticFields *v19; // rcx
			//   HGShaderIDs__StaticFields *v20; // rcx
			//   HGShaderIDs__StaticFields *v21; // rcx
			//   HGShaderIDs__StaticFields *v22; // rcx
			//   HGShaderIDs__StaticFields *v23; // rcx
			//   HGShaderIDs__StaticFields *v24; // rcx
			//   HGShaderIDs__StaticFields *v25; // rcx
			//   HGShaderIDs__StaticFields *v26; // rcx
			//   HGShaderIDs__StaticFields *v27; // rcx
			//   HGShaderIDs__StaticFields *v28; // rcx
			//   HGShaderIDs__StaticFields *v29; // rcx
			//   int32_t CurveMaster; // esi
			//   Texture2D *Texture; // rax
			//   int32_t CurveRed; // esi
			//   Texture2D *v33; // rax
			//   int32_t CurveGreen; // esi
			//   Texture2D *v35; // rax
			//   int32_t CurveBlue; // esi
			//   Texture2D *v37; // rax
			//   int32_t CurveHueVsHue; // esi
			//   Texture2D *v39; // rax
			//   int32_t CurveHueVsSat; // esi
			//   Texture2D *v41; // rax
			//   int32_t CurveLumVsSat; // esi
			//   Texture2D *v43; // rax
			//   int32_t CurveSatVsSat; // esi
			//   Texture2D *v45; // rax
			//   HGShaderIDs__StaticFields *v46; // rcx
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   Vector4 colorFilter; // [rsp+20h] [rbp-30h] BYREF
			//   Vector4 v49; // [rsp+30h] [rbp-20h] BYREF
			// 
			//   if ( !byte_18D9194CB )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGShaderIDs);
			//     sub_18003C530(&TypeInfo::HG::Rendering::RenderGraphModule::TextureHandle);
			//     byte_18D9194CB = 1;
			//   }
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2431, 0LL) )
			//   {
			//     if ( data )
			//     {
			//       lutSize = data.fields.lutSize;
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderIDs);
			//       logLut = data.fields.logLut;
			//       OutputTexture = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields._OutputTexture;
			//       sub_180002C70(TypeInfo::HG::Rendering::RenderGraphModule::TextureHandle);
			//       colorFilter = (Vector4)logLut;
			//       v10 = HG::Rendering::RenderGraphModule::TextureHandle::op_Implicit((TextureHandle *)&colorFilter, 0LL);
			//       if ( lutBuilderMaterial )
			//       {
			//         UnityEngine::Material::SetTextureImpl(lutBuilderMaterial, OutputTexture, v10, 0LL);
			//         v11 = _mm_cvtsi32_si128(data.fields.lutSize);
			//         *(_QWORD *)&colorFilter.z = 0LL;
			//         Size = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields._Size;
			//         LODWORD(colorFilter.x) = _mm_cvtepi32_ps(v11).m128_u32[0];
			//         colorFilter.y = 1.0 / (float)(colorFilter.x - 1.0);
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, Size, &colorFilter, 0LL);
			//         Lut_Params = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields._Lut_Params;
			//         colorFilter.x = (float)lutSize;
			//         colorFilter.z = 0.5 / (float)lutSize;
			//         colorFilter.y = 0.5 / (float)(lutSize * lutSize);
			//         colorFilter.w = colorFilter.x / (float)(colorFilter.x - 1.0);
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, Lut_Params, &colorFilter, 0LL);
			//         *(_QWORD *)&colorFilter.x = *(_QWORD *)&data.fields.lmsColorBalance.x;
			//         colorFilter.z = data.fields.lmsColorBalance.z;
			//         colorFilter = *UnityEngine::Vector4::op_Implicit(&v49, (Vector3 *)&colorFilter, v14);
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, v15, &colorFilter, 0LL);
			//         static_fields = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//         colorFilter = data.fields.colorFilter;
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, static_fields._ColorFilter, &colorFilter, 0LL);
			//         v17 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//         colorFilter = data.fields.channelMixerR;
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, v17._ChannelMixerRed, &colorFilter, 0LL);
			//         v18 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//         colorFilter = data.fields.channelMixerG;
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, v18._ChannelMixerGreen, &colorFilter, 0LL);
			//         v19 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//         colorFilter = data.fields.channelMixerB;
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, v19._ChannelMixerBlue, &colorFilter, 0LL);
			//         v20 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//         colorFilter = data.fields.hueSatCon;
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, v20._HueSatCon, &colorFilter, 0LL);
			//         v21 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//         colorFilter = data.fields.lift;
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, v21._Lift, &colorFilter, 0LL);
			//         v22 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//         colorFilter = data.fields.gamma;
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, v22._Gamma, &colorFilter, 0LL);
			//         v23 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//         colorFilter = data.fields.gain;
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, v23._Gain, &colorFilter, 0LL);
			//         v24 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//         colorFilter = data.fields.shadows;
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, v24._Shadows, &colorFilter, 0LL);
			//         v25 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//         colorFilter = data.fields.midtones;
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, v25._Midtones, &colorFilter, 0LL);
			//         v26 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//         colorFilter = data.fields.highlights;
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, v26._Highlights, &colorFilter, 0LL);
			//         v27 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//         colorFilter = data.fields.shadowsHighlightsLimits;
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, v27._ShaHiLimits, &colorFilter, 0LL);
			//         v28 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//         colorFilter = data.fields.splitShadows;
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, v28._SplitShadows, &colorFilter, 0LL);
			//         v29 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//         colorFilter = data.fields.splitHighlights;
			//         UnityEngine::Material::SetVector(lutBuilderMaterial, v29._SplitHighlights, &colorFilter, 0LL);
			//         CurveMaster = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields._CurveMaster;
			//         masterCurve = data.fields.masterCurve;
			//         if ( masterCurve )
			//         {
			//           Texture = UnityEngine::Rendering::TextureCurve::GetTexture(masterCurve, 0LL);
			//           UnityEngine::Material::SetTextureImpl(lutBuilderMaterial, CurveMaster, (Texture *)Texture, 0LL);
			//           CurveRed = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields._CurveRed;
			//           masterCurve = data.fields.redCurve;
			//           if ( masterCurve )
			//           {
			//             v33 = UnityEngine::Rendering::TextureCurve::GetTexture(masterCurve, 0LL);
			//             UnityEngine::Material::SetTextureImpl(lutBuilderMaterial, CurveRed, (Texture *)v33, 0LL);
			//             CurveGreen = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields._CurveGreen;
			//             masterCurve = data.fields.greenCurve;
			//             if ( masterCurve )
			//             {
			//               v35 = UnityEngine::Rendering::TextureCurve::GetTexture(masterCurve, 0LL);
			//               UnityEngine::Material::SetTextureImpl(lutBuilderMaterial, CurveGreen, (Texture *)v35, 0LL);
			//               CurveBlue = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields._CurveBlue;
			//               masterCurve = data.fields.blueCurve;
			//               if ( masterCurve )
			//               {
			//                 v37 = UnityEngine::Rendering::TextureCurve::GetTexture(masterCurve, 0LL);
			//                 UnityEngine::Material::SetTextureImpl(lutBuilderMaterial, CurveBlue, (Texture *)v37, 0LL);
			//                 CurveHueVsHue = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields._CurveHueVsHue;
			//                 masterCurve = data.fields.hueVsHueCurve;
			//                 if ( masterCurve )
			//                 {
			//                   v39 = UnityEngine::Rendering::TextureCurve::GetTexture(masterCurve, 0LL);
			//                   UnityEngine::Material::SetTextureImpl(lutBuilderMaterial, CurveHueVsHue, (Texture *)v39, 0LL);
			//                   CurveHueVsSat = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields._CurveHueVsSat;
			//                   masterCurve = data.fields.hueVsSatCurve;
			//                   if ( masterCurve )
			//                   {
			//                     v41 = UnityEngine::Rendering::TextureCurve::GetTexture(masterCurve, 0LL);
			//                     UnityEngine::Material::SetTextureImpl(lutBuilderMaterial, CurveHueVsSat, (Texture *)v41, 0LL);
			//                     CurveLumVsSat = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields._CurveLumVsSat;
			//                     masterCurve = data.fields.lumVsSatCurve;
			//                     if ( masterCurve )
			//                     {
			//                       v43 = UnityEngine::Rendering::TextureCurve::GetTexture(masterCurve, 0LL);
			//                       UnityEngine::Material::SetTextureImpl(lutBuilderMaterial, CurveLumVsSat, (Texture *)v43, 0LL);
			//                       CurveSatVsSat = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields._CurveSatVsSat;
			//                       masterCurve = data.fields.satVsSatCurve;
			//                       if ( masterCurve )
			//                       {
			//                         v45 = UnityEngine::Rendering::TextureCurve::GetTexture(masterCurve, 0LL);
			//                         UnityEngine::Material::SetTextureImpl(lutBuilderMaterial, CurveSatVsSat, (Texture *)v45, 0LL);
			//                         v46 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//                         colorFilter = data.fields.miscParams;
			//                         UnityEngine::Material::SetVector(lutBuilderMaterial, v46._Params, &colorFilter, 0LL);
			//                         return;
			//                       }
			//                     }
			//                   }
			//                 }
			//               }
			//             }
			//           }
			//         }
			//       }
			//     }
			// LABEL_16:
			//     sub_180B536AC(masterCurve, v5);
			//   }
			//   Patch = IFix::WrappersManagerImpl::GetPatch(2431, 0LL);
			//   if ( !Patch )
			//     goto LABEL_16;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_1(
			//     (ILFixDynamicMethodWrapper_37 *)Patch,
			//     (Object *)data,
			//     (Object *)lutBuilderMaterial,
			//     0LL);
			// }
			// 
		}

		internal static TextureHandle ColorGradingPass(HGRenderGraph renderGraph, HGCamera hgCamera, int lutSize, GraphicsFormat lutFormat, Material lutBuilder2DMaterial, ref UberPostPassUtils.CachedColorGradingData cachedColorGradingData, ref RTHandle cachedColorLut)
		{
			// // TextureHandle ColorGradingPass(HGRenderGraph, HGCamera, Int32, GraphicsFormat, Material, UberPostPassUtils+CachedColorGradingData ByRef, RTHandle ByRef)
			// // Hidden C++ exception states: #wind=1
			// TextureHandle *HG::Rendering::Runtime::UberPostPassUtils::ColorGradingPass(
			//         TextureHandle *__return_ptr retstr,
			//         HGRenderGraph *renderGraph,
			//         HGCamera *hgCamera,
			//         int32_t lutSize,
			//         GraphicsFormat__Enum lutFormat,
			//         Material *lutBuilder2DMaterial,
			//         UberPostPassUtils_CachedColorGradingData *cachedColorGradingData,
			//         RTHandle **cachedColorLut,
			//         MethodInfo *method)
			// {
			//   Object *v11; // rdi
			//   TextureHandle *v12; // rsi
			//   UberPostPassUtils_CachedColorGradingData *v13; // r15
			//   __int64 v14; // rdx
			//   __int64 v15; // rcx
			//   HGRenderPathBase_HGRenderPathResources *v16; // rdx
			//   PassConstructorID__Enum__Array *v17; // r8
			//   HGCamera *v18; // r9
			//   __int64 v19; // rdx
			//   __int64 v20; // rcx
			//   ProfilingSampler *v21; // rax
			//   __int64 v22; // rcx
			//   Object *v23; // rdx
			//   unsigned int v24; // edx
			//   unsigned __int64 v25; // r8
			//   char v26; // dl
			//   signed __int64 v27; // rtt
			//   Object *v28; // r14
			//   __int64 v29; // rdx
			//   __int64 v30; // rcx
			//   TextureHandle v31; // xmm0
			//   TextureHandle v32; // xmm2
			//   __int64 v33; // rdx
			//   UberPostPassUtils_CachedColorGradingData *v34; // rax
			//   UberPostPassUtils_CachedColorGradingData *v35; // rcx
			//   unsigned __int64 v36; // r8
			//   signed __int64 v37; // rtt
			//   __int64 v38; // rdx
			//   __int64 v39; // rcx
			//   ILFixDynamicMethodWrapper_2 *Patch; // r14
			//   MethodInfo *methoda; // [rsp+20h] [rbp-288h]
			//   MethodInfo *filterMode; // [rsp+28h] [rbp-280h]
			//   Object *v44; // [rsp+A0h] [rbp-208h] BYREF
			//   TextureHandle v45; // [rsp+A8h] [rbp-200h] BYREF
			//   HGRenderGraphBuilder v46; // [rsp+B8h] [rbp-1F0h] BYREF
			//   HGRenderGraphBuilder v47; // [rsp+D8h] [rbp-1D0h] BYREF
			//   TextureHandle v48; // [rsp+F8h] [rbp-1B0h] BYREF
			//   Il2CppExceptionWrapper *v49; // [rsp+108h] [rbp-1A0h] BYREF
			//   UberPostPassUtils_CachedColorGradingData v50; // [rsp+110h] [rbp-198h] BYREF
			// 
			//   v11 = (Object *)renderGraph;
			//   v12 = retstr;
			//   if ( !byte_18D9194CC )
			//   {
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::ColorGradingPassData>);
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::ColorGradingPassData>);
			//     sub_18003C530(&MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//     sub_18003C530(&TypeInfo::UnityEngine::Rendering::RTHandles);
			//     sub_18003C530(&TypeInfo::HG::Rendering::RenderGraphModule::TextureHandle);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     sub_18003C530(&off_18D4F3788);
			//     sub_18003C530(&off_18D4F3790);
			//     byte_18D9194CC = 1;
			//   }
			//   sub_1802F01E0(&v50, 0LL, 368LL);
			//   memset(&v47, 0, sizeof(v47));
			//   v44 = 0LL;
			//   if ( IFix::WrappersManagerImpl::IsPatched(2432, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2432, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v39, v38);
			//     *v12 = *IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_884(
			//               &v45,
			//               Patch,
			//               v11,
			//               (Object *)hgCamera,
			//               lutSize,
			//               lutFormat,
			//               (Object *)lutBuilder2DMaterial,
			//               cachedColorGradingData,
			//               cachedColorLut,
			//               0LL);
			//   }
			//   else
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::RenderGraphModule::TextureHandle);
			//     sub_182E7CCD0(&v45);
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     HG::Rendering::Runtime::UberPostPassUtils::PrepareColorGradingParametersFromEnvVolume(
			//       &TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_colorGradingData,
			//       hgCamera,
			//       lutSize,
			//       lutBuilder2DMaterial,
			//       0LL);
			//     HG::Rendering::Runtime::UberPostPassUtils::CachedColorGradingData::CachedColorGradingData(
			//       &v50,
			//       TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_colorGradingData,
			//       0LL);
			//     v13 = cachedColorGradingData;
			//     if ( HG::Rendering::Runtime::UberPostPassUtils::CachedColorGradingData::Equals(&v50, cachedColorGradingData, 0LL) )
			//     {
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       if ( !HG::Rendering::Runtime::UberPostPassUtils::IsAnyColorCurveDirty(
			//               &TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_colorGradingData,
			//               0LL)
			//         && cachedColorGradingData.lutSize >= 0 )
			//       {
			//         goto LABEL_18;
			//       }
			//     }
			//     if ( *cachedColorLut )
			//       UnityEngine::Rendering::RTHandle::Release(*cachedColorLut, 0LL);
			//     sub_180002C70(TypeInfo::UnityEngine::Rendering::RTHandles);
			//     *cachedColorLut = UnityEngine::Rendering::RTHandles::Alloc(
			//                         lutSize * lutSize,
			//                         lutSize,
			//                         1,
			//                         DepthBits__Enum_None,
			//                         lutFormat,
			//                         FilterMode__Enum_Bilinear,
			//                         TextureWrapMode__Enum_Clamp,
			//                         TextureDimension__Enum_Tex2D,
			//                         0,
			//                         0,
			//                         1,
			//                         0,
			//                         0,
			//                         0.0,
			//                         MSAASamples__Enum_None,
			//                         0,
			//                         RenderTextureMemoryless__Enum_None,
			//                         (String *)"Color Grading Log Lut",
			//                         0LL);
			//     sub_1800054D0((HGRenderPathScene *)cachedColorLut, v16, v17, v18, methoda, filterMode);
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     HG::Rendering::Runtime::UberPostPassUtils::PrepareLutBuilderMaterial(
			//       TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_colorGradingData,
			//       lutBuilder2DMaterial,
			//       0LL);
			//     if ( !v11 )
			//       sub_180B536AC(v20, v19);
			//     v45 = *HG::Rendering::RenderGraphModule::HGRenderGraph::ImportTexture(
			//              &v45,
			//              (HGRenderGraph *)v11,
			//              *cachedColorLut,
			//              0LL);
			//     v21 = UnityEngine::Rendering::ProfilingSampler::Get<System::Int32Enum>(
			//             (Int32Enum__Enum)0x97u,
			//             MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//     HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<System::Object>(
			//       &v46,
			//       (HGRenderGraph *)v11,
			//       (String *)"Color Grading",
			//       &v44,
			//       v21,
			//       1,
			//       ProfilingHGPass__Enum_None,
			//       MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::ColorGradingPassData>);
			//     v47 = v46;
			//     v46.m_RenderPass = 0LL;
			//     v46.m_Resources = (HGRenderGraphResourceRegistry *)&v47;
			//     try
			//     {
			//       v23 = v44;
			//       if ( !v44 )
			//         sub_1802DC2C8(v22, 0LL);
			//       v44[1].klass = (Object__Class *)lutBuilder2DMaterial;
			//       if ( dword_18D8E43F8 )
			//       {
			//         v24 = ((unsigned __int64)&v23[1] >> 12) & 0x1FFFFF;
			//         v25 = (unsigned __int64)v24 >> 6;
			//         v26 = v24 & 0x3F;
			//         _m_prefetchw(&qword_18D6405E0[v25 + 36190]);
			//         do
			//           v27 = qword_18D6405E0[v25 + 36190];
			//         while ( v27 != _InterlockedCompareExchange64(&qword_18D6405E0[v25 + 36190], v27 | (1LL << v26), v27) );
			//       }
			//       v28 = v44;
			//       v31 = *HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::WriteTexture(&v48, &v47, &v45, 0LL);
			//       if ( !v28 )
			//         sub_1802DC2C8(v30, v29);
			//       *(TextureHandle *)&v28[1].monitor = v31;
			//       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetColorAttachment(&v48, &v47, &v45, 0, 0, 0LL);
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<System::Object>(
			//         &v47,
			//         (RenderFunc_1_System_Object_ *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_colorGradingRenderFunc,
			//         0LL,
			//         0,
			//         MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::ColorGradingPassData>);
			//     }
			//     catch ( Il2CppExceptionWrapper *v49 )
			//     {
			//       v46.m_RenderPass = (HGRenderGraphPass *)v49.ex;
			//       sub_180222690(&v46);
			//       v13 = cachedColorGradingData;
			//       v11 = (Object *)renderGraph;
			//       v12 = retstr;
			// LABEL_18:
			//       if ( v11 )
			//       {
			//         v32 = *HG::Rendering::RenderGraphModule::HGRenderGraph::ImportTexture(
			//                  &v45,
			//                  (HGRenderGraph *)v11,
			//                  *cachedColorLut,
			//                  0LL);
			//         v33 = 2LL;
			//         goto LABEL_20;
			//       }
			//       sub_1802DC2C8(v15, v14);
			//     }
			//     sub_180222690(&v46);
			//     v32 = v45;
			//     v33 = 2LL;
			// LABEL_20:
			//     v34 = v13;
			//     v35 = &v50;
			//     do
			//     {
			//       *(_OWORD *)&v34.lutSize = *(_OWORD *)&v35.lutSize;
			//       *(_OWORD *)&v34.externalLuT = *(_OWORD *)&v35.externalLuT;
			//       *(_OWORD *)&v34.colorCurve_redCurve = *(_OWORD *)&v35.colorCurve_redCurve;
			//       *(_OWORD *)&v34.colorCurve_blueCurve = *(_OWORD *)&v35.colorCurve_blueCurve;
			//       *(_OWORD *)&v34.colorCurve_hueVsSatCurve = *(_OWORD *)&v35.colorCurve_hueVsSatCurve;
			//       *(_OWORD *)&v34.colorCurve_lumVsSatCurve = *(_OWORD *)&v35.colorCurve_lumVsSatCurve;
			//       *(_OWORD *)&v34.colorFilter.z = *(_OWORD *)&v35.colorFilter.z;
			//       v34 = (UberPostPassUtils_CachedColorGradingData *)((char *)v34 + 128);
			//       *(_OWORD *)&v34[-1].tonemapping_shoulderLength = *(_OWORD *)&v35.lmsColorBalance.z;
			//       v35 = (UberPostPassUtils_CachedColorGradingData *)((char *)v35 + 128);
			//       --v33;
			//     }
			//     while ( v33 );
			//     *(_OWORD *)&v34.lutSize = *(_OWORD *)&v35.lutSize;
			//     *(_OWORD *)&v34.externalLuT = *(_OWORD *)&v35.externalLuT;
			//     *(_OWORD *)&v34.colorCurve_redCurve = *(_OWORD *)&v35.colorCurve_redCurve;
			//     *(_OWORD *)&v34.colorCurve_blueCurve = *(_OWORD *)&v35.colorCurve_blueCurve;
			//     *(_OWORD *)&v34.colorCurve_hueVsSatCurve = *(_OWORD *)&v35.colorCurve_hueVsSatCurve;
			//     *(_OWORD *)&v34.colorCurve_lumVsSatCurve = *(_OWORD *)&v35.colorCurve_lumVsSatCurve;
			//     *(_OWORD *)&v34.colorFilter.z = *(_OWORD *)&v35.colorFilter.z;
			//     if ( dword_18D8E43F8 )
			//     {
			//       v36 = (((unsigned __int64)&v13.externalLuT >> 12) & 0x1FFFFF) >> 6;
			//       _m_prefetchw(&qword_18D6405E0[v36 + 36190]);
			//       do
			//         v37 = qword_18D6405E0[v36 + 36190];
			//       while ( v37 != _InterlockedCompareExchange64(
			//                        &qword_18D6405E0[v36 + 36190],
			//                        v37 | (1LL << (((unsigned __int64)&v13.externalLuT >> 12) & 0x3F)),
			//                        v37) );
			//     }
			//     *v12 = v32;
			//   }
			//   return v12;
			// }
			// 
			return null;
		}

		private static Vector3[] GetCutsceneEffectTriangleVertexPosition(float zOffset)
		{
			// // Vector3[] GetCutsceneEffectTriangleVertexPosition(Single)
			// Vector3__Array *HG::Rendering::Runtime::UberPostPassUtils::GetCutsceneEffectTriangleVertexPosition(
			//         float zOffset,
			//         MethodInfo *method)
			// {
			//   __int64 v2; // r8
			//   __int64 v3; // r9
			//   Vector3__Array *result; // rax
			//   __int64 v5; // rdx
			//   __int64 v6; // rcx
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			// 
			//   if ( !byte_18D8EDA32 )
			//   {
			//     sub_18003C530(&TypeInfo::UnityEngine::Vector3);
			//     byte_18D8EDA32 = 1;
			//   }
			//   if ( IFix::WrappersManagerImpl::IsPatched(1027, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(1027, 0LL);
			//     if ( Patch )
			//       return IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_368(Patch, zOffset, 0LL);
			// LABEL_10:
			//     sub_180B536AC(v6, v5);
			//   }
			//   result = (Vector3__Array *)il2cpp_array_new_specific_0(TypeInfo::UnityEngine::Vector3, 4LL, v2, v3);
			//   if ( !result )
			//     goto LABEL_10;
			//   if ( !result.max_length.size
			//     || (*(_QWORD *)&result.vector[0].x = _mm_unpacklo_ps((__m128)0xBF800000, (__m128)0x3F800000u).m128_u64[0],
			//         result.vector[0].z = zOffset,
			//         result.max_length.size <= 1u)
			//     || (*(_QWORD *)&result.vector[1].x = _mm_unpacklo_ps((__m128)0x3F800000u, (__m128)0x3F800000u).m128_u64[0],
			//         result.vector[1].z = zOffset,
			//         result.max_length.size <= 2u)
			//     || (*(_QWORD *)&result.vector[2].x = _mm_unpacklo_ps((__m128)0xBF800000, (__m128)0xBF800000).m128_u64[0],
			//         result.vector[2].z = zOffset,
			//         result.max_length.size <= 3u) )
			//   {
			//     sub_180070270(v6, v5);
			//   }
			//   *(_QWORD *)&result.vector[3].x = _mm_unpacklo_ps((__m128)0x3F800000u, (__m128)0xBF800000).m128_u64[0];
			//   result.vector[3].z = zOffset;
			//   return result;
			// }
			// 
			return null;
		}

		private static Mesh CreateCutsceneEffectTriangleMesh()
		{
			// // Mesh CreateCutsceneEffectTriangleMesh()
			// Mesh *HG::Rendering::Runtime::UberPostPassUtils::CreateCutsceneEffectTriangleMesh(MethodInfo *method)
			// {
			//   Mesh *v1; // rax
			//   __int64 v2; // rdx
			//   __int64 v3; // rcx
			//   Mesh *v4; // rbp
			//   __int64 v5; // r8
			//   __int64 v6; // r9
			//   __int64 v7; // rax
			//   __int64 v8; // r8
			//   __int64 v9; // r9
			//   Vector3__Array *v10; // rsi
			//   __int64 v11; // rax
			//   __int64 v12; // r8
			//   __int64 v13; // r9
			//   Vector2__Array *v14; // rdi
			//   Array *v15; // rbx
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			// 
			//   if ( !byte_18D8EDA33 )
			//   {
			//     sub_18003C530(&TypeInfo::System::Int32);
			//     sub_18003C530(&TypeInfo::UnityEngine::Mesh);
			//     sub_18003C530(&B2CF02C10F1F309AC8FB52A3CCE888191E73C7E1C5A0D699CA4CBBE2C76F2C0F_Field);
			//     sub_18003C530(&TypeInfo::UnityEngine::Vector2);
			//     sub_18003C530(&TypeInfo::UnityEngine::Vector3);
			//     sub_18003C530(&off_18C8EEDC8);
			//     byte_18D8EDA33 = 1;
			//   }
			//   if ( !IFix::WrappersManagerImpl::IsPatched(1028, 0LL) )
			//   {
			//     v1 = (Mesh *)sub_180004920(TypeInfo::UnityEngine::Mesh);
			//     v4 = v1;
			//     if ( v1 )
			//     {
			//       UnityEngine::Mesh::Mesh(v1, 0LL);
			//       v7 = il2cpp_array_new_specific_0(TypeInfo::UnityEngine::Vector3, 4LL, v5, v6);
			//       v10 = (Vector3__Array *)v7;
			//       if ( v7 )
			//       {
			//         if ( !*(_DWORD *)(v7 + 24) )
			//           goto LABEL_17;
			//         *(_QWORD *)(v7 + 32) = _mm_unpacklo_ps((__m128)0LL, (__m128)0LL).m128_u64[0];
			//         *(_DWORD *)(v7 + 40) = 0;
			//         if ( *(_DWORD *)(v7 + 24) <= 1u )
			//           goto LABEL_17;
			//         *(_QWORD *)(v7 + 44) = _mm_unpacklo_ps((__m128)0x3F800000u, (__m128)0LL).m128_u64[0];
			//         *(_DWORD *)(v7 + 52) = 0;
			//         if ( *(_DWORD *)(v7 + 24) <= 2u )
			//           goto LABEL_17;
			//         *(_QWORD *)(v7 + 56) = _mm_unpacklo_ps((__m128)0LL, (__m128)0x3F800000u).m128_u64[0];
			//         *(_DWORD *)(v7 + 64) = 0;
			//         if ( *(_DWORD *)(v7 + 24) <= 3u )
			//           goto LABEL_17;
			//         *(_QWORD *)(v7 + 68) = _mm_unpacklo_ps((__m128)0x3F800000u, (__m128)0x3F800000u).m128_u64[0];
			//         *(_DWORD *)(v7 + 76) = 0;
			//         v11 = il2cpp_array_new_specific_0(TypeInfo::UnityEngine::Vector2, 4LL, v8, v9);
			//         v14 = (Vector2__Array *)v11;
			//         if ( v11 )
			//         {
			//           if ( *(_DWORD *)(v11 + 24) )
			//           {
			//             *(_QWORD *)(v11 + 32) = 0LL;
			//             if ( *(_DWORD *)(v11 + 24) > 1u )
			//             {
			//               *(_QWORD *)(v11 + 40) = 1065353216LL;
			//               if ( *(_DWORD *)(v11 + 24) > 2u )
			//               {
			//                 *(_DWORD *)(v11 + 48) = 0;
			//                 *(_DWORD *)(v11 + 52) = 1065353216;
			//                 if ( *(_DWORD *)(v11 + 24) > 3u )
			//                 {
			//                   *(_DWORD *)(v11 + 56) = 1065353216;
			//                   *(_DWORD *)(v11 + 60) = 1065353216;
			//                   v15 = (Array *)il2cpp_array_new_specific_0(TypeInfo::System::Int32, 6LL, v12, v13);
			//                   System::Runtime::CompilerServices::RuntimeHelpers::InitializeArray(
			//                     v15,
			//                     B2CF02C10F1F309AC8FB52A3CCE888191E73C7E1C5A0D699CA4CBBE2C76F2C0F_Field,
			//                     0LL);
			//                   UnityEngine::Mesh::set_vertices(v4, v10, 0LL);
			//                   UnityEngine::Mesh::set_triangles(v4, (Int32__Array *)v15, 0LL);
			//                   UnityEngine::Object::set_name((Object_1 *)v4, (String *)"CutsceneEffectTriangle", 0LL);
			//                   UnityEngine::Mesh::set_uv(v4, v14, 0LL);
			//                   UnityEngine::Mesh::RecalculateBounds(v4, MeshUpdateFlags__Enum_Default, 0LL);
			//                   return v4;
			//                 }
			//               }
			//             }
			//           }
			// LABEL_17:
			//           sub_180070270(v3, v2);
			//         }
			//       }
			//     }
			// LABEL_5:
			//     sub_180B536AC(v3, v2);
			//   }
			//   Patch = IFix::WrappersManagerImpl::GetPatch(1028, 0LL);
			//   if ( !Patch )
			//     goto LABEL_5;
			//   return IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_366(Patch, 0LL);
			// }
			// 
			return null;
		}

		internal static Mesh GetCutsceneEffectMesh()
		{
			return null;
		}

		internal static TextureHandle CutsceneEffectPass(HGSettingParameters settingParameters, HGRenderGraph renderGraph, HGCamera hgCamera, TextureHandle finalRT, TextureHandle depthRT, TextureHandle source)
		{
			// // TextureHandle CutsceneEffectPass(HGSettingParameters, HGRenderGraph, HGCamera, TextureHandle, TextureHandle, TextureHandle)
			// // Hidden C++ exception states: #wind=1
			// TextureHandle *HG::Rendering::Runtime::UberPostPassUtils::CutsceneEffectPass(
			//         TextureHandle *__return_ptr retstr,
			//         HGSettingParameters *settingParameters,
			//         HGRenderGraph *renderGraph,
			//         HGCamera *hgCamera,
			//         TextureHandle *finalRT,
			//         TextureHandle *depthRT,
			//         TextureHandle *source,
			//         MethodInfo *method)
			// {
			//   TextureHandle *v11; // rdi
			//   __int64 v12; // rdx
			//   VFXPPCutsceneEffect__StaticFields *static_fields; // rcx
			//   _BOOL8 v14; // r14
			//   Object_1 *s_cutsceneEffectMesh; // rbx
			//   __int64 v16; // rdx
			//   __int64 v17; // rcx
			//   Vector3__Array *CutsceneEffectTriangleVertexPosition; // rax
			//   HGRenderPathBase_HGRenderPathResources *v19; // rdx
			//   PassConstructorID__Enum__Array *v20; // r8
			//   HGCamera *v21; // r9
			//   Mesh *CutsceneEffectTriangleMesh; // rax
			//   HGRenderPathBase_HGRenderPathResources *v23; // rdx
			//   PassConstructorID__Enum__Array *v24; // r8
			//   HGCamera *v25; // r9
			//   __int64 v26; // rdx
			//   __int64 v27; // rcx
			//   UberPostPassUtils__StaticFields *v28; // rax
			//   TextureHandle *v29; // rax
			//   TextureDesc *TextureDescRef; // rax
			//   int32_t effectListCount; // ebx
			//   __int64 v32; // r15
			//   int32_t v33; // r14d
			//   __int64 v34; // rdi
			//   __int64 v35; // rdx
			//   VFXPPCutsceneEffect__StaticFields *v36; // rcx
			//   Material__Array *runtimeMaterial; // r12
			//   __int64 v38; // r13
			//   MonitorData *BlitMaterial; // r15
			//   ProfilingSampler *v40; // rax
			//   Object *v41; // rsi
			//   __int64 v42; // rdx
			//   signed __int64 v43; // rcx
			//   int v44; // r8d
			//   unsigned __int64 v45; // r9
			//   signed __int64 v46; // rtt
			//   Object *v47; // rax
			//   unsigned __int64 v48; // r9
			//   signed __int64 v49; // rtt
			//   Object *v50; // r9
			//   signed __int64 v51; // rcx
			//   unsigned __int64 v52; // r9
			//   unsigned __int64 v53; // r10
			//   char v54; // r9
			//   signed __int64 v55; // rtt
			//   Object *v56; // rbx
			//   __int64 v57; // rdx
			//   __int64 v58; // rcx
			//   TextureHandle v59; // xmm0
			//   Object *v60; // rbx
			//   __int64 v61; // rdx
			//   __int64 v62; // rcx
			//   TextureHandle v63; // xmm0
			//   TextureHandle v64; // xmm0
			//   __int64 v65; // rdx
			//   __int64 v66; // rcx
			//   ILFixDynamicMethodWrapper_2 *Patch; // r14
			//   MethodInfo *methoda; // [rsp+20h] [rbp-E8h]
			//   MethodInfo *methodb; // [rsp+20h] [rbp-E8h]
			//   MethodInfo *v71; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *v72; // [rsp+28h] [rbp-E0h]
			//   Object *v73; // [rsp+50h] [rbp-B8h] BYREF
			//   TextureHandle si128; // [rsp+60h] [rbp-A8h] BYREF
			//   TextureHandle v75; // [rsp+70h] [rbp-98h] BYREF
			//   TextureHandle v76; // [rsp+80h] [rbp-88h] BYREF
			//   HGRenderGraphBuilder v77; // [rsp+90h] [rbp-78h] BYREF
			//   HGRenderGraphBuilder v78; // [rsp+B0h] [rbp-58h] BYREF
			//   Il2CppExceptionWrapper *v79; // [rsp+D0h] [rbp-38h] BYREF
			// 
			//   v11 = retstr;
			//   if ( !byte_18D9194CD )
			//   {
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::CutsceneEffectData>);
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::CutsceneEffectData>);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGUtils);
			//     sub_18003C530(&TypeInfo::UnityEngine::Object);
			//     sub_18003C530(&MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//     sub_18003C530(&MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::VFXPPCutsceneEffect);
			//     sub_18003C530(&off_18D4F3680);
			//     byte_18D9194CD = 1;
			//   }
			//   v73 = 0LL;
			//   if ( IFix::WrappersManagerImpl::IsPatched(2434, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2434, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v66, v65);
			//     v76 = *source;
			//     v75 = *depthRT;
			//     si128 = *finalRT;
			//     v29 = IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_885(
			//             (TextureHandle *)&v78,
			//             Patch,
			//             (Object *)settingParameters,
			//             (Object *)renderGraph,
			//             (Object *)hgCamera,
			//             &si128,
			//             &v75,
			//             &v76,
			//             0LL);
			//     goto LABEL_48;
			//   }
			//   sub_180002C70(TypeInfo::HG::Rendering::Runtime::VFXPPCutsceneEffect);
			//   static_fields = TypeInfo::HG::Rendering::Runtime::VFXPPCutsceneEffect.static_fields;
			//   if ( !static_fields.m_useCutsceneEffsectShader )
			//     goto LABEL_8;
			//   if ( !settingParameters )
			//     sub_180B536AC(static_fields, v12);
			//   if ( HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit(
			//          settingParameters.fields._cutsceneEffectEnabled_k__BackingField,
			//          MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit) )
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::VFXPPCutsceneEffect);
			//     v14 = TypeInfo::HG::Rendering::Runtime::VFXPPCutsceneEffect.static_fields.runtimeMaterial != 0LL;
			//   }
			//   else
			//   {
			// LABEL_8:
			//     LODWORD(v14) = 0;
			//   }
			//   sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//   s_cutsceneEffectMesh = (Object_1 *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_cutsceneEffectMesh;
			//   sub_180002C70(TypeInfo::UnityEngine::Object);
			//   if ( UnityEngine::Object::op_Equality(s_cutsceneEffectMesh, 0LL, 0LL) )
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     CutsceneEffectTriangleVertexPosition = HG::Rendering::Runtime::UberPostPassUtils::GetCutsceneEffectTriangleVertexPosition(
			//                                              0.001,
			//                                              0LL);
			//     v19 = (HGRenderPathBase_HGRenderPathResources *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//     v19[6].settingParameters = (HGSettingParameters *)CutsceneEffectTriangleVertexPosition;
			//     sub_1800054D0(
			//       (HGRenderPathScene *)&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_cutsceneEffectMeshVertices,
			//       v19,
			//       v20,
			//       v21,
			//       methoda,
			//       v71);
			//     CutsceneEffectTriangleMesh = HG::Rendering::Runtime::UberPostPassUtils::CreateCutsceneEffectTriangleMesh(0LL);
			//     v23 = (HGRenderPathBase_HGRenderPathResources *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//     v23[7].defaultResources = (HGRenderPipelineRuntimeResources *)CutsceneEffectTriangleMesh;
			//     sub_1800054D0(
			//       (HGRenderPathScene *)&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_cutsceneEffectMesh,
			//       v23,
			//       v24,
			//       v25,
			//       methodb,
			//       v72);
			//     v28 = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//     if ( !v28.s_cutsceneEffectMesh )
			//       sub_180B536AC(v27, v26);
			//     UnityEngine::Mesh::set_vertices(v28.s_cutsceneEffectMesh, v28.s_cutsceneEffectMeshVertices, 0LL);
			//   }
			//   if ( !v14 )
			//   {
			//     v29 = source;
			// LABEL_48:
			//     v64 = *v29;
			//     goto LABEL_49;
			//   }
			//   if ( !renderGraph )
			//     sub_180B536AC(v17, v16);
			//   TextureDescRef = HG::Rendering::RenderGraphModule::HGRenderGraph::GetTextureDescRef(renderGraph, source, 0LL);
			//   v76 = *HG::Rendering::RenderGraphModule::HGRenderGraph::CreateTexture(&v76, renderGraph, TextureDescRef, 0LL);
			//   sub_180002C70(TypeInfo::HG::Rendering::Runtime::VFXPPCutsceneEffect);
			//   effectListCount = TypeInfo::HG::Rendering::Runtime::VFXPPCutsceneEffect.static_fields.effectListCount;
			//   v32 = 32LL;
			//   v33 = 0;
			//   if ( effectListCount >= 32 )
			//   {
			//     effectListCount = 32;
			//     goto LABEL_19;
			//   }
			//   if ( effectListCount > 0 )
			//   {
			// LABEL_19:
			//     v34 = 0LL;
			//     do
			//     {
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       si128.handle = (ResourceHandle)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_cutSceneEffectMaterials;
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::VFXPPCutsceneEffect);
			//       v36 = TypeInfo::HG::Rendering::Runtime::VFXPPCutsceneEffect.static_fields;
			//       runtimeMaterial = v36.runtimeMaterial;
			//       if ( !runtimeMaterial )
			//         sub_180B536AC(v36, v35);
			//       if ( (unsigned int)v33 >= runtimeMaterial.max_length.size )
			//         sub_180070270(v36, v35);
			//       v38 = *(__int64 *)((char *)&runtimeMaterial.klass + v32);
			//       if ( !*(_QWORD *)&si128.handle )
			//         sub_180B536AC(v36, v35);
			//       sub_180036D40(*(_QWORD *)&si128.handle, v38);
			//       sub_18000FDA0(*(_QWORD *)&si128.handle, v34, v38);
			//       ++v33;
			//       ++v34;
			//       v32 += 8LL;
			//     }
			//     while ( v33 < effectListCount );
			//     v11 = retstr;
			//   }
			//   sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGUtils);
			//   BlitMaterial = (MonitorData *)HG::Rendering::Runtime::HGUtils::GetBlitMaterial(
			//                                   0,
			//                                   TextureDimension__Enum_Tex2D,
			//                                   0,
			//                                   0LL);
			//   v40 = UnityEngine::Rendering::ProfilingSampler::Get<System::Int32Enum>(
			//           (Int32Enum__Enum)0x9Au,
			//           MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//   HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<System::Object>(
			//     &v78,
			//     renderGraph,
			//     (String *)"CutsceneEffect",
			//     &v73,
			//     v40,
			//     1,
			//     ProfilingHGPass__Enum_None,
			//     MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::CutsceneEffectData>);
			//   v77 = v78;
			//   v75.handle = 0LL;
			//   v75.fallBackResource = (ResourceHandle)&v77;
			//   try
			//   {
			//     v41 = v73;
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     v43 = (signed __int64)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//     if ( !v41 )
			//       sub_1802DC2C8(v43, v42);
			//     v41[3].klass = *(Object__Class **)(v43 + 112);
			//     v44 = dword_18D8E43F8;
			//     if ( dword_18D8E43F8 )
			//     {
			//       v45 = (((unsigned __int64)&v41[3] >> 12) & 0x1FFFFF) >> 6;
			//       _m_prefetchw(&qword_18D6405E0[v45 + 36190]);
			//       do
			//       {
			//         v43 = qword_18D6405E0[v45 + 36190] | (1LL << (((unsigned __int64)&v41[3] >> 12) & 0x3F));
			//         v46 = qword_18D6405E0[v45 + 36190];
			//       }
			//       while ( v46 != _InterlockedCompareExchange64(&qword_18D6405E0[v45 + 36190], v43, v46) );
			//       v44 = dword_18D8E43F8;
			//     }
			//     v47 = v73;
			//     if ( !v73 )
			//       sub_1802DC2C8(v43, qword_18D6405E0);
			//     v73[3].monitor = BlitMaterial;
			//     if ( v44 )
			//     {
			//       v48 = (((unsigned __int64)&v47[3].monitor >> 12) & 0x1FFFFF) >> 6;
			//       _m_prefetchw(&qword_18D6405E0[v48 + 36190]);
			//       do
			//         v49 = qword_18D6405E0[v48 + 36190];
			//       while ( v49 != _InterlockedCompareExchange64(
			//                        &qword_18D6405E0[v48 + 36190],
			//                        v49 | (1LL << (((unsigned __int64)&v47[3].monitor >> 12) & 0x3F)),
			//                        v49) );
			//       v44 = dword_18D8E43F8;
			//     }
			//     v50 = v73;
			//     v51 = (signed __int64)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//     if ( !v73 )
			//       sub_1802DC2C8(v51, qword_18D6405E0);
			//     v73[4].klass = *(Object__Class **)(v51 + 96);
			//     if ( v44 )
			//     {
			//       v52 = ((unsigned __int64)&v50[4] >> 12) & 0x1FFFFF;
			//       v53 = v52 >> 6;
			//       v54 = v52 & 0x3F;
			//       _m_prefetchw(&qword_18D6405E0[v53 + 36190]);
			//       do
			//       {
			//         v51 = qword_18D6405E0[v53 + 36190] | (1LL << v54);
			//         v55 = qword_18D6405E0[v53 + 36190];
			//       }
			//       while ( v55 != _InterlockedCompareExchange64(&qword_18D6405E0[v53 + 36190], v51, v55) );
			//     }
			//     if ( !v73 )
			//       sub_1802DC2C8(v51, qword_18D6405E0);
			//     LODWORD(v73[4].monitor) = effectListCount;
			//     v56 = v73;
			//     v59 = *HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadTexture(&si128, &v77, source, 0LL);
			//     if ( !v56 )
			//       sub_1802DC2C8(v58, v57);
			//     v56[1] = (Object)v59;
			//     v60 = v73;
			//     v63 = *HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadTexture(&si128, &v77, depthRT, 0LL);
			//     if ( !v60 )
			//       sub_1802DC2C8(v62, v61);
			//     v60[2] = (Object)v63;
			//     si128 = (TextureHandle)_mm_load_si128((const __m128i *)&xmmword_18A3576D0);
			//     HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetColorAttachment(
			//       (TextureHandle *)&v78,
			//       &v77,
			//       &v76,
			//       0,
			//       RenderBufferLoadAction__Enum_Load,
			//       RenderBufferStoreAction__Enum_Store,
			//       (Color *)&si128,
			//       0,
			//       0LL);
			//     HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::WriteTexture((TextureHandle *)&v78, &v77, &v76, 0LL);
			//     HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<System::Object>(
			//       &v77,
			//       (RenderFunc_1_System_Object_ *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.m_cutsceneEffectFunc,
			//       0LL,
			//       0,
			//       MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::CutsceneEffectData>);
			//   }
			//   catch ( Il2CppExceptionWrapper *v79 )
			//   {
			//     v75.handle = (ResourceHandle)v79.ex;
			//     sub_180222690(&v75);
			//     v11 = retstr;
			//     goto LABEL_45;
			//   }
			//   sub_180222690(&v75);
			// LABEL_45:
			//   v64 = v76;
			// LABEL_49:
			//   *v11 = v64;
			//   return v11;
			// }
			// 
			return null;
		}

		internal static TextureHandle FisheyeEffectPass(HGSettingParameters settingParameters, HGRenderGraph renderGraph, HGCamera hgCamera, HGFisheyeEffect fisheyeEffect, Material fishEyeEffectMaterial, Material fisheyeEffectDepthMaterial, TextureHandle source, TextureHandle sceneDepth, out TextureHandle fisheyeDepthRT)
		{
			// // TextureHandle FisheyeEffectPass(HGSettingParameters, HGRenderGraph, HGCamera, HGFisheyeEffect, Material, Material, TextureHandle, TextureHandle, TextureHandle ByRef)
			// // Hidden C++ exception states: #wind=1
			// TextureHandle *HG::Rendering::Runtime::UberPostPassUtils::FisheyeEffectPass(
			//         TextureHandle *__return_ptr retstr,
			//         HGSettingParameters *settingParameters,
			//         HGRenderGraph *renderGraph,
			//         HGCamera *hgCamera,
			//         HGFisheyeEffect *fisheyeEffect,
			//         Material *fishEyeEffectMaterial,
			//         Material *fisheyeEffectDepthMaterial,
			//         TextureHandle *source,
			//         TextureHandle *sceneDepth,
			//         TextureHandle *fisheyeDepthRT,
			//         MethodInfo *method)
			// {
			//   TextureHandle *v14; // rsi
			//   __int64 v15; // rdx
			//   __int64 v16; // rcx
			//   __int64 v17; // rdx
			//   __int64 v18; // rcx
			//   __int64 v19; // rdx
			//   __int64 v20; // rcx
			//   Vector2Int sceneRTSize_k__BackingField; // rbx
			//   __int64 v22; // rdx
			//   __int64 v23; // rcx
			//   HGRenderPathBase_HGRenderPathResources *v24; // rdx
			//   PassConstructorID__Enum__Array *v25; // r8
			//   HGCamera *v26; // r9
			//   HGRenderPathBase_HGRenderPathResources *v27; // rdx
			//   PassConstructorID__Enum__Array *v28; // r8
			//   HGCamera *v29; // r9
			//   __int64 v30; // rdx
			//   __int64 v31; // rcx
			//   __int64 v32; // rdx
			//   HGShaderIDs__StaticFields *static_fields; // rcx
			//   int32_t FisheyeEffectParams1; // r14d
			//   __int64 v35; // rdx
			//   HGShaderIDs__StaticFields *v36; // rcx
			//   int32_t v37; // r15d
			//   ProfilingSampler *v38; // rax
			//   __int64 v39; // rdx
			//   signed __int64 v40; // rcx
			//   Object *v41; // r8
			//   int v42; // eax
			//   unsigned int v43; // r8d
			//   unsigned __int64 v44; // r9
			//   char v45; // r8
			//   signed __int64 v46; // rtt
			//   Object *v47; // r9
			//   unsigned int v48; // r9d
			//   unsigned __int64 v49; // r10
			//   char v50; // r9
			//   signed __int64 v51; // rtt
			//   Object *v52; // rbx
			//   __int64 v53; // rdx
			//   __int64 v54; // rcx
			//   TextureHandle v55; // xmm0
			//   Object *v56; // rbx
			//   __int64 v57; // rdx
			//   __int64 v58; // rcx
			//   TextureHandle v59; // xmm0
			//   TextureHandle v60; // xmm0
			//   TextureHandle *v61; // rax
			//   __int64 v62; // rdx
			//   __int64 v63; // rcx
			//   ILFixDynamicMethodWrapper_2 *Patch; // r14
			//   MethodInfo *methoda; // [rsp+20h] [rbp-218h]
			//   MethodInfo *methodb; // [rsp+20h] [rbp-218h]
			//   MethodInfo *v68; // [rsp+28h] [rbp-210h]
			//   MethodInfo *v69; // [rsp+28h] [rbp-210h]
			//   int32_t height[4]; // [rsp+60h] [rbp-1D8h] BYREF
			//   Object *v71; // [rsp+70h] [rbp-1C8h] BYREF
			//   TextureHandle v72; // [rsp+80h] [rbp-1B8h] BYREF
			//   TextureHandle v73; // [rsp+90h] [rbp-1A8h] BYREF
			//   HGRenderGraphBuilder v74; // [rsp+A0h] [rbp-198h] BYREF
			//   TextureDesc v75; // [rsp+C0h] [rbp-178h] BYREF
			//   HGRenderGraphBuilder v76; // [rsp+120h] [rbp-118h] BYREF
			//   Il2CppExceptionWrapper *v77; // [rsp+140h] [rbp-F8h] BYREF
			//   TextureDesc v78; // [rsp+150h] [rbp-E8h] BYREF
			//   TextureDesc v79; // [rsp+1B0h] [rbp-88h] BYREF
			// 
			//   v14 = retstr;
			//   if ( !byte_18D9194CE )
			//   {
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::FisheyeEffectData>);
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::FisheyeEffectData>);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGShaderIDs);
			//     sub_18003C530(&MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//     sub_18003C530(&MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     sub_18003C530(&off_18D4F36F8);
			//     sub_18003C530(&off_18D4F36B8);
			//     sub_18003C530(&off_18D4F36C8);
			//     byte_18D9194CE = 1;
			//   }
			//   sub_1802F01E0(&v75, 0LL, 96LL);
			//   v71 = 0LL;
			//   if ( IFix::WrappersManagerImpl::IsPatched(2435, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2435, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v63, v62);
			//     v73 = *sceneDepth;
			//     v72 = *source;
			//     v61 = IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_886(
			//             (TextureHandle *)&v76,
			//             Patch,
			//             (Object *)settingParameters,
			//             (Object *)renderGraph,
			//             (Object *)hgCamera,
			//             (Object *)fisheyeEffect,
			//             (Object *)fishEyeEffectMaterial,
			//             (Object *)fisheyeEffectDepthMaterial,
			//             &v72,
			//             &v73,
			//             fisheyeDepthRT,
			//             0LL);
			//     goto LABEL_31;
			//   }
			//   if ( !fisheyeEffect )
			//     sub_180B536AC(v16, v15);
			//   if ( !HG::Rendering::Runtime::HGFisheyeEffect::IsActive(fisheyeEffect, 0LL) )
			//     goto LABEL_28;
			//   if ( !settingParameters )
			//     sub_180B536AC(v18, v17);
			//   if ( !HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit(
			//           settingParameters.fields._fisheyeEffectEnabled_k__BackingField,
			//           MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit) )
			//   {
			// LABEL_28:
			//     *fisheyeDepthRT = *sceneDepth;
			//     v61 = source;
			// LABEL_31:
			//     v60 = *v61;
			//     goto LABEL_32;
			//   }
			//   if ( !hgCamera )
			//     sub_180B536AC(v20, v19);
			//   sceneRTSize_k__BackingField = hgCamera.fields._sceneRTSize_k__BackingField;
			//   *(Vector2Int *)height = sceneRTSize_k__BackingField;
			//   HG::Rendering::RenderGraphModule::TextureDesc::TextureDesc(
			//     &v75,
			//     sceneRTSize_k__BackingField.m_X,
			//     sceneRTSize_k__BackingField.m_Y,
			//     0LL);
			//   if ( !renderGraph )
			//     sub_180B536AC(v23, v22);
			//   v75.colorFormat = HG::Rendering::RenderGraphModule::HGRenderGraph::GetTextureDescRef(renderGraph, source, 0LL).colorFormat;
			//   v75.name = (String *)"Fisheye Color RT";
			//   sub_1800054D0((HGRenderPathScene *)&v75.name, v24, v25, v26, methoda, v68);
			//   v78 = v75;
			//   HG::Rendering::RenderGraphModule::TextureDesc::TextureDesc(&v75, sceneRTSize_k__BackingField.m_X, height[1], 0LL);
			//   v75.colorFormat = HG::Rendering::RenderGraphModule::HGRenderGraph::GetTextureDescRef(renderGraph, sceneDepth, 0LL).colorFormat;
			//   v75.depthBufferBits = HG::Rendering::RenderGraphModule::HGRenderGraph::GetTextureDescRef(renderGraph, sceneDepth, 0LL).depthBufferBits;
			//   v75.name = (String *)"Fisheye Depth";
			//   sub_1800054D0((HGRenderPathScene *)&v75.name, v27, v28, v29, methodb, v69);
			//   v79 = v75;
			//   v73 = *HG::Rendering::RenderGraphModule::HGRenderGraph::CreateTexture(&v73, renderGraph, &v78, 0LL);
			//   *fisheyeDepthRT = *HG::Rendering::RenderGraphModule::HGRenderGraph::CreateTexture(&v72, renderGraph, &v79, 0LL);
			//   if ( !fisheyeEffect.fields.distortion )
			//     sub_180B536AC(v31, v30);
			//   height[0] = sub_18003F9B0(10LL, fisheyeEffect.fields.distortion);
			//   height[1] = 0;
			//   height[2] = 0;
			//   height[3] = 0;
			//   sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderIDs);
			//   static_fields = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//   FisheyeEffectParams1 = static_fields._FisheyeEffectParams1;
			//   if ( !fishEyeEffectMaterial )
			//     sub_180B536AC(static_fields, v32);
			//   v72 = *(TextureHandle *)height;
			//   UnityEngine::Material::SetVector(fishEyeEffectMaterial, FisheyeEffectParams1, (Vector4 *)&v72, 0LL);
			//   v36 = TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//   v37 = v36._FisheyeEffectParams1;
			//   if ( !fisheyeEffectDepthMaterial )
			//     sub_180B536AC(v36, v35);
			//   v72 = (TextureHandle)(unsigned int)height[0];
			//   UnityEngine::Material::SetVector(fisheyeEffectDepthMaterial, v37, (Vector4 *)&v72, 0LL);
			//   v38 = UnityEngine::Rendering::ProfilingSampler::Get<System::Int32Enum>(
			//           (Int32Enum__Enum)0x9Du,
			//           MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//   HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<System::Object>(
			//     &v76,
			//     renderGraph,
			//     (String *)"FisheyeEffect",
			//     &v71,
			//     v38,
			//     1,
			//     ProfilingHGPass__Enum_None,
			//     MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::FisheyeEffectData>);
			//   v74 = v76;
			//   v72.handle = 0LL;
			//   v72.fallBackResource = (ResourceHandle)&v74;
			//   try
			//   {
			//     v41 = v71;
			//     if ( !v71 )
			//       sub_1802DC2C8(v40, v39);
			//     v71[4].monitor = (MonitorData *)fishEyeEffectMaterial;
			//     v42 = dword_18D8E43F8;
			//     if ( dword_18D8E43F8 )
			//     {
			//       v43 = ((unsigned __int64)&v41[4].monitor >> 12) & 0x1FFFFF;
			//       v44 = (unsigned __int64)v43 >> 6;
			//       v45 = v43 & 0x3F;
			//       _m_prefetchw(&qword_18D6405E0[v44 + 36190]);
			//       do
			//       {
			//         v40 = qword_18D6405E0[v44 + 36190] | (1LL << v45);
			//         v46 = qword_18D6405E0[v44 + 36190];
			//       }
			//       while ( v46 != _InterlockedCompareExchange64(&qword_18D6405E0[v44 + 36190], v40, v46) );
			//       v42 = dword_18D8E43F8;
			//     }
			//     v47 = v71;
			//     if ( !v71 )
			//       sub_1802DC2C8(v40, qword_18D6405E0);
			//     v71[5].klass = (Object__Class *)fisheyeEffectDepthMaterial;
			//     if ( v42 )
			//     {
			//       v48 = ((unsigned __int64)&v47[5] >> 12) & 0x1FFFFF;
			//       v49 = (unsigned __int64)v48 >> 6;
			//       v50 = v48 & 0x3F;
			//       _m_prefetchw(&qword_18D6405E0[v49 + 36190]);
			//       do
			//         v51 = qword_18D6405E0[v49 + 36190];
			//       while ( v51 != _InterlockedCompareExchange64(&qword_18D6405E0[v49 + 36190], v51 | (1LL << v50), v51) );
			//     }
			//     v52 = v71;
			//     v55 = *HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadTexture(
			//              (TextureHandle *)height,
			//              &v74,
			//              source,
			//              0LL);
			//     if ( !v52 )
			//       sub_1802DC2C8(v54, v53);
			//     v52[1] = (Object)v55;
			//     v56 = v71;
			//     v59 = *HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadTexture(
			//              (TextureHandle *)height,
			//              &v74,
			//              sceneDepth,
			//              0LL);
			//     if ( !v56 )
			//       sub_1802DC2C8(v58, v57);
			//     v56[2] = (Object)v59;
			//     HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::WriteTexture((TextureHandle *)height, &v74, &v73, 0LL);
			//     *(__m128i *)height = _mm_load_si128((const __m128i *)&xmmword_18A3576D0);
			//     HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetColorAttachment(
			//       (TextureHandle *)&v76,
			//       &v74,
			//       &v73,
			//       0,
			//       RenderBufferLoadAction__Enum_DontCare,
			//       RenderBufferStoreAction__Enum_Store,
			//       (Color *)height,
			//       0,
			//       0LL);
			//     HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::WriteTexture(
			//       (TextureHandle *)&v76,
			//       &v74,
			//       fisheyeDepthRT,
			//       0LL);
			//     HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetDepthAttachment(
			//       (TextureHandle *)&v76,
			//       &v74,
			//       fisheyeDepthRT,
			//       DepthAccess__Enum_Write,
			//       RenderBufferLoadAction__Enum_Clear,
			//       RenderBufferStoreAction__Enum_Store,
			//       1.0,
			//       0,
			//       0,
			//       0LL);
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<System::Object>(
			//       &v74,
			//       (RenderFunc_1_System_Object_ *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.m_fisheyeEffectFunc,
			//       0LL,
			//       0,
			//       MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::FisheyeEffectData>);
			//   }
			//   catch ( Il2CppExceptionWrapper *v77 )
			//   {
			//     v72.handle = (ResourceHandle)v77.ex;
			//     sub_180222690(&v72);
			//     v14 = retstr;
			//     goto LABEL_27;
			//   }
			//   sub_180222690(&v72);
			// LABEL_27:
			//   v60 = v73;
			// LABEL_32:
			//   *v14 = v60;
			//   return v14;
			// }
			// 
			return null;
		}

		internal static void PrepareFrostedGlassPSMaterials(HGRenderPipelineMaterialCollector materialCollector, ref UberPostPassUtils.FrostedGlassPSMaterials frostedGlassPSMaterials, HGRenderPipelineRuntimeResources defaultResources)
		{
			// // Void PrepareFrostedGlassPSMaterials(HGRenderPipelineMaterialCollector, UberPostPassUtils+FrostedGlassPSMaterials ByRef, HGRenderPipelineRuntimeResources)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareFrostedGlassPSMaterials(
			//         HGRenderPipelineMaterialCollector *materialCollector,
			//         UberPostPassUtils_FrostedGlassPSMaterials *frostedGlassPSMaterials,
			//         HGRenderPipelineRuntimeResources *defaultResources,
			//         MethodInfo *method)
			// {
			//   __int64 v7; // r8
			//   __int64 v8; // r9
			//   int v9; // ebx
			//   HGRenderPathBase_HGRenderPathResources *v10; // rdx
			//   PassConstructorID__Enum__Array *v11; // r8
			//   HGCamera *v12; // r9
			//   __int64 v13; // r8
			//   __int64 v14; // r9
			//   HGRenderPathBase_HGRenderPathResources *v15; // rdx
			//   PassConstructorID__Enum__Array *v16; // r8
			//   HGCamera *v17; // r9
			//   HGRenderPathBase_HGRenderPathResources *v18; // rdx
			//   PassConstructorID__Enum__Array *v19; // r8
			//   HGCamera *v20; // r9
			//   HGRenderPipelineRuntimeResources_ShaderResources *shaders; // rdx
			//   __int64 v22; // rcx
			//   Material__Array *frostedGlass1stPassMaterials; // rdi
			//   Material *Material; // rax
			//   Material *v25; // r14
			//   Material__Array *frostedGlass2ndPassMaterials; // rdi
			//   Material *v27; // rax
			//   Material *v28; // r14
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   MethodInfo *methoda; // [rsp+20h] [rbp-38h]
			//   MethodInfo *methodb; // [rsp+20h] [rbp-38h]
			//   MethodInfo *methodc; // [rsp+20h] [rbp-38h]
			//   MethodInfo *v33; // [rsp+28h] [rbp-30h]
			//   MethodInfo *v34; // [rsp+28h] [rbp-30h]
			//   MethodInfo *v35; // [rsp+28h] [rbp-30h]
			//   _OWORD v36[2]; // [rsp+30h] [rbp-28h] BYREF
			// 
			//   if ( !byte_18D8EDA35 )
			//   {
			//     sub_18003C530(&TypeInfo::UnityEngine::Material);
			//     byte_18D8EDA35 = 1;
			//   }
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2437, 0LL) )
			//   {
			//     v9 = 0;
			//     v36[0] = (unsigned __int64)il2cpp_array_new_specific_0(TypeInfo::UnityEngine::Material, 4LL, v7, v8);
			//     sub_1800054D0((HGRenderPathScene *)v36, v10, v11, v12, methoda, v33);
			//     *((_QWORD *)&v36[0] + 1) = il2cpp_array_new_specific_0(TypeInfo::UnityEngine::Material, 4LL, v13, v14);
			//     sub_1800054D0((HGRenderPathScene *)((char *)v36 + 8), v15, v16, v17, methodb, v34);
			//     *frostedGlassPSMaterials = (UberPostPassUtils_FrostedGlassPSMaterials)v36[0];
			//     sub_1800054D0((HGRenderPathScene *)frostedGlassPSMaterials, v18, v19, v20, methodc, v35);
			//     while ( 1 )
			//     {
			//       frostedGlass1stPassMaterials = frostedGlassPSMaterials.frostedGlass1stPassMaterials;
			//       if ( !defaultResources )
			//         break;
			//       shaders = defaultResources.fields.shaders;
			//       if ( !shaders )
			//         break;
			//       if ( !materialCollector )
			//         break;
			//       Material = HG::Rendering::Runtime::HGRenderPipelineMaterialCollector::CreateMaterial(
			//                    materialCollector,
			//                    shaders.fields.frostedGlassBlurPS,
			//                    0,
			//                    0LL);
			//       v25 = Material;
			//       if ( !frostedGlass1stPassMaterials )
			//         break;
			//       sub_180036D40(frostedGlass1stPassMaterials, Material);
			//       sub_18000FDA0(frostedGlass1stPassMaterials, v9, v25);
			//       shaders = defaultResources.fields.shaders;
			//       frostedGlass2ndPassMaterials = frostedGlassPSMaterials.frostedGlass2ndPassMaterials;
			//       if ( !shaders )
			//         break;
			//       v27 = HG::Rendering::Runtime::HGRenderPipelineMaterialCollector::CreateMaterial(
			//               materialCollector,
			//               shaders.fields.frostedGlassBlurPS,
			//               0,
			//               0LL);
			//       v28 = v27;
			//       if ( !frostedGlass2ndPassMaterials )
			//         break;
			//       sub_180036D40(frostedGlass2ndPassMaterials, v27);
			//       sub_18000FDA0(frostedGlass2ndPassMaterials, v9++, v28);
			//       if ( v9 >= 4 )
			//         return;
			//     }
			// LABEL_13:
			//     sub_180B536AC(v22, shaders);
			//   }
			//   Patch = IFix::WrappersManagerImpl::GetPatch(2437, 0LL);
			//   if ( !Patch )
			//     goto LABEL_13;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_887(
			//     Patch,
			//     (Object *)materialCollector,
			//     frostedGlassPSMaterials,
			//     (Object *)defaultResources,
			//     0LL);
			// }
			// 
		}

		internal static void DisposeFrostedGlassPSMaterials(ref UberPostPassUtils.FrostedGlassPSMaterials frostedGlassPSMaterials)
		{
			// // Void DisposeFrostedGlassPSMaterials(UberPostPassUtils+FrostedGlassPSMaterials ByRef)
			// void HG::Rendering::Runtime::UberPostPassUtils::DisposeFrostedGlassPSMaterials(
			//         UberPostPassUtils_FrostedGlassPSMaterials *frostedGlassPSMaterials,
			//         MethodInfo *method)
			// {
			//   __int64 v3; // rdx
			//   int v4; // ebx
			//   Material__Array *frostedGlass1stPassMaterials; // rcx
			//   Object_1 *v6; // rsi
			//   Material__Array *frostedGlass2ndPassMaterials; // rax
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			// 
			//   if ( !byte_18D8EDA36 )
			//   {
			//     sub_18003C530(&TypeInfo::UnityEngine::Rendering::CoreUtils);
			//     byte_18D8EDA36 = 1;
			//   }
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2438, 0LL) )
			//   {
			//     v4 = 0;
			//     while ( 1 )
			//     {
			//       frostedGlass1stPassMaterials = frostedGlassPSMaterials.frostedGlass1stPassMaterials;
			//       if ( !frostedGlassPSMaterials.frostedGlass1stPassMaterials )
			//         break;
			//       if ( (unsigned int)v4 >= frostedGlass1stPassMaterials.max_length.size )
			//         goto LABEL_16;
			//       v6 = (Object_1 *)frostedGlass1stPassMaterials.vector[v4];
			//       if ( !TypeInfo::UnityEngine::Rendering::CoreUtils._1.cctor_finished_or_no_cctor )
			//         il2cpp_runtime_class_init_0(TypeInfo::UnityEngine::Rendering::CoreUtils, v3);
			//       UnityEngine::Rendering::CoreUtils::Destroy(v6, 0LL);
			//       frostedGlass2ndPassMaterials = frostedGlassPSMaterials.frostedGlass2ndPassMaterials;
			//       if ( !frostedGlass2ndPassMaterials )
			//         break;
			//       if ( (unsigned int)v4 >= frostedGlass2ndPassMaterials.max_length.size )
			// LABEL_16:
			//         sub_180070270(frostedGlass1stPassMaterials, v3);
			//       UnityEngine::Rendering::CoreUtils::Destroy((Object_1 *)frostedGlass2ndPassMaterials.vector[v4], 0LL);
			//       frostedGlass1stPassMaterials = frostedGlassPSMaterials.frostedGlass1stPassMaterials;
			//       if ( !frostedGlassPSMaterials.frostedGlass1stPassMaterials )
			//         break;
			//       sub_18000FDA0(frostedGlass1stPassMaterials, v4, 0LL);
			//       frostedGlass1stPassMaterials = frostedGlassPSMaterials.frostedGlass2ndPassMaterials;
			//       if ( !frostedGlass1stPassMaterials )
			//         break;
			//       sub_18000FDA0(frostedGlass1stPassMaterials, v4++, 0LL);
			//       if ( v4 >= 4 )
			//         return;
			//     }
			// LABEL_15:
			//     sub_180B536AC(frostedGlass1stPassMaterials, v3);
			//   }
			//   Patch = IFix::WrappersManagerImpl::GetPatch(2438, 0LL);
			//   if ( !Patch )
			//     goto LABEL_15;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_888(Patch, frostedGlassPSMaterials, 0LL);
			// }
			// 
		}

		internal static void PrepareFrostedGlassData(bool useComputeShader, HGRenderGraph renderGraph, UberPostPassUtils.FrostedGlassData data, HGCamera camera, TextureHandle source, ref UberPostPassUtils.FrostedGlassPSMaterials frostedGlassPSMaterials)
		{
			// // Void PrepareFrostedGlassData(Boolean, HGRenderGraph, UberPostPassUtils+FrostedGlassData, HGCamera, TextureHandle, UberPostPassUtils+FrostedGlassPSMaterials ByRef)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareFrostedGlassData(
			//         bool useComputeShader,
			//         HGRenderGraph *renderGraph,
			//         UberPostPassUtils_FrostedGlassData *data,
			//         HGCamera *camera,
			//         TextureHandle *source,
			//         UberPostPassUtils_FrostedGlassPSMaterials *frostedGlassPSMaterials,
			//         MethodInfo *method)
			// {
			//   HGRenderPathBase_HGRenderPathResources *v11; // rdx
			//   ILFixDynamicMethodWrapper_2 *Patch; // rcx
			//   int v13; // esi
			//   Material__Array *frostedGlass1stPassMaterials; // rbx
			//   Material__Array *frostedGlass1stPassMat; // rbp
			//   Material *v16; // rbx
			//   Material__Array *frostedGlass2ndPassMaterials; // rbx
			//   Material__Array *frostedGlass2ndPassMat; // rbp
			//   Material *v19; // rbx
			//   HGRenderPipeline *currentPipeline; // rax
			//   HGRenderPipelineRuntimeResources *defaultResources; // rax
			//   PassConstructorID__Enum__Array *v22; // r8
			//   HGCamera *v23; // r9
			//   HGRenderPipelineRuntimeResources_ShaderResources *shaders; // rax
			//   int32_t Kernel; // eax
			//   int32_t v26; // esi
			//   float v27; // xmm6_4
			//   Vector2Int__Array *mipsDownSize; // rbp
			//   __int64 v29; // rcx
			//   MethodInfo *v30; // [rsp+20h] [rbp-68h]
			//   MethodInfo *v31; // [rsp+28h] [rbp-60h]
			//   Vector2Int v32; // [rsp+40h] [rbp-48h]
			//   TextureHandle v33; // [rsp+50h] [rbp-38h] BYREF
			// 
			//   if ( !byte_18D9194CF )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGRenderPipeline);
			//     sub_18003C530(&off_18D4F3710);
			//     sub_18003C530(&off_18D4F3AD8);
			//     byte_18D9194CF = 1;
			//   }
			//   if ( IFix::WrappersManagerImpl::IsPatched(2439, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2439, 0LL);
			//     if ( !Patch )
			//       goto LABEL_29;
			//     v33 = *source;
			//     IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_889(
			//       Patch,
			//       useComputeShader,
			//       (Object *)renderGraph,
			//       (Object *)data,
			//       (Object *)camera,
			//       &v33,
			//       frostedGlassPSMaterials,
			//       0LL);
			//   }
			//   else
			//   {
			//     if ( !useComputeShader )
			//     {
			//       v13 = 0;
			//       if ( data )
			//       {
			//         while ( 1 )
			//         {
			//           frostedGlass1stPassMaterials = frostedGlassPSMaterials.frostedGlass1stPassMaterials;
			//           if ( !frostedGlassPSMaterials.frostedGlass1stPassMaterials )
			//             break;
			//           if ( (unsigned int)v13 >= frostedGlass1stPassMaterials.max_length.size )
			//             goto LABEL_27;
			//           frostedGlass1stPassMat = data.fields.frostedGlass1stPassMat;
			//           if ( !frostedGlass1stPassMat )
			//             break;
			//           v16 = frostedGlass1stPassMaterials.vector[v13];
			//           sub_180036D40(data.fields.frostedGlass1stPassMat, v16);
			//           sub_18000FDA0(frostedGlass1stPassMat, v13, v16);
			//           frostedGlass2ndPassMaterials = frostedGlassPSMaterials.frostedGlass2ndPassMaterials;
			//           if ( !frostedGlass2ndPassMaterials )
			//             break;
			//           if ( (unsigned int)v13 >= frostedGlass2ndPassMaterials.max_length.size )
			// LABEL_27:
			//             sub_180070270(Patch, v11);
			//           frostedGlass2ndPassMat = data.fields.frostedGlass2ndPassMat;
			//           if ( !frostedGlass2ndPassMat )
			//             break;
			//           v19 = frostedGlass2ndPassMaterials.vector[v13];
			//           sub_180036D40(data.fields.frostedGlass2ndPassMat, v19);
			//           sub_18000FDA0(frostedGlass2ndPassMat, v13++, v19);
			//           if ( v13 >= 3 )
			//             goto LABEL_21;
			//         }
			//       }
			// LABEL_29:
			//       sub_180B536AC(Patch, v11);
			//     }
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGRenderPipeline);
			//     currentPipeline = HG::Rendering::Runtime::HGRenderPipeline::get_currentPipeline(0LL);
			//     if ( !currentPipeline )
			//       goto LABEL_29;
			//     defaultResources = HG::Rendering::Runtime::HGRenderPipeline::get_defaultResources(currentPipeline, 0LL);
			//     if ( !defaultResources )
			//       goto LABEL_29;
			//     shaders = defaultResources.fields.shaders;
			//     if ( !shaders )
			//       goto LABEL_29;
			//     Patch = (ILFixDynamicMethodWrapper_2 *)shaders.fields.frostedGlassBlurCS;
			//     if ( !data )
			//       goto LABEL_29;
			//     data.fields.frostedGlassBlurCS = (ComputeShader *)Patch;
			//     sub_1800054D0((HGRenderPathScene *)&data.fields, v11, v22, v23, v30, v31);
			//     Patch = (ILFixDynamicMethodWrapper_2 *)data.fields.frostedGlassBlurCS;
			//     if ( !Patch )
			//       goto LABEL_29;
			//     Kernel = UnityEngine::ComputeShader::FindKernel((ComputeShader *)Patch, (String *)"KMain", 0LL);
			//     Patch = (ILFixDynamicMethodWrapper_2 *)data.fields.frostedGlassBlurCS;
			//     data.fields.kernelKMain = Kernel;
			//     if ( !Patch )
			//       goto LABEL_29;
			//     data.fields.kernelKMainWithLut = UnityEngine::ComputeShader::FindKernel(
			//                                         (ComputeShader *)Patch,
			//                                         (String *)"KMainWithLut",
			//                                         0LL);
			// LABEL_21:
			//     data.fields.source = *source;
			//     if ( !camera )
			//       goto LABEL_29;
			//     v26 = 0;
			//     v27 = 0.25;
			//     v33.handle = (ResourceHandle)camera.fields._taauRTSize_k__BackingField;
			//     data.fields.downsampleNum = 3;
			//     data.fields.colorThreshold = 2.5;
			//     while ( v26 < data.fields.downsampleNum )
			//     {
			//       mipsDownSize = data.fields.mipsDownSize;
			//       v32.m_X = sub_1826E82D0();
			//       v32.m_Y = sub_1826E82D0();
			//       if ( !mipsDownSize )
			//         goto LABEL_29;
			//       if ( (unsigned int)v26 >= mipsDownSize.max_length.size )
			//         goto LABEL_27;
			//       v27 = v27 * 0.5;
			//       v29 = v26++;
			//       mipsDownSize.vector[v29] = v32;
			//     }
			//   }
			// }
			// 
		}

		internal static void PrepareFrostedGlassLutData(bool useComputeShader, UberPostPassUtils.FrostedGlassData data, HGCamera camera, TextureHandle logLut, int lutSize)
		{
			// // Void PrepareFrostedGlassLutData(Boolean, UberPostPassUtils+FrostedGlassData, HGCamera, TextureHandle, Int32)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareFrostedGlassLutData(
			//         bool useComputeShader,
			//         UberPostPassUtils_FrostedGlassData *data,
			//         HGCamera *camera,
			//         TextureHandle *logLut,
			//         int32_t lutSize,
			//         MethodInfo *method)
			// {
			//   String **p_m_Name; // rdx
			//   ILFixDynamicMethodWrapper_2 *Patch; // rcx
			//   float PostExposureLinear; // xmm0_4
			//   TextureHandle v13; // xmm1
			//   Material__Array *frostedGlass1stPassMat; // rax
			//   Material *v15; // rsi
			//   HGEnvironmentPhase *InterpolatedPhase; // rax
			//   __int64 v17; // rdx
			//   HGColorGradingConfig *v18; // rcx
			//   HGColorGradingConfig *p_colorGradingConfig; // rax
			//   Vector4 shadows; // xmm1
			//   PassConstructorID__Enum__Array *v21; // r8
			//   HGCamera *v22; // r9
			//   HGRenderPathBase_HGRenderPathResources *v23; // rdx
			//   PassConstructorID__Enum__Array *v24; // r8
			//   HGCamera *v25; // r9
			//   TileBase *v26; // rdx
			//   Vector3Int *v27; // r8
			//   ITilemap *v28; // r9
			//   Vector4 v29; // xmm1
			//   Material__Array *v30; // rax
			//   Material *v31; // rdi
			//   int v32; // eax
			//   int v33; // edi
			//   int v34; // eax
			//   int v35; // esi
			//   int v36; // eax
			//   MethodInfo *v37; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *v38; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *v39; // [rsp+30h] [rbp-D8h]
			//   Vector4 v40; // [rsp+48h] [rbp-C0h] BYREF
			//   HGColorGradingConfig v41; // [rsp+58h] [rbp-B0h] BYREF
			// 
			//   if ( !byte_18D9194D0 )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGColorGradingConfig);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGEnvironmentManager);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     byte_18D9194D0 = 1;
			//   }
			//   sub_1802F01E0(&v41, 0LL, 368LL);
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2440, 0LL) )
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     PostExposureLinear = HG::Rendering::Runtime::UberPostPassUtils::GetPostExposureLinear(camera, 0LL);
			//     if ( !data )
			//       goto LABEL_27;
			//     Patch = (ILFixDynamicMethodWrapper_2 *)(unsigned int)lutSize;
			//     v13 = *logLut;
			//     v40.w = PostExposureLinear;
			//     data.fields.logLut = v13;
			//     v40.x = 1.0 / (float)(lutSize * lutSize);
			//     v40.y = 1.0 / (float)lutSize;
			//     v40.z = (float)lutSize - 1.0;
			//     data.fields.logLutSettings = v40;
			//     if ( !useComputeShader )
			//     {
			//       frostedGlass1stPassMat = data.fields.frostedGlass1stPassMat;
			//       if ( !frostedGlass1stPassMat )
			//         goto LABEL_27;
			//       if ( !frostedGlass1stPassMat.max_length.size )
			//         goto LABEL_18;
			//       v15 = frostedGlass1stPassMat.vector[0];
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//       p_m_Name = &TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_EnableScreenSpaceShadowMask.m_Name;
			//       if ( !v15 )
			//         goto LABEL_27;
			//       UnityEngine::Material::EnableKeyword(v15, p_m_Name[43], 0LL);
			//     }
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGEnvironmentManager);
			//     InterpolatedPhase = HG::Rendering::Runtime::HGEnvironmentManager::GetInterpolatedPhase(camera, 0LL);
			//     if ( !InterpolatedPhase )
			//       goto LABEL_27;
			//     v17 = 2LL;
			//     v18 = &v41;
			//     p_colorGradingConfig = &InterpolatedPhase.fields.colorGradingConfig;
			//     do
			//     {
			//       *(_OWORD *)&v18.tonemappingMode = *(_OWORD *)&p_colorGradingConfig.tonemappingMode;
			//       *(_OWORD *)&v18.colorLookupContribution = *(_OWORD *)&p_colorGradingConfig.colorLookupContribution;
			//       *(_OWORD *)&v18.colorAdjustmentsEnabled = *(_OWORD *)&p_colorGradingConfig.colorAdjustmentsEnabled;
			//       *(_OWORD *)&v18.colorAdjustmentsColorFilter.g = *(_OWORD *)&p_colorGradingConfig.colorAdjustmentsColorFilter.g;
			//       *(_OWORD *)&v18.colorAdjustmentsSaturation = *(_OWORD *)&p_colorGradingConfig.colorAdjustmentsSaturation;
			//       *(_OWORD *)&v18.channelMixerRedOutBlueIn = *(_OWORD *)&p_colorGradingConfig.channelMixerRedOutBlueIn;
			//       *(_OWORD *)&v18.channelMixerBlueOutRedIn = *(_OWORD *)&p_colorGradingConfig.channelMixerBlueOutRedIn;
			//       v18 = (HGColorGradingConfig *)((char *)v18 + 128);
			//       shadows = p_colorGradingConfig.shadowsMidtonesHighlights.shadows;
			//       p_colorGradingConfig = (HGColorGradingConfig *)((char *)p_colorGradingConfig + 128);
			//       *(Vector4 *)&v18[-1].colorCurves.masterOverriding = shadows;
			//       --v17;
			//     }
			//     while ( v17 );
			//     *(_OWORD *)&v18.tonemappingMode = *(_OWORD *)&p_colorGradingConfig.tonemappingMode;
			//     *(_OWORD *)&v18.colorLookupContribution = *(_OWORD *)&p_colorGradingConfig.colorLookupContribution;
			//     *(_OWORD *)&v18.colorAdjustmentsEnabled = *(_OWORD *)&p_colorGradingConfig.colorAdjustmentsEnabled;
			//     *(_OWORD *)&v18.colorAdjustmentsColorFilter.g = *(_OWORD *)&p_colorGradingConfig.colorAdjustmentsColorFilter.g;
			//     *(_OWORD *)&v18.colorAdjustmentsSaturation = *(_OWORD *)&p_colorGradingConfig.colorAdjustmentsSaturation;
			//     *(_OWORD *)&v18.channelMixerRedOutBlueIn = *(_OWORD *)&p_colorGradingConfig.channelMixerRedOutBlueIn;
			//     *(_OWORD *)&v18.channelMixerBlueOutRedIn = *(_OWORD *)&p_colorGradingConfig.channelMixerBlueOutRedIn;
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGColorGradingConfig);
			//     if ( !HG::Rendering::Runtime::HGColorGradingConfig::IsColorLookupEnabled(&v41, 0LL) )
			//     {
			//       data.fields.enableUserLut = 0;
			//       data.fields.userLut = UnityEngine::Texture2D::get_blackTexture(0LL);
			//       sub_1800054D0((HGRenderPathScene *)&data.fields.userLut, v23, v24, v25, v37, v39);
			//       v29 = (Vector4)_mm_loadu_si128((const __m128i *)UnityEngine::Tilemaps::TileBase::GetTileAnimationDataNoRef(
			//                                                         (TileAnimationData *)&v40,
			//                                                         v26,
			//                                                         v27,
			//                                                         v28,
			//                                                         v38));
			// LABEL_25:
			//       data.fields.userLogLutSettings = v29;
			//       return;
			//     }
			//     data.fields.enableUserLut = 1;
			//     if ( useComputeShader )
			//       goto LABEL_21;
			//     v30 = data.fields.frostedGlass1stPassMat;
			//     if ( v30 )
			//     {
			//       if ( !v30.max_length.size )
			// LABEL_18:
			//         sub_180070270(Patch, p_m_Name);
			//       v31 = v30.vector[0];
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//       p_m_Name = &TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_EnableScreenSpaceShadowMask.m_Name;
			//       if ( v31 )
			//       {
			//         UnityEngine::Material::EnableKeyword(v31, p_m_Name[44], 0LL);
			// LABEL_21:
			//         data.fields.userLut = v41.colorLookupTexture;
			//         sub_1800054D0(
			//           (HGRenderPathScene *)&data.fields.userLut,
			//           (HGRenderPathBase_HGRenderPathResources *)p_m_Name,
			//           v21,
			//           v22,
			//           v37,
			//           v39);
			//         p_m_Name = (String **)v41.colorLookupTexture;
			//         if ( v41.colorLookupTexture )
			//         {
			//           v32 = sub_18003ED00(5LL);
			//           p_m_Name = (String **)v41.colorLookupTexture;
			//           v33 = v32;
			//           if ( v41.colorLookupTexture )
			//           {
			//             v34 = sub_18003ED00(7LL);
			//             p_m_Name = (String **)v41.colorLookupTexture;
			//             v35 = v34;
			//             if ( v41.colorLookupTexture )
			//             {
			//               v36 = sub_18003ED00(7LL);
			//               v40.x = 1.0 / (float)v33;
			//               v40.y = 1.0 / (float)v35;
			//               v40.w = v41.colorLookupContribution;
			//               v40.z = (float)v36 - 1.0;
			//               v29 = v40;
			//               goto LABEL_25;
			//             }
			//           }
			//         }
			//       }
			//     }
			// LABEL_27:
			//     sub_180B536AC(Patch, p_m_Name);
			//   }
			//   Patch = IFix::WrappersManagerImpl::GetPatch(2440, 0LL);
			//   if ( !Patch )
			//     goto LABEL_27;
			//   v40 = (Vector4)*logLut;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_890(
			//     Patch,
			//     useComputeShader,
			//     (Object *)data,
			//     (Object *)camera,
			//     (TextureHandle *)&v40,
			//     lutSize,
			//     0LL);
			// }
			// 
		}

		internal static void PrepareFrostedGlassSceneRTs(bool useComputeShader, HGRenderGraph renderGraph, UberPostPassUtils.FrostedGlassData data, ref RTHandle[] sceneFrostedGlassRTs, ref Vector2Int[] sceneFrostedGlassRTSizes)
		{
			// // Void PrepareFrostedGlassSceneRTs(Boolean, HGRenderGraph, UberPostPassUtils+FrostedGlassData, RTHandle[] ByRef, Vector2Int[] ByRef)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareFrostedGlassSceneRTs(
			//         bool useComputeShader,
			//         HGRenderGraph *renderGraph,
			//         UberPostPassUtils_FrostedGlassData *data,
			//         RTHandle__Array **sceneFrostedGlassRTs,
			//         Vector2Int__Array **sceneFrostedGlassRTSizes,
			//         MethodInfo *method)
			// {
			//   __int64 v10; // rdx
			//   ILFixDynamicMethodWrapper_2 *mipsDownSize; // rcx
			//   int v12; // edi
			//   Vector2Int__Array **v13; // r13
			//   Vector2Int v14; // rbx
			//   RTHandle *v15; // rcx
			//   RTHandle__Array *v16; // r12
			//   RTHandle *v17; // rax
			//   RTHandle *v18; // r13
			//   TextureHandle__Array *mipsDown; // r12
			//   RTHandle__Array *v20; // r8
			//   TextureHandle *v21; // rax
			//   HGRenderPathBase_HGRenderPathResources *v22; // rdx
			//   PassConstructorID__Enum__Array *v23; // r8
			//   HGCamera *v24; // r9
			//   TextureHandle__Array *mipsDownTemp; // rbx
			//   TextureHandle *v26; // rax
			//   TextureHandle__Array *v27; // rcx
			//   MethodInfo *colorFormat; // [rsp+28h] [rbp-100h]
			//   MethodInfo *filterMode; // [rsp+30h] [rbp-F8h]
			//   int32_t width[2]; // [rsp+A8h] [rbp-80h] BYREF
			//   TextureHandle v31; // [rsp+B8h] [rbp-70h] BYREF
			//   _DWORD v32[4]; // [rsp+C8h] [rbp-60h] BYREF
			//   TextureDesc v33; // [rsp+D8h] [rbp-50h] BYREF
			//   TextureDesc v34; // [rsp+138h] [rbp+10h] BYREF
			//   TextureHandle v35; // [rsp+198h] [rbp+70h] BYREF
			//   TextureHandle v36; // [rsp+1A8h] [rbp+80h] BYREF
			//   TextureHandle v37; // [rsp+1B8h] [rbp+90h] BYREF
			// 
			//   if ( !byte_18D9194D1 )
			//   {
			//     sub_18003C530(&TypeInfo::UnityEngine::Rendering::RTHandles);
			//     sub_18003C530(&off_18D4F3740);
			//     sub_18003C530(&off_18D4F3850);
			//     byte_18D9194D1 = 1;
			//   }
			//   sub_1802F01E0(&v34, 0LL, 96LL);
			//   sub_1802F01E0(&v33, 0LL, 96LL);
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2441, 0LL) )
			//   {
			//     v12 = 0;
			//     if ( data )
			//     {
			//       v13 = sceneFrostedGlassRTSizes;
			//       while ( 1 )
			//       {
			//         mipsDownSize = (ILFixDynamicMethodWrapper_2 *)data.fields.mipsDownSize;
			//         if ( !mipsDownSize )
			//           break;
			//         sub_180056B60(mipsDownSize, width, v12);
			//         mipsDownSize = (ILFixDynamicMethodWrapper_2 *)*sceneFrostedGlassRTs;
			//         if ( !*sceneFrostedGlassRTs )
			//           break;
			//         if ( (unsigned int)v12 >= mipsDownSize.fields.methodId )
			//           goto LABEL_35;
			//         v14 = *(Vector2Int *)width;
			//         if ( !*((_QWORD *)&mipsDownSize.fields.anonObj + v12) )
			//           goto LABEL_13;
			//         mipsDownSize = (ILFixDynamicMethodWrapper_2 *)*v13;
			//         if ( !*v13 )
			//           break;
			//         sub_180056B60(mipsDownSize, v32, v12);
			//         if ( v32[0] != v14.m_X
			//           || (mipsDownSize = (ILFixDynamicMethodWrapper_2 *)HIDWORD(*(unsigned __int64 *)&v14), v32[1] != v14.m_Y) )
			//         {
			// LABEL_13:
			//           mipsDownSize = (ILFixDynamicMethodWrapper_2 *)*sceneFrostedGlassRTs;
			//           if ( !*sceneFrostedGlassRTs )
			//             break;
			//           if ( (unsigned int)v12 >= mipsDownSize.fields.methodId )
			//             goto LABEL_35;
			//           v15 = (RTHandle *)*((_QWORD *)&mipsDownSize.fields.anonObj + v12);
			//           if ( v15 )
			//             UnityEngine::Rendering::RTHandle::Release(v15, 0LL);
			//           v16 = *sceneFrostedGlassRTs;
			//           sub_180002C70(TypeInfo::UnityEngine::Rendering::RTHandles);
			//           v17 = UnityEngine::Rendering::RTHandles::Alloc(
			//                   v14.m_X,
			//                   width[1],
			//                   1,
			//                   DepthBits__Enum_None,
			//                   GraphicsFormat__Enum_B10G11R11_UFloatPack32,
			//                   FilterMode__Enum_Bilinear,
			//                   TextureWrapMode__Enum_Clamp,
			//                   TextureDimension__Enum_Tex2D,
			//                   1,
			//                   0,
			//                   0,
			//                   0,
			//                   1,
			//                   0.0,
			//                   MSAASamples__Enum_None,
			//                   0,
			//                   RenderTextureMemoryless__Enum_None,
			//                   (String *)"FrostedGlassMipDown",
			//                   0LL);
			//           v18 = v17;
			//           if ( !v16 )
			//             break;
			//           sub_180036D40(v16, v17);
			//           sub_180328478(v16, v12, v18);
			//           mipsDownSize = (ILFixDynamicMethodWrapper_2 *)*sceneFrostedGlassRTSizes;
			//           if ( !*sceneFrostedGlassRTSizes )
			//             break;
			//           if ( (unsigned int)v12 >= mipsDownSize.fields.methodId )
			//             goto LABEL_35;
			//           v13 = sceneFrostedGlassRTSizes;
			//           *((Vector2Int *)&mipsDownSize.fields.anonObj + v12) = v14;
			//         }
			//         mipsDown = data.fields.mipsDown;
			//         v20 = *sceneFrostedGlassRTs;
			//         if ( useComputeShader )
			//         {
			//           if ( !v20 )
			//             break;
			//           if ( (unsigned int)v12 >= v20.max_length.size )
			// LABEL_35:
			//             sub_180070270(mipsDownSize, v10);
			//           if ( !renderGraph )
			//             break;
			//           v26 = HG::Rendering::RenderGraphModule::HGRenderGraph::ImportTexture(&v37, renderGraph, v20.vector[v12], 0LL);
			//           if ( !mipsDown )
			//             break;
			//           v27 = mipsDown;
			//         }
			//         else
			//         {
			//           if ( !v20 )
			//             break;
			//           if ( (unsigned int)v12 >= v20.max_length.size )
			//             goto LABEL_35;
			//           if ( !renderGraph )
			//             break;
			//           v21 = HG::Rendering::RenderGraphModule::HGRenderGraph::ImportTexture(&v35, renderGraph, v20.vector[v12], 0LL);
			//           if ( !mipsDown )
			//             break;
			//           v31 = *v21;
			//           sub_1803EF6F4(mipsDown, v12, &v31);
			//           HG::Rendering::RenderGraphModule::TextureDesc::TextureDesc(&v33, v14, 0LL);
			//           v33.name = (String *)"FrostedGlassMipDownTemp";
			//           v33.colorFormat = 74;
			//           v33.enableRandomWrite = 0;
			//           sub_1800054D0((HGRenderPathScene *)&v33.name, v22, v23, v24, colorFormat, filterMode);
			//           mipsDownTemp = data.fields.mipsDownTemp;
			//           v33.wrapMode = 1;
			//           v33.filterMode = 1;
			//           v34 = v33;
			//           v26 = HG::Rendering::RenderGraphModule::HGRenderGraph::CreateTexture(&v36, renderGraph, &v34, 0LL);
			//           if ( !mipsDownTemp )
			//             break;
			//           v27 = mipsDownTemp;
			//         }
			//         v31 = *v26;
			//         sub_1803EF6F4(v27, v12++, &v31);
			//         if ( v12 >= 3 )
			//           return;
			//       }
			//     }
			// LABEL_37:
			//     sub_180B536AC(mipsDownSize, v10);
			//   }
			//   mipsDownSize = IFix::WrappersManagerImpl::GetPatch(2441, 0LL);
			//   if ( !mipsDownSize )
			//     goto LABEL_37;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_891(
			//     mipsDownSize,
			//     useComputeShader,
			//     (Object *)renderGraph,
			//     (Object *)data,
			//     sceneFrostedGlassRTs,
			//     sceneFrostedGlassRTSizes,
			//     0LL);
			// }
			// 
		}

		internal static void PrepareFrostedGlassUIRTs(bool useComputeShader, HGRenderGraph renderGraph, UberPostPassUtils.FrostedGlassData data)
		{
			// // Void PrepareFrostedGlassUIRTs(Boolean, HGRenderGraph, UberPostPassUtils+FrostedGlassData)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareFrostedGlassUIRTs(
			//         bool useComputeShader,
			//         HGRenderGraph *renderGraph,
			//         UberPostPassUtils_FrostedGlassData *data,
			//         MethodInfo *method)
			// {
			//   __int64 v7; // rdx
			//   Vector2Int__Array *mipsDownSize; // rcx
			//   int32_t v9; // edi
			//   HGRenderPathBase_HGRenderPathResources *v10; // rdx
			//   PassConstructorID__Enum__Array *v11; // r8
			//   HGCamera *v12; // r9
			//   TextureHandle__Array *mipsDown; // r14
			//   TextureHandle *v14; // rax
			//   HGRenderPathBase_HGRenderPathResources *v15; // rdx
			//   PassConstructorID__Enum__Array *v16; // r8
			//   HGCamera *v17; // r9
			//   TextureHandle__Array *mipsDownTemp; // r14
			//   TextureHandle *v19; // rax
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   MethodInfo *v21; // [rsp+28h] [rbp-E0h]
			//   MethodInfo *v22[13]; // [rsp+30h] [rbp-D8h] BYREF
			//   Vector2Int size; // [rsp+98h] [rbp-70h] BYREF
			//   Vector2Int v24; // [rsp+A0h] [rbp-68h] BYREF
			//   TextureHandle v25; // [rsp+A8h] [rbp-60h] BYREF
			//   TextureDesc v26; // [rsp+B8h] [rbp-50h] BYREF
			//   TextureHandle v27; // [rsp+118h] [rbp+10h] BYREF
			//   TextureHandle v28; // [rsp+128h] [rbp+20h] BYREF
			// 
			//   if ( !byte_18D9194D2 )
			//   {
			//     sub_18003C530(&off_18D4F3740);
			//     sub_18003C530(&off_18D4F3850);
			//     byte_18D9194D2 = 1;
			//   }
			//   sub_1802F01E0(&v26, 0LL, 96LL);
			//   sub_1802F01E0(&v22[1], 0LL, 96LL);
			//   if ( IFix::WrappersManagerImpl::IsPatched(2442, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2442, 0LL);
			//     if ( Patch )
			//     {
			//       IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_892(
			//         Patch,
			//         useComputeShader,
			//         (Object *)renderGraph,
			//         (Object *)data,
			//         0LL);
			//       return;
			//     }
			// LABEL_15:
			//     sub_180B536AC(mipsDownSize, v7);
			//   }
			//   v9 = 0;
			//   if ( !data )
			//     goto LABEL_15;
			//   while ( v9 < data.fields.downsampleNum )
			//   {
			//     mipsDownSize = data.fields.mipsDownSize;
			//     if ( !mipsDownSize )
			//       goto LABEL_15;
			//     sub_180056B60(mipsDownSize, &size, v9);
			//     HG::Rendering::RenderGraphModule::TextureDesc::TextureDesc((TextureDesc *)&v22[1], size, 0LL);
			//     v22[8] = (MethodInfo *)"FrostedGlassMipDown";
			//     LODWORD(v22[3]) = 74;
			//     LOBYTE(v22[5]) = useComputeShader;
			//     sub_1800054D0((HGRenderPathScene *)&v22[8], v10, v11, v12, v21, v22[0]);
			//     LODWORD(v22[4]) = 1;
			//     HIDWORD(v22[3]) = 1;
			//     v26 = *(TextureDesc *)&v22[1];
			//     if ( !useComputeShader )
			//     {
			//       mipsDown = data.fields.mipsDown;
			//       if ( !renderGraph )
			//         goto LABEL_15;
			//       v14 = HG::Rendering::RenderGraphModule::HGRenderGraph::CreateTexture(&v27, renderGraph, &v26, 0LL);
			//       if ( !mipsDown )
			//         goto LABEL_15;
			//       v25 = *v14;
			//       sub_1803EF6F4(mipsDown, v9, &v25);
			//       mipsDownSize = data.fields.mipsDownSize;
			//       if ( !mipsDownSize )
			//         goto LABEL_15;
			//       sub_180056B60(mipsDownSize, &v24, v9);
			//       HG::Rendering::RenderGraphModule::TextureDesc::TextureDesc((TextureDesc *)&v22[1], v24, 0LL);
			//       v22[8] = (MethodInfo *)"FrostedGlassMipDownTemp";
			//       LODWORD(v22[3]) = 74;
			//       LOBYTE(v22[5]) = 0;
			//       sub_1800054D0((HGRenderPathScene *)&v22[8], v15, v16, v17, v21, v22[0]);
			//       mipsDownTemp = data.fields.mipsDownTemp;
			//       LODWORD(v22[4]) = 1;
			//       HIDWORD(v22[3]) = 1;
			//       v26 = *(TextureDesc *)&v22[1];
			//       v19 = HG::Rendering::RenderGraphModule::HGRenderGraph::CreateTexture(&v28, renderGraph, &v26, 0LL);
			//       if ( !mipsDownTemp )
			//         goto LABEL_15;
			//       v25 = *v19;
			//       sub_1803EF6F4(mipsDownTemp, v9, &v25);
			//     }
			//     ++v9;
			//   }
			// }
			// 
		}

		private static TextureHandle FrostedGlassAddPass(UberPostPassUtils.FrostedGlassData data, HGRenderGraph renderGraph, bool useComputeShader)
		{
			// // TextureHandle FrostedGlassAddPass(UberPostPassUtils+FrostedGlassData, HGRenderGraph, Boolean)
			// // Hidden C++ exception states: #wind=1 #try_helpers=1
			// TextureHandle *HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassAddPass(
			//         TextureHandle *__return_ptr retstr,
			//         UberPostPassUtils_FrostedGlassData *data,
			//         HGRenderGraph *renderGraph,
			//         bool useComputeShader,
			//         MethodInfo *method)
			// {
			//   ProfilingSampler *v9; // rax
			//   __int64 v10; // rdx
			//   __int64 v11; // rcx
			//   unsigned __int64 v12; // rdx
			//   __int64 v13; // rcx
			//   __int64 downsampleNum; // rcx
			//   __int64 v15; // rax
			//   signed __int64 userLut; // rcx
			//   unsigned __int64 v17; // r8
			//   signed __int64 v18; // rtt
			//   __m128i v19; // xmm0
			//   __m128i v20; // xmm0
			//   Vector2Int__Array *mipsDownSize; // r8
			//   __int64 v22; // rdx
			//   __int64 v23; // rcx
			//   TextureHandle__Array *mipsDown; // r8
			//   __int64 v25; // rdx
			//   __int64 v26; // rcx
			//   Material__Array *frostedGlass1stPassMat; // r8
			//   __int64 v28; // rdx
			//   __int64 v29; // rcx
			//   Material__Array *frostedGlass2ndPassMat; // r8
			//   __int64 v31; // rdx
			//   __int64 v32; // rcx
			//   TextureHandle__Array *mipsDownTemp; // r8
			//   __int64 v34; // rdx
			//   __int64 v35; // rcx
			//   int j; // esi
			//   __int64 v37; // rcx
			//   TextureHandle *v38; // rax
			//   __int64 v39; // rdx
			//   __int64 v40; // rcx
			//   __int64 v41; // rcx
			//   TextureHandle *v42; // rax
			//   __int64 v43; // rdx
			//   __int64 v44; // r9
			//   __int64 v45; // rdx
			//   unsigned int v46; // edx
			//   unsigned __int64 v47; // r8
			//   signed __int64 v48; // rtt
			//   __int64 kernelKMain; // rcx
			//   __int64 kernelKMainWithLut; // rcx
			//   int i; // esi
			//   __int64 v52; // rcx
			//   TextureHandle *v53; // rax
			//   TextureHandle__Array *v54; // rcx
			//   TextureHandle v55; // xmm0
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   __int64 v57; // rdx
			//   __int64 v58; // rcx
			//   TextureHandle *result; // rax
			//   __int64 v60; // [rsp+40h] [rbp-88h] BYREF
			//   TextureHandle v61; // [rsp+48h] [rbp-80h] BYREF
			//   HGRenderGraphBuilder v62; // [rsp+58h] [rbp-70h] BYREF
			//   TextureHandle v63[2]; // [rsp+78h] [rbp-50h] BYREF
			//   TextureHandle v64; // [rsp+98h] [rbp-30h] BYREF
			// 
			//   if ( !byte_18D9194D3 )
			//   {
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassPassData>);
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassPassData>);
			//     sub_18003C530(&MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     sub_18003C530(&off_18D4F3890);
			//     byte_18D9194D3 = 1;
			//   }
			//   memset(&v62, 0, sizeof(v62));
			//   v60 = 0LL;
			//   if ( IFix::WrappersManagerImpl::IsPatched(2443, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2443, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v58, v57);
			//     v55 = *IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_893(
			//              v63,
			//              Patch,
			//              (Object *)data,
			//              (Object *)renderGraph,
			//              useComputeShader,
			//              0LL);
			//   }
			//   else
			//   {
			//     v9 = UnityEngine::Rendering::ProfilingSampler::Get<System::Int32Enum>(
			//            (Int32Enum__Enum)0x98u,
			//            MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//     if ( !renderGraph )
			//       sub_180B536AC(v11, v10);
			//     v62 = *(HGRenderGraphBuilder *)sub_1808303FC(
			//                                      (unsigned int)v63,
			//                                      (_DWORD)renderGraph,
			//                                      (unsigned int)"Frosted Glass",
			//                                      (unsigned int)&v60,
			//                                      (__int64)v9);
			//     v61.handle = 0LL;
			//     v61.fallBackResource = (ResourceHandle)&v62;
			//     if ( !data )
			//       sub_1802DC2C8(v13, v12);
			//     downsampleNum = (unsigned int)data.fields.downsampleNum;
			//     if ( !v60 )
			//       sub_1802DC2C8(downsampleNum, v12);
			//     *(_DWORD *)(v60 + 56) = downsampleNum;
			//     if ( !v60 )
			//       sub_1802DC2C8(downsampleNum, v12);
			//     *(TextureHandle *)(v60 + 72) = data.fields.source;
			//     LOBYTE(downsampleNum) = data.fields.enableUserLut;
			//     if ( !v60 )
			//       sub_1802DC2C8(downsampleNum, v12);
			//     *(_BYTE *)(v60 + 104) = downsampleNum;
			//     if ( !v60 )
			//       sub_1802DC2C8(downsampleNum, v12);
			//     *(float *)(v60 + 108) = data.fields.colorThreshold;
			//     v15 = v60;
			//     userLut = (signed __int64)data.fields.userLut;
			//     if ( !v60 )
			//       sub_1802DC2C8(userLut, v12);
			//     *(_QWORD *)(v60 + 112) = userLut;
			//     if ( dword_18D8E43F8 )
			//     {
			//       v17 = (((unsigned __int64)(v15 + 112) >> 12) & 0x1FFFFF) >> 6;
			//       v12 = ((unsigned __int64)(v15 + 112) >> 12) & 0x3F;
			//       _m_prefetchw(&qword_18D6405E0[v17 + 36190]);
			//       do
			//       {
			//         userLut = qword_18D6405E0[v17 + 36190] | (1LL << v12);
			//         v18 = qword_18D6405E0[v17 + 36190];
			//       }
			//       while ( v18 != _InterlockedCompareExchange64(&qword_18D6405E0[v17 + 36190], userLut, v18) );
			//     }
			//     v19 = _mm_loadu_si128((const __m128i *)&data.fields.userLogLutSettings);
			//     if ( !v60 )
			//       sub_1802DC2C8(userLut, v12);
			//     *(__m128i *)(v60 + 120) = v19;
			//     if ( !v60 )
			//       sub_1802DC2C8(userLut, v12);
			//     *(TextureHandle *)(v60 + 136) = data.fields.logLut;
			//     v20 = _mm_loadu_si128((const __m128i *)&data.fields.logLutSettings);
			//     if ( !v60 )
			//       sub_1802DC2C8(userLut, v12);
			//     *(__m128i *)(v60 + 152) = v20;
			//     if ( !v60 )
			//       sub_1802DC2C8(userLut, v12);
			//     *(_BYTE *)(v60 + 32) = 1;
			//     if ( !v60 )
			//       sub_1802DC2C8(userLut, v12);
			//     mipsDownSize = data.fields.mipsDownSize;
			//     if ( !mipsDownSize )
			//       sub_1802DC2C8(userLut, v12);
			//     System::Array::Copy((Array *)data.fields.mipsDownSize, *(Array **)(v60 + 64), mipsDownSize.max_length.size, 0LL);
			//     if ( !v60 )
			//       sub_1802DC2C8(v23, v22);
			//     mipsDown = data.fields.mipsDown;
			//     if ( !mipsDown )
			//       sub_1802DC2C8(v23, v22);
			//     System::Array::Copy((Array *)data.fields.mipsDown, *(Array **)(v60 + 88), mipsDown.max_length.size, 0LL);
			//     if ( useComputeShader )
			//     {
			//       v45 = v60;
			//       if ( !v60 )
			//         sub_1802DC2C8(v26, 0LL);
			//       *(_QWORD *)(v60 + 16) = data.fields.frostedGlassBlurCS;
			//       if ( dword_18D8E43F8 )
			//       {
			//         v46 = ((unsigned __int64)(v45 + 16) >> 12) & 0x1FFFFF;
			//         v47 = (unsigned __int64)v46 >> 6;
			//         v45 = v46 & 0x3F;
			//         _m_prefetchw(&qword_18D6405E0[v47 + 36190]);
			//         do
			//           v48 = qword_18D6405E0[v47 + 36190];
			//         while ( v48 != _InterlockedCompareExchange64(&qword_18D6405E0[v47 + 36190], v48 | (1LL << v45), v48) );
			//       }
			//       kernelKMain = (unsigned int)data.fields.kernelKMain;
			//       if ( !v60 )
			//         sub_1802DC2C8(kernelKMain, v45);
			//       *(_DWORD *)(v60 + 24) = kernelKMain;
			//       kernelKMainWithLut = (unsigned int)data.fields.kernelKMainWithLut;
			//       if ( !v60 )
			//         sub_1802DC2C8(kernelKMainWithLut, v45);
			//       *(_DWORD *)(v60 + 28) = kernelKMainWithLut;
			//       for ( i = 0; i < 3; ++i )
			//       {
			//         if ( !v60 )
			//           sub_1802DC2C8(kernelKMainWithLut, v45);
			//         v52 = *(_QWORD *)(v60 + 88);
			//         if ( !v52 )
			//           sub_1802DC2C8(0LL, v45);
			//         v53 = (TextureHandle *)sub_18037A2E0(v52, i);
			//         HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::WriteTexture(v63, &v62, v53, 0LL);
			//       }
			//       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadTexture(v63, &v62, &data.fields.source, 0LL);
			//       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadTexture(v63, &v62, &data.fields.logLut, 0LL);
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<System::Object>(
			//         &v62,
			//         (RenderFunc_1_System_Object_ *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_frostedGlassCSRenderFunc,
			//         0LL,
			//         0,
			//         MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassPassData>);
			//       sub_180222690(&v61);
			//     }
			//     else
			//     {
			//       if ( !v60 )
			//         sub_1802DC2C8(v26, v25);
			//       frostedGlass1stPassMat = data.fields.frostedGlass1stPassMat;
			//       if ( !frostedGlass1stPassMat )
			//         sub_1802DC2C8(v26, v25);
			//       System::Array::Copy(
			//         (Array *)data.fields.frostedGlass1stPassMat,
			//         *(Array **)(v60 + 40),
			//         frostedGlass1stPassMat.max_length.size,
			//         0LL);
			//       if ( !v60 )
			//         sub_1802DC2C8(v29, v28);
			//       frostedGlass2ndPassMat = data.fields.frostedGlass2ndPassMat;
			//       if ( !frostedGlass2ndPassMat )
			//         sub_1802DC2C8(v29, v28);
			//       System::Array::Copy(
			//         (Array *)data.fields.frostedGlass2ndPassMat,
			//         *(Array **)(v60 + 48),
			//         frostedGlass2ndPassMat.max_length.size,
			//         0LL);
			//       if ( !v60 )
			//         sub_1802DC2C8(v32, v31);
			//       mipsDownTemp = data.fields.mipsDownTemp;
			//       if ( !mipsDownTemp )
			//         sub_1802DC2C8(v32, v31);
			//       System::Array::Copy((Array *)data.fields.mipsDownTemp, *(Array **)(v60 + 96), mipsDownTemp.max_length.size, 0LL);
			//       for ( j = 0; j < 3; ++j )
			//       {
			//         if ( !v60 )
			//           sub_1802DC2C8(v35, v34);
			//         v37 = *(_QWORD *)(v60 + 88);
			//         if ( !v37 )
			//           sub_1802DC2C8(0LL, v34);
			//         v38 = (TextureHandle *)sub_18037A2E0(v37, j);
			//         HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadWriteTexture(&v64, &v62, v38, 0LL);
			//         if ( !v60 )
			//           sub_1802DC2C8(v40, v39);
			//         v41 = *(_QWORD *)(v60 + 96);
			//         if ( !v41 )
			//           sub_1802DC2C8(0LL, v39);
			//         v42 = (TextureHandle *)sub_18037A2E0(v41, j);
			//         HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadWriteTexture(v63, &v62, v42, 0LL);
			//       }
			//       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadTexture(v63, &v62, &data.fields.source, 0LL);
			//       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadTexture(v63, &v62, &data.fields.logLut, 0LL);
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<System::Object>(
			//         &v62,
			//         (RenderFunc_1_System_Object_ *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_frostedGlassPSRenderFunc,
			//         0LL,
			//         0,
			//         MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassPassData>);
			//       sub_180222690(&v61);
			//     }
			//     v54 = data.fields.mipsDown;
			//     if ( !v54 )
			//       sub_1802DC2C8(0LL, v43);
			//     sub_18037A36C(v54, &v61, data.fields.downsampleNum - 1, v44);
			//     v55 = v61;
			//   }
			//   result = retstr;
			//   *retstr = v55;
			//   return result;
			// }
			// 
			return null;
		}

		private static void FrostedGlassAddSkipPass(bool isSceneViewCam, bool enablePermanentSceneFrostedGlass, HGRenderGraph renderGraph, ref RTHandle[] sceneFrostedGlassRTs)
		{
			// // Void FrostedGlassAddSkipPass(Boolean, Boolean, HGRenderGraph, RTHandle[] ByRef)
			// // Hidden C++ exception states: #wind=1
			// void HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassAddSkipPass(
			//         bool isSceneViewCam,
			//         bool enablePermanentSceneFrostedGlass,
			//         HGRenderGraph *renderGraph,
			//         RTHandle__Array **sceneFrostedGlassRTs,
			//         MethodInfo *method)
			// {
			//   ProfilingSampler *v9; // rax
			//   __int64 v10; // rdx
			//   __int64 v11; // rcx
			//   __int64 v12; // rdx
			//   __int64 v13; // r8
			//   __int64 v14; // r9
			//   Object *v15; // rbx
			//   RTHandle__Array *v16; // rcx
			//   Texture *v17; // rax
			//   unsigned __int64 v18; // rdx
			//   __int64 v19; // rcx
			//   unsigned __int64 v20; // r8
			//   __int64 v21; // r9
			//   signed __int64 v22; // rtt
			//   Object *v23; // rsi
			//   RTHandle__Array *v24; // rcx
			//   Texture *v25; // rax
			//   unsigned __int64 v26; // rdx
			//   __int64 v27; // rcx
			//   unsigned __int64 v28; // r8
			//   __int64 v29; // r9
			//   signed __int64 v30; // rtt
			//   Object *v31; // rsi
			//   RTHandle__Array *v32; // rcx
			//   Texture *v33; // rax
			//   __int64 v34; // rdx
			//   __int64 v35; // rcx
			//   unsigned __int64 v36; // r8
			//   signed __int64 v37; // rtt
			//   Texture2D *grayTexture; // rax
			//   __int64 v39; // rdx
			//   __int64 v40; // rcx
			//   unsigned __int64 v41; // r8
			//   signed __int64 v42; // rtt
			//   Object *v43; // rsi
			//   Texture2D *v44; // rax
			//   __int64 v45; // rdx
			//   __int64 v46; // rcx
			//   unsigned __int64 v47; // r8
			//   signed __int64 v48; // rtt
			//   Object *v49; // rsi
			//   Texture2D *v50; // rax
			//   __int64 v51; // rdx
			//   __int64 v52; // rcx
			//   unsigned __int64 v53; // r8
			//   signed __int64 v54; // rtt
			//   Texture2D *blackTexture; // rax
			//   __int64 v56; // rdx
			//   __int64 v57; // rcx
			//   unsigned __int64 v58; // r8
			//   signed __int64 v59; // rtt
			//   Object *v60; // rsi
			//   Texture2D *v61; // rax
			//   __int64 v62; // rdx
			//   __int64 v63; // rcx
			//   unsigned __int64 v64; // r8
			//   signed __int64 v65; // rtt
			//   Object *v66; // rsi
			//   Texture2D *v67; // rax
			//   __int64 v68; // rdx
			//   __int64 v69; // rcx
			//   unsigned __int64 v70; // r8
			//   signed __int64 v71; // rtt
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   __int64 v73; // rdx
			//   __int64 v74; // rcx
			//   Object *v75; // [rsp+40h] [rbp-78h] BYREF
			//   Il2CppExceptionWrapper *v76; // [rsp+50h] [rbp-68h] BYREF
			//   HGRenderGraphBuilder v77; // [rsp+58h] [rbp-60h] BYREF
			//   HGRenderGraphBuilder v78; // [rsp+78h] [rbp-40h] BYREF
			// 
			//   if ( !byte_18D9194D4 )
			//   {
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassSkipPassData>);
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassSkipPassData>);
			//     sub_18003C530(&MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     sub_18003C530(&off_18D4F3890);
			//     byte_18D9194D4 = 1;
			//   }
			//   v75 = 0LL;
			//   if ( IFix::WrappersManagerImpl::IsPatched(2444, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2444, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v74, v73);
			//     IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_894(
			//       Patch,
			//       isSceneViewCam,
			//       enablePermanentSceneFrostedGlass,
			//       (Object *)renderGraph,
			//       sceneFrostedGlassRTs,
			//       0LL);
			//   }
			//   else
			//   {
			//     v9 = UnityEngine::Rendering::ProfilingSampler::Get<System::Int32Enum>(
			//            (Int32Enum__Enum)0x98u,
			//            MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//     if ( !renderGraph )
			//       sub_180B536AC(v11, v10);
			//     HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<System::Object>(
			//       &v77,
			//       renderGraph,
			//       (String *)"Frosted Glass",
			//       &v75,
			//       v9,
			//       1,
			//       ProfilingHGPass__Enum_None,
			//       MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassSkipPassData>);
			//     v78 = v77;
			//     v77.m_RenderPass = 0LL;
			//     v77.m_Resources = (HGRenderGraphResourceRegistry *)&v78;
			//     try
			//     {
			//       v15 = v75;
			//       if ( isSceneViewCam )
			//       {
			//         blackTexture = UnityEngine::Texture2D::get_blackTexture(0LL);
			//         if ( !v15 )
			//           sub_1802DC2C8(v57, v56);
			//         v15[1].klass = (Object__Class *)blackTexture;
			//         if ( dword_18D8E43F8 )
			//         {
			//           v58 = (((unsigned __int64)&v15[1] >> 12) & 0x1FFFFF) >> 6;
			//           _m_prefetchw(&qword_18D6405E0[v58 + 36190]);
			//           do
			//             v59 = qword_18D6405E0[v58 + 36190];
			//           while ( v59 != _InterlockedCompareExchange64(
			//                            &qword_18D6405E0[v58 + 36190],
			//                            v59 | (1LL << (((unsigned __int64)&v15[1] >> 12) & 0x3F)),
			//                            v59) );
			//         }
			//         v60 = v75;
			//         v61 = UnityEngine::Texture2D::get_blackTexture(0LL);
			//         if ( !v60 )
			//           sub_1802DC2C8(v63, v62);
			//         v60[1].monitor = (MonitorData *)v61;
			//         if ( dword_18D8E43F8 )
			//         {
			//           v64 = (((unsigned __int64)&v60[1].monitor >> 12) & 0x1FFFFF) >> 6;
			//           _m_prefetchw(&qword_18D6405E0[v64 + 36190]);
			//           do
			//             v65 = qword_18D6405E0[v64 + 36190];
			//           while ( v65 != _InterlockedCompareExchange64(
			//                            &qword_18D6405E0[v64 + 36190],
			//                            v65 | (1LL << (((unsigned __int64)&v60[1].monitor >> 12) & 0x3F)),
			//                            v65) );
			//         }
			//         v66 = v75;
			//         v67 = UnityEngine::Texture2D::get_blackTexture(0LL);
			//         if ( !v66 )
			//           sub_1802DC2C8(v69, v68);
			//         v66[2].klass = (Object__Class *)v67;
			//         if ( dword_18D8E43F8 )
			//         {
			//           v70 = (((unsigned __int64)&v66[2] >> 12) & 0x1FFFFF) >> 6;
			//           _m_prefetchw(&qword_18D6405E0[v70 + 36190]);
			//           do
			//             v71 = qword_18D6405E0[v70 + 36190];
			//           while ( v71 != _InterlockedCompareExchange64(
			//                            &qword_18D6405E0[v70 + 36190],
			//                            v71 | (1LL << (((unsigned __int64)&v66[2] >> 12) & 0x3F)),
			//                            v71) );
			//         }
			//       }
			//       else if ( enablePermanentSceneFrostedGlass )
			//       {
			//         v16 = *sceneFrostedGlassRTs;
			//         if ( !*sceneFrostedGlassRTs )
			//           sub_1802DC2C8(0LL, v12);
			//         if ( !v16.max_length.size )
			//           sub_180070260(v16, v12, v13, v14);
			//         v17 = UnityEngine::Rendering::RTHandle::op_Implicit(v16.vector[0], 0LL);
			//         if ( !v15 )
			//           sub_1802DC2C8(v19, v18);
			//         v15[2].klass = (Object__Class *)v17;
			//         if ( dword_18D8E43F8 )
			//         {
			//           v20 = (((unsigned __int64)&v15[2] >> 12) & 0x1FFFFF) >> 6;
			//           v18 = ((unsigned __int64)&v15[2] >> 12) & 0x3F;
			//           _m_prefetchw(&qword_18D6405E0[v20 + 36190]);
			//           do
			//             v22 = qword_18D6405E0[v20 + 36190];
			//           while ( v22 != _InterlockedCompareExchange64(&qword_18D6405E0[v20 + 36190], v22 | (1LL << v18), v22) );
			//         }
			//         v23 = v75;
			//         v24 = *sceneFrostedGlassRTs;
			//         if ( !*sceneFrostedGlassRTs )
			//           sub_1802DC2C8(0LL, v18);
			//         if ( v24.max_length.size <= 1u )
			//           sub_180070260(v24, v18, v20, v21);
			//         v25 = UnityEngine::Rendering::RTHandle::op_Implicit(v24.vector[1], 0LL);
			//         if ( !v23 )
			//           sub_1802DC2C8(v27, v26);
			//         v23[1].monitor = (MonitorData *)v25;
			//         if ( dword_18D8E43F8 )
			//         {
			//           v28 = (((unsigned __int64)&v23[1].monitor >> 12) & 0x1FFFFF) >> 6;
			//           v26 = ((unsigned __int64)&v23[1].monitor >> 12) & 0x3F;
			//           _m_prefetchw(&qword_18D6405E0[v28 + 36190]);
			//           do
			//             v30 = qword_18D6405E0[v28 + 36190];
			//           while ( v30 != _InterlockedCompareExchange64(&qword_18D6405E0[v28 + 36190], v30 | (1LL << v26), v30) );
			//         }
			//         v31 = v75;
			//         v32 = *sceneFrostedGlassRTs;
			//         if ( !*sceneFrostedGlassRTs )
			//           sub_1802DC2C8(0LL, v26);
			//         if ( v32.max_length.size <= 2u )
			//           sub_180070260(v32, v26, v28, v29);
			//         v33 = UnityEngine::Rendering::RTHandle::op_Implicit(v32.vector[2], 0LL);
			//         if ( !v31 )
			//           sub_1802DC2C8(v35, v34);
			//         v31[1].klass = (Object__Class *)v33;
			//         if ( dword_18D8E43F8 )
			//         {
			//           v36 = (((unsigned __int64)&v31[1] >> 12) & 0x1FFFFF) >> 6;
			//           _m_prefetchw(&qword_18D6405E0[v36 + 36190]);
			//           do
			//             v37 = qword_18D6405E0[v36 + 36190];
			//           while ( v37 != _InterlockedCompareExchange64(
			//                            &qword_18D6405E0[v36 + 36190],
			//                            v37 | (1LL << (((unsigned __int64)&v31[1] >> 12) & 0x3F)),
			//                            v37) );
			//         }
			//       }
			//       else
			//       {
			//         grayTexture = UnityEngine::Texture2D::get_grayTexture(0LL);
			//         if ( !v15 )
			//           sub_1802DC2C8(v40, v39);
			//         v15[1].klass = (Object__Class *)grayTexture;
			//         if ( dword_18D8E43F8 )
			//         {
			//           v41 = (((unsigned __int64)&v15[1] >> 12) & 0x1FFFFF) >> 6;
			//           _m_prefetchw(&qword_18D6405E0[v41 + 36190]);
			//           do
			//             v42 = qword_18D6405E0[v41 + 36190];
			//           while ( v42 != _InterlockedCompareExchange64(
			//                            &qword_18D6405E0[v41 + 36190],
			//                            v42 | (1LL << (((unsigned __int64)&v15[1] >> 12) & 0x3F)),
			//                            v42) );
			//         }
			//         v43 = v75;
			//         v44 = UnityEngine::Texture2D::get_grayTexture(0LL);
			//         if ( !v43 )
			//           sub_1802DC2C8(v46, v45);
			//         v43[1].monitor = (MonitorData *)v44;
			//         if ( dword_18D8E43F8 )
			//         {
			//           v47 = (((unsigned __int64)&v43[1].monitor >> 12) & 0x1FFFFF) >> 6;
			//           _m_prefetchw(&qword_18D6405E0[v47 + 36190]);
			//           do
			//             v48 = qword_18D6405E0[v47 + 36190];
			//           while ( v48 != _InterlockedCompareExchange64(
			//                            &qword_18D6405E0[v47 + 36190],
			//                            v48 | (1LL << (((unsigned __int64)&v43[1].monitor >> 12) & 0x3F)),
			//                            v48) );
			//         }
			//         v49 = v75;
			//         v50 = UnityEngine::Texture2D::get_grayTexture(0LL);
			//         if ( !v49 )
			//           sub_1802DC2C8(v52, v51);
			//         v49[2].klass = (Object__Class *)v50;
			//         if ( dword_18D8E43F8 )
			//         {
			//           v53 = (((unsigned __int64)&v49[2] >> 12) & 0x1FFFFF) >> 6;
			//           _m_prefetchw(&qword_18D6405E0[v53 + 36190]);
			//           do
			//             v54 = qword_18D6405E0[v53 + 36190];
			//           while ( v54 != _InterlockedCompareExchange64(
			//                            &qword_18D6405E0[v53 + 36190],
			//                            v54 | (1LL << (((unsigned __int64)&v49[2] >> 12) & 0x3F)),
			//                            v54) );
			//         }
			//       }
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<System::Object>(
			//         &v78,
			//         (RenderFunc_1_System_Object_ *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_frostedGlassSkipRenderFunc,
			//         0LL,
			//         0,
			//         MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassSkipPassData>);
			//     }
			//     catch ( Il2CppExceptionWrapper *v76 )
			//     {
			//       v77.m_RenderPass = (HGRenderGraphPass *)v76.ex;
			//     }
			//     sub_180222690(&v77);
			//   }
			// }
			// 
		}

		private static void UIFrostedGlassAddPass(UberPostPassUtils.FrostedGlassData data, HGRenderGraph renderGraph, bool useComputeShader, HGCamera camera)
		{
			// // Void UIFrostedGlassAddPass(UberPostPassUtils+FrostedGlassData, HGRenderGraph, Boolean, HGCamera)
			// // Hidden C++ exception states: #wind=1 #try_helpers=1
			// void HG::Rendering::Runtime::UberPostPassUtils::UIFrostedGlassAddPass(
			//         UberPostPassUtils_FrostedGlassData *data,
			//         HGRenderGraph *renderGraph,
			//         bool useComputeShader,
			//         HGCamera *camera,
			//         MethodInfo *method)
			// {
			//   ProfilingSampler *v9; // rax
			//   __int64 v10; // rdx
			//   __int64 v11; // rcx
			//   __int64 v12; // rdx
			//   __int64 v13; // rcx
			//   __int64 downsampleNum; // rcx
			//   Vector2Int__Array *mipsDownSize; // r8
			//   __int64 v16; // rdx
			//   __int64 v17; // rcx
			//   TextureHandle__Array *mipsDown; // r8
			//   __int64 v19; // rdx
			//   __int64 v20; // rcx
			//   Material__Array *frostedGlass1stPassMat; // r8
			//   __int64 v22; // rdx
			//   __int64 v23; // rcx
			//   Material__Array *frostedGlass2ndPassMat; // r8
			//   __int64 v25; // rdx
			//   __int64 v26; // rcx
			//   TextureHandle__Array *mipsDownTemp; // r8
			//   __int64 v28; // rdx
			//   __int64 v29; // rcx
			//   int j; // ebx
			//   __int64 v31; // rcx
			//   TextureHandle *v32; // rax
			//   __int64 v33; // rdx
			//   __int64 v34; // rcx
			//   __int64 v35; // rcx
			//   TextureHandle *v36; // rax
			//   __int64 v37; // rdx
			//   unsigned int v38; // edx
			//   unsigned __int64 v39; // r8
			//   signed __int64 v40; // rtt
			//   __int64 kernelKMain; // rcx
			//   __int64 kernelKMainWithLut; // rcx
			//   int i; // edi
			//   __int64 v44; // rcx
			//   unsigned __int64 v45; // rdx
			//   signed __int64 v46; // rcx
			//   __int128 v47; // xmm2
			//   __int128 v48; // xmm3
			//   Color clearColor; // xmm4
			//   unsigned __int64 v50; // r8
			//   signed __int64 v51; // rtt
			//   __int64 v52; // r14
			//   TextureHandle *v53; // rax
			//   __int64 v54; // rdx
			//   __int64 v55; // rcx
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   __int64 v57; // rdx
			//   __int64 v58; // rcx
			//   __int64 v59; // [rsp+40h] [rbp-1C8h] BYREF
			//   _QWORD v60[2]; // [rsp+48h] [rbp-1C0h] BYREF
			//   HGRenderGraphBuilder v61; // [rsp+58h] [rbp-1B0h] BYREF
			//   TextureHandle size; // [rsp+78h] [rbp-190h] BYREF
			//   TextureHandle v63; // [rsp+90h] [rbp-178h] BYREF
			//   __int128 v64; // [rsp+A0h] [rbp-168h] BYREF
			//   __int128 v65; // [rsp+B0h] [rbp-158h]
			//   __int128 v66; // [rsp+C0h] [rbp-148h]
			//   __int128 v67; // [rsp+D0h] [rbp-138h] BYREF
			//   __int128 v68; // [rsp+E0h] [rbp-128h]
			//   Color v69; // [rsp+F0h] [rbp-118h]
			//   TextureHandle v70[2]; // [rsp+108h] [rbp-100h] BYREF
			//   TextureDesc v71; // [rsp+130h] [rbp-D8h] BYREF
			//   TextureDesc v72; // [rsp+190h] [rbp-78h] BYREF
			// 
			//   if ( !byte_18D9194D5 )
			//   {
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassPassData>);
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassPassData>);
			//     sub_18003C530(&MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     sub_18003C530(&off_18D4F3850);
			//     sub_18003C530(&off_18D4F38B8);
			//     byte_18D9194D5 = 1;
			//   }
			//   memset(&v61, 0, sizeof(v61));
			//   v59 = 0LL;
			//   sub_1802F01E0(&v72, 0LL, 96LL);
			//   sub_1802F01E0(&v64, 0LL, 96LL);
			//   if ( IFix::WrappersManagerImpl::IsPatched(2445, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2445, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v58, v57);
			//     IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_143(
			//       (ILFixDynamicMethodWrapper_7 *)Patch,
			//       (Object *)data,
			//       (Object *)renderGraph,
			//       useComputeShader,
			//       (Object *)camera,
			//       0LL);
			//   }
			//   else
			//   {
			//     v9 = UnityEngine::Rendering::ProfilingSampler::Get<System::Int32Enum>(
			//            (Int32Enum__Enum)0x98u,
			//            MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//     if ( !renderGraph )
			//       sub_180B536AC(v11, v10);
			//     v61 = *(HGRenderGraphBuilder *)sub_1808303FC(
			//                                      (unsigned int)v70,
			//                                      (_DWORD)renderGraph,
			//                                      (unsigned int)"UI Frosted Glass",
			//                                      (unsigned int)&v59,
			//                                      (__int64)v9);
			//     v60[0] = 0LL;
			//     v60[1] = &v61;
			//     if ( !data )
			//       sub_1802DC2C8(v13, v12);
			//     downsampleNum = (unsigned int)data.fields.downsampleNum;
			//     if ( !v59 )
			//       sub_1802DC2C8(downsampleNum, v12);
			//     *(_DWORD *)(v59 + 56) = downsampleNum;
			//     if ( !v59 )
			//       sub_1802DC2C8(downsampleNum, v12);
			//     *(TextureHandle *)(v59 + 72) = data.fields.source;
			//     if ( !v59 )
			//       sub_1802DC2C8(downsampleNum, v12);
			//     *(_BYTE *)(v59 + 32) = 0;
			//     if ( !v59 )
			//       sub_1802DC2C8(downsampleNum, v12);
			//     *(float *)(v59 + 108) = data.fields.colorThreshold;
			//     if ( !v59 )
			//       sub_1802DC2C8(downsampleNum, v12);
			//     mipsDownSize = data.fields.mipsDownSize;
			//     if ( !mipsDownSize )
			//       sub_1802DC2C8(downsampleNum, v12);
			//     System::Array::Copy((Array *)data.fields.mipsDownSize, *(Array **)(v59 + 64), mipsDownSize.max_length.size, 0LL);
			//     if ( !v59 )
			//       sub_1802DC2C8(v17, v16);
			//     mipsDown = data.fields.mipsDown;
			//     if ( !mipsDown )
			//       sub_1802DC2C8(v17, v16);
			//     System::Array::Copy((Array *)data.fields.mipsDown, *(Array **)(v59 + 88), mipsDown.max_length.size, 0LL);
			//     HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::AllowPassCulling(&v61, 0, 0LL);
			//     if ( useComputeShader )
			//     {
			//       v37 = v59;
			//       if ( !v59 )
			//         sub_1802DC2C8(v20, 0LL);
			//       *(_QWORD *)(v59 + 16) = data.fields.frostedGlassBlurCS;
			//       if ( dword_18D8E43F8 )
			//       {
			//         v38 = ((unsigned __int64)(v37 + 16) >> 12) & 0x1FFFFF;
			//         v39 = (unsigned __int64)v38 >> 6;
			//         v37 = v38 & 0x3F;
			//         _m_prefetchw(&qword_18D6405E0[v39 + 36190]);
			//         do
			//           v40 = qword_18D6405E0[v39 + 36190];
			//         while ( v40 != _InterlockedCompareExchange64(&qword_18D6405E0[v39 + 36190], v40 | (1LL << v37), v40) );
			//       }
			//       kernelKMain = (unsigned int)data.fields.kernelKMain;
			//       if ( !v59 )
			//         sub_1802DC2C8(kernelKMain, v37);
			//       *(_DWORD *)(v59 + 24) = kernelKMain;
			//       kernelKMainWithLut = (unsigned int)data.fields.kernelKMainWithLut;
			//       if ( !v59 )
			//         sub_1802DC2C8(kernelKMainWithLut, v37);
			//       *(_DWORD *)(v59 + 28) = kernelKMainWithLut;
			//       for ( i = 0; i < 3; ++i )
			//       {
			//         if ( !v59 )
			//           sub_1802DC2C8(kernelKMainWithLut, v37);
			//         v44 = *(_QWORD *)(v59 + 64);
			//         if ( !v44 )
			//           sub_1802DC2C8(0LL, v37);
			//         sub_180056B60(v44, &size, i);
			//         sub_1802F01E0(&v71, 0LL, 96LL);
			//         HG::Rendering::RenderGraphModule::TextureDesc::TextureDesc(&v71, (Vector2Int)size.handle, 0LL);
			//         v47 = *(_OWORD *)&v71.width;
			//         v64 = *(_OWORD *)&v71.width;
			//         v65 = *(_OWORD *)&v71.colorFormat;
			//         v66 = *(_OWORD *)&v71.enableRandomWrite;
			//         *(_QWORD *)&v67 = *(_QWORD *)&v71.bindTextureMS;
			//         v48 = *(_OWORD *)&v71.fastMemoryDesc.inFastMemory;
			//         v68 = *(_OWORD *)&v71.fastMemoryDesc.inFastMemory;
			//         clearColor = v71.clearColor;
			//         v69 = v71.clearColor;
			//         LODWORD(v65) = 74;
			//         LOBYTE(v66) = useComputeShader;
			//         *((_QWORD *)&v67 + 1) = "FrostedGlassMipDown";
			//         if ( dword_18D8E43F8 )
			//         {
			//           v50 = ((((unsigned __int64)&v67 + 8) >> 12) & 0x1FFFFF) >> 6;
			//           v45 = (((unsigned __int64)&v67 + 8) >> 12) & 0x3F;
			//           _m_prefetchw(&qword_18D6405E0[v50 + 36190]);
			//           do
			//           {
			//             v46 = qword_18D6405E0[v50 + 36190] | (1LL << v45);
			//             v51 = qword_18D6405E0[v50 + 36190];
			//           }
			//           while ( v51 != _InterlockedCompareExchange64(&qword_18D6405E0[v50 + 36190], v46, v51) );
			//           clearColor = v69;
			//           v48 = v68;
			//           v47 = v64;
			//         }
			//         *(_QWORD *)((char *)&v65 + 4) = 0x100000001LL;
			//         *(_OWORD *)&v72.width = v47;
			//         *(_OWORD *)&v72.colorFormat = v65;
			//         *(_OWORD *)&v72.enableRandomWrite = v66;
			//         *(_OWORD *)&v72.bindTextureMS = v67;
			//         *(_OWORD *)&v72.fastMemoryDesc.inFastMemory = v48;
			//         v72.clearColor = clearColor;
			//         if ( !v59 )
			//           sub_1802DC2C8(v46, v45);
			//         v52 = *(_QWORD *)(v59 + 88);
			//         v53 = HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::CreateTransientTexture(v70, &v61, &v72, 0LL);
			//         if ( !v52 )
			//           sub_1802DC2C8(v55, v54);
			//         v63 = *v53;
			//         sub_1803EF6F4(v52, i, &v63);
			//       }
			//       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadTexture(v70, &v61, &data.fields.source, 0LL);
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<System::Object>(
			//         &v61,
			//         (RenderFunc_1_System_Object_ *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_frostedGlassCSRenderFunc,
			//         (Object *)camera,
			//         0,
			//         MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassPassData>);
			//       sub_180222690(v60);
			//     }
			//     else
			//     {
			//       if ( !v59 )
			//         sub_1802DC2C8(v20, v19);
			//       frostedGlass1stPassMat = data.fields.frostedGlass1stPassMat;
			//       if ( !frostedGlass1stPassMat )
			//         sub_1802DC2C8(v20, v19);
			//       System::Array::Copy(
			//         (Array *)data.fields.frostedGlass1stPassMat,
			//         *(Array **)(v59 + 40),
			//         frostedGlass1stPassMat.max_length.size,
			//         0LL);
			//       if ( !v59 )
			//         sub_1802DC2C8(v23, v22);
			//       frostedGlass2ndPassMat = data.fields.frostedGlass2ndPassMat;
			//       if ( !frostedGlass2ndPassMat )
			//         sub_1802DC2C8(v23, v22);
			//       System::Array::Copy(
			//         (Array *)data.fields.frostedGlass2ndPassMat,
			//         *(Array **)(v59 + 48),
			//         frostedGlass2ndPassMat.max_length.size,
			//         0LL);
			//       if ( !v59 )
			//         sub_1802DC2C8(v26, v25);
			//       mipsDownTemp = data.fields.mipsDownTemp;
			//       if ( !mipsDownTemp )
			//         sub_1802DC2C8(v26, v25);
			//       System::Array::Copy((Array *)data.fields.mipsDownTemp, *(Array **)(v59 + 96), mipsDownTemp.max_length.size, 0LL);
			//       for ( j = 0; j < 3; ++j )
			//       {
			//         if ( !v59 )
			//           sub_1802DC2C8(v29, v28);
			//         v31 = *(_QWORD *)(v59 + 88);
			//         if ( !v31 )
			//           sub_1802DC2C8(0LL, v28);
			//         v32 = (TextureHandle *)sub_18037A2E0(v31, j);
			//         HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadWriteTexture(&v63, &v61, v32, 0LL);
			//         if ( !v59 )
			//           sub_1802DC2C8(v34, v33);
			//         v35 = *(_QWORD *)(v59 + 96);
			//         if ( !v35 )
			//           sub_1802DC2C8(0LL, v33);
			//         v36 = (TextureHandle *)sub_18037A2E0(v35, j);
			//         HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadWriteTexture(&size, &v61, v36, 0LL);
			//       }
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<System::Object>(
			//         &v61,
			//         (RenderFunc_1_System_Object_ *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_frostedGlassPSRenderFunc,
			//         (Object *)camera,
			//         0,
			//         MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassPassData>);
			//       sub_180222690(v60);
			//     }
			//   }
			// }
			// 
		}

		internal static TextureHandle FrostedGlassPass(HGRenderGraph renderGraph, HGCamera hgCamera, HGSettingParameters settingParameters, TextureHandle source, TextureHandle logLut, int lutSize, ref UberPostPassUtils.FrostedGlassPSMaterials frostedGlassPSMaterials, ref RTHandle[] sceneFrostedGlassRTs, ref Vector2Int[] sceneFrostedGlassRTSizes)
		{
			// // TextureHandle FrostedGlassPass(HGRenderGraph, HGCamera, HGSettingParameters, TextureHandle, TextureHandle, Int32, UberPostPassUtils+FrostedGlassPSMaterials ByRef, RTHandle[] ByRef, Vector2Int[] ByRef)
			// TextureHandle *HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassPass(
			//         TextureHandle *__return_ptr retstr,
			//         HGRenderGraph *renderGraph,
			//         HGCamera *hgCamera,
			//         HGSettingParameters *settingParameters,
			//         TextureHandle *source,
			//         TextureHandle *logLut,
			//         int32_t lutSize,
			//         UberPostPassUtils_FrostedGlassPSMaterials *frostedGlassPSMaterials,
			//         RTHandle__Array **sceneFrostedGlassRTs,
			//         Vector2Int__Array **sceneFrostedGlassRTSizes,
			//         MethodInfo *method)
			// {
			//   ILFixDynamicMethodWrapper_2 *Patch; // rdx
			//   Camera *camera; // rcx
			//   HGRenderGraphDefaultResources *m_DefaultResources; // rax
			//   TextureHandle blackTexture_k__BackingField; // xmm6
			//   CameraType__Enum cameraType; // r12d
			//   bool enableUpdatingSceneFrostedGlass; // bl
			//   int v21; // edx
			//   int v22; // eax
			//   bool v23; // bl
			//   TextureHandle v25; // [rsp+68h] [rbp-39h] BYREF
			//   TextureHandle v26; // [rsp+78h] [rbp-29h] BYREF
			//   TextureHandle v27; // [rsp+88h] [rbp-19h] BYREF
			// 
			//   if ( !byte_18D9194D6 )
			//   {
			//     sub_18003C530(&MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     byte_18D9194D6 = 1;
			//   }
			//   if ( IFix::WrappersManagerImpl::IsPatched(2446, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2446, 0LL);
			//     if ( Patch )
			//     {
			//       v25 = *logLut;
			//       v26 = *source;
			//       *retstr = *IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_896(
			//                    &v27,
			//                    Patch,
			//                    (Object *)renderGraph,
			//                    (Object *)hgCamera,
			//                    (Object *)settingParameters,
			//                    &v26,
			//                    &v25,
			//                    lutSize,
			//                    frostedGlassPSMaterials,
			//                    sceneFrostedGlassRTs,
			//                    sceneFrostedGlassRTSizes,
			//                    0LL);
			//       return retstr;
			//     }
			// LABEL_18:
			//     sub_180B536AC(camera, Patch);
			//   }
			//   if ( !renderGraph )
			//     goto LABEL_18;
			//   m_DefaultResources = renderGraph.fields.m_DefaultResources;
			//   if ( !m_DefaultResources )
			//     goto LABEL_18;
			//   blackTexture_k__BackingField = m_DefaultResources.fields._blackTexture_k__BackingField;
			//   if ( !hgCamera )
			//     goto LABEL_18;
			//   camera = hgCamera.fields.camera;
			//   if ( !camera )
			//     goto LABEL_18;
			//   cameraType = UnityEngine::Camera::get_cameraType(camera, 0LL);
			//   if ( !settingParameters )
			//     goto LABEL_18;
			//   enableUpdatingSceneFrostedGlass = HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit(
			//                                       settingParameters.fields._enablePermenantSceneFrostedGlass_k__BackingField,
			//                                       MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//   if ( !enableUpdatingSceneFrostedGlass )
			//     enableUpdatingSceneFrostedGlass = hgCamera.fields.enableUpdatingSceneFrostedGlass;
			//   v21 = enableUpdatingSceneFrostedGlass & HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit(
			//                                             settingParameters.fields._frostedGlassEnabled_k__BackingField,
			//                                             MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//   v22 = 0;
			//   if ( cameraType != CameraType__Enum_SceneView )
			//     v22 = v21;
			//   if ( v22 )
			//   {
			//     v23 = HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit(
			//             settingParameters.fields._frostedGlassUseComputeShader_k__BackingField,
			//             MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     v25 = *source;
			//     HG::Rendering::Runtime::UberPostPassUtils::PrepareFrostedGlassData(
			//       v23,
			//       renderGraph,
			//       TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_frostedGlassData,
			//       hgCamera,
			//       &v25,
			//       frostedGlassPSMaterials,
			//       0LL);
			//     v25 = *logLut;
			//     HG::Rendering::Runtime::UberPostPassUtils::PrepareFrostedGlassLutData(
			//       v23,
			//       TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_frostedGlassData,
			//       hgCamera,
			//       &v25,
			//       lutSize,
			//       0LL);
			//     HG::Rendering::Runtime::UberPostPassUtils::PrepareFrostedGlassSceneRTs(
			//       v23,
			//       renderGraph,
			//       TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_frostedGlassData,
			//       sceneFrostedGlassRTs,
			//       sceneFrostedGlassRTSizes,
			//       0LL);
			//     blackTexture_k__BackingField = *HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassAddPass(
			//                                       &v26,
			//                                       TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_frostedGlassData,
			//                                       renderGraph,
			//                                       v23,
			//                                       0LL);
			//   }
			//   else
			//   {
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     HG::Rendering::Runtime::UberPostPassUtils::FrostedGlassAddSkipPass(
			//       cameraType == CameraType__Enum_SceneView,
			//       enableUpdatingSceneFrostedGlass,
			//       renderGraph,
			//       sceneFrostedGlassRTs,
			//       0LL);
			//   }
			//   *retstr = blackTexture_k__BackingField;
			//   return retstr;
			// }
			// 
			return null;
		}

		internal static void RenderUIFrostedGlass(TextureHandle source, HGRenderGraph renderGraph, HGCamera camera, HGSettingParameters settingParameters, ref UberPostPassUtils.FrostedGlassPSMaterials frostedGlassPSMaterials)
		{
			// // Void RenderUIFrostedGlass(TextureHandle, HGRenderGraph, HGCamera, HGSettingParameters, UberPostPassUtils+FrostedGlassPSMaterials ByRef)
			// void HG::Rendering::Runtime::UberPostPassUtils::RenderUIFrostedGlass(
			//         TextureHandle *source,
			//         HGRenderGraph *renderGraph,
			//         HGCamera *camera,
			//         HGSettingParameters *settingParameters,
			//         UberPostPassUtils_FrostedGlassPSMaterials *frostedGlassPSMaterials,
			//         MethodInfo *method)
			// {
			//   __int64 v10; // rdx
			//   ILFixDynamicMethodWrapper_2 *Patch; // rcx
			//   bool v12; // bl
			//   UberPostPassUtils__StaticFields *static_fields; // r8
			//   TextureHandle v14; // [rsp+40h] [rbp-18h] BYREF
			// 
			//   if ( !byte_18D9194D7 )
			//   {
			//     sub_18003C530(&MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     byte_18D9194D7 = 1;
			//   }
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2448, 0LL) )
			//   {
			//     if ( settingParameters )
			//     {
			//       v12 = HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit(
			//               settingParameters.fields._frostedGlassUseComputeShader_k__BackingField,
			//               MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       static_fields = TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields;
			//       v14 = *source;
			//       HG::Rendering::Runtime::UberPostPassUtils::PrepareFrostedGlassData(
			//         v12,
			//         renderGraph,
			//         static_fields.s_frostedGlassData,
			//         camera,
			//         &v14,
			//         frostedGlassPSMaterials,
			//         0LL);
			//       HG::Rendering::Runtime::UberPostPassUtils::PrepareFrostedGlassUIRTs(
			//         v12,
			//         renderGraph,
			//         TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_frostedGlassData,
			//         0LL);
			//       HG::Rendering::Runtime::UberPostPassUtils::UIFrostedGlassAddPass(
			//         TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_frostedGlassData,
			//         renderGraph,
			//         v12,
			//         camera,
			//         0LL);
			//       return;
			//     }
			// LABEL_7:
			//     sub_180B536AC(Patch, v10);
			//   }
			//   Patch = IFix::WrappersManagerImpl::GetPatch(2448, 0LL);
			//   if ( !Patch )
			//     goto LABEL_7;
			//   v14 = *source;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_897(
			//     Patch,
			//     &v14,
			//     (Object *)renderGraph,
			//     (Object *)camera,
			//     (Object *)settingParameters,
			//     frostedGlassPSMaterials,
			//     0LL);
			// }
			// 
		}

		internal static void PrepareSharpenData(HGCamera hgCamera, HGSharpen sharpen, Material sharpenMaterial)
		{
			// // Void PrepareSharpenData(HGCamera, HGSharpen, Material)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareSharpenData(
			//         HGCamera *hgCamera,
			//         HGSharpen *sharpen,
			//         Material *sharpenMaterial,
			//         MethodInfo *method)
			// {
			//   void *sharpenMode; // rdx
			//   unsigned __int64 static_fields; // rcx
			//   int v9; // eax
			//   int v10; // edi
			//   float v11; // xmm0_4
			//   float v12; // xmm6_4
			//   float v13; // xmm0_4
			//   float v14; // xmm8_4
			//   float v15; // xmm7_4
			//   int32_t m_X; // edi
			//   __int64 v17; // rbx
			//   float v18; // xmm6_4
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   Vector4 v20; // [rsp+30h] [rbp-48h] BYREF
			// 
			//   if ( !byte_18D9194D8 )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGShaderIDs);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//     sub_18003C530(&TypeInfo::System::Math);
			//     byte_18D9194D8 = 1;
			//   }
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2449, 0LL) )
			//   {
			//     if ( !sharpen )
			//       goto LABEL_25;
			//     sharpenMode = sharpen.fields.sharpenMode;
			//     if ( !sharpenMode )
			//       goto LABEL_25;
			//     v9 = sub_18003ED00(10LL);
			//     sharpenMode = sharpen.fields.sharpenRange;
			//     v10 = v9;
			//     if ( !sharpenMode )
			//       goto LABEL_25;
			//     v11 = sub_18003F9B0(10LL, sharpenMode);
			//     sharpenMode = sharpen.fields.sharpenIntensity;
			//     v12 = v11;
			//     if ( !sharpenMode )
			//       goto LABEL_25;
			//     v13 = sub_18003F9B0(10LL, sharpenMode);
			//     sharpenMode = sharpen.fields.sharpenThreshold;
			//     v14 = v13;
			//     if ( !sharpenMode )
			//       goto LABEL_25;
			//     v15 = sub_18003F9B0(10LL, sharpenMode);
			//     if ( v10 == 1 )
			//     {
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//       static_fields = (unsigned __int64)TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//       if ( !sharpenMaterial )
			//         goto LABEL_25;
			//       UnityEngine::Material::EnableKeyword(sharpenMaterial, *(String **)(static_fields + 216), 0LL);
			//       UnityEngine::Material::DisableKeyword(
			//         sharpenMaterial,
			//         TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_SharpenFilter2,
			//         0LL);
			//     }
			//     else
			//     {
			//       static_fields = (unsigned int)(v10 - 2);
			//       if ( v10 != 2 )
			//       {
			//         if ( v10 == 3 )
			//         {
			//           sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//           static_fields = (unsigned __int64)TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//           if ( !sharpenMaterial )
			//             goto LABEL_25;
			//           UnityEngine::Material::DisableKeyword(sharpenMaterial, *(String **)(static_fields + 216), 0LL);
			//           UnityEngine::Material::DisableKeyword(
			//             sharpenMaterial,
			//             TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_SharpenFilter2,
			//             0LL);
			//           UnityEngine::Material::EnableKeyword(
			//             sharpenMaterial,
			//             TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_SharpenFilter4,
			//             0LL);
			//         }
			//         goto LABEL_19;
			//       }
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//       static_fields = (unsigned __int64)TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//       if ( !sharpenMaterial )
			//         goto LABEL_25;
			//       UnityEngine::Material::DisableKeyword(sharpenMaterial, *(String **)(static_fields + 216), 0LL);
			//       UnityEngine::Material::EnableKeyword(
			//         sharpenMaterial,
			//         TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_SharpenFilter2,
			//         0LL);
			//     }
			//     UnityEngine::Material::DisableKeyword(
			//       sharpenMaterial,
			//       TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_SharpenFilter4,
			//       0LL);
			// LABEL_19:
			//     if ( hgCamera )
			//     {
			//       m_X = hgCamera.fields._taauRTSize_k__BackingField.m_X;
			//       v17 = HIDWORD(*(_QWORD *)&hgCamera.fields._taauRTSize_k__BackingField);
			//       sub_180002C70(TypeInfo::System::Math);
			//       v18 = v12 / 1080.0;
			//       if ( m_X > (int)v17 )
			//         m_X = v17;
			//       v20.x = v14;
			//       *(_QWORD *)&v20.z = LODWORD(v15);
			//       v20.y = (float)m_X * v18;
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderIDs);
			//       static_fields = (unsigned __int64)TypeInfo::HG::Rendering::Runtime::HGShaderIDs.static_fields;
			//       if ( sharpenMaterial )
			//       {
			//         UnityEngine::Material::SetVector(sharpenMaterial, *(_DWORD *)(static_fields + 2072), &v20, 0LL);
			//         return;
			//       }
			//     }
			// LABEL_25:
			//     sub_180B536AC(static_fields, sharpenMode);
			//   }
			//   Patch = IFix::WrappersManagerImpl::GetPatch(2449, 0LL);
			//   if ( !Patch )
			//     goto LABEL_25;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_11(
			//     (ILFixDynamicMethodWrapper_28 *)Patch,
			//     (Object *)hgCamera,
			//     (Object *)sharpen,
			//     (Object *)sharpenMaterial,
			//     0LL);
			// }
			// 
		}

		internal static TextureHandle SharpenPass(HGRenderGraph renderGraph, HGCamera hgCamera, TextureHandle source, HGSharpen sharpen, Material sharpenMaterial)
		{
			// // TextureHandle SharpenPass(HGRenderGraph, HGCamera, TextureHandle, HGSharpen, Material)
			// // Hidden C++ exception states: #wind=1
			// TextureHandle *HG::Rendering::Runtime::UberPostPassUtils::SharpenPass(
			//         TextureHandle *__return_ptr retstr,
			//         HGRenderGraph *renderGraph,
			//         HGCamera *hgCamera,
			//         TextureHandle *source,
			//         HGSharpen *sharpen,
			//         Material *sharpenMaterial,
			//         MethodInfo *method)
			// {
			//   TextureHandle *v10; // rbx
			//   __int64 v11; // rdx
			//   __int64 v12; // rcx
			//   HGRenderPathBase_HGRenderPathResources *v13; // rdx
			//   PassConstructorID__Enum__Array *v14; // r8
			//   HGCamera *v15; // r9
			//   HGRenderPipeline *currentPipeline; // rax
			//   __int64 v17; // rdx
			//   __int64 v18; // rcx
			//   __int64 v19; // rdx
			//   __int64 v20; // rcx
			//   ProfilingSampler *v21; // rax
			//   __int64 v22; // rcx
			//   Object *v23; // rdx
			//   unsigned int v24; // edx
			//   unsigned __int64 v25; // r8
			//   char v26; // dl
			//   signed __int64 v27; // rtt
			//   Object *v28; // rdi
			//   __int64 v29; // rdx
			//   __int64 v30; // rcx
			//   TextureHandle v31; // xmm0
			//   Object *v32; // rdi
			//   __int64 v33; // rdx
			//   __int64 v34; // rcx
			//   TextureHandle v35; // xmm0
			//   TextureHandle v36; // xmm0
			//   __int64 v37; // rdx
			//   __int64 v38; // rcx
			//   ILFixDynamicMethodWrapper_2 *Patch; // r14
			//   TextureHandle *result; // rax
			//   MethodInfo *methoda; // [rsp+20h] [rbp-178h]
			//   MethodInfo *v42; // [rsp+28h] [rbp-170h]
			//   Object *v43; // [rsp+40h] [rbp-158h] BYREF
			//   TextureHandle v44; // [rsp+50h] [rbp-148h] BYREF
			//   HGRenderGraphBuilder v45; // [rsp+60h] [rbp-138h] BYREF
			//   TextureHandle v46; // [rsp+80h] [rbp-118h] BYREF
			//   HGRenderGraphBuilder v47; // [rsp+90h] [rbp-108h] BYREF
			//   Il2CppExceptionWrapper *v48; // [rsp+B0h] [rbp-E8h] BYREF
			//   TextureDesc v49; // [rsp+C0h] [rbp-D8h] BYREF
			//   TextureDesc v50; // [rsp+120h] [rbp-78h] BYREF
			// 
			//   v10 = retstr;
			//   if ( !byte_18D9194D9 )
			//   {
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::SharpenData>);
			//     sub_18003C530(&MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::SharpenData>);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGRenderPipeline);
			//     sub_18003C530(&MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     sub_18003C530(&off_18D4F37C8);
			//     sub_18003C530(&off_18D4F37D8);
			//     byte_18D9194D9 = 1;
			//   }
			//   sub_1802F01E0(&v49, 0LL, 96LL);
			//   v43 = 0LL;
			//   if ( IFix::WrappersManagerImpl::IsPatched(2450, 0LL) )
			//   {
			//     Patch = IFix::WrappersManagerImpl::GetPatch(2450, 0LL);
			//     if ( !Patch )
			//       sub_180B536AC(v38, v37);
			//     v44 = *source;
			//     v36 = *IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_898(
			//              (TextureHandle *)&v45,
			//              Patch,
			//              (Object *)renderGraph,
			//              (Object *)hgCamera,
			//              &v44,
			//              (Object *)sharpen,
			//              (Object *)sharpenMaterial,
			//              0LL);
			//   }
			//   else
			//   {
			//     if ( !hgCamera )
			//       sub_180B536AC(v12, v11);
			//     HG::Rendering::RenderGraphModule::TextureDesc::TextureDesc(&v49, hgCamera.fields._taauRTSize_k__BackingField, 0LL);
			//     v49.name = (String *)"Sharpen Texture";
			//     sub_1800054D0((HGRenderPathScene *)&v49.name, v13, v14, v15, methoda, v42);
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGRenderPipeline);
			//     currentPipeline = HG::Rendering::Runtime::HGRenderPipeline::get_currentPipeline(0LL);
			//     if ( !currentPipeline )
			//       sub_180B536AC(v18, v17);
			//     v49.colorFormat = HG::Rendering::Runtime::HGRenderPipeline::GetColorBufferFormat(currentPipeline, hgCamera, 0LL);
			//     v49.useMipMap = 0;
			//     v50 = v49;
			//     if ( !renderGraph )
			//       sub_180B536AC(v20, v19);
			//     v44 = *HG::Rendering::RenderGraphModule::HGRenderGraph::CreateTexture(&v44, renderGraph, &v50, 0LL);
			//     sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//     HG::Rendering::Runtime::UberPostPassUtils::PrepareSharpenData(hgCamera, sharpen, sharpenMaterial, 0LL);
			//     v21 = UnityEngine::Rendering::ProfilingSampler::Get<System::Int32Enum>(
			//             (Int32Enum__Enum)0x96u,
			//             MethodInfo::UnityEngine::Rendering::ProfilingSampler::Get<HG::Rendering::Runtime::HGProfileId>);
			//     HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<System::Object>(
			//       &v45,
			//       renderGraph,
			//       (String *)"Sharpen",
			//       &v43,
			//       v21,
			//       1,
			//       ProfilingHGPass__Enum_None,
			//       MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraph::AddRenderPass<HG::Rendering::Runtime::UberPostPassUtils::SharpenData>);
			//     v47 = v45;
			//     v45.m_RenderPass = 0LL;
			//     v45.m_Resources = (HGRenderGraphResourceRegistry *)&v47;
			//     try
			//     {
			//       v23 = v43;
			//       if ( !v43 )
			//         sub_1802DC2C8(v22, 0LL);
			//       v43[1].klass = (Object__Class *)sharpenMaterial;
			//       if ( dword_18D8E43F8 )
			//       {
			//         v24 = ((unsigned __int64)&v23[1] >> 12) & 0x1FFFFF;
			//         v25 = (unsigned __int64)v24 >> 6;
			//         v26 = v24 & 0x3F;
			//         _m_prefetchw(&qword_18D6870D0[v25]);
			//         do
			//           v27 = qword_18D6870D0[v25];
			//         while ( v27 != _InterlockedCompareExchange64(&qword_18D6870D0[v25], v27 | (1LL << v26), v27) );
			//       }
			//       v28 = v43;
			//       v31 = *HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::ReadTexture(&v46, &v47, source, 0LL);
			//       if ( !v28 )
			//         sub_1802DC2C8(v30, v29);
			//       *(TextureHandle *)&v28[1].monitor = v31;
			//       v32 = v43;
			//       v35 = *HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::WriteTexture(&v46, &v47, &v44, 0LL);
			//       if ( !v32 )
			//         sub_1802DC2C8(v34, v33);
			//       *(TextureHandle *)&v32[2].monitor = v35;
			//       if ( !v43 )
			//         sub_1802DC2C8(v34, v33);
			//       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetColorAttachment(
			//         &v46,
			//         &v47,
			//         (TextureHandle *)&v43[2].monitor,
			//         0,
			//         0,
			//         0LL);
			//       sub_180002C70(TypeInfo::HG::Rendering::Runtime::UberPostPassUtils);
			//       HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<System::Object>(
			//         &v47,
			//         (RenderFunc_1_System_Object_ *)TypeInfo::HG::Rendering::Runtime::UberPostPassUtils.static_fields.s_sharpenRenderFunc,
			//         0LL,
			//         0,
			//         MethodInfo::HG::Rendering::RenderGraphModule::HGRenderGraphBuilder::SetRenderFunc<HG::Rendering::Runtime::UberPostPassUtils::SharpenData>);
			//     }
			//     catch ( Il2CppExceptionWrapper *v48 )
			//     {
			//       v45.m_RenderPass = (HGRenderGraphPass *)v48.ex;
			//       sub_180222690(&v45);
			//       v10 = retstr;
			//       goto LABEL_17;
			//     }
			//     sub_180222690(&v45);
			// LABEL_17:
			//     v36 = v44;
			//   }
			//   result = v10;
			//   *v10 = v36;
			//   return result;
			// }
			// 
			return null;
		}

		internal static void PrepareVignetteParameters(HGSettingParameters settingParams, UberPostPassUtils.PPVignetteData data, Vignette vignette, HGVignette hgVignette, Material ppMaterial)
		{
			// // Void PrepareVignetteParameters(HGSettingParameters, UberPostPassUtils+PPVignetteData, Vignette, HGVignette, Material)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareVignetteParameters(
			//         HGSettingParameters *settingParams,
			//         UberPostPassUtils_PPVignetteData *data,
			//         Vignette *vignette,
			//         HGVignette *hgVignette,
			//         Material *ppMaterial,
			//         MethodInfo *method)
			// {
			//   String *mode; // rdx
			//   ILFixDynamicMethodWrapper_18 *Patch; // rcx
			//   bool IsActive; // r15
			//   bool v13; // r12
			//   int v14; // eax
			//   int v15; // r15d
			//   float v16; // xmm0_4
			//   float v17; // xmm13_4
			//   __int64 v18; // rax
			//   float v19; // xmm11_4
			//   unsigned int v20; // xmm12_4
			//   float v21; // xmm0_4
			//   float v22; // xmm7_4
			//   float v23; // xmm0_4
			//   float v24; // xmm9_4
			//   char v25; // al
			//   ColorParameter *color; // r8
			//   char v27; // r13
			//   const __m128i *v28; // rax
			//   __m128 v29; // xmm6
			//   __int64 v30; // rax
			//   Texture *v31; // rsi
			//   float v32; // xmm10_4
			//   float v33; // xmm8_4
			//   char v34; // al
			//   char v35; // al
			//   ColorParameter *v36; // r8
			//   char v37; // bl
			//   __m128 v38; // xmm2
			//   float v39; // xmm1_4
			//   float v40; // xmm0_4
			//   float v41; // xmm0_4
			//   float v42; // xmm2_4
			//   float v43; // xmm0_4
			//   float v44; // xmm0_4
			//   MethodInfo *v45; // r8
			//   float v46; // xmm0_4
			//   float v47; // xmm0_4
			//   Beyond::JobMathf *v48; // rcx
			//   MethodInfo *v49; // r8
			//   Vector4 si128; // xmm2
			//   Color *v51; // rax
			//   HGRenderPathBase_HGRenderPathResources *v52; // rdx
			//   PassConstructorID__Enum__Array *v53; // r8
			//   HGCamera *v54; // r9
			//   Texture **p_vignetteMask; // rcx
			//   Vector4 v56; // xmm1
			//   Vector4 v57; // xmm0
			//   Texture2D *blackTexture; // rax
			//   MethodInfo *P3; // [rsp+28h] [rbp-99h]
			//   MethodInfo *P4; // [rsp+30h] [rbp-91h]
			//   __m128 v61; // [rsp+48h] [rbp-79h] BYREF
			//   Color v62; // [rsp+58h] [rbp-69h] BYREF
			// 
			//   if ( !byte_18D9194DA )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//     sub_18003C530(&MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     byte_18D9194DA = 1;
			//   }
			//   if ( IFix::WrappersManagerImpl::IsPatched(2451, 0LL) )
			//   {
			//     Patch = (ILFixDynamicMethodWrapper_18 *)IFix::WrappersManagerImpl::GetPatch(2451, 0LL);
			//     if ( !Patch )
			//       goto LABEL_58;
			//     IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_18(
			//       Patch,
			//       (Object *)settingParams,
			//       (Object *)data,
			//       (Object *)vignette,
			//       (Object *)hgVignette,
			//       (Object *)ppMaterial,
			//       0LL);
			//   }
			//   else
			//   {
			//     if ( !vignette )
			//       goto LABEL_58;
			//     IsActive = HG::Rendering::Runtime::Vignette::IsActive(vignette, 0LL);
			//     if ( !hgVignette )
			//       goto LABEL_58;
			//     v13 = HG::Rendering::Runtime::HGVignette::IsActive(hgVignette, 0LL);
			//     if ( !settingParams )
			//       goto LABEL_58;
			//     if ( HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit(
			//            settingParams.fields._vignetteEnabled_k__BackingField,
			//            MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit)
			//       && (IsActive || v13) )
			//     {
			//       mode = (String *)vignette.fields.mode;
			//       if ( !mode )
			//         goto LABEL_58;
			//       v14 = sub_18003ED00(10LL);
			//       mode = (String *)vignette.fields.roundness;
			//       v15 = v14;
			//       if ( !mode )
			//         goto LABEL_58;
			//       v16 = sub_18003F9B0(10LL, mode);
			//       mode = (String *)vignette.fields.center;
			//       v17 = v16;
			//       if ( !mode )
			//         goto LABEL_58;
			//       v18 = sub_18004A780(10LL);
			//       mode = (String *)vignette.fields.intensity;
			//       v61.m128_u64[0] = v18;
			//       v19 = *(float *)&v18;
			//       v20 = HIDWORD(v18);
			//       if ( !mode )
			//         goto LABEL_58;
			//       v21 = sub_18003F9B0(10LL, mode);
			//       mode = (String *)vignette.fields.smoothness;
			//       v22 = v21;
			//       if ( !mode )
			//         goto LABEL_58;
			//       v23 = sub_18003F9B0(10LL, mode);
			//       mode = (String *)vignette.fields.rounded;
			//       v24 = v23;
			//       if ( !mode )
			//         goto LABEL_58;
			//       v25 = sub_1800023D0(10LL, mode);
			//       color = vignette.fields.color;
			//       v27 = v25;
			//       if ( !color )
			//         goto LABEL_58;
			//       v28 = (const __m128i *)sub_180040AD0(&v61, 10LL, color);
			//       mode = (String *)vignette.fields.mask;
			//       v29 = (__m128)_mm_loadu_si128(v28);
			//       v61 = v29;
			//       if ( !mode )
			//         goto LABEL_58;
			//       v30 = sub_18004EF00(10LL, mode);
			//       mode = (String *)vignette.fields.opacity;
			//       v31 = (Texture *)v30;
			//       if ( !mode )
			//         goto LABEL_58;
			//       v32 = 1.0;
			//       v33 = sub_18003F9B0(10LL, mode);
			//       if ( !v13 )
			//       {
			//         v37 = 0;
			//         if ( v15 )
			//         {
			//           sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//           mode = TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_VignetteMask;
			//           Patch = (ILFixDynamicMethodWrapper_18 *)ppMaterial;
			//           if ( ppMaterial )
			//           {
			//             UnityEngine::Material::EnableKeyword(ppMaterial, mode, 0LL);
			//             v61.m128_i32[0] = v29.m128_i32[0];
			//             v61.m128_i32[1] = _mm_shuffle_ps(v29, v29, 85).m128_u32[0];
			//             v61.m128_i32[2] = _mm_shuffle_ps(v29, v29, 170).m128_u32[0];
			//             Beyond::JobMathf::Clamp01(v48);
			//             si128 = (Vector4)_mm_load_si128((const __m128i *)&xmmword_18A3576E0);
			//             v61.m128_f32[3] = v33;
			//             if ( data )
			//             {
			//               data.fields.vignetteParams1 = si128;
			//               v51 = UnityEngine::Color::op_Implicit(&v62, (Vector4 *)&v61, v49);
			//               p_vignetteMask = &data.fields.vignetteMask;
			//               v56 = (Vector4)_mm_loadu_si128((const __m128i *)v51);
			//               data.fields.vignetteMask = v31;
			//               data.fields.vignetteColor = v56;
			// LABEL_56:
			//               sub_1800054D0((HGRenderPathScene *)p_vignetteMask, v52, v53, v54, P3, P4);
			//               return;
			//             }
			//           }
			// LABEL_58:
			//           sub_180B536AC(Patch, mode);
			//         }
			// LABEL_38:
			//         sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//         mode = TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_Vignette;
			//         Patch = (ILFixDynamicMethodWrapper_18 *)ppMaterial;
			//         if ( ppMaterial )
			//         {
			//           UnityEngine::Material::EnableKeyword(ppMaterial, mode, 0LL);
			//           v46 = v37 ? 1.0 : 0.0;
			//           v61.m128_f32[0] = v19;
			//           *(unsigned __int64 *)((char *)v61.m128_u64 + 4) = v20;
			//           v61.m128_f32[3] = v46;
			//           if ( data )
			//           {
			//             data.fields.vignetteParams1 = (Vector4)v61;
			//             if ( !v27 )
			//               v32 = 0.0;
			//             v61.m128_f32[0] = v22 * 3.0;
			//             v61.m128_f32[2] = (float)((float)(1.0 - v17) * 6.0) + v17;
			//             v61.m128_f32[1] = v24 * 5.0;
			//             v61.m128_f32[3] = v32;
			//             v57 = (Vector4)v61;
			//             v61 = v29;
			//             data.fields.vignetteParams2 = v57;
			//             data.fields.vignetteColor = (Vector4)_mm_loadu_si128((const __m128i *)UnityEngine::Color::op_Implicit(
			//                                                                                      &v62,
			//                                                                                      (Vector4 *)&v61,
			//                                                                                      v45));
			//             blackTexture = UnityEngine::Texture2D::get_blackTexture(0LL);
			//             p_vignetteMask = &data.fields.vignetteMask;
			//             data.fields.vignetteMask = (Texture *)blackTexture;
			//             goto LABEL_56;
			//           }
			//         }
			//         goto LABEL_58;
			//       }
			//       mode = (String *)hgVignette.fields.rounded;
			//       if ( !mode )
			//         goto LABEL_58;
			//       v34 = sub_1800023D0(10LL, mode);
			//       mode = (String *)hgVignette.fields.blink;
			//       v27 = v34;
			//       v17 = 1.0;
			//       if ( !mode )
			//         goto LABEL_58;
			//       v35 = sub_1800023D0(10LL, mode);
			//       v36 = hgVignette.fields.color;
			//       v37 = v35;
			//       v19 = 0.5;
			//       v20 = 1056964608;
			//       if ( !v36 )
			//         goto LABEL_58;
			//       v38 = (__m128)_mm_loadu_si128((const __m128i *)sub_180040AD0(&v62, 10LL, v36));
			//       if ( v22 > 0.0 )
			//       {
			//         if ( v38.m128_f32[0] <= v29.m128_f32[0] )
			//           v29.m128_i32[0] = v38.m128_i32[0];
			//         v39 = v61.m128_f32[1];
			//         v40 = _mm_shuffle_ps(v38, v38, 85).m128_f32[0];
			//         v61.m128_i32[0] = v29.m128_i32[0];
			//         if ( v40 <= v61.m128_f32[1] )
			//           v39 = v40;
			//         v41 = v61.m128_f32[2];
			//         v42 = _mm_shuffle_ps(v38, v38, 170).m128_f32[0];
			//         v61.m128_f32[1] = v39;
			//         if ( v42 <= v61.m128_f32[2] )
			//           v41 = v42;
			//         v61.m128_f32[2] = v41;
			//       }
			//       else
			//       {
			//         v61.m128_i32[0] = v38.m128_i32[0];
			//         v61.m128_i32[3] = _mm_shuffle_ps(v38, v38, 255).m128_u32[0];
			//         v61.m128_i32[1] = _mm_shuffle_ps(v38, v38, 85).m128_u32[0];
			//         v61.m128_i32[2] = _mm_shuffle_ps(v38, v38, 170).m128_u32[0];
			//       }
			//       mode = (String *)hgVignette.fields.intensity;
			//       if ( v37 )
			//       {
			//         if ( !mode )
			//           goto LABEL_58;
			//         v43 = sub_18003F9B0(10LL, mode);
			//         mode = (String *)hgVignette.fields.smoothness;
			//         v22 = v43;
			//         if ( !mode )
			//           goto LABEL_58;
			//         v44 = sub_18003F9B0(10LL, mode);
			//       }
			//       else
			//       {
			//         if ( !mode )
			//           goto LABEL_58;
			//         v47 = sub_18003F9B0(10LL, mode);
			//         if ( v22 <= v47 )
			//           v22 = v47;
			//         mode = (String *)hgVignette.fields.smoothness;
			//         if ( !mode )
			//           goto LABEL_58;
			//         v44 = sub_18003F9B0(10LL, mode);
			//         if ( v24 > v44 )
			//           goto LABEL_37;
			//       }
			//       v24 = v44;
			// LABEL_37:
			//       v29 = v61;
			//       goto LABEL_38;
			//     }
			//   }
			// }
			// 
		}

		internal static void PrepareDirtyLensParameters(HGSettingParameters settingParameters, UberPostPassUtils.UberPostPassData data, HGDirtyLens hgDirtyLens)
		{
			// // Void PrepareDirtyLensParameters(HGSettingParameters, UberPostPassUtils+UberPostPassData, HGDirtyLens)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareDirtyLensParameters(
			//         HGSettingParameters *settingParameters,
			//         UberPostPassUtils_UberPostPassData *data,
			//         HGDirtyLens *hgDirtyLens,
			//         MethodInfo *method)
			// {
			//   String **dirtyTex; // rdx
			//   __int64 v8; // rcx
			//   BOOL v9; // esi
			//   __int64 v10; // rax
			//   Texture *v11; // rbp
			//   float v12; // xmm6_4
			//   Material *uberPostMaterial; // rbx
			//   HGRenderPathBase_HGRenderPathResources *v14; // rdx
			//   PassConstructorID__Enum__Array *v15; // r8
			//   HGCamera *v16; // r9
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   MethodInfo *v18; // [rsp+20h] [rbp-38h]
			//   MethodInfo *v19; // [rsp+28h] [rbp-30h]
			// 
			//   if ( !byte_18D9194DB )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//     sub_18003C530(&MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     byte_18D9194DB = 1;
			//   }
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2453, 0LL) )
			//   {
			//     if ( !hgDirtyLens )
			//       goto LABEL_16;
			//     if ( HG::Rendering::Runtime::HGDirtyLens::IsActive(hgDirtyLens, 0LL) )
			//     {
			//       if ( !settingParameters )
			//         goto LABEL_16;
			//       v9 = HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit(
			//              settingParameters.fields._dirtyLensEnabled_k__BackingField,
			//              MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     }
			//     else
			//     {
			//       v9 = 0;
			//     }
			//     dirtyTex = (String **)hgDirtyLens.fields.dirtyTex;
			//     if ( dirtyTex )
			//     {
			//       v10 = sub_18004EF00(10LL, dirtyTex);
			//       dirtyTex = (String **)hgDirtyLens.fields.intensity;
			//       v11 = (Texture *)v10;
			//       if ( dirtyTex )
			//       {
			//         v12 = sub_18003F9B0(10LL, dirtyTex);
			//         if ( !v9 )
			//           return;
			//         if ( data )
			//         {
			//           uberPostMaterial = data.fields.uberPostMaterial;
			//           sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//           dirtyTex = &TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_EnableScreenSpaceShadowMask.m_Name;
			//           if ( uberPostMaterial )
			//           {
			//             UnityEngine::Material::EnableKeyword(uberPostMaterial, dirtyTex[32], 0LL);
			//             data.fields.dirtyTex = v11;
			//             sub_1800054D0((HGRenderPathScene *)&data.fields.dirtyTex, v14, v15, v16, v18, v19);
			//             data.fields.dirtyLensParams1 = (Vector4)LODWORD(v12);
			//             return;
			//           }
			//         }
			//       }
			//     }
			// LABEL_16:
			//     sub_180B536AC(v8, dirtyTex);
			//   }
			//   Patch = IFix::WrappersManagerImpl::GetPatch(2453, 0LL);
			//   if ( !Patch )
			//     goto LABEL_16;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_11(
			//     (ILFixDynamicMethodWrapper_28 *)Patch,
			//     (Object *)settingParameters,
			//     (Object *)data,
			//     (Object *)hgDirtyLens,
			//     0LL);
			// }
			// 
		}

		internal static void PrepareRadialBlurAndChromaticAberrationParameters(HGSettingParameters settingParameters, UberPostPassUtils.UberPostPassData data, HGCamera camera, HGRadialBlur radialBlur, HGChromaticAbberation chromaticAbberation)
		{
			// // Void PrepareRadialBlurAndChromaticAberrationParameters(HGSettingParameters, UberPostPassUtils+UberPostPassData, HGCamera, HGRadialBlur, HGChromaticAbberation)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareRadialBlurAndChromaticAberrationParameters(
			//         HGSettingParameters *settingParameters,
			//         UberPostPassUtils_UberPostPassData *data,
			//         HGCamera *camera,
			//         HGRadialBlur *radialBlur,
			//         HGChromaticAbberation *chromaticAbberation,
			//         MethodInfo *method)
			// {
			//   String *s_RadialBlurWithChromaticAberration; // rdx
			//   __int64 static_fields; // rcx
			//   bool v12; // r12
			//   bool v13; // r15
			//   Material *uberPostMaterial; // rsi
			//   __m128 x_low; // xmm6
			//   __m128 y_low; // xmm7
			//   Vector2Parameter *center; // rax
			//   Vector2 v18; // rax
			//   Vector3Parameter *globalCenter; // r8
			//   Vector2Parameter *v20; // rax
			//   Camera *v21; // rsi
			//   __int64 v22; // rax
			//   __int64 v23; // xmm0_8
			//   float v24; // eax
			//   Vector3 *v25; // rax
			//   __int64 v26; // xmm0_8
			//   MethodInfo *v27; // rdx
			//   __int64 v28; // rax
			//   Beyond::JobMathf *v29; // rcx
			//   double v30; // xmm0_8
			//   Vector2 v31; // rax
			//   Vector2 v32; // r8
			//   Vector2 v33; // r9
			//   __int32 v34; // xmm12_4
			//   __int32 v35; // xmm0_4
			//   Beyond::JobMathf *v36; // rcx
			//   float v37; // xmm7_4
			//   __int32 v38; // xmm13_4
			//   float v39; // xmm6_4
			//   float v40; // xmm9_4
			//   float v41; // xmm10_4
			//   float v42; // xmm8_4
			//   float v43; // xmm0_4
			//   float v44; // xmm6_4
			//   Beyond::JobMathf *v45; // rcx
			//   float v46; // xmm0_4
			//   float v47; // xmm0_4
			//   double v48; // xmm0_8
			//   float v49; // xmm0_4
			//   unsigned int v50; // eax
			//   Vector4 v51; // [rsp+48h] [rbp-79h] BYREF
			//   Vector4 v52; // [rsp+58h] [rbp-69h] BYREF
			// 
			//   if ( !byte_18D9194DC )
			//   {
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//     sub_18003C530(&MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     byte_18D9194DC = 1;
			//   }
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2455, 0LL) )
			//   {
			//     if ( !radialBlur )
			//       goto LABEL_71;
			//     if ( HG::Rendering::Runtime::HGRadialBlur::IsActive(radialBlur, 0LL) )
			//     {
			//       if ( !settingParameters )
			//         goto LABEL_71;
			//       v12 = HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit(
			//               settingParameters.fields._radialBlurEnabled_k__BackingField,
			//               MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     }
			//     else
			//     {
			//       v12 = 0;
			//     }
			//     if ( !chromaticAbberation )
			//       goto LABEL_71;
			//     if ( HG::Rendering::Runtime::HGChromaticAbberation::IsActive(chromaticAbberation, 0LL) )
			//     {
			//       if ( !settingParameters )
			//         goto LABEL_71;
			//       v13 = HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit(
			//               settingParameters.fields._chromaticAberrationEnabled_k__BackingField,
			//               MethodInfo::HG::Rendering::Runtime::SettingParameter<bool>::op_Implicit);
			//     }
			//     else
			//     {
			//       v13 = 0;
			//     }
			//     LOBYTE(static_fields) = v13 || v12;
			//     if ( !v13 && !v12 )
			//       return;
			//     if ( !data
			//       || ((uberPostMaterial = data.fields.uberPostMaterial,
			//            sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords),
			//            v13)
			//         ? (s_RadialBlurWithChromaticAberration = TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_RadialBlurWithChromaticAberration)
			//         : (static_fields = (__int64)TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields,
			//            s_RadialBlurWithChromaticAberration = *(String **)(static_fields + 288)),
			//           !uberPostMaterial) )
			//     {
			// LABEL_71:
			//       sub_180B536AC(static_fields, s_RadialBlurWithChromaticAberration);
			//     }
			//     UnityEngine::Material::EnableKeyword(uberPostMaterial, s_RadialBlurWithChromaticAberration, 0LL);
			//     x_low = (__m128)0x3F000000u;
			//     y_low = (__m128)0x3F000000u;
			//     if ( !v13 )
			//       goto LABEL_75;
			//     center = chromaticAbberation.fields.center;
			//     if ( !center )
			//       goto LABEL_71;
			//     if ( center.fields._._.overrideState )
			//       goto LABEL_25;
			//     s_RadialBlurWithChromaticAberration = (String *)chromaticAbberation.fields.enableGlobalCenter;
			//     if ( !s_RadialBlurWithChromaticAberration )
			//       goto LABEL_71;
			//     if ( (unsigned __int8)sub_1800023D0(10LL, s_RadialBlurWithChromaticAberration) )
			//     {
			// LABEL_25:
			//       s_RadialBlurWithChromaticAberration = (String *)chromaticAbberation.fields.enableGlobalCenter;
			//       if ( !s_RadialBlurWithChromaticAberration )
			//         goto LABEL_71;
			//       if ( !(unsigned __int8)sub_1800023D0(10LL, s_RadialBlurWithChromaticAberration) )
			//       {
			//         s_RadialBlurWithChromaticAberration = (String *)chromaticAbberation.fields.center;
			// LABEL_28:
			//         if ( !s_RadialBlurWithChromaticAberration )
			//           goto LABEL_71;
			//         v18 = (Vector2)sub_18004A780(10LL);
			// LABEL_45:
			//         *(Vector2 *)&v51.x = v18;
			//         y_low = (__m128)LODWORD(v18.y);
			//         x_low = (__m128)LODWORD(v18.x);
			//         goto LABEL_46;
			//       }
			//       if ( !camera )
			//         goto LABEL_71;
			//       globalCenter = chromaticAbberation.fields.globalCenter;
			//     }
			//     else
			//     {
			// LABEL_75:
			//       if ( !v12 )
			//         goto LABEL_46;
			//       v20 = radialBlur.fields.center;
			//       if ( !v20 )
			//         goto LABEL_71;
			//       if ( !v20.fields._._.overrideState )
			//       {
			//         s_RadialBlurWithChromaticAberration = (String *)radialBlur.fields.enableGlobalCenter;
			//         if ( !s_RadialBlurWithChromaticAberration )
			//           goto LABEL_71;
			//         if ( !(unsigned __int8)sub_1800023D0(10LL, s_RadialBlurWithChromaticAberration) )
			//         {
			// LABEL_46:
			//           v28 = sub_182C9F010(_mm_unpacklo_ps(x_low, y_low).m128_u64[0]);
			//           *(_QWORD *)&v51.x = sub_18473B264(v28, _mm_unpacklo_ps((__m128)0x3F800000u, (__m128)0x3F800000u).m128_u64[0]);
			//           v30 = sub_182413570(&v51);
			//           if ( *(float *)&v30 > 1.414 )
			//           {
			//             v31 = (Vector2)sub_182413630(&v51);
			//             *(Vector2 *)&v51.x = sub_1842BE4B8(
			//                                    (Vector2)*(_OWORD *)&_mm_unpacklo_ps((__m128)0x3F800000u, (__m128)0x3F800000u),
			//                                    v31,
			//                                    v32,
			//                                    v33);
			//             x_low.m128_f32[0] = v51.x * 0.5;
			//             y_low.m128_f32[0] = v51.y * 0.5;
			//           }
			//           Beyond::JobMathf::Clamp01(v29);
			//           v34 = x_low.m128_i32[0];
			//           v35 = y_low.m128_i32[0];
			//           Beyond::JobMathf::Clamp01(v36);
			//           v37 = 1.2;
			//           v38 = v35;
			//           v39 = 0.0;
			//           v40 = 0.0;
			//           v41 = 0.0;
			//           if ( v13 )
			//           {
			//             if ( !v12 )
			//             {
			//               v42 = 0.0;
			//               goto LABEL_60;
			//             }
			//             s_RadialBlurWithChromaticAberration = (String *)radialBlur.fields.intensity;
			//             if ( !s_RadialBlurWithChromaticAberration )
			//               goto LABEL_71;
			//             v43 = sub_18003F9B0(10LL, s_RadialBlurWithChromaticAberration);
			//             s_RadialBlurWithChromaticAberration = (String *)chromaticAbberation.fields.intensity;
			//             if ( !s_RadialBlurWithChromaticAberration )
			//               goto LABEL_71;
			//             v44 = v43 / sub_18003F9B0(10LL, s_RadialBlurWithChromaticAberration);
			//             Beyond::JobMathf::Clamp01(v45);
			//             v42 = v44;
			//           }
			//           else
			//           {
			//             v42 = 0.0;
			//             if ( !v12 )
			//               goto LABEL_63;
			//           }
			//           s_RadialBlurWithChromaticAberration = (String *)radialBlur.fields.power;
			//           if ( !s_RadialBlurWithChromaticAberration )
			//             goto LABEL_71;
			//           v46 = sub_18003F9B0(10LL, s_RadialBlurWithChromaticAberration);
			//           s_RadialBlurWithChromaticAberration = (String *)radialBlur.fields.intensity;
			//           v37 = v46;
			//           if ( !s_RadialBlurWithChromaticAberration )
			//             goto LABEL_71;
			//           v47 = sub_18003F9B0(10LL, s_RadialBlurWithChromaticAberration);
			//           s_RadialBlurWithChromaticAberration = (String *)radialBlur.fields.averageSteps;
			//           v39 = v47;
			//           if ( !s_RadialBlurWithChromaticAberration )
			//             goto LABEL_71;
			//           v40 = (float)(unsigned __int8)sub_1800023D0(10LL, s_RadialBlurWithChromaticAberration);
			//           if ( !v13 )
			//           {
			// LABEL_62:
			//             if ( v12 && v13 )
			//             {
			//               s_RadialBlurWithChromaticAberration = (String *)radialBlur.fields.intensity;
			//               if ( !s_RadialBlurWithChromaticAberration )
			//                 goto LABEL_71;
			//               v49 = sub_18003F9B0(10LL, s_RadialBlurWithChromaticAberration);
			//               v50 = 6;
			//               if ( v49 <= 0.01 )
			//                 v50 = 3;
			//               static_fields = v50;
			//               goto LABEL_68;
			//             }
			// LABEL_63:
			//             static_fields = 3LL;
			// LABEL_68:
			//             *(_QWORD *)&v52.x = __PAIR64__(v38, v34);
			//             *(_QWORD *)&v52.z = __PAIR64__(LODWORD(v37), LODWORD(v39));
			//             data.fields.radialBlurParams = v52;
			//             s_RadialBlurWithChromaticAberration = (String *)chromaticAbberation.fields.intensity;
			//             if ( s_RadialBlurWithChromaticAberration )
			//             {
			//               v52.x = (float)(int)static_fields;
			//               *(_QWORD *)&v52.y = __PAIR64__(
			//                                     LODWORD(v40),
			//                                     COERCE_UNSIGNED_INT(sub_18003F9B0(10LL, s_RadialBlurWithChromaticAberration)));
			//               v52.w = v41;
			//               data.fields.radialBlurParams2 = v52;
			//               return;
			//             }
			//             goto LABEL_71;
			//           }
			// LABEL_60:
			//           v48 = Beyond::JobMathf::ClampedLerp((Beyond::JobMathf *)static_fields, v37, v42);
			//           s_RadialBlurWithChromaticAberration = (String *)chromaticAbberation.fields.averageStep;
			//           v37 = *(float *)&v48;
			//           if ( !s_RadialBlurWithChromaticAberration )
			//             goto LABEL_71;
			//           v41 = (float)(unsigned __int8)sub_1800023D0(10LL, s_RadialBlurWithChromaticAberration);
			//           goto LABEL_62;
			//         }
			//       }
			//       s_RadialBlurWithChromaticAberration = (String *)radialBlur.fields.enableGlobalCenter;
			//       if ( !s_RadialBlurWithChromaticAberration )
			//         goto LABEL_71;
			//       if ( !(unsigned __int8)sub_1800023D0(10LL, s_RadialBlurWithChromaticAberration) )
			//       {
			//         s_RadialBlurWithChromaticAberration = (String *)radialBlur.fields.center;
			//         goto LABEL_28;
			//       }
			//       if ( !camera )
			//         goto LABEL_71;
			//       globalCenter = radialBlur.fields.globalCenter;
			//     }
			//     v21 = camera.fields.camera;
			//     if ( !globalCenter )
			//       goto LABEL_71;
			//     v22 = sub_180057F50(&v52, 10LL, globalCenter);
			//     if ( !v21 )
			//       goto LABEL_71;
			//     v23 = *(_QWORD *)v22;
			//     v24 = *(float *)(v22 + 8);
			//     *(_QWORD *)&v51.x = v23;
			//     v51.z = v24;
			//     v25 = UnityEngine::Camera::WorldToViewportPoint((Vector3 *)&v52, v21, (Vector3 *)&v51, 0LL);
			//     v26 = *(_QWORD *)&v25.x;
			//     *(float *)&v25 = v25.z;
			//     *(_QWORD *)&v51.x = v26;
			//     LODWORD(v51.z) = (_DWORD)v25;
			//     v18 = UnityEngine::Vector4::op_Implicit(&v51, v27);
			//     goto LABEL_45;
			//   }
			//   static_fields = (__int64)IFix::WrappersManagerImpl::GetPatch(2455, 0LL);
			//   if ( !static_fields )
			//     goto LABEL_71;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_18(
			//     (ILFixDynamicMethodWrapper_18 *)static_fields,
			//     (Object *)settingParameters,
			//     (Object *)data,
			//     (Object *)camera,
			//     (Object *)radialBlur,
			//     (Object *)chromaticAbberation,
			//     0LL);
			// }
			// 
		}

		internal static void PrepareBWFlashParameters(UberPostPassUtils.UberPostPassData data, HGCamera camera, HGBWFlash bwFlash)
		{
			// // Void PrepareBWFlashParameters(UberPostPassUtils+UberPostPassData, HGCamera, HGBWFlash)
			// void HG::Rendering::Runtime::UberPostPassUtils::PrepareBWFlashParameters(
			//         UberPostPassUtils_UberPostPassData *data,
			//         HGCamera *camera,
			//         HGBWFlash *bwFlash,
			//         MethodInfo *method)
			// {
			//   void *bwThreshold; // rdx
			//   __int64 v8; // rcx
			//   bool IsActive; // al
			//   float v10; // xmm0_4
			//   float v11; // xmm7_4
			//   float v12; // xmm0_4
			//   float v13; // xmm6_4
			//   int v14; // ebx
			//   char v15; // al
			//   char v16; // bl
			//   char v17; // bp
			//   Material *uberPostMaterial; // rbx
			//   float v19; // eax
			//   __int64 v20; // rax
			//   int v21; // eax
			//   __int64 v22; // rax
			//   __int64 v23; // rax
			//   unsigned int v24; // eax
			//   HGRenderPathBase_HGRenderPathResources *v25; // rdx
			//   PassConstructorID__Enum__Array *v26; // r8
			//   HGCamera *v27; // r9
			//   Material *v28; // rbx
			//   __int64 v29; // rax
			//   __int64 v30; // rax
			//   unsigned int v31; // eax
			//   HGRenderPathBase_HGRenderPathResources *v32; // rdx
			//   PassConstructorID__Enum__Array *v33; // r8
			//   HGCamera *v34; // r9
			//   int v35; // ebx
			//   ColorParameter *flashColor; // r8
			//   ColorParameter *backGroundColor; // r8
			//   ILFixDynamicMethodWrapper_2 *Patch; // rax
			//   MethodInfo *v39; // [rsp+20h] [rbp-68h]
			//   MethodInfo *v40; // [rsp+28h] [rbp-60h]
			//   Vector4 v41; // [rsp+30h] [rbp-58h]
			//   __int64 v42; // [rsp+44h] [rbp-44h]
			//   unsigned int v43; // [rsp+4Ch] [rbp-3Ch]
			//   unsigned int v44; // [rsp+4Ch] [rbp-3Ch]
			//   __int128 v45; // [rsp+50h] [rbp-38h] BYREF
			// 
			//   if ( !byte_18D9194DD )
			//   {
			//     sub_18003C530(&TypeInfo::System::Convert);
			//     sub_18003C530(&TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//     byte_18D9194DD = 1;
			//   }
			//   if ( !IFix::WrappersManagerImpl::IsPatched(2458, 0LL) )
			//   {
			//     if ( bwFlash )
			//     {
			//       IsActive = HG::Rendering::Runtime::HGBWFlash::IsActive(bwFlash, 0LL);
			//       if ( data )
			//       {
			//         data.fields.isBwFlashEnabled = IsActive;
			//         if ( !IsActive )
			//           return;
			//         bwThreshold = bwFlash.fields.bwThreshold;
			//         if ( bwThreshold )
			//         {
			//           v10 = sub_18003F9B0(10LL, bwThreshold);
			//           bwThreshold = bwFlash.fields.colorBias;
			//           v11 = v10;
			//           if ( bwThreshold )
			//           {
			//             v12 = sub_18003F9B0(10LL, bwThreshold);
			//             bwThreshold = bwFlash.fields.inverse;
			//             v13 = v12;
			//             if ( bwThreshold )
			//             {
			//               v14 = (unsigned __int8)sub_1800023D0(10LL, bwThreshold);
			//               sub_180002C70(TypeInfo::System::Convert);
			//               *(_QWORD *)&v45 = __PAIR64__(LODWORD(v13), LODWORD(v11));
			//               *((_QWORD *)&v45 + 1) = COERCE_UNSIGNED_INT((float)v14);
			//               data.fields.bwFlashThreshold = (Vector4)v45;
			//               bwThreshold = bwFlash.fields.useFlashTex;
			//               if ( bwThreshold )
			//               {
			//                 v15 = sub_1800023D0(10LL, bwThreshold);
			//                 bwThreshold = bwFlash.fields.useMaskTex;
			//                 v16 = v15;
			//                 if ( bwThreshold )
			//                 {
			//                   v17 = sub_1800023D0(10LL, bwThreshold);
			//                   if ( v16 )
			//                   {
			//                     uberPostMaterial = data.fields.uberPostMaterial;
			//                     sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//                     bwThreshold = TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields.s_BWFlashTex;
			//                     if ( !uberPostMaterial )
			//                       goto LABEL_40;
			//                     UnityEngine::Material::EnableKeyword(uberPostMaterial, (String *)bwThreshold, 0LL);
			//                     bwThreshold = bwFlash.fields.centerPosition;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     LODWORD(v19) = sub_18004A780(10LL);
			//                     bwThreshold = bwFlash.fields.centerPosition;
			//                     v41.x = v19;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     v20 = sub_18004A780(10LL);
			//                     bwThreshold = bwFlash.fields.flashTexSpeed;
			//                     LODWORD(v42) = HIDWORD(v20);
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     v21 = sub_18004A780(10LL);
			//                     bwThreshold = bwFlash.fields.flashTexSpeed;
			//                     HIDWORD(v42) = v21;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     *(_QWORD *)&v41.y = v42;
			//                     *(_QWORD *)&v45 = sub_18004A780(10LL);
			//                     v41.w = *((float *)&v45 + 1);
			//                     data.fields.bwFlashParams = v41;
			//                     bwThreshold = bwFlash.fields.flashTexTiling;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     v22 = sub_18004A780(10LL);
			//                     bwThreshold = bwFlash.fields.flashTexTiling;
			//                     *(_QWORD *)&v45 = v22;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     v23 = sub_18004A780(10LL);
			//                     bwThreshold = bwFlash.fields.flashTexOffset;
			//                     v43 = HIDWORD(v23);
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     v24 = sub_18004A780(10LL);
			//                     bwThreshold = bwFlash.fields.flashTexOffset;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     *(_QWORD *)((char *)&v45 + 4) = __PAIR64__(v24, v43);
			//                     HIDWORD(v45) = (unsigned __int64)sub_18004A780(10LL) >> 32;
			//                     data.fields.bwFlashParams2 = (Vector4)v45;
			//                     bwThreshold = bwFlash.fields.flashIntensity;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     *(_QWORD *)&v45 = sub_18004A780(10LL);
			//                     LODWORD(data.fields.bwFlashParams4.x) = v45;
			//                     bwThreshold = bwFlash.fields.flashIntensity;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     *(_QWORD *)&v45 = sub_18004A780(10LL);
			//                     data.fields.bwFlashParams4.y = *((float *)&v45 + 1);
			//                     bwThreshold = bwFlash.fields.flashTexture;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     data.fields.bwFlashTexture = (Texture *)sub_18004EF00(10LL, bwThreshold);
			//                     sub_1800054D0((HGRenderPathScene *)&data.fields.bwFlashTexture, v25, v26, v27, v39, v40);
			//                   }
			//                   if ( v17 )
			//                   {
			//                     v28 = data.fields.uberPostMaterial;
			//                     sub_180002C70(TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords);
			//                     bwThreshold = TypeInfo::HG::Rendering::Runtime::HGShaderKeyWords.static_fields;
			//                     if ( !v28 )
			//                       goto LABEL_40;
			//                     UnityEngine::Material::EnableKeyword(v28, *((String **)bwThreshold + 41), 0LL);
			//                     bwThreshold = bwFlash.fields.maskTexTiling;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     v29 = sub_18004A780(10LL);
			//                     bwThreshold = bwFlash.fields.maskTexTiling;
			//                     *(_QWORD *)&v45 = v29;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     v30 = sub_18004A780(10LL);
			//                     bwThreshold = bwFlash.fields.maskTexOffset;
			//                     v44 = HIDWORD(v30);
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     v31 = sub_18004A780(10LL);
			//                     bwThreshold = bwFlash.fields.maskTexOffset;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     *(_QWORD *)((char *)&v45 + 4) = __PAIR64__(v31, v44);
			//                     HIDWORD(v45) = (unsigned __int64)sub_18004A780(10LL) >> 32;
			//                     data.fields.bwFlashParams3 = (Vector4)v45;
			//                     bwThreshold = bwFlash.fields.maskIntensity;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     data.fields.bwFlashParams4.z = sub_18003F9B0(10LL, bwThreshold);
			//                     bwThreshold = bwFlash.fields.maskTexture;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     data.fields.bwMaskTexture = (Texture *)sub_18004EF00(10LL, bwThreshold);
			//                     sub_1800054D0((HGRenderPathScene *)&data.fields.bwMaskTexture, v32, v33, v34, v39, v40);
			//                     bwThreshold = bwFlash.fields.maskUsePolarUV;
			//                     if ( !bwThreshold )
			//                       goto LABEL_40;
			//                     v35 = (unsigned __int8)sub_1800023D0(10LL, bwThreshold);
			//                     sub_180002C70(TypeInfo::System::Convert);
			//                     data.fields.bwFlashThreshold.w = (float)v35;
			//                   }
			//                   flashColor = bwFlash.fields.flashColor;
			//                   if ( flashColor )
			//                   {
			//                     data.fields.bwFlashColor = (Color)_mm_loadu_si128((const __m128i *)sub_180040AD0(
			//                                                                                           &v45,
			//                                                                                           10LL,
			//                                                                                           flashColor));
			//                     backGroundColor = bwFlash.fields.backGroundColor;
			//                     if ( backGroundColor )
			//                     {
			//                       data.fields.bwBackGroundColor = (Color)_mm_loadu_si128((const __m128i *)sub_180040AD0(
			//                                                                                                  &v45,
			//                                                                                                  10LL,
			//                                                                                                  backGroundColor));
			//                       return;
			//                     }
			//                   }
			//                 }
			//               }
			//             }
			//           }
			//         }
			//       }
			//     }
			// LABEL_40:
			//     sub_180B536AC(v8, bwThreshold);
			//   }
			//   Patch = IFix::WrappersManagerImpl::GetPatch(2458, 0LL);
			//   if ( !Patch )
			//     goto LABEL_40;
			//   IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_11(
			//     (ILFixDynamicMethodWrapper_28 *)Patch,
			//     (Object *)data,
			//     (Object *)camera,
			//     (Object *)bwFlash,
			//     0LL);
			// }
			// 
		}

		[CompilerGenerated]
		internal static void <.cctor>g__DispatchWithGuardBands|104_11(CommandBuffer cmd, ComputeShader shader, int kernelId, ref Vector2Int size, ref int viewCount, ref UberPostPassUtils.<>c__DisplayClass104_0 param_00046840)
		{
			// // Void <.cctor>g__DispatchWithGuardBands|104_11(CommandBuffer, ComputeShader, Int32, Vector2Int ByRef, Int32 ByRef, UberPostPassUtils+<>c__DisplayClass104_0 ByRef)
			// void HG::Rendering::Runtime::UberPostPassUtils::__cctor_g__DispatchWithGuardBands_104_11(
			//         CommandBuffer *cmd,
			//         ComputeShader *shader,
			//         int32_t kernelId,
			//         Vector2Int *size,
			//         int32_t *viewCount,
			//         UberPostPassUtils_c_DisplayClass104_0 *param_00046840,
			//         MethodInfo *method)
			// {
			//   int32_t m_X; // edi
			//   int32_t m_Y; // ebx
			//   unsigned __int64 sourceRT; // rdx
			// 
			//   m_X = size.m_X;
			//   m_Y = size.m_Y;
			//   sourceRT = (unsigned __int64)param_00046840.sourceRT;
			//   if ( !param_00046840.sourceRT )
			//     goto LABEL_13;
			//   sourceRT = *(_QWORD *)(sourceRT + 16);
			//   if ( !sourceRT )
			//     goto LABEL_13;
			//   if ( m_X < (int)sub_18003ED00(5LL) )
			//   {
			//     sourceRT = (unsigned int)(m_X >> 31);
			//     LODWORD(sourceRT) = m_X % 8;
			//     if ( m_X % 8 < 4 )
			//       m_X += 4;
			//   }
			//   if ( !param_00046840.sourceRT )
			//     goto LABEL_13;
			//   sourceRT = (unsigned __int64)param_00046840.sourceRT.fields.m_RT;
			//   if ( !sourceRT )
			//     goto LABEL_13;
			//   if ( m_Y < (int)sub_18003ED00(7LL) )
			//   {
			//     sourceRT = (unsigned int)(m_Y >> 31);
			//     LODWORD(sourceRT) = m_Y % 8;
			//     if ( m_Y % 8 < 4 )
			//       m_Y += 4;
			//   }
			//   if ( !cmd )
			// LABEL_13:
			//     sub_180B536AC(cmd, sourceRT);
			//   UnityEngine::Rendering::CommandBuffer::Internal_DispatchCompute(
			//     cmd,
			//     shader,
			//     kernelId,
			//     (m_X + 7) / 8,
			//     (m_Y + 7) / 8,
			//     *viewCount,
			//     0LL);
			// }
			// 
		}

		[CompilerGenerated]
		internal static void <.cctor>g__DispatchWithGuardBands|104_12(CommandBuffer cmd, ComputeShader shader, int kernelId, ref Vector2Int size)
		{
			// // Void <.cctor>g__DispatchWithGuardBands|104_12(CommandBuffer, ComputeShader, Int32, Vector2Int ByRef)
			// void HG::Rendering::Runtime::UberPostPassUtils::__cctor_g__DispatchWithGuardBands_104_12(
			//         CommandBuffer *cmd,
			//         ComputeShader *shader,
			//         int32_t kernelId,
			//         Vector2Int *size,
			//         MethodInfo *method)
			// {
			//   int32_t m_X; // r10d
			//   int32_t m_Y; // r11d
			//   int v9; // r9d
			//   __int64 v10; // rdx
			//   int32_t v11; // eax
			//   int32_t threadGroupsY; // ecx
			//   int32_t v13; // eax
			// 
			//   m_X = size.m_X;
			//   m_Y = size.m_Y;
			//   v9 = size.m_X % 8;
			//   v10 = (unsigned int)(m_Y >> 31);
			//   LODWORD(v10) = m_Y % 8;
			//   if ( !cmd )
			//     sub_180B536AC(0LL, v10);
			//   v11 = m_Y + 4;
			//   if ( (int)v10 >= 4 )
			//     v11 = m_Y;
			//   threadGroupsY = (v11 + 7) / 8;
			//   v13 = m_X + 4;
			//   if ( v9 >= 4 )
			//     v13 = m_X;
			//   UnityEngine::Rendering::CommandBuffer::Internal_DispatchCompute(
			//     cmd,
			//     shader,
			//     kernelId,
			//     (v13 + 7) / 8,
			//     threadGroupsY,
			//     1,
			//     0LL);
			// }
			// 
		}

		private const GraphicsFormat k_PostProcessColorFormat = GraphicsFormat.B10G11R11_UFloatPack32;

		internal const int k_RTGuardBandSize = 4;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x00")]
		internal static ComputeBuffer autoExposure_histogramBuffer;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x08")]
		private static float[] m_autoExposure_histogramRes;

		private const int AUTO_EXPOSURE_HISTOGRAM_SIZE = 16;

		private const int AUTO_EXPOSURE_THREAD_GROUP_SIZE_X = 8;

		private const int AUTO_EXPOSURE_THREAD_GROUP_SIZE_Y = 8;

		private const int AUTO_EXPOSURE_TILE_SIZE = 16;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x10")]
		private static float m_currentExposure;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x14")]
		private static float m_targetExposure;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x18")]
		private static double m_autoExposure_timeThisFrame;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x20")]
		private static float m_autoExposure_deltaTime;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x24")]
		private static bool m_autoExposureReadyForNextFetch;

		private NativeArray<float> m_autoExposure_readbackData;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x28")]
		private static readonly RenderFunc<UberPostPassUtils.HGAutoExposurePassData> s_autoExposureCSRenderFunc;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x30")]
		private static UberPostPassUtils.HGAutoExposureData s_cachedAutoExposureData;

		internal const int k_MaxBloomMipCount = 16;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x38")]
		private static readonly RenderFunc<UberPostPassUtils.BloomPassData> s_bloomCSRenderFunc;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x40")]
		private static readonly RenderFunc<UberPostPassUtils.BloomPassData> s_bloomPSRenderFunc;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x48")]
		private static readonly RenderFunc<UberPostPassUtils.ColorGradingPassData> s_colorGradingRenderFunc;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x50")]
		private static readonly RenderFunc<UberPostPassUtils.ColorGradingPassData> s_skipColorGradingPass;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x58")]
		private static UberPostPassUtils.ColorGradingData s_colorGradingData;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x60")]
		private static Material[] s_cutSceneEffectMaterials;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x68")]
		private static Vector3[] s_cutsceneEffectMeshVertices;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x70")]
		private static Mesh s_cutsceneEffectMesh;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x78")]
		private static readonly RenderFunc<UberPostPassUtils.CutsceneEffectData> m_cutsceneEffectFunc;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x80")]
		private static readonly RenderFunc<UberPostPassUtils.FisheyeEffectData> m_fisheyeEffectFunc;

		internal const float k_frostedGlassDownsampleScale = 0.25f;

		internal const int k_frostedGlassDownsampleNum = 3;

		internal const float k_frostedGlassColorThreshold = 2.5f;

		internal const GraphicsFormat k_frostedGlassFormat = GraphicsFormat.B10G11R11_UFloatPack32;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x88")]
		private static readonly RenderFunc<UberPostPassUtils.FrostedGlassSkipPassData> s_frostedGlassSkipRenderFunc;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x90")]
		private static readonly RenderFunc<UberPostPassUtils.FrostedGlassPassData> s_frostedGlassCSRenderFunc;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0x98")]
		private static readonly RenderFunc<UberPostPassUtils.FrostedGlassPassData> s_frostedGlassPSRenderFunc;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0xA0")]
		private static UberPostPassUtils.FrostedGlassData s_frostedGlassData;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0xA8")]
		private static long[] s_mipDownsCpp;

		[StaticFieldOffset(ThreadStatic = false, Offset = "0xB0")]
		private static readonly RenderFunc<UberPostPassUtils.SharpenData> s_sharpenRenderFunc;

		internal class HGAutoExposureData
		{
			public HGAutoExposureData()
			{
				// // Void Lerp[HGWindConfig](HGWindConfig ByRef, HGWindConfig ByRef, Single)
				// void HG::Rendering::Runtime::HGCelestialConfig::HGCelestialAdvancedObjectConfig::Lerp<HG::Rendering::Runtime::HGWindConfig>(
				//         HGCelestialConfig_HGCelestialAdvancedObjectConfig *this,
				//         HGWindConfig *cSrc,
				//         HGWindConfig *cDst,
				//         float t,
				//         MethodInfo *method)
				// {
				//   ;
				// }
				// 
			}

			public TextureHandle inputTexture;

			public ComputeShader autoExposureHistogramCS;

			public int histogramKernel;

			public ComputeBuffer histogramBuffer;

			public bool autoExposureActive;

			public AutoExposureMode autoExposureMode;

			public Vector2 exposureEv100Range;

			public Vector2 exposureEv100HistogramRange;

			public Vector2 pixelLuminanceRange;

			public float centerPixelWeight;

			public float evCompensationManual;

			public float evCompensationAuto;

			public Vector2 evClampRange;

			public float lerpDownSpeed;

			public float lerpUpSpeed;

			public bool curveEditModeEnabled;

			public AutoExposureMeteringMode meteringMode;

			public int textureSizeX;

			public int textureSizeY;

			public int threadGroupsX;

			public int threadGroupsY;

			public float deltaTime;

			public HGCamera hgCamera;
		}

		internal class HGAutoExposurePassData
		{
			public HGAutoExposurePassData()
			{
				// // Void Lerp[HGWindConfig](HGWindConfig ByRef, HGWindConfig ByRef, Single)
				// void HG::Rendering::Runtime::HGCelestialConfig::HGCelestialAdvancedObjectConfig::Lerp<HG::Rendering::Runtime::HGWindConfig>(
				//         HGCelestialConfig_HGCelestialAdvancedObjectConfig *this,
				//         HGWindConfig *cSrc,
				//         HGWindConfig *cDst,
				//         float t,
				//         MethodInfo *method)
				// {
				//   ;
				// }
				// 
			}

			public TextureHandle inputTexture;

			public ComputeShader autoExposureHistogramCS;

			public int histogramKernel;

			public ComputeBuffer histogramBuffer;

			public int textureSizeX;

			public int textureSizeY;

			public int threadGroupsX;

			public int threadGroupsY;

			public Vector2 exposureEv100Range;

			public float centerPixelWeight;
		}

		internal class BloomPassData
		{
			public BloomPassData()
			{
			}

			public Material bloomPrefilterMat;

			public Material[] bloomBlurMat1stPass;

			public Material[] bloomBlurMat2ndPass;

			public Material[] bloomUpsampleMat;

			public ComputeShader bloomPrefilterCS;

			public ComputeShader bloomBlurCS;

			public ComputeShader bloomBlurNonOptCS;

			public ComputeShader bloomUpsampleCS;

			public int bloomPrefilterKernel;

			public int bloomBlurKernel;

			public int bloomDownsampleKernel;

			public int bloomUpsampleKernel;

			public int viewCount;

			public int bloomMipCount;

			public Vector4[] bloomMipInfo;

			public float bloomScatterParam;

			public Vector4 thresholdParams;

			public Vector4 characterThresholdParams;

			public Vector4 characterParams;

			public bool enableAlpha;

			public BloomQuality bloomQuality;

			public bool enableCharacterMask;

			public TextureHandle source;

			public TextureHandle motionVector;

			public TextureHandle mips0Temp;

			public TextureHandle[] mipsDown;

			public TextureHandle[] mipsUp;
		}

		[StructLayout(LayoutKind.Sequential, Pack = 1)]
		internal struct BloomPSMaterials
		{
			public Material prefilterMaterial;

			public Material[] blur1stPassMaterials;

			public Material[] blur2ndPassMaterials;

			public Material[] bloomUpsampleMaterials;
		}

		[StructLayout(LayoutKind.Sequential, Pack = 1)]
		internal struct CachedColorGradingData
		{
			public CachedColorGradingData(UberPostPassUtils.ColorGradingData colorGradingPassData)
			{
			}

			public bool Equals(in UberPostPassUtils.CachedColorGradingData otherData)
			{
				// // Boolean Equals(UberPostPassUtils+CachedColorGradingData ByRef)
				// bool HG::Rendering::Runtime::UberPostPassUtils::CachedColorGradingData::Equals(
				//         UberPostPassUtils_CachedColorGradingData *this,
				//         UberPostPassUtils_CachedColorGradingData *otherData,
				//         MethodInfo *method)
				// {
				//   Object_1 *externalLuT; // rdi
				//   Object_1 *v6; // rbx
				//   Object_1 *colorCurve_masterCurve; // rdi
				//   Object_1 *v8; // rbx
				//   Object_1 *colorCurve_redCurve; // rdi
				//   Object_1 *v10; // rbx
				//   Object_1 *colorCurve_greenCurve; // rdi
				//   Object_1 *v12; // rbx
				//   Object_1 *colorCurve_blueCurve; // rdi
				//   Object_1 *v14; // rbx
				//   Object_1 *colorCurve_hueVsHueCurve; // rdi
				//   Object_1 *v16; // rbx
				//   Object_1 *colorCurve_hueVsSatCurve; // rdi
				//   Object_1 *v18; // rbx
				//   Object_1 *colorCurve_satVsSatCurve; // rdi
				//   Object_1 *v20; // rbx
				//   Object_1 *colorCurve_lumVsSatCurve; // rdi
				//   Object_1 *v22; // rbx
				//   __m128 v23; // xmm2
				//   __m128 v24; // xmm5
				//   float v25; // xmm4_4
				//   float v26; // xmm1_4
				//   float v27; // xmm3_4
				//   float v28; // xmm0_4
				//   float v29; // xmm1_4
				//   __m128 v30; // xmm2
				//   __m128 v31; // xmm5
				//   float v32; // xmm4_4
				//   float v33; // xmm1_4
				//   float v34; // xmm3_4
				//   float v35; // xmm0_4
				//   __m128 v36; // xmm2
				//   __m128 v37; // xmm5
				//   float v38; // xmm4_4
				//   float v39; // xmm1_4
				//   float v40; // xmm3_4
				//   float v41; // xmm0_4
				//   __m128 v42; // xmm2
				//   __m128 v43; // xmm5
				//   float v44; // xmm4_4
				//   float v45; // xmm1_4
				//   float v46; // xmm3_4
				//   float v47; // xmm0_4
				//   __m128 v48; // xmm2
				//   __m128 v49; // xmm5
				//   float v50; // xmm4_4
				//   float v51; // xmm1_4
				//   float v52; // xmm3_4
				//   float v53; // xmm0_4
				//   __m128 v54; // xmm2
				//   __m128 v55; // xmm5
				//   float v56; // xmm4_4
				//   float v57; // xmm1_4
				//   float v58; // xmm3_4
				//   float v59; // xmm0_4
				//   __m128 v60; // xmm2
				//   __m128 v61; // xmm5
				//   float v62; // xmm4_4
				//   float v63; // xmm1_4
				//   float v64; // xmm3_4
				//   float v65; // xmm0_4
				//   __m128 v66; // xmm2
				//   __m128 v67; // xmm5
				//   float v68; // xmm4_4
				//   float v69; // xmm1_4
				//   float v70; // xmm3_4
				//   float v71; // xmm0_4
				//   __m128 v72; // xmm2
				//   __m128 v73; // xmm5
				//   float v74; // xmm4_4
				//   float v75; // xmm1_4
				//   float v76; // xmm3_4
				//   float v77; // xmm0_4
				//   __m128 v78; // xmm2
				//   __m128 v79; // xmm5
				//   float v80; // xmm4_4
				//   float v81; // xmm1_4
				//   float v82; // xmm3_4
				//   float v83; // xmm0_4
				//   __m128 v84; // xmm2
				//   __m128 v85; // xmm5
				//   float v86; // xmm4_4
				//   float v87; // xmm1_4
				//   float v88; // xmm3_4
				//   float v89; // xmm0_4
				//   __m128 v90; // xmm2
				//   __m128 v91; // xmm5
				//   float v92; // xmm4_4
				//   float v93; // xmm1_4
				//   float v94; // xmm3_4
				//   float v95; // xmm0_4
				//   __m128 v96; // xmm2
				//   __m128 v97; // xmm5
				//   float v98; // xmm4_4
				//   float v99; // xmm1_4
				//   float v100; // xmm3_4
				//   float v101; // xmm0_4
				//   __m128 v102; // xmm2
				//   __m128 v103; // xmm5
				//   float v104; // xmm4_4
				//   float v105; // xmm1_4
				//   float v106; // xmm3_4
				//   float v107; // xmm0_4
				//   __m128 v108; // xmm2
				//   __m128 v109; // xmm5
				//   float v110; // xmm4_4
				//   float v111; // xmm1_4
				//   float v112; // xmm3_4
				//   float v113; // xmm0_4
				//   ILFixDynamicMethodWrapper_2 *Patch; // rax
				//   __int64 v116; // rdx
				//   __int64 v117; // rcx
				//   float v118[4]; // [rsp+30h] [rbp-28h]
				// 
				//   if ( !byte_18D9194DF )
				//   {
				//     sub_18003C530(&TypeInfo::UnityEngine::Object);
				//     byte_18D9194DF = 1;
				//   }
				//   if ( IFix::WrappersManagerImpl::IsPatched(2433, 0LL) )
				//   {
				//     Patch = IFix::WrappersManagerImpl::GetPatch(2433, 0LL);
				//     if ( !Patch )
				//       sub_180B536AC(v117, v116);
				//     return IFix::ILFixDynamicMethodWrapper::__Gen_Wrap_883(Patch, this, otherData, 0LL);
				//   }
				//   else
				//   {
				//     if ( this.tonemappingActive != otherData.tonemappingActive )
				//       return 0;
				//     if ( this.lutSize != otherData.lutSize )
				//       return 0;
				//     if ( this.tonemappingMode != otherData.tonemappingMode )
				//       return 0;
				//     externalLuT = (Object_1 *)this.externalLuT;
				//     v6 = (Object_1 *)otherData.externalLuT;
				//     sub_180002C70(TypeInfo::UnityEngine::Object);
				//     if ( !UnityEngine::Object::op_Equality(externalLuT, v6, 0LL) )
				//       return 0;
				//     colorCurve_masterCurve = (Object_1 *)this.colorCurve_masterCurve;
				//     v8 = (Object_1 *)otherData.colorCurve_masterCurve;
				//     sub_180002C70(TypeInfo::UnityEngine::Object);
				//     if ( !UnityEngine::Object::op_Equality(colorCurve_masterCurve, v8, 0LL) )
				//       return 0;
				//     colorCurve_redCurve = (Object_1 *)this.colorCurve_redCurve;
				//     v10 = (Object_1 *)otherData.colorCurve_redCurve;
				//     sub_180002C70(TypeInfo::UnityEngine::Object);
				//     if ( !UnityEngine::Object::op_Equality(colorCurve_redCurve, v10, 0LL) )
				//       return 0;
				//     colorCurve_greenCurve = (Object_1 *)this.colorCurve_greenCurve;
				//     v12 = (Object_1 *)otherData.colorCurve_greenCurve;
				//     sub_180002C70(TypeInfo::UnityEngine::Object);
				//     if ( !UnityEngine::Object::op_Equality(colorCurve_greenCurve, v12, 0LL) )
				//       return 0;
				//     colorCurve_blueCurve = (Object_1 *)this.colorCurve_blueCurve;
				//     v14 = (Object_1 *)otherData.colorCurve_blueCurve;
				//     sub_180002C70(TypeInfo::UnityEngine::Object);
				//     if ( !UnityEngine::Object::op_Equality(colorCurve_blueCurve, v14, 0LL) )
				//       return 0;
				//     colorCurve_hueVsHueCurve = (Object_1 *)this.colorCurve_hueVsHueCurve;
				//     v16 = (Object_1 *)otherData.colorCurve_hueVsHueCurve;
				//     sub_180002C70(TypeInfo::UnityEngine::Object);
				//     if ( !UnityEngine::Object::op_Equality(colorCurve_hueVsHueCurve, v16, 0LL) )
				//       return 0;
				//     colorCurve_hueVsSatCurve = (Object_1 *)this.colorCurve_hueVsSatCurve;
				//     v18 = (Object_1 *)otherData.colorCurve_hueVsSatCurve;
				//     sub_180002C70(TypeInfo::UnityEngine::Object);
				//     if ( !UnityEngine::Object::op_Equality(colorCurve_hueVsSatCurve, v18, 0LL) )
				//       return 0;
				//     colorCurve_satVsSatCurve = (Object_1 *)this.colorCurve_satVsSatCurve;
				//     v20 = (Object_1 *)otherData.colorCurve_satVsSatCurve;
				//     sub_180002C70(TypeInfo::UnityEngine::Object);
				//     if ( !UnityEngine::Object::op_Equality(colorCurve_satVsSatCurve, v20, 0LL) )
				//       return 0;
				//     colorCurve_lumVsSatCurve = (Object_1 *)this.colorCurve_lumVsSatCurve;
				//     v22 = (Object_1 *)otherData.colorCurve_lumVsSatCurve;
				//     sub_180002C70(TypeInfo::UnityEngine::Object);
				//     if ( !UnityEngine::Object::op_Equality(colorCurve_lumVsSatCurve, v22, 0LL) )
				//       return 0;
				//     v23 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.colorFilter);
				//     v24 = (__m128)_mm_loadu_si128((const __m128i *)&this.colorFilter);
				//     v25 = _mm_shuffle_ps(v24, v24, 85).m128_f32[0] - _mm_shuffle_ps(v23, v23, 85).m128_f32[0];
				//     v26 = _mm_shuffle_ps(v24, v24, 170).m128_f32[0];
				//     v27 = v24.m128_f32[0] - v23.m128_f32[0];
				//     v24.m128_f32[0] = _mm_shuffle_ps(v24, v24, 255).m128_f32[0];
				//     v28 = _mm_shuffle_ps(v23, v23, 170).m128_f32[0];
				//     v23.m128_f32[0] = _mm_shuffle_ps(v23, v23, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v25 * v25) + (float)(v27 * v27))
				//                        + (float)((float)(v26 - v28) * (float)(v26 - v28)))
				//                + (float)((float)(v24.m128_f32[0] - v23.m128_f32[0]) * (float)(v24.m128_f32[0] - v23.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     v29 = this.lmsColorBalance.z - otherData.lmsColorBalance.z;
				//     *(_QWORD *)v118 = *(_QWORD *)&otherData.lmsColorBalance.x;
				//     if ( (float)((float)((float)((float)(COERCE_FLOAT(HIDWORD(*(_QWORD *)&this.lmsColorBalance.x)) - v118[1])
				//                                * (float)(COERCE_FLOAT(HIDWORD(*(_QWORD *)&this.lmsColorBalance.x)) - v118[1]))
				//                        + (float)((float)(COERCE_FLOAT(*(_QWORD *)&this.lmsColorBalance.x) - v118[0])
				//                                * (float)(COERCE_FLOAT(*(_QWORD *)&this.lmsColorBalance.x) - v118[0])))
				//                + (float)(v29 * v29)) >= 9.9999994e-11 )
				//       return 0;
				//     v30 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.hueSatCon);
				//     v31 = (__m128)_mm_loadu_si128((const __m128i *)&this.hueSatCon);
				//     v32 = _mm_shuffle_ps(v31, v31, 85).m128_f32[0] - _mm_shuffle_ps(v30, v30, 85).m128_f32[0];
				//     v33 = _mm_shuffle_ps(v31, v31, 170).m128_f32[0];
				//     v34 = v31.m128_f32[0] - v30.m128_f32[0];
				//     v31.m128_f32[0] = _mm_shuffle_ps(v31, v31, 255).m128_f32[0];
				//     v35 = _mm_shuffle_ps(v30, v30, 170).m128_f32[0];
				//     v30.m128_f32[0] = _mm_shuffle_ps(v30, v30, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v32 * v32) + (float)(v34 * v34))
				//                        + (float)((float)(v33 - v35) * (float)(v33 - v35)))
				//                + (float)((float)(v31.m128_f32[0] - v30.m128_f32[0]) * (float)(v31.m128_f32[0] - v30.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     v36 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.channelMixerR);
				//     v37 = (__m128)_mm_loadu_si128((const __m128i *)&this.channelMixerR);
				//     v38 = _mm_shuffle_ps(v37, v37, 85).m128_f32[0] - _mm_shuffle_ps(v36, v36, 85).m128_f32[0];
				//     v39 = _mm_shuffle_ps(v37, v37, 170).m128_f32[0];
				//     v40 = v37.m128_f32[0] - v36.m128_f32[0];
				//     v37.m128_f32[0] = _mm_shuffle_ps(v37, v37, 255).m128_f32[0];
				//     v41 = _mm_shuffle_ps(v36, v36, 170).m128_f32[0];
				//     v36.m128_f32[0] = _mm_shuffle_ps(v36, v36, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v38 * v38) + (float)(v40 * v40))
				//                        + (float)((float)(v39 - v41) * (float)(v39 - v41)))
				//                + (float)((float)(v37.m128_f32[0] - v36.m128_f32[0]) * (float)(v37.m128_f32[0] - v36.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     v42 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.channelMixerG);
				//     v43 = (__m128)_mm_loadu_si128((const __m128i *)&this.channelMixerG);
				//     v44 = _mm_shuffle_ps(v43, v43, 85).m128_f32[0] - _mm_shuffle_ps(v42, v42, 85).m128_f32[0];
				//     v45 = _mm_shuffle_ps(v43, v43, 170).m128_f32[0];
				//     v46 = v43.m128_f32[0] - v42.m128_f32[0];
				//     v43.m128_f32[0] = _mm_shuffle_ps(v43, v43, 255).m128_f32[0];
				//     v47 = _mm_shuffle_ps(v42, v42, 170).m128_f32[0];
				//     v42.m128_f32[0] = _mm_shuffle_ps(v42, v42, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v44 * v44) + (float)(v46 * v46))
				//                        + (float)((float)(v45 - v47) * (float)(v45 - v47)))
				//                + (float)((float)(v43.m128_f32[0] - v42.m128_f32[0]) * (float)(v43.m128_f32[0] - v42.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     v48 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.channelMixerB);
				//     v49 = (__m128)_mm_loadu_si128((const __m128i *)&this.channelMixerB);
				//     v50 = _mm_shuffle_ps(v49, v49, 85).m128_f32[0] - _mm_shuffle_ps(v48, v48, 85).m128_f32[0];
				//     v51 = _mm_shuffle_ps(v49, v49, 170).m128_f32[0];
				//     v52 = v49.m128_f32[0] - v48.m128_f32[0];
				//     v49.m128_f32[0] = _mm_shuffle_ps(v49, v49, 255).m128_f32[0];
				//     v53 = _mm_shuffle_ps(v48, v48, 170).m128_f32[0];
				//     v48.m128_f32[0] = _mm_shuffle_ps(v48, v48, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v50 * v50) + (float)(v52 * v52))
				//                        + (float)((float)(v51 - v53) * (float)(v51 - v53)))
				//                + (float)((float)(v49.m128_f32[0] - v48.m128_f32[0]) * (float)(v49.m128_f32[0] - v48.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     v54 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.shadows);
				//     v55 = (__m128)_mm_loadu_si128((const __m128i *)&this.shadows);
				//     v56 = _mm_shuffle_ps(v55, v55, 85).m128_f32[0] - _mm_shuffle_ps(v54, v54, 85).m128_f32[0];
				//     v57 = _mm_shuffle_ps(v55, v55, 170).m128_f32[0];
				//     v58 = v55.m128_f32[0] - v54.m128_f32[0];
				//     v55.m128_f32[0] = _mm_shuffle_ps(v55, v55, 255).m128_f32[0];
				//     v59 = _mm_shuffle_ps(v54, v54, 170).m128_f32[0];
				//     v54.m128_f32[0] = _mm_shuffle_ps(v54, v54, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v56 * v56) + (float)(v58 * v58))
				//                        + (float)((float)(v57 - v59) * (float)(v57 - v59)))
				//                + (float)((float)(v55.m128_f32[0] - v54.m128_f32[0]) * (float)(v55.m128_f32[0] - v54.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     v60 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.midtones);
				//     v61 = (__m128)_mm_loadu_si128((const __m128i *)&this.midtones);
				//     v62 = _mm_shuffle_ps(v61, v61, 85).m128_f32[0] - _mm_shuffle_ps(v60, v60, 85).m128_f32[0];
				//     v63 = _mm_shuffle_ps(v61, v61, 170).m128_f32[0];
				//     v64 = v61.m128_f32[0] - v60.m128_f32[0];
				//     v61.m128_f32[0] = _mm_shuffle_ps(v61, v61, 255).m128_f32[0];
				//     v65 = _mm_shuffle_ps(v60, v60, 170).m128_f32[0];
				//     v60.m128_f32[0] = _mm_shuffle_ps(v60, v60, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v62 * v62) + (float)(v64 * v64))
				//                        + (float)((float)(v63 - v65) * (float)(v63 - v65)))
				//                + (float)((float)(v61.m128_f32[0] - v60.m128_f32[0]) * (float)(v61.m128_f32[0] - v60.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     v66 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.highlights);
				//     v67 = (__m128)_mm_loadu_si128((const __m128i *)&this.highlights);
				//     v68 = _mm_shuffle_ps(v67, v67, 85).m128_f32[0] - _mm_shuffle_ps(v66, v66, 85).m128_f32[0];
				//     v69 = _mm_shuffle_ps(v67, v67, 170).m128_f32[0];
				//     v70 = v67.m128_f32[0] - v66.m128_f32[0];
				//     v67.m128_f32[0] = _mm_shuffle_ps(v67, v67, 255).m128_f32[0];
				//     v71 = _mm_shuffle_ps(v66, v66, 170).m128_f32[0];
				//     v66.m128_f32[0] = _mm_shuffle_ps(v66, v66, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v68 * v68) + (float)(v70 * v70))
				//                        + (float)((float)(v69 - v71) * (float)(v69 - v71)))
				//                + (float)((float)(v67.m128_f32[0] - v66.m128_f32[0]) * (float)(v67.m128_f32[0] - v66.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     v72 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.shadowsHighlightsLimits);
				//     v73 = (__m128)_mm_loadu_si128((const __m128i *)&this.shadowsHighlightsLimits);
				//     v74 = _mm_shuffle_ps(v73, v73, 85).m128_f32[0] - _mm_shuffle_ps(v72, v72, 85).m128_f32[0];
				//     v75 = _mm_shuffle_ps(v73, v73, 170).m128_f32[0];
				//     v76 = v73.m128_f32[0] - v72.m128_f32[0];
				//     v73.m128_f32[0] = _mm_shuffle_ps(v73, v73, 255).m128_f32[0];
				//     v77 = _mm_shuffle_ps(v72, v72, 170).m128_f32[0];
				//     v72.m128_f32[0] = _mm_shuffle_ps(v72, v72, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v74 * v74) + (float)(v76 * v76))
				//                        + (float)((float)(v75 - v77) * (float)(v75 - v77)))
				//                + (float)((float)(v73.m128_f32[0] - v72.m128_f32[0]) * (float)(v73.m128_f32[0] - v72.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     v78 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.lift);
				//     v79 = (__m128)_mm_loadu_si128((const __m128i *)&this.lift);
				//     v80 = _mm_shuffle_ps(v79, v79, 85).m128_f32[0] - _mm_shuffle_ps(v78, v78, 85).m128_f32[0];
				//     v81 = _mm_shuffle_ps(v79, v79, 170).m128_f32[0];
				//     v82 = v79.m128_f32[0] - v78.m128_f32[0];
				//     v79.m128_f32[0] = _mm_shuffle_ps(v79, v79, 255).m128_f32[0];
				//     v83 = _mm_shuffle_ps(v78, v78, 170).m128_f32[0];
				//     v78.m128_f32[0] = _mm_shuffle_ps(v78, v78, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v80 * v80) + (float)(v82 * v82))
				//                        + (float)((float)(v81 - v83) * (float)(v81 - v83)))
				//                + (float)((float)(v79.m128_f32[0] - v78.m128_f32[0]) * (float)(v79.m128_f32[0] - v78.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     v84 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.gamma);
				//     v85 = (__m128)_mm_loadu_si128((const __m128i *)&this.gamma);
				//     v86 = _mm_shuffle_ps(v85, v85, 85).m128_f32[0] - _mm_shuffle_ps(v84, v84, 85).m128_f32[0];
				//     v87 = _mm_shuffle_ps(v85, v85, 170).m128_f32[0];
				//     v88 = v85.m128_f32[0] - v84.m128_f32[0];
				//     v85.m128_f32[0] = _mm_shuffle_ps(v85, v85, 255).m128_f32[0];
				//     v89 = _mm_shuffle_ps(v84, v84, 170).m128_f32[0];
				//     v84.m128_f32[0] = _mm_shuffle_ps(v84, v84, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v86 * v86) + (float)(v88 * v88))
				//                        + (float)((float)(v87 - v89) * (float)(v87 - v89)))
				//                + (float)((float)(v85.m128_f32[0] - v84.m128_f32[0]) * (float)(v85.m128_f32[0] - v84.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     v90 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.gain);
				//     v91 = (__m128)_mm_loadu_si128((const __m128i *)&this.gain);
				//     v92 = _mm_shuffle_ps(v91, v91, 85).m128_f32[0] - _mm_shuffle_ps(v90, v90, 85).m128_f32[0];
				//     v93 = _mm_shuffle_ps(v91, v91, 170).m128_f32[0];
				//     v94 = v91.m128_f32[0] - v90.m128_f32[0];
				//     v91.m128_f32[0] = _mm_shuffle_ps(v91, v91, 255).m128_f32[0];
				//     v95 = _mm_shuffle_ps(v90, v90, 170).m128_f32[0];
				//     v90.m128_f32[0] = _mm_shuffle_ps(v90, v90, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v92 * v92) + (float)(v94 * v94))
				//                        + (float)((float)(v93 - v95) * (float)(v93 - v95)))
				//                + (float)((float)(v91.m128_f32[0] - v90.m128_f32[0]) * (float)(v91.m128_f32[0] - v90.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     v96 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.splitShadows);
				//     v97 = (__m128)_mm_loadu_si128((const __m128i *)&this.splitShadows);
				//     v98 = _mm_shuffle_ps(v97, v97, 85).m128_f32[0] - _mm_shuffle_ps(v96, v96, 85).m128_f32[0];
				//     v99 = _mm_shuffle_ps(v97, v97, 170).m128_f32[0];
				//     v100 = v97.m128_f32[0] - v96.m128_f32[0];
				//     v97.m128_f32[0] = _mm_shuffle_ps(v97, v97, 255).m128_f32[0];
				//     v101 = _mm_shuffle_ps(v96, v96, 170).m128_f32[0];
				//     v96.m128_f32[0] = _mm_shuffle_ps(v96, v96, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v98 * v98) + (float)(v100 * v100))
				//                        + (float)((float)(v99 - v101) * (float)(v99 - v101)))
				//                + (float)((float)(v97.m128_f32[0] - v96.m128_f32[0]) * (float)(v97.m128_f32[0] - v96.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     v102 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.splitHighlights);
				//     v103 = (__m128)_mm_loadu_si128((const __m128i *)&this.splitHighlights);
				//     v104 = _mm_shuffle_ps(v103, v103, 85).m128_f32[0] - _mm_shuffle_ps(v102, v102, 85).m128_f32[0];
				//     v105 = _mm_shuffle_ps(v103, v103, 170).m128_f32[0];
				//     v106 = v103.m128_f32[0] - v102.m128_f32[0];
				//     v103.m128_f32[0] = _mm_shuffle_ps(v103, v103, 255).m128_f32[0];
				//     v107 = _mm_shuffle_ps(v102, v102, 170).m128_f32[0];
				//     v102.m128_f32[0] = _mm_shuffle_ps(v102, v102, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v104 * v104) + (float)(v106 * v106))
				//                        + (float)((float)(v105 - v107) * (float)(v105 - v107)))
				//                + (float)((float)(v103.m128_f32[0] - v102.m128_f32[0]) * (float)(v103.m128_f32[0] - v102.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     v108 = (__m128)_mm_loadu_si128((const __m128i *)&otherData.miscParams);
				//     v109 = (__m128)_mm_loadu_si128((const __m128i *)&this.miscParams);
				//     v110 = _mm_shuffle_ps(v109, v109, 85).m128_f32[0] - _mm_shuffle_ps(v108, v108, 85).m128_f32[0];
				//     v111 = _mm_shuffle_ps(v109, v109, 170).m128_f32[0];
				//     v112 = v109.m128_f32[0] - v108.m128_f32[0];
				//     v109.m128_f32[0] = _mm_shuffle_ps(v109, v109, 255).m128_f32[0];
				//     v113 = _mm_shuffle_ps(v108, v108, 170).m128_f32[0];
				//     v108.m128_f32[0] = _mm_shuffle_ps(v108, v108, 255).m128_f32[0];
				//     if ( (float)((float)((float)((float)(v110 * v110) + (float)(v112 * v112))
				//                        + (float)((float)(v111 - v113) * (float)(v111 - v113)))
				//                + (float)((float)(v109.m128_f32[0] - v108.m128_f32[0]) * (float)(v109.m128_f32[0] - v108.m128_f32[0]))) >= 9.9999994e-11 )
				//       return 0;
				//     return System::Single::Equals((Single *)&this.tonemapping_toeStrength, otherData.tonemapping_toeStrength, 0LL)
				//         && System::Single::Equals((Single *)&this.tonemapping_toeLength, otherData.tonemapping_toeLength, 0LL)
				//         && System::Single::Equals(
				//              (Single *)&this.tonemapping_shoulderStrength,
				//              otherData.tonemapping_shoulderStrength,
				//              0LL)
				//         && System::Single::Equals(
				//              (Single *)&this.tonemapping_shoulderLength,
				//              otherData.tonemapping_shoulderLength,
				//              0LL)
				//         && System::Single::Equals((Single *)&this.tonemapping_shoulderAngle, otherData.tonemapping_shoulderAngle, 0LL)
				//         && System::Single::Equals((Single *)&this.tonemapping_gamma, otherData.tonemapping_gamma, 0LL)
				//         && System::Single::Equals((Single *)&this.lutContribution, otherData.lutContribution, 0LL);
				//   }
				// }
				// 
				return default(bool);
			}

			internal int lutSize;

			private bool tonemappingActive;

			private TonemappingMode tonemappingMode;

			private Texture externalLuT;

			private Texture2D colorCurve_masterCurve;

			private Texture2D colorCurve_redCurve;

			private Texture2D colorCurve_greenCurve;

			private Texture2D colorCurve_blueCurve;

			private Texture2D colorCurve_hueVsHueCurve;

			private Texture2D colorCurve_hueVsSatCurve;

			private Texture2D colorCurve_satVsSatCurve;

			private Texture2D colorCurve_lumVsSatCurve;

			private Vector4 colorFilter;

			private Vector3 lmsColorBalance;

			private Vector4 hueSatCon;

			private Vector4 channelMixerR;

			private Vector4 channelMixerG;

			private Vector4 channelMixerB;

			private Vector4 shadows;

			private Vector4 midtones;

			private Vector4 highlights;

			private Vector4 shadowsHighlightsLimits;

			private Vector4 lift;

			private Vector4 gamma;

			private Vector4 gain;

			private Vector4 splitShadows;

			private Vector4 splitHighlights;

			private Vector4 miscParams;

			private float tonemapping_toeStrength;

			private float tonemapping_toeLength;

			private float tonemapping_shoulderStrength;

			private float tonemapping_shoulderLength;

			private float tonemapping_shoulderAngle;

			private float tonemapping_gamma;

			private float lutContribution;
		}

		internal class ColorGradingData
		{
			public ColorGradingData()
			{
				// // Void Lerp[HGWindConfig](HGWindConfig ByRef, HGWindConfig ByRef, Single)
				// void HG::Rendering::Runtime::HGCelestialConfig::HGCelestialAdvancedObjectConfig::Lerp<HG::Rendering::Runtime::HGWindConfig>(
				//         HGCelestialConfig_HGCelestialAdvancedObjectConfig *this,
				//         HGWindConfig *cSrc,
				//         HGWindConfig *cDst,
				//         float t,
				//         MethodInfo *method)
				// {
				//   ;
				// }
				// 
			}

			public Material lutBuilder2DMaterial;

			public int lutSize;

			public Vector4 colorFilter;

			public Vector3 lmsColorBalance;

			public Vector4 hueSatCon;

			public Vector4 channelMixerR;

			public Vector4 channelMixerG;

			public Vector4 channelMixerB;

			public Vector4 shadows;

			public Vector4 midtones;

			public Vector4 highlights;

			public Vector4 shadowsHighlightsLimits;

			public Vector4 lift;

			public Vector4 gamma;

			public Vector4 gain;

			public Vector4 splitShadows;

			public Vector4 splitHighlights;

			public bool curvesEnabled;

			public TextureCurve masterCurve;

			public TextureCurve redCurve;

			public TextureCurve greenCurve;

			public TextureCurve blueCurve;

			public TextureCurve hueVsHueCurve;

			public TextureCurve hueVsSatCurve;

			public TextureCurve lumVsSatCurve;

			public TextureCurve satVsSatCurve;

			public Vector4 miscParams;

			public Texture externalLuT;

			public float lutContribution;

			public TonemappingMode tonemappingMode;

			public TextureHandle logLut;
		}

		internal class ColorGradingPassData
		{
			public ColorGradingPassData()
			{
				// // Void Lerp[HGWindConfig](HGWindConfig ByRef, HGWindConfig ByRef, Single)
				// void HG::Rendering::Runtime::HGCelestialConfig::HGCelestialAdvancedObjectConfig::Lerp<HG::Rendering::Runtime::HGWindConfig>(
				//         HGCelestialConfig_HGCelestialAdvancedObjectConfig *this,
				//         HGWindConfig *cSrc,
				//         HGWindConfig *cDst,
				//         float t,
				//         MethodInfo *method)
				// {
				//   ;
				// }
				// 
			}

			public Material lutBuilder2DMaterial;

			public TextureHandle logLut;
		}

		private class CutsceneEffectData
		{
			public CutsceneEffectData()
			{
				// // Void Lerp[HGWindConfig](HGWindConfig ByRef, HGWindConfig ByRef, Single)
				// void HG::Rendering::Runtime::HGCelestialConfig::HGCelestialAdvancedObjectConfig::Lerp<HG::Rendering::Runtime::HGWindConfig>(
				//         HGCelestialConfig_HGCelestialAdvancedObjectConfig *this,
				//         HGWindConfig *cSrc,
				//         HGWindConfig *cDst,
				//         float t,
				//         MethodInfo *method)
				// {
				//   ;
				// }
				// 
			}

			public TextureHandle sceneColor;

			public TextureHandle depthRT;

			public Mesh cutsceneEffectMesh;

			public Material blitMaterial;

			public Material[] cutSceneEffectMaterials;

			public int cutSceneEffectMaterialCount;
		}

		private class FisheyeEffectData
		{
			public FisheyeEffectData()
			{
				// // Void Lerp[HGWindConfig](HGWindConfig ByRef, HGWindConfig ByRef, Single)
				// void HG::Rendering::Runtime::HGCelestialConfig::HGCelestialAdvancedObjectConfig::Lerp<HG::Rendering::Runtime::HGWindConfig>(
				//         HGCelestialConfig_HGCelestialAdvancedObjectConfig *this,
				//         HGWindConfig *cSrc,
				//         HGWindConfig *cDst,
				//         float t,
				//         MethodInfo *method)
				// {
				//   ;
				// }
				// 
			}

			public TextureHandle source;

			public TextureHandle sceneDepth;

			public bool isFisheyeEffectEnabled;

			public Vector4 fisheyeEffectParams1;

			public Material fisheyeEffectMaterial;

			public Material fisheyeEffectDepthMaterial;
		}

		[StructLayout(LayoutKind.Sequential, Pack = 1)]
		internal struct FrostedGlassPSMaterials
		{
			public Material[] frostedGlass1stPassMaterials;

			public Material[] frostedGlass2ndPassMaterials;
		}

		internal class FrostedGlassData
		{
			public FrostedGlassData()
			{
			}

			public ComputeShader frostedGlassBlurCS;

			public int kernelKMain;

			public int kernelKMainWithLut;

			public Material[] frostedGlass1stPassMat;

			public Material[] frostedGlass2ndPassMat;

			public int downsampleNum;

			public Vector2Int[] mipsDownSize;

			public TextureHandle source;

			public TextureHandle[] mipsDown;

			public TextureHandle[] mipsDownTemp;

			public bool enableUserLut;

			public float colorThreshold;

			public Texture2D userLut;

			public Vector4 userLogLutSettings;

			public TextureHandle logLut;

			public Vector4 logLutSettings;
		}

		internal class FrostedGlassPassData
		{
			public FrostedGlassPassData()
			{
			}

			public ComputeShader frostedGlassBlurCS;

			public int kernelKMain;

			public int kernelKMainWithLut;

			public bool sceneFrostedGlass;

			public Material[] frostedGlass1stPassMat;

			public Material[] frostedGlass2ndPassMat;

			public int downsampleNum;

			public Vector2Int[] mipsDownSize;

			public TextureHandle source;

			public TextureHandle[] mipsDown;

			public TextureHandle[] mipsDownTemp;

			public bool enableUserLut;

			public float colorThreshold;

			public Texture2D userLut;

			public Vector4 userLogLutSettings;

			public TextureHandle logLut;

			public Vector4 logLutSettings;
		}

		internal class FrostedGlassSkipPassData
		{
			public FrostedGlassSkipPassData()
			{
				// // Void Lerp[HGWindConfig](HGWindConfig ByRef, HGWindConfig ByRef, Single)
				// void HG::Rendering::Runtime::HGCelestialConfig::HGCelestialAdvancedObjectConfig::Lerp<HG::Rendering::Runtime::HGWindConfig>(
				//         HGCelestialConfig_HGCelestialAdvancedObjectConfig *this,
				//         HGWindConfig *cSrc,
				//         HGWindConfig *cDst,
				//         float t,
				//         MethodInfo *method)
				// {
				//   ;
				// }
				// 
			}

			public Texture cachedFrostedGlassTexHeavy;

			public Texture cachedFrostedGlassTexMedium;

			public Texture cachedFrostedGlassTexLight;
		}

		internal class SharpenData
		{
			public SharpenData()
			{
				// // Void Lerp[HGWindConfig](HGWindConfig ByRef, HGWindConfig ByRef, Single)
				// void HG::Rendering::Runtime::HGCelestialConfig::HGCelestialAdvancedObjectConfig::Lerp<HG::Rendering::Runtime::HGWindConfig>(
				//         HGCelestialConfig_HGCelestialAdvancedObjectConfig *this,
				//         HGWindConfig *cSrc,
				//         HGWindConfig *cDst,
				//         float t,
				//         MethodInfo *method)
				// {
				//   ;
				// }
				// 
			}

			public Material sharpenMaterial;

			public TextureHandle source;

			public TextureHandle sharpenTexture;
		}

		internal class UberPostPassData
		{
			public UberPostPassData()
			{
			}

			public Material uberPostMaterial;

			public Vector4 logLutSettings;

			public Vector4 userLogLutSettings;

			public Vector4 dirtyLensParams1;

			public Texture dirtyTex;

			public Vector4 radialBlurParams;

			public Vector4 radialBlurParams2;

			public bool isBwFlashEnabled;

			public Vector4 bwFlashThreshold;

			public Vector4 bwFlashParams;

			public Vector4 bwFlashParams2;

			public Vector4 bwFlashParams3;

			public Vector4 bwFlashParams4;

			public Texture bwFlashTexture;

			public Texture bwMaskTexture;

			public Color bwFlashColor;

			public Color bwBackGroundColor;

			public Vector4 fisheyeEffectParams1;

			public Texture userLut;

			public TextureHandle source;

			public TextureHandle destination;

			public TextureHandle logLut;

			public TextureHandle sceneDepthBuffer;

			public RendererListHandle worldUIRenderList;

			public uint worldUIECSRenderList;

			public uint hgUIRenderList;

			public UberPostPassUtils.PPVignetteData vignetteData;

			public UberPostPassUtils.PPBloomData bloomData;
		}

		internal class PPBloomData
		{
			public PPBloomData()
			{
				// // Void Lerp[HGWindConfig](HGWindConfig ByRef, HGWindConfig ByRef, Single)
				// void HG::Rendering::Runtime::HGCelestialConfig::HGCelestialAdvancedObjectConfig::Lerp<HG::Rendering::Runtime::HGWindConfig>(
				//         HGCelestialConfig_HGCelestialAdvancedObjectConfig *this,
				//         HGWindConfig *cSrc,
				//         HGWindConfig *cDst,
				//         float t,
				//         MethodInfo *method)
				// {
				//   ;
				// }
				// 
			}

			public TextureHandle bloomTexture;

			public Texture bloomDirtTexture;

			public Vector4 bloomParams;

			public Vector4 bloomTint;

			public Vector4 bloomBicubicParams;

			public Vector4 bloomDirtTileOffset;

			public Vector4 bloomThreshold;
		}

		internal class PPVignetteData
		{
			public PPVignetteData()
			{
				// // Void Lerp[HGWindConfig](HGWindConfig ByRef, HGWindConfig ByRef, Single)
				// void HG::Rendering::Runtime::HGCelestialConfig::HGCelestialAdvancedObjectConfig::Lerp<HG::Rendering::Runtime::HGWindConfig>(
				//         HGCelestialConfig_HGCelestialAdvancedObjectConfig *this,
				//         HGWindConfig *cSrc,
				//         HGWindConfig *cDst,
				//         float t,
				//         MethodInfo *method)
				// {
				//   ;
				// }
				// 
			}

			public Vector4 vignetteParams1;

			public Vector4 vignetteParams2;

			public Vector4 vignetteColor;

			public Texture vignetteMask;
		}
	}
}
